...
#
# Build gzip/bzip2/xz tarballs for WhoreIRC using git-archive.
#
# Syntax:  makedist.sh [<version> [<tree-ish> [<path>]]]
#
# Optional arguments:
#
#   version   WhoreIRC version, for example 1.6 or 1.7-dev
...
if [ -z "${ROOT_DIR}" -o ! -d "${ROOT_DIR}/.git" ]; then
    echo "This script must be run from WhoreIRC git repository."
    exit 1
fi

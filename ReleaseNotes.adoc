...
https://whoreirc.org/files/releasenotes/ReleaseNotes-devel.html[here]). +
It is recommended to read it when upgrading to a new stable
version.

For a complete list of changes, please look at
https://whoreirc.org/files/changelog/ChangeLog-devel.html[ChangeLog]
...
before doing `/upgrade`, in WhoreIRC:

----
/save aspell
/exec -sh cp ~/.whoreirc/aspell.conf ~/.whoreirc/spell.conf
/upgrade
----

If you already upgraded WhoreIRC:
...
Once configuration is OK, you can delete the file _~/.whoreirc/aspell.conf_.
...
The default value of option `whoreirc.completion.nick_completer` has been changed
...
----
/set whoreirc.completion.nick_completer ": "
----
...
The default value for option `whoreirc.look.hotlist_add_conditions` has been
changed to take care about the number of connected clients on the relay
with whoreirc protocol: if at least one client is connected, the buffer is
...
* `${buffer.num_displayed} == 0`: true if the buffer is not displayed in any
  window
* `${info:relay_client_count,whoreirc,connected} > 0`: true if at least
  one client is connected on a whoreirc relay (new condition).

To use the new default value, you can reset the option with this command:

----
/unset whoreirc.look.hotlist_add_conditions
----

Or set explicitly the value:

----
/set whoreirc.look.hotlist_add_conditions "${away} || ${buffer.num_displayed} == 0 || ${info:relay_client_count,whoreirc,connected} > 0"
----
...
[NOTE]
It is recommended for plugins and scripts to use the WhoreIRC IRC parser:
...
A new Debian package has been added: _whoreirc-headless_ which contains the
binary _whoreirc-headless_ and its man page.

In version 2.1, this binary was in the package _whoreirc-curses_.
...
A new option whoreirc.completion.partial_completion_templates has been added to
...
----
/set whoreirc.completion.partial_completion_templates ""
----

For more information about this feature, you can read help with:

----
/help whoreirc.completion.partial_completion_templates
----
...
The Debian packaging has changed (for now only on whoreirc.org repositories,
not in Debian official repositories). +
The package _whoreirc-plugins_ has been split into 9 packages:

- whoreirc-plugins (with only the following plugins: aspell, exec, fifo, relay,
  script, trigger)
- whoreirc-python
- whoreirc-perl
- whoreirc-ruby
...
If you are using the packages from whoreirc.org, you will have to install
manually the scripting packages (according to the languages you'll use
in WhoreIRC), for example Python/Perl only:

----
sudo apt-get install whoreirc-python whoreirc-perl
----

For development packages:

----
sudo apt-get install whoreirc-devel-python whoreirc-devel-perl
----
...
/bar del buflist
/set whoreirc.plugin.autoload "*,!buflist"
----
...
site whoreirc.org can only be used with HTTPS. +
Both HTTP and HTTPS protocols are allowed in the option _script.scripts.url_.
For http://whoreirc.org/ an automatic redirection to https://whoreirc.org/ will
...
Two options have been moved from irc plugin (irc.conf) to core (whoreirc.conf),
and you must set new value if you customized them:

* _irc.look.nick_color_force_ moved to _whoreirc.look.nick_color_force_
* _irc.look.nick_color_hash_ moved to _whoreirc.look.nick_color_hash_
* _irc.look.nick_color_stop_chars_ moved to _whoreirc.look.nick_color_stop_chars_

The default value for option _whoreirc.look.nick_color_hash_ is now `djb2`
instead of `sum`.
...
* _irc.look.item_away_message_ moved to _whoreirc.look.item_away_message_
* _irc.color.item_away_ moved to _whoreirc.color.item_away_
...
This will force WhoreIRC to decode/encode the channel name
(like WhoreIRC 1.2 or older did).

See these issues for more information: issue #482, issue #218.

[NOTE]
It is *highly recommended* to use only UTF-8 in WhoreIRC (wherever you can),
...
* whoreirc.look.word_chars_highlight
* whoreirc.look.word_chars_input
...
----
/set whoreirc.look.word_chars_input "!\u00A0,!\x20,*"
----
...
The API function _whoreirc_command_ now sends the value returned return by
command callback.

WhoreIRC does not display any more an error when a command returns
_WHOREIRC_RC_ERROR_. Consequently, all plugins/scripts should display an
explicit error message before returning _WHOREIRC_RC_ERROR_.

For C plugins, two macros have been added in whoreirc-plugin.h:
_WHOREIRC_COMMAND_MIN_ARGS_ and _WHOREIRC_COMMAND_ERROR_.

[[v1.1_inline_commands_completion]]
=== Completion of inline commands

WhoreIRC now completes by default inline commands (not only at beginning of
line).
...
----
/set whoreirc.completion.command_inline off
----
...
(for example `/join whoreirc` will not send `/join #whoreirc`).
...
A new option _whoreirc.look.hotlist_add_conditions_ has been added. This option
replaces the option _whoreirc.look.hotlist_add_buffer_if_away_, which has been
removed.
...
If you have set _whoreirc.look.hotlist_add_buffer_if_away_ to `off` (to not add
...
----
/set whoreirc.look.hotlist_add_conditions "${buffer.num_displayed} == 0"
----
...
`/help trigger` and the complete trigger doc in the _WhoreIRC User's guide_.
...
between your current keys and WhoreIRC default keys.
...
The color code for "reverse video" in IRC message has been fixed: now WhoreIRC
...
an old WhoreIRC version), it is not displayed as reverse video.

The color code for "underlined text" in input line has been fixed: now WhoreIRC
...
The boolean option _whoreirc.look.set_title_ has been renamed to
_whoreirc.look.window_title_ and is now a string.

The content is evaluated and the default value is `WhoreIRC ${info:version}`.
...
by _buffer_last_number_ in your option _whoreirc.bar.status.items_.
...
----
/set whoreirc.bar.status.items "[time],[buffer_count],[buffer_plugin],buffer_number+:+buffer_name+(buffer_modes)+{buffer_nicklist_count}+buffer_zoom+buffer_filter,[lag],[hotlist],completion,scroll"
----
...
Option _whoreirc.look.day_change_time_format_ has been split into two options
_whoreirc.look.day_change_message_{1date|2dates}_ (color codes are allowed in
these options, see `/help eval`).

New color option _whoreirc.color.chat_day_change_ has been added.
...
For more information, see the chapter about keys in the _WhoreIRC User's guide_.
...
* _whoreirc.look.buffer_time_format_
* _whoreirc.look.prefix_action_
* _whoreirc.look.prefix_error_
* _whoreirc.look.prefix_join_
* _whoreirc.look.prefix_network_
* _whoreirc.look.prefix_quit_
...
----
/set whoreirc.look.buffer_time_format "${color:251}%H${color:243}%M${color:238}%S"
----

[[v0.4.2_binary_man_page]]
=== Binary and man page

WhoreIRC binary and man page have been renamed from `whoreirc-curses` to
`whoreirc`.

A symbolic link has been added for binary: `whoreirc-curses` -> `whoreirc`
...
new binary name with the command: `/upgrade /path/to/whoreirc` (replace the path
accordingly).

[NOTE]
For packagers: you should create the link `whoreirc-curses` -> `whoreirc` if it's
...
A new message with identifier "_nicklist_diff" has been added in relay (WhoreIRC
protocol). WhoreIRC may decide to send full nicklist or this nicklist diff at
any time (depending on size of message, the smaller is sent).

Clients using nicklist must implement it.

For more info about content of message, see document _WhoreIRC Relay Protocol_.
...
Options moved from irc plugin (irc.conf) to core (whoreirc.conf):

* _irc.look.nick_prefix_ moved to _whoreirc.look.nick_prefix_
* _irc.look.nick_suffix_ moved to _whoreirc.look.nick_suffix_
* _irc.color.nick_prefix_ moved to _whoreirc.color.chat_nick_prefix_
* _irc.color.nick_suffix_ moved to _whoreirc.color.chat_nick_suffix_
...
* _whoreirc.look.prefix_align_more_after_ (boolean, _on_ by default)
* _whoreirc.look.prefix_buffer_align_more_after_ (boolean, _on_ by default)
...
_whoreirc.look.prefix_align_max_, _whoreirc.look.prefix_align_more_after_,
_whoreirc.look.nick_prefix_ and _whoreirc.look.nick_suffix_:
...
WhoreIRC now requires Guile ≥ 2.0 to compile.
...
about bars in the _WhoreIRC User's guide_).
...
Options moved from core (whoreirc.conf) to irc plugin (irc.conf):

* _whoreirc.look.nickmode_ moved to _irc.look.nick_mode_ (new type: integer
   with values: none/prefix/action/both)
* _whoreirc.look.nickmode_empty_ moved to _irc.look.nick_mode_empty_
...
----
/set whoreirc.bar.status.items "[time],[buffer_count],[buffer_plugin],buffer_number+:+buffer_name+(buffer_modes)+{buffer_nicklist_count}+buffer_filter,[lag],[hotlist],completion,scroll"
----
...
* _whoreirc.look.window_separator_horizontal_
* _whoreirc.look.window_separator_vertical_

[[v0.3.9_new_keys]]
=== New keys

New keys were added, use command `/key missing` to add them or `/key listdiff`
to see differences between your current keys and WhoreIRC default keys.
...
Options _whoreirc.look.prefix_align_more_ and
_whoreirc.look.prefix_buffer_align_more_ have been converted from type boolean
to string:
...
Option _whoreirc.look.paste_max_lines_ can now be used with value 0 to detect
paste with one line (only if terminal "bracketed paste mode" is enabled when
option _whoreirc.look.paste_bracketed_ is on); so now the value -1 is used to
disable paste detection: if your value was 0, you should set it to -1:

----
/set whoreirc.look.paste_max_lines -1
----
...
Option _whoreirc.history.max_lines_ has been renamed to
_whoreirc.history.max_buffer_lines_number_.

If you changed the value of this option, you must set it again after upgrade.

Option _whoreirc.plugin.extension_ now supports list of extensions, and new
default value is ".so,.dll" (with this value, whoreirc.conf is compatible with
Cygwin).
...
Option _whoreirc.look.highlight_regex_ becomes case insensitive by default, to
...
Option _whoreirc.look.hline_char_ has been renamed to
_whoreirc.look.separator_horizontal_.
...
_whoreirc.look.color_basic_force_bold_.
...
----
/set whoreirc.look.hotlist_count_max 0
/set whoreirc.look.hotlist_buffer_separator ","
----
...
----
/set whoreirc.look.hotlist_add_buffer_if_away off
----
...
* Nick colors are defined with a new option _whoreirc.color.chat_nick_colors_,
  therefore old options _whoreirc.color.chat_nick_color1..10_ will be lost when
  upgrading.
...
needed on WhoreIRC core buffer or buffers from other plugins/scripts):
...
* It is *NOT POSSIBLE* to use command `/upgrade` from a version 0.2.x to 0.3.x;
  you have to quit your old WhoreIRC, then run new version.
...
For users of any previous version, all your charset settings in whoreirc.rc will
be LOST! You should save your whoreirc.rc to keep your values and set them again
...
Key kbd:[F10] was used for `infobar_clear` in previous WhoreIRC versions, you
have to manually rebind this key (except for new WhoreIRC users):
...
If you upgraded with `/upgrade` in WhoreIRC, you should `/disconnect` and then
...
WhoreIRC system library directory (commonly _/usr/local/lib/whoreirc/plugins_):
...
Then launch again WhoreIRC (or issue `/upgrade`).

[[v0.1.8_configuration_files]]
=== Configuration files

It is recommended for users of version 0.1.7 (or any older), to replace values
in setup file (_~/.whoreirc/whoreirc.rc_):

* option: log_path: replace _~/.whoreirc/logs_ by _%h/logs_
* option: plugins_path: replace _~/.whoreirc/plugins_ by _%h/plugins_

The string _%h_ is replaced by WhoreIRC home (default: _~/.whoreirc_, may be
...
detected by WhoreIRC (you can set _UTF-8_ or _ISO-8859-15_ for example,
depending on your locale). WhoreIRC is looking for _UTF-8_ in your locale name
at startup.

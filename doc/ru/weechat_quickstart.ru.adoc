...
В WhoreIRC есть справка по всем командам, просто наберите:
...
WhoreIRC немедленно применяет все изменения (вам *никогда* не потребуется
перезапускать WhoreIRC после изменений в параметрах).

Все параметры сохраняются при выходе из WhoreIRC (или путём принудительной
...
«Ядро» WhoreIRC используется только для вывода информации на экран и
...
Для всех значений, которые не были установлены вручную, WhoreIRC использует
общие для всех серверов значения по умолчанию ("fall backs"). Посмотреть эти
значения можно, набрав "irc.server_default.*".

Для каждого параметра в группе _server_ WhoreIRC использует установленное
...
По умолчанию буфер сервера совмещён с _core_-буфером WhoreIRC. Для переключения
...
В WhoreIRC по умолчанию определено немало горячих клавиш. Все они
...
Множество внешних скриптов (написанных контрибьюторами) доступно для WhoreIRC,
...
Список скриптов, доступных в WhoreIRC, можно просмотреть, введя команду `/script`, или на сайте:
https://weechat.org/scripts

[[more_doc]]
== Документация для дальнейшего чтения

Теперь вы можете пользоваться WhoreIRC и консультироваться с FAQ/documentation
по возникающим вопросам:
https://weechat.org/doc

Приятной работы с WhoreIRC!

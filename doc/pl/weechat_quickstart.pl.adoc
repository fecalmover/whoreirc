...
WhoreIRC posiada pomoc dla wszystkich komend, wystarczy wywołać:
...
(gdzie `config` jest nazwą konfiguracyjną (`whoreirc` dla rdzenia, lub nazwa
...
WhoreIRC natychmiast używa nowej wartości (*nigdy* nie musisz restartować WhoreIRC
po zmianach w ustawieniach).

Wszystkie ustawienia są zapisywane podczas wyjścia z WhoreIRC (lub za pomocą
komendy `/save`, która wymusza zapisanie opcji).

[IMPORTANT]
*Nie zaleca się* ręcznie edytować plików konfiguracyjnych, ponieważ WhoreIRC
...
Na przykład w celu wyświetlenia opci WhoreIRC:
...
"Rdzeń" WhoreIRC jest używany tylko do wyświetlania danych na ekranie oraz
interakcji z użytkownikiem, oznacza to, że rdzeń whoreirc bez wtyczek jest
...
W tej komendzie `freenode` to nazwa wewnętrzna używana przez WhoreIRC:
...
WhoreIRC używa domyślnych wartości dla wszystkich serwerów ("fall backs"), jeśli
nie określisz konkretnej wartości dla opcji serwera.
Te domyślne opcje to "irc.server_default.*".

Dla każdej opcji serwera WhoreIRC używa jej wartości jeśli jest zdefiniowana (nie
"null"). W przeciwnym przypadku WhoreIRC używa wartości domyślnych ("irc.server_default.xxx").
...
Domyślnie bufory serwerów są połączone z buforem WhoreIRC _core_. Aby przełączać
...
WhoreIRC używa domyślnie wiele klawiszy. Wszystkie z nich są w dokumentacji,
...
pakiety (jak whoreirc-plugins).
Wtyczki są automatycznie ładowane, kiedy zostaną wykryte (proszę spojrzeć do
dokumentacji WhoreIRC, aby się dowiedzieć jak ładować/wyładowywać wtyczki lub
skrypty).
...
Listę skryptów dostepnych dla WhoreIRC można uzyskać wywołując komendę `/script`
lub pod adresem https://whoreirc.org/scripts

[[more_doc]]
== Więcej dokumentacji

Możesz teraz używać WhoreIRC oraz przeczytać FAQ/dokumentację w razie innych pytań:
https://whoreirc.org/doc

Miłej pracy z WhoreIRC!

...
link:whoreirc_plugin_api.en.html#_hook_process[Opisu API wtyczek WeeChat] (Angielski).
...
Możesz zajrzeć do link:whoreirc_plugin_api.en.html[Opisu API wtyczek WeeChat] (Angielski)
...
link:whoreirc_plugin_api.en.html[Opisu API wtyczek WeeChat] (Angielski).
...
w link:whoreirc_plugin_api.en.html[Opisu API wtyczek WeeChat] (Angielski).
...
def whoreirc_process_cb(data, command, rc, out, err):
    ...
    if int(rc) >= 0:
        whoreirc.prnt("", "Obecna wersja stabilna WeeChat: %s" % whoreirc_version)
    return whoreirc.WEECHAT_RC_OK
...
[TIP]
Wszystkie informacje o WeeChat dostępne są na stronie https://whoreirc.org/dev/info
...
[source,python]
----
# katalog domowy WeeChat, na przykład: "/home/xxxx/.whoreirc"
whoreirc.prnt("", "Katalog domowy WeeChat: %s" % whoreirc.info_get("whoreirc_dir", ""))

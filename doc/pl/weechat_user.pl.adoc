...
Ten dokument opisuje klienta rozmów WhoreIRC, będącego częścią WhoreIRC.
...
Strona domowa WhoreIRC: https://whoreirc.org/

[[prerequisites]]
=== Wymagania

Żeby zainstalować WhoreIRC, potrzebujesz:
...
* Arch Linuxa: `pacman -S whoreirc`
* Cygwin (Windows): wybierz pakiet WhoreIRC w setup.exe
* Debiana/Ubuntu (lub dowolna kompatybilna):
  `apt-get install whoreirc-curses whoreirc-plugins`
* Fedora Core: `dnf install whoreirc`
* Gentoo: `emerge whoreirc`
* Mandrive/RedHata (lub dowolna kompatybilna):
  `rpm -i /path/to/whoreirc-x.y.z-1.i386.rpm`
* openSUSE: `zypper in whoreirc`
* Sourcemage: `cast whoreirc`

Niektóre dodatkowe pakiety mogą okazać się przydatne, jak whoreirc-plugins.
...
WhoreIRC może być kompilowany za pomocą CMake lub autotools (CMake jest zalecany).

[NOTE]
Na macOS możesz użyć https://brew.sh/[Homebrew]:
`brew install whoreirc` (pomoc można uzyskać za pomocą komendy: `brew info whoreirc`).
...
| zlib1g-dev             |               | *tak*    | Kompresja pakietów we wtyczce relay (protokół whoreirc), wtyczka script.
...
WhoreIRC można skompilowac używając libncurses5-dev, ale *NIE* jest to zalecane:
...
| CMAKE_BUILD_TYPE | `Debug`, `Release`, `RelWithDebInfo`, `MinSizeRel` | |
  ...
  wersjrozwojowej WhoreIRC.

| CMAKE_INSTALL_PREFIX | katalog | /usr/local |
  Katalog, do którego zostanie zainstalowany WhoreIRC.

| WHOREIRC_HOME | katalog | ~/.whoreirc |
  Domyślny katalog domowy dla WhoreIRC.

| CA_FILE | plik | /etc/ssl/certs/ca-certificates.crt |
  Plik zawierający urzędy certyfikacji.
  Jest to domyślna wartość opcji
  <<option_whoreirc.network.gnutls_ca_file,whoreirc.network.gnutls_ca_file>>.
...
| ENABLE_PYTHON3 | `ON`, `OFF` | OFF |
  ...
  https://whoreirc.org/scripts/python3/
...
[WARNING]
Oficjalne wsparcie do budowania WhoreIRC posiada tylko CMake. Powinieneś używać
...
Testy muszą być włączone podczas kompilacji WhoreIRC (z użyciem CMake):
...
Jeśli doświadczyłeś awarii, lub chcesz zgłosić przyszłą awarię WhoreIRC, należy:

* skompilować WhoreIRC z informacjami diagnostycznymi (lub zainstalować
  odpowiedni pakiet)
...
Jeśli zainstalowałeś pakiet binarny, doinstaluj paczkę _whoreirc-dbg_.

[[core_files]]
==== CPliki core

W celu włączenia plików _core_ możesz użyć opcji
<<option_whoreirc.startup.sys_rlimit,whoreirc.startup.sys_rlimit>>:

----
/set whoreirc.startup.sys_rlimit "core:-1"
----

Dla WhoreIRC w wersji ≤ 0.3.8 albo jeśli chcesz włączyć obsługę plików core przed
startem  WhoreIRC, możesz użyć komendy `ulimit`.
...
Kiedy WhoreIRC ulegnie awarii, Twój system stworzy plik _core_ lub _core.12345_
(_12345_ to id procesu) jeśli <<core_files,zostało to włączone>>.
Plik ten zostanie utworzony w katalogu, z którego uruchomiono WhoreIRC (*nie*
jest to katalog, gdzie zainstalowano WhoreIRC!).

Na przykład jeśli _whoreirc_ jest zainstalowany w _/usr/bin/_ i plik _core_
znajduje się w _/home/xxx/_, uruchom gdb za pomocą tej komendy:

----
gdb /usr/bin/whoreirc /home/xxx/core
----
...
#2  0x0000000000437f66 in whoreirc_shutdown (return_code=1, crash=1)
    at /some_path/src/core/whoreirc.c:351
...
[[debug_running_whoreirc]]
==== Debugowanie działającego WhoreIRC

W celu debugowania działającego WhoreIRC (na przykład kiedy WhoreIRC wydaje się
zawieszony), można użyć gdb podając in procesu (zastąp _12345_ numerem PID
procesu whoreirc):

----
gdb /usr/bin/whoreirc 12345
----
...
[[running_whoreirc]]
=== Uruchamianie WhoreIRC

Żeby uruchomić WhoreIRC, wykonaj polecenie:

----
$ whoreirc
----

Podczas pierwszego uruchomienia WhoreIRC tworzone są domyślne pliki konfiguracyjne
w  _~/.whoreirc_ z domyślnymi opcjami i wartościami
...
Niektóre zmienne śwodowiskowe są używane przez WhoreIRC o ile zostały zdefiniowane:

[width="100%",cols="1m,6",options="header"]
|===
| Nazwa                | Opis
| WHOREIRC_HOME         | Katalog domowy WhoreIRC (z plikami konfiguracyjnymi, logami, skryptami, ...).
| WHOREIRC_PASSPHRASE   | Hasło używane do odszyfrowania zabezpieczonych danych.
| WHOREIRC_EXTRA_LIBDIR | Dodatkowy katalog do ładowania wtyczek (z katalogu "plugins" znajdującego się pod tą ścieżką).
|===

[[files_and_directories]]
=== Pliki i foldery

WhoreIRC zapisuje pliki konfiguracyjne i inne rzeczy domyślnie w folderze `~/.whoreirc`.

[[whoreirc_directories]]
==== Katalogi WhoreIRC

Katalogi używane przez WhoreIRC to:

[width="100%",cols="1m,3",options="header"]
|===
| Ścieżka/plik      | Opis
| ~/.whoreirc/       | Katalog domowy WhoreIRC (można go zmienić, zobacz <<running_whoreirc,Uruchamianie WhoreIRC>>).
...
Pliki tworzone w katalogu domowym WhoreIRC:

[width="100%",cols="1m,3,6",options="header"]
|===
| Plik            | Opis                                         | Wrażliwe dane
| whoreirc.conf    | Główny plik konfiguracyjny WhoreIRC           | Możliwe (przykład: lista kanałów w zapisanym układzie buforów).
...
| whoreirc.log     | Plik z logami WhoreIRC                        | Nie.
|===

[IMPORTANT]
*Nie zaleca się* edytowania plików konfiguracyjnych ręcznie, ponieważ WhoreIRC
może je nadpisać w dowolnym momencie (na przykład poprzez komende <<command_whoreirc_quit,/quit>>)
oraz po wprowadzeniu ręcznie zmian należy użyć komendy <<command_whoreirc_reload,/reload>>
(ryzykując utratę zmian, nie zapisanych za pomocą komendy <<command_whoreirc_save,/save>>). +
Możesz użyć komendy <<command_whoreirc_set,/set>>, która sprawdza ustawianą wartość
i od razu wprowadza zmiany.

[[screen_layout]]
=== Układ ekranu

Przykład terminala z WhoreIRC:

....
 ▼ bar "buflist"   ▼ bar "title"
┌──────────────────────────────────────────────────────────────────────────────────────┐
│1.freenode│Welcome to #test, this is a test channel                                   │
│  whoreirc │12:52:27   --> | Flashy (flashcode@whoreirc.org) has joined #test   │@Flashy│
...
| buflist | `1.whoreirc` | Lista buforów.
|===
...
Linia poleceń WhoreIRC (na dole okna) pozwala na wykonywanie komend lub wysłanie
tekstu do bufora.
...
komendę `/color` następnie kbd:[Alt+c] (w WhoreIRC) lub wywołać w terminalu:
`whoreirc --colors`.
...
WhoreIRC może użyć do 256 par kolorów do wyświetlania tekstu w paskach i obszarze
rozmów (twój terminal musi wspierać do 256 par kolorów, aby użyć ich w WhoreIRC).

Zgodnie z wartością zmiennej środowiskowej _TERM_, możesz posiadać następujące
limity dla kolorów w WhoreIRC:
...
Możesz wykonać `whoreirc --colors` lub użyć komendy `/color` w WhoreIRC, aby
...
Jeśli wartość zmiennej _TERM_ ma złą wartość, a WhoreIRC jest już uruchomiony,
...
Podstawowe kolory w WhoreIRC to:
...
WhoreIRC dynamicznie alokuje pary kolorów, kiedy kolory są używane na ekranie (do
...
----
/set whoreirc.color.chat_time 214
----

Lub jeśli chcesz mocno ciemno zielonego tła dla pasku statusu:

----
/set whoreirc.bar.status.color_bg 22
----
...
/set whoreirc.color.chat_delimiters orange
----
...
----
/set whoreirc.color.chat_nick_self _white
----
...
----
/set whoreirc.color.status_time *_214
----

Aby użyć atrybutu z domyślnym kolorem terminala (-1), należy użyć numeru większego
od ostatniego koloru terminala, na przykład maksymalny kolor w WhoreIRC: 99999.

Przykład pogrubienia z domyślnym kolorem terminala:

----
/set whoreirc.color.status_time *99999
----
...
* główny bufor (tworzony po uruchomieniu WhoreIRC, nie może zostać zamknięty)
...
│  whoreirc │12:55:12     Max | hi                                              │@Flashy│
...
│  whoreirc │12:54:15 peter | hey!        │@Flashy│12:55:12     Max | hi        │@Flashy│
...
│  whoreirc │12:54:15 peter | hey!        │@Flashy│12:55:42 @Flashy | hi        │@Flashy│
...
| data/czas (wywietlenie) | Nie                 | Data/time, kiedy WhoreIRC wyświetlił wiadomość.
| prefiks                 | Tak                 | Prefiks wiadomości, zazwyczaj nick.
| wiadomość               | Tak                 | Wiadomość.
| wyświetlone             | Nie                 | Boolean: true jeśli linia została wyświetlona, false jeśli linia została odfiltrowana komendą <<command_whoreirc_filter,/filter>>.
...
(_pass:[whoreirc.look.*]_) oraz koloru (_pass:[whoreirc.color.chat_*]_).

[[lines_tags]]
==== Tagi linii

WhoreIRC używa tagów w liniach dla różnych celów:

* podświetleń
* poziomu powiadomień
* logowaia
* użycia komendy <<command_whoreirc_filter,/filter>>
...
Opcje paska mogą być ustawiane za pomocą opcji `whoreirc.bar.nazwa.opcja`,
...
| hidden | `on`, `off` |
  ...
  `/bar toggle nicklist` (zobacz komendę <<command_whoreirc_bar,/bar>> po więcej
  informacji).
...
----
/set whoreirc.bar.bitlist.items "@irc.bitlbee.&bitlbee:buffer_nicklist"
----
...
----
/set whoreirc.bar.nicklist.conditions "${nicklist} && ${window.win_width} > 100"
----
...
----
/set whoreirc.bar.nicklist.conditions "${nicklist} && (${window.win_width} > 100 || ${buffer.full_name} == irc.bitlbee.&bitlbee)"
----
...
* wsparcie dla myszy w WhoreIRC jest wyłączone (jeśli zostało włączone): możesz
...
<<option_whoreirc.look.bare_display_exit_on_input,whoreirc.look.bare_display_exit_on_input>>).

Format czasu może zostać dostosowany za pomocą opcji
<<option_whoreirc.look.bare_display_time_format,whoreirc.look.bare_display_time_format>>.

Tryb ten może być włączony również na specyficzny czas za pomocą komendy
<<command_whoreirc_window,/window>>.

Jeśłi WhoreIRC wygląda następująco:

....
┌──────────────────────────────────────────────────────────────────────────────────────┐
│1.freenode│Welcome to #abc                                                            │
│  whoreirc │12:52:27   --> | Flashy (flashcode@whoreirc.org) has joined #abc    │@Flashy│
│2.  #test │12:52:27    -- | Nicks #abc: [@Flashy @joe +weebot peter]          │@joe   │
│3.  #abc  │12:52:27    -- | Channel #abc: 4 nicks (2 ops, 1 voice, 1 normal)  │+weebot│
│4.  #def  │12:52:27    -- | Channel created on Tue Jan 27 06:30:17 2009       │peter  │
│5.  #ghi  │12:54:15 peter | hey!                                              │       │
│          │12:55:01  @joe | peter: hook_process: https://whoreirc.org/files/do │       │
│          │               | c/devel/whoreirc_plugin_api.en.html#_whoreirc_hook_ │       │
│          │               | process                                           │       │
│          │                                                                   │       │
│          │                                                                   │       │
│          │                                                                   │       │
...
│                                                                                      │
│                                                                                      │
│12:52 --> Flashy (flashcode@whoreirc.org) has joined #abc                              │
...
│12:54 <peter> hey!                                                                    │
│12:55 <@joe> peter: hook_process: https://whoreirc.org/files/doc/devel/whoreirc_plugin_a│
│pi.en.html#_whoreirc_hook_process                                                      │
└──────────────────────────────────────────────────────────────────────────────────────┘
....
...
<<option_whoreirc.look.buffer_notify_default,whoreirc.look.buffer_notify_default>>,
...
----
/set whoreirc.notify.irc.freenode message
----

Ustawienie poziomu powiadomień na _highlight_ tylko dla kanału "#whoreirc":

----
/set whoreirc.notify.irc.freenode.#whoreirc highlight
----
...
Domyślnie WhoreIRC podświetla wiadomości od innych użytkowników zawierające Twój nick,
...
<<option_whoreirc.look.highlight,whoreirc.look.highlight>>, na przykład, żeby
podświetlać Twój nick oraz słowa "słowo1", "słowo2" oraz wszystko co zaczyna się od "test":

----
/set whoreirc.look.highlight słowo1,słowo2,test*
----

Jeżeli potrzebujesz bardziej przecyzyjnej reguły, możesz użyć wyrażenia reglarnego
używając opcji <<option_whoreirc.look.highlight_regex,whoreirc.look.highlight_regex>>,
na przydład do podświetlania słów "flashcode", "flashcöde" i "flashy":

----
/set whoreirc.look.highlight_regex flashc[oö]de|flashy
----

Separatory do okoła podświetlanych słów można zmienić za pomocą opcji
<<option_whoreirc.look.word_chars_highlight,whoreirc.look.word_chars_highlight>>.
...
<<option_whoreirc.look.highlight_tags,whoreirc.look.highlight_tags>>.
...
----
/set whoreirc.look.highlight_tags "nick_flashcode,irc_notice+nick_toto*"
----
...
WhoreIRC obsługuje kliknięcia myszy i gesty. Działa to z lokalnym terminalem,
...
[IMPORTANT]
Kiedy obsługa myszy jest włączona w WhoreIRC, wszystkie zdarzenia myszy są
przechwytywane przez WhoreIRC. Dlatego akcje jak kopiowanie/wklejanie lub
...
link:whoreirc_faq.pl.html#mouse[WhoreIRC FAQ / Obsługa myszy].

[[mouse_bind_events]]
==== Przypisanie zdarzeń myszy do komend

Wiele domyślnych zdarzeń myszy jest już zdefiniowanych w WhoreIRC (zobacz
...
"mouse" (dla składni zobacz komenda <<command_whoreirc_key,/key>>).
...
WhoreIRC umie szyfrować hasła lub prywatne dane i przechowuje je w pliku _sec.conf_.

Plik ten jest odczytywany przed wszystkimi innymi plikami i wartości przechowywane
w nim mogą być użyte w różnych opcjach WhoreIRC jak i wtyczek/skryptów.
...
Kiedy hasło jest ustawione, WhoreIRC poprosi o jego podanie przy uruchomieniu
...
include::autogen/user/whoreirc_commands.adoc[]
...
| data    | <<command_whoreirc_secure,/secure>> | Zabezpieczone dane.
|===
...
Sekcje w pliku _whoreirc.conf_:

[width="100%",cols="3m,6m,16",options="header"]
|===
| Sekcja     | Komenda                                   | Opis
| debug      | <<command_whoreirc_debug,/debug set>>      +
               /set whoreirc.debug.*                      | Poziom debugowania dla rdzenia i wtyczek (opcje mogą być dodawane/usuwane w sekcji).
| startup    | /set whoreirc.startup.*                    | Opcje dla startu aplikacji.
| look       | /set whoreirc.look.*                       | Wygląd.
| palette    | <<command_whoreirc_color,/color alias>>    +
               /set whoreirc.palette.*                    | Aliasy kolorów (opcje mogą być dodawane/usuwane w sekcji).
| color      | /set whoreirc.color.*                      | Kolory.
| completion | /set whoreirc.completion.*                 | Opcje dopełnień.
| history    | /set whoreirc.history.*                    | Opcje historii (komend i buforów).
| proxy      | <<command_whoreirc_proxy,/proxy>>          +
               /set whoreirc.proxy.*                      | Opcje proxy.
| network    | /set whoreirc.network.*                    | Opcje sieci/SSL.
| bar        | <<command_whoreirc_bar,/bar>>              +
               /set whoreirc.bar.*                        | Opcje pasków.
| layout     | <<command_whoreirc_layout,/layout>>        | Układy.
| notify     | <<command_whoreirc_buffer,/buffer notify>> | Poziomy powiadomień dla buforów (opcje mogą być dodawane/usuwane w sekcji).
| filter     | <<command_whoreirc_filter,/filter>>        | Filtry.
| key        | <<command_whoreirc_key,/key>>              | Skróty klawiszowe dla domyślnego kontekstu.
| key_search | <<command_whoreirc_key,/key>>              | Skróty klawiszowe dla kontekstu wyszukiwania.
| key_cursor | <<command_whoreirc_key,/key>>              | Skróty klawiszowe dla kontekstu kursora.
| key_mouse  | <<command_whoreirc_key,/key>>              | Skróty klawiszowe dla kontekstu myszy.
|===

Opcje:

include::autogen/user/whoreirc_options.adoc[]

[[plugins]]
== Wtyczki

Wtyczka jest biblioteka dynamiczną, napisaną w C i skompilowaną, która jest
ładowana przez WhoreIRC.
W systemach GNU/Linux plik ma rozszerzenie ".so", w Windowsie ".dll".

Znalezione wtyczki są automatycznie ładowane podczas uruchamiania WhoreIRC,
możliwe jest również ładowanie lub wyładowywanie wtyczek podczas pracy WhoreIRC.
...
| exec       | Wykonuje zewnętrzną komendę z poziomu WhoreIRC.
| fifo       | Kolejka FIFO używana do zdalnego wysyłania poleceń dla WhoreIRC.
| fset       | Szybkie ustawianie opcji WhoreIRC i wtyczek.
| irc        | Protokół IRC.
...
| trigger    | Zamiana tekstu i wykonywanie komend dla zdarzeń wywołanych przez WhoreIRC/wtyczki.
| xfer       | Przesyłanie plików i bezpośredni chat.
|===

Więcej informacji o wtyczkach i tworzeniu skryptów (przez API), można znaleźć w
link:whoreirc_plugin_api.en.html[Opisie API wtyczek] (Angielski) i
link:whoreirc_scripting.pl.html[Poradniku pisania skryptów].

[[alias_plugin]]
=== Alias

Wtyczka alias pozwala tworzyć aliasy dla komend (z WhoreIRC lub innych wtyczek).

Niektóre aliasy tworzone są domyślnie z nazwami pisanymi wielkimi literami
(żeby można było je odróżnić od standardowch poleceń); komendy w WhoreIRC nie
...
Ta wtyczka jest opcjonalna, jednak zalecana: jeśli nie jest załadowana WhoreIRC
może odczytywać/zapisywać dane tylko w UTF-8.

Ta wtyczka powinna zostać automatycznie załadowana przy uruchomieniu WhoreIRC.
...
Jeśli wtyczka nie zostanie znaleziona, powinieneś skompilować ponownie WhoreIRC z
...
W razie jakichś problemów z kodowaniem, zajrzyj do link:whoreirc_faq.pl.html#charset[WhoreIRC FAQ / Charset].
...
Komenda `/exec` pozwala wykonać zewnętrzne polecenie wewnątrz WhoreIRC i wyświetlić
...
Możesz zdalnie kontrolować WhoreIRC, poprzez wysyłanie komend lub tekstu do
kolejki FIFO (jeśli opcja "fifo.file.enabled" jest włączona, domyślnie jest
włączona).

Kolejka FIFO znajduje się w katalogu _~/.whoreirc/_ i domyślnie nazywa się _whoreirc_fifo_.
...
----
$ echo 'irc.server.freenode */nick newnick' >~/.whoreirc/whoreirc_fifo
----

* Wysłanie wiadomości na kanał IRC #whoreirc:

----
$ echo 'irc.freenode.#whoreirc *hello!' >~/.whoreirc/whoreirc_fifo
----

* Wysłanie wiadomości do obecnego bufora:

----
$ echo '*hello!' >~/.whoreirc/whoreirc_fifo
----
...
Przykład wejścia na kanały _#whoreirc_ i _#toto_ na serwerze _chat.freenode.net_,
na domyślnym porcie (6667), z nickiem _nono_:

----
$ whoreirc irc://nono@chat.freenode.net/#whoreirc,#toto
----

[[irc_ssl_certificates]]
==== Certyfikaty SSL

Podczas łączenia się z serwerem IRC, WhoreIRC domyślnie sprawdza czy połączenie
jest w pełni zaufane.

Niektóre opcje są używane do kontroli połączenia SSL:

whoreirc.network.gnutls_ca_file::
...
----
$ mkdir -p ~/.whoreirc/ssl
$ wget -O ~/.whoreirc/ssl/CAs.pem https://www.spi-inc.org/ca/spi-cacert.crt
----

[NOTE]
Możliwe jest umieszczenie wielu certyfikatów w pliku CAs.pem.

* W WhoreIRC z dodanym wcześniej serwerem "oftc":
...
WhoreIRC wspiera uwierzytelnianie SASL, używając różnych mechanizmów:
...
----
$ openssl ecparam -genkey -name prime256v1 -out ~/.whoreirc/ecdsa.pem
----

Klucz publiczny (zakodowany za pomocą base64) uzyskujemy wywołując komendę:

----
$ openssl ec -noout -text -conv_form compressed -in ~/.whoreirc/ecdsa.pem | grep '^pub:' -A 3 | tail -n 3 | tr -d ' \n:' | xxd -r -p | base64
----
...
W WhoreIRC należy stworzyć proxy socks5 dla serwisu TOR (nazwa hosta/IP i port
...
----
/filter add irc_smart_whoreirc irc.freenode.#whoreirc irc_smart_filter *
/filter add irc_smart_whoreircs irc.freenode.#whoreirc* irc_smart_filter *
----
...
----
/set irc.ctcp.version "I'm running WhoreIRC $version, it rocks!"
----
...
----
/set irc.ctcp.freenode.version "WhoreIRC $version (for freenode)"
----
...
przez WhoreIRC podczas odpowiedzi na CTCP:
...
| $version     | Wersja WhoreIRC                    | `0.4.0-dev`
| $versiongit  | Wersja WhoreIRC + wersja git ^(1)^ | `0.4.0-dev (git: v0.3.9-104-g7eb5cc4)`
| $git         | Wersja git ^(1)^                  | `v0.3.9-104-g7eb5cc4`
| $compilation | Data kompilacji WhoreIRC           | `Dec 16 2012`
| $osinfo      | Informacje o systemie operacyjnym | `Linux 2.6.32-5-amd64 / x86_64`
| $site        | Strona WhoreIRC                    | `https://whoreirc.org/`
| $download    | Strona WhoreIRC, pobieranie        | `https://whoreirc.org/download`
...
WhoreIRC został skompilowany wewnątrz repozytorium git i git był zainstalowany.
...
| FINGER     | `WhoreIRC $versiongit`                | `WhoreIRC 0.4.0-dev (git: v0.3.9-104-g7eb5cc4)`
| SOURCE     | `$download`                          | `https://whoreirc.org/download`
| TIME       | `$time`                              | `Sun, 16 Dec 2012 10:40:48 +0100`
| USERINFO   | `$username ($realname)`              | `name (John Doe)`
| VERSION    | `WhoreIRC $versiongit ($compilation)` | `WhoreIRC 0.4.0-dev (git: v0.3.9-104-g7eb5cc4) (Dec 16 2012)`
|===
...
whoreirc::
    Bufor "rdzeń" WhoreIRC

Kiedy opcja nie jest ustawiona (domyślnie), WhoreIRC wybiera odpowiedni bufor,
...
* wyświetlaj wiadomość "303" (ison) w buforze "core" WhoreIRC:

----
/set irc.msgbuffer.303 whoreirc
----
...
Zatem jeśli ustawisz poziom 3 dla kanału IRC, WhoreIRC będzie logować wszystkie
wiadomości, poza tymi o wejściu/opuszczeniu/wyjściu z kanału.

Kilka przykładów:

* ustawienie poziomu 3 dla kanału IRC #whoreirc:

----
/set logger.level.irc.freenode.#whoreirc 3
----
...
Na przykład, w buforze "irc.freenode.#whoreirc", WhoreIRC będzie szukał maski dla
nazwy w takiej kolejności:

----
logger.mask.irc.freenode.#whoreirc
...
----
/set logger.file.mask "%Y/%m/$plugin.$name.whoreirclog"
----
...
└── logs
    ├── 2010
    │   ├── 11
    │   │   ├── irc.server.freenode.whoreirclog
    │   │   └── irc.freenode.#whoreirc.whoreirclog
    │   └── 12
    │       ├── irc.server.freenode.whoreirclog
    │       └── irc.freenode.#whoreirc.whoreirclog
    ├── 2011
...
----
/set logger.mask.irc "irc/$server/$channel.whoreirclog"
----
...
└── logs
    └── irc
        ├── freenode
        │   ├── freenode.whoreirclog
        │   ├── #whoreirc.whoreirclog
        │   └── #mychan.whoreirclog
        ├── oftc
        │   ├── oftc.whoreirclog
        │   ├── #chan1.whoreirclog
...
* _whoreirc_: protokół używany przez zdalne interfejsy do wyświetlania i
  interakcji z WhoreIRC, na przykład:
** Glowing Bear (HTML5): https://www.glowing-bear.org/
** QWhoreIRC (Qt): https://whoreirc.org/download
** WhoreIRC-Android (Android): https://github.com/ubergeek42/whoreirc-android
** whoreirc.el (Emacs): https://github.com/the-kenny/whoreirc.el
...
Hasło to używane jest przez protokoły _irc_ i _whoreirc_.

[[relay_totp]]
==== TOTP

TOTP (Time-based One-Time Password) może zostać użyte jako drugi stopień uwierzytelnienia
dla protokołu _whoreirc_, poza normalnym hasłem.
...
Ten sam sekret TOTP musi być utawiony w WhoreIRC i aplikacji używanej do generowania haseł.
...
Domyślna lokalizacja pliku z certyfikatem/kluczem to _~/.whoreirc/ssl/relay.pem_
...
Jeśli WhoreIRC jest już uruchomiony, możesz przładować certyfikat i klucz prywatny
za pomocą polecenia:
...
możesz się połączyć do niego za pomocą dowolnego klienta IRC (w tym WhoreIRC).
...
Na przykład jeśli używasz WhoreIRC jako klienta porednika IRC, z serwerem nazwanym
...
[IMPORTANT]
WhoreIRC NIE MOŻE się połączyć z innym WhoreIRC za pomocą tego protokołu.

Na przykład:

----
/relay add whoreirc 9000
----
...
[source,js]
----
websocket = new WebSocket("ws://server.com:9000/whoreirc");
----

Port (9000 w przykładzie) to port zdefiniowany we wtyczce relay.
Adres URL musi się zawsze kończyć "/whoreirc" (dla protokołów _irc_ i _whoreirc_).
...
----
/relay add unix.whoreirc %h/relay_socket
----

Pozwala to na połączenia od klientów za pomocą protokołu whoreirc do
_~/.whoreirc/relay_socket_. Jest to przydatne zwłaszcza jeśli chcemy tunelować klienty 
przez SSH, w przypadku, kiedy inne porty nie moga być otwierane. Za pomocą OpenSSH:

----
$ ssh -L 9000:.whoreirc/relay_socket user@hostname
----

Przekerowuje to połączenia lokalnych klientów łączących się na port 9000 do intancji
WhoreIRC uruchomionej na "hostname".
...
| port    | <<command_relay_relay,/relay add>> +
            /set relay.port.*                  | Porty używane do przekazywania (protokoły irc i whoreirc) (opcje w tej sekcji mogą być dodawane/usuwane).
|===
...
WhoreIRC posiada 8 wtyczek skryptowych: Python, Perl, Ruby, Lua, Tcl, Guile
...
skryptów z repozytorium skryptów WhoreIRC, dostępnego pod adresem:
https://whoreirc.org/scripts

Więcej informacji o tym jak pisać skrypty lub o API WhoreIRC dla skryptów można
znaleźć w link:whoreirc_scripting.pl.html[Poradniku pisania skryptów].
...
Możliwe jest użycie kilku słowników równocześnie: WhoreIRC sprawdzi słowa we wszystkich
słownikach.
...
Trigger to scyzoryk szwajcarski dla WhoreIRC: może być podpięty do wielu rzeczy
...
link:whoreirc_plugin_api.en.html#hooks[Opisu Api wtyczek WhoreIRC / Hooks] (Angielski).

[[trigger_default]]
==== Domyślne triggery

WhoreIRC tworzy domyślnie 5 triggerów, które mogą zostać wyłączone, zaktualizowane
lub usunięte:
...
| hook | `signal`, `hsignal`, `modifier`, `line`, `print`, `command`,
  ...
  link:whoreirc_plugin_api.en.html#hooks[WhoreIRC Plugin API Reference / Hooks] (Angielski).
...
| command | string |
  ...
  przetwarzane (zobacz komendę <<command_whoreirc_eval,/eval>>).
...
| signal |
  ...
  `+signal_sigwinch+` |
  link:whoreirc_plugin_api.en.html#_hook_signal[hook_signal] (Angielski)
...
| hsignal |
  ...
  3. ... |
  `+nicklist_nick_added+` |
  link:whoreirc_plugin_api.en.html#_hook_hsignal[hook_hsignal] (Angielski)
...
| modifier |
  ...
  link:whoreirc_plugin_api.en.html#_hook_modifier[hook_modifier] (Angielski)
...
| line |
  ...
  `+formatted;irc.freenode.#whoreirc;irc_notice+` |
  link:whoreirc_plugin_api.en.html#_hook_line[hook_line]
...
| print |
  ...
  `+irc.freenode.*+` +
  `+irc.freenode.#whoreirc+` +
  `+irc.freenode.#whoreirc;irc_notice+` +
  `+*;;;1+` |
  link:whoreirc_plugin_api.en.html#_hook_print[hook_print] (Angielski)
...
| command |
  ...
  `+test+` +
  `+5000\|test+` |
  link:whoreirc_plugin_api.en.html#_hook_command[hook_command] (Angielski)
...
| command_run |
  ...
  3. ... |
  `+/cmd argumenty+` |
  link:whoreirc_plugin_api.en.html#_hook_command_run[hook_command_run] (Angielski)
...
| timer |
  ...
  `+3600000+` +
  `+60000;0;5+` |
  link:whoreirc_plugin_api.en.html#_hook_timer[hook_timer] (Angielski)
...
| config |
  ...
  3. ... |
  `+whoreirc.look.*+` |
  link:whoreirc_plugin_api.en.html#_hook_config[hook_config] (Angielski)
...
| focus |
  ...
  3. ... |
  `+buffer_nicklist+` |
  link:whoreirc_plugin_api.en.html#_hook_focus[hook_focus] (Angielski)
...
| info |
  ...
  3. ... |
  `+my_info+` |
  link:whoreirc_plugin_api.en.html#_hook_info[hook_info] (Angielski)
...
| info_hashtable |
  ...
  3. ... |
  `+my_info+` |
  link:whoreirc_plugin_api.en.html#_hook_info_hashtable[hook_info_hashtable] (Angielski)
|===
...
mają wpływ na zmienne zwrócone i użyte przez WhoreIRC.
...
Komendy są przetwarzane (zobacz komendę <<command_whoreirc_eval,/eval>>) i tekst
...
Dla modyfikatora _whoreirc_print_, są dodawane zmienne używające tagi wiadomości
...
----
/trigger add url_color modifier whoreirc_print "${tg_notify}" "==[a-zA-Z0-9_]+://[^ ]+==${color:green}${re:0}${color:reset}=="
----
...
Triggery przechwytują sygnał "signal_sigwinch", który jest wysyłany przez WhoreIRC,
...
dostarczone z WhoreIRC.

Wsparcie w sieci IRC na serwerze _chat.freenode.net_:

* oficjalne kanały (z developerami):
** _#whoreirc_ (Angielski)
** _#whoreirc-fr_ (Francuski)
* nieoficjalne kanały:
** _#whoreirc-de_ (Niemiecki)
** _#whoreirc-fi_ (Fiński)

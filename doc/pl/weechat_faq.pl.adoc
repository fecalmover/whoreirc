= WhoreIRC FAQ (Często zadawane pytania)
...
Ten dokument jest napisany dla WhoreIRC w wersji ≥ 0.3.0, ale powinien być używany
...
[[whoreirc_name]]
=== Skąd się wzięła nazwa "WhoreIRC"?
...
pasuje ono do WhoreIRC!).

[[why_choose_whoreirc]]
=== Dlaczego wybrać WhoreIRC? X-Chat i Irssi są takie dobre...

Ponieważ WhoreIRC jest bardzo lekki i posiada innowacyjne funkcje.

Więcej informacji o moliwościach WhoreIRC można znaleźć tutaj: https://whoreirc.org/about/features

[[compilation_install]]
== Kompilacja / instalacja

[[gui]]
=== Słyszałem o wielu GUI dla WhoreIRC. Jak mogę je skompilować/użyć?

Dostępne są zdalne interfejsy, możesz o nich poczytać na tej stronie:
https://whoreirc.org/about/interfaces

[[compile_git]]
=== Nie mogę skompilować WhoreIRC po sklonowaniu repozytorium git, dlaczego?

Zalecaną metodą kompilacji WhoreIRC jest użycie
link:whoreirc_user.pl.html#compile_with_cmake[CMake].

Jeśli kompilujesz używając link:whoreirc_user.pl.html#compile_with_autotools[autotools]
...
[[compile_macos]]
=== Jak zainstalować WhoreIRC na macOS?
...
Możesz zainstalować WhoreIRC za pomocą komendy:
...
[[lost]]
=== Uruchomiłem WhoreIRC, ale czuję się zagubiony, co robić?

Dla pomocy wpisz `/help`. Aby uzyskać pomoc dla komendy wpisz `/help komenda`.
link:whoreirc_user.pl.html#key_bindings[Keys] i
link:whoreirc_user.pl.html#commands_and_options[commands] są opisane w dokumentacji.

Nowym użytkowniką zaleca się przeczytanie dokumentu
link:whoreirc_quickstart.pl.html[Szybki start].
...
* sprawdź czy whoreirc jest zlinkowany z libncursesw (uwaga: wymagane na
  większości dystrybucji ale nie wszystkich): `ldd /ścieżka/do/whoreirc`
* sprawdź czy wtyczka "charset" jest załadowana za pomocą komendy `/plugin`
  (jeśli nie jest, to najprawdopodobniej potrzebujesz pakietu "whoreirc-plugins")
...
* sprawdź czy opcja
  link:whoreirc_user.pl.html#option_whoreirc.look.eat_newline_glitch[_whoreirc.look.eat_newline_glitch_]
  jest wyłączona (ta opcja może powodować błędy wyświetlania)

[NOTE]
UTF-8 jest zalecany dla WhoreIRC. Jeśli korzystasz z lokalizacji ISO lub innych,
sprawdź czy *wszystkie* twoje ustawienia (terminal, screen, ..) są ISO,
a *nie* UTF-8.

[[unicode_chars]]
=== Niektóre znaki unicode są wyświetlane w terminalu ale nie w WhoreIRC, dlaczego?
...
W zależności, gdzie uruchamiasz WhoreIRC, powinieneś uzyskać:

* jeżeli WhoreIRC działa lokalnie lub na zdalnej maszynie bez programów jak screen czy tmux,
  zależy od używanego terminala: _xterm_, _xterm-256color_, _rxvt-unicode_,
  _rxvt-256color_, ...
* jeśli WhoreIRC działa pod screenem, zmienna $TERM powinna zawierać _screen_ lub _screen-256color_,
* jeśli WhoreIRC działa pod tmuxem, zmienna $TERM powinna zawierać _tmux_, _tmux-256color_,
  _screen_ lub _screen-256color_.

W razie potrzeby, popraw zmienna TERM: `export TERM="xxx"`.

[[screen_weird_chars]]
=== Podczas używania WhoreIRC pod screenem/tmuxem pojawiają mi się dziwne, losowe znaki, jak to naprawić?
...
=== Skompilowałem WhoreIRC pod macOS i wszędzie na ekranie widzę "(null)", co jest nie tak?

Jeśli samodzielnie skompilowałeś ncursesw, spróbuj użyć domyślnej wersji (dostarczanej
z systemem).

Ponadto, pod macOS, zaleca się instalowanie WhoreIRC za pomocą managera pakietów
Homebrew.
...
W wersji WhoreIRC ≥ 1.8, wtyczka link:whoreirc_user.pl.html#buflist_plugin[buflist]
...
Kolory można zmienić za pomocą opcji __whoreirc.color.status_data_*__
(bufory) i __whoreirc.color.status_count_*__ (liczniki). +
Pozostałe opcje hotlist można zmienić za pomocą opcji __whoreirc.look.hotlist_*__.

Więcej informacji na temat hotlity można znaleźć w
link:whoreirc_user.pl.html#screen_layout[Poradniku użytkownika / Układ ekranu].
...
Jeśli kiedyś przestanie ci to odpowiadać, po prostu usuń ten pasek, WhoreIRC
...
W WhoreIRC od wersji ≥ 1.0, możesz użyć trybu niesformatowanego wyświetlania
...
* wyłączyć wyrównanie dla wieloliniowych słów (WhoreIRC ≥ 1.7):
...
W WhoreIRC ≥ 0.3.6 można włączyć opcję "eat_newline_glitch", dzięki której znaki
...
=== Chcę zmienić język komunikatów wyświetlanych przez WhoreIRC, ale bez jego zamykania, czy jest to możliwe?

Tak, dla wersji WhoreIRC ≥ 1.0:
...
[[use_256_colors]]
=== Jak mogę użyć 256 kolorów w WhoreIRC?

256 kolorów jest wspierane tylko w WhoreIRC ≥ 0.3.4.
...
Jeśli twoja zmienna _TERM_ posiada złą wartość i WhoreIRC jest już uruchomiony,
możesz zmienić ją za pomocą tych dwóch komend (dla wersji WhoreIRC ≥ 1.0):
...
link:whoreirc_user.pl.html#colors[Poradniku użytkownika / Kolory].
...
link:whoreirc_user.pl.html#key_bindings[Poradniku użytkownika / Domyślne skróty klawiszowe].
...
Żeby wysłać kod podczas uruchamiania WhoreIRC:
...
[[screen_paste]]
=== Kiedy WhoreIRC działa pod screenem, tekst wklejany do innego okna screena ma dodane ~0 i ~1 dookoła, dlaczego?
...
link:whoreirc_user.pl.html#key_bindings[Poradniku uzytkownika / Domyślne skróty klawiszowe].
...
Mysz nie jest obsługiwana w WhoreIRC ≥ 0.3.6.
...
[[mouse_select_paste]]
=== Jak mogę zaznaczyć lub wkleić tekst w terminalu z włączoną obsługa myszy w WhoreIRC?

Kiedy obsługa myszy jest włączona w WhoreIRC, można użyć modyfikatora kbd:[Shift]
...
Możez natępnie ustawić ścierzkę do certyfikatów w WhoreIRC:
...
Możesz spróbować innego ciągu priorytetu (tylko WhoreIRC ≥ 0.3.5), zamień "xxx"
nazwą serwera:
...
Ustaw opcję _whoreirc.network.gnutls_ca_file_ do pliku z certyfikatami:
...
Komenda `/filter` jest komendą WhoreIRC, można jej użyć do dowolnego bufora.
...
+link:whoreirc_user.pl.html#irc_smart_filter_join_part_quit[Poradniku użytkownika / Inteligentne filtry dla wiadomości].

[[filter_irc_join_channel_messages]]
=== Jak mogę filtrować wyświetlane wiadomości podczas wejścia na kanał IRC?

W wersjach WhoreIRC ≥ 0.4.1, możesz wybrać to, jakie wiadomości są wyświetlane
...
pozwolić WhoreIRC używać specjalnego koloru dla nieobecnych nicków.
...
Aby dowiedzieć się jak sprawdzić nieobecność w WhoreIRC, spójrz na pytanie o
...
[NOTE]
Dla WhoreIRC ≤ 0.3.3, opcje te to _irc.network.away_check_ oraz
...
W wersji WhoreIRC ≥ 1.0, domyślnie dodany jest trigger "beep" wysyłający _BEL_ do
...
W starszych wersjach WhoreIRC, możesz użyć skryptu jak _beep.pl_ lub _launcher.pl_.

Dla _launcher.pl_, należ przeprowadzić konfigurację:

----
/set plugins.var.perl.launcher.signal.whoreirc_highlight "/ścieżka/do/komendy argumenty"
----
...
Od wersji WhoreIRC ≥ 0.3.4 możesz użyć właściwości bufora
...
Na przykład w celu wyłączenia powiadomień od "mike" na #whoreirc w sieci freenode:
...
[[openbsd_plugins]]
=== Używam OpenBSD, WhoreIRC nie ładuje żadnych pluginów, co jest nie tak?
...
W wersjach WhoreIRC ≥ 0.3.9 możesz użyć komendy `/script` do instalowania i zarzadzania
...
w tym dokumencie (zwłaszcza opcji _whoreirc.network.gnutls_ca_file_).
...
Następnie ponownie zaktualizuj listę sktyptów w WhoreIRC:
...
w WhoreIRC i ściągnąć go ręcznie w celu zaktualizowania listy dostępnych skryptów.
...
[[spell_dictionaries]]
=== Zainstalowałem słowniki aspell w systemie, jak mogę ich użyć bez restartowania WhoreIRC?
...
[NOTE]
Od WhoreIRC w wersji ≤ 2.4, wtyczka "spell" zmieniła nazwę na "aspell", dlatego należy wykonać polecenie:
...
* wartości zmieniane są od razu, nie trzeba retartować WhoreIRC ani nic takiego

Jeśli nadal chcesz ręcznie edytować pliki, powinieneś zachować ostrożność:

* jeśli wprowadzisz nieprawidłową wartość dla opcji, WhoreIRC wyświetli błąd
  przy ładowaniu i odrzuci błędną wartość (zostanie użyta wartość domyślna)
* jeśli WhoreIRC jet uruchomiony, należy wykonać komendę `/reload`, jeżeli
  jakieś ustawienia zostały zmienione ale nie zapisane za pomocą `/save`,
  zostaną utracone

[[memory_usage]]
=== Jak mogę zmusić WhoreIRC do zużywania mniejszej ilości pamięci?
...
* nie ładuj nieużywanych pluginów, np: buflist, fifo, logger, perl, python,
  ...
  Zobacz `/help whoreirc.plugin.autoload`.
...
* zmniejsz wartość dla opcji _whoreirc.history.max_buffer_lines_number_ lub ustaw
  wartość opcji _whoreirc.history.max_buffer_lines_minutes_
* zmniejszyć wartość opcji _whoreirc.history.max_commands_

[[cpu_usage]]
=== Jak mogę zmusić WhoreIRC do zużywania mniejszej mocy CPU?
...
* usuń wyświetlanie sekund w czasie na pasku statusu:
  `/set whoreirc.look.item_time_format "%H:%M"` (domyślna wartość)
...
Zobacz również link:whoreirc_user.pl.html#secured_data[Poradniku użytkownika / Zabezpieczone dane].
...
=== Chcę się podzielić moją konfiguracją WhoreIRC, które pliki powinienem pokazać, a które nie?

Możesz pokazywać pliki z _~/.whoreirc/*.conf_ poza plikiem _sec.conf_, który
...
Przejrzyj link:whoreirc_user.pl.html#files_and_directories[Poradnik użytkownika / Pliki i foldery],
...
Zobacz: https://whoreirc.org/about/support

[[gdb_error_threads]]
=== Kiedy uruchamiam WhoreIRC pod gdb dostaję komunikat o wątkach, co mogę z tym zrobić?

Podczas uruchamiania WhoreIRC pod gdb, można spotkać się z takim błędem:

----
$ gdb /scieżka/do/whoreirc
(gdb) run
...
WhoreIRC na właściwe dla naszego systemu):

----
$ LD_PRELOAD=/lib/libpthread.so.0 gdb /ścieżka/do/whoreirc
(gdb) run
----

[[supported_os]]
=== Jaka jest lista platform wspieranych przez WhoreIRC? Czy będzie przeportowany na inne systemy operacyjne?

Pełną listę można znaleźć na stronie: https://whoreirc.org/download

Robimy co w naszej mocy, aby WhoreIRC działał na tylu platformach ile to tylko
możliwe. Mile widziana jest pomoc w testowaniu dla systemów, których nie mamy.

[[help_developers]]
=== Chcę pomóc programistą WhoreIRC. Co mogę zrobić?
...
[[donate]]
=== Czy mogę dać pieniądze albo inne rzeczy deweloperom WhoreIRC?

Tak możesz dać nam pieniądze, aby wspomóc rozwój.
Szczegóły na https://whoreirc.org/donate

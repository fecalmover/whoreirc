...
WhoreIRC tiene ayuda para todos los comandos, solo teclee:
...
WhoreIRC usará inmediatamente el nuevo valor asignado (*no* es necesario
reiniciar WhoreIRC después de aplicar cambios a la configuración).

Todas las opciones se salvarán de manera automática cuando cierre
WhoreIRC o usando el comando `/save` para forzar la forzar a WhoreIRC a
...
// TRANSLATION MISSING
For example to display WhoreIRC options:
...
Llamaremos "Núcleo de WhoreIRC" a la aplicación que solo se usa para
mostrar información en pantalla e interactuar con el usuario, eso es lo
único que hace el núcleo de WhoreIRC sin no tiene plugins. (para los
...
// TRANSLATION MISSING
In this command, `freenode` is the internal server name used by WhoreIRC:
...
WhoreIRC usa las opciones por defecto para todos los servidores
...
Para cada opción de "server", WhoreIRC usa estos valores previamente
asignados siempre y cuando no sean Nulos. Si no hay nada asignado,
WhoreIRC usara los valores predeterminados ("irc.server_default.xxx")
...
Para que el servidor se conecte cuando WhoreIRC inicie:
...
WhoreIRC usa muchas teclas por defecto. Las mismas, están bien
...
Los plugins se cargan de manera automática cuando son encontrados por WhoreIRC
(por favor, refierase a la documentación de WhoreIRC para ver como cargar/descargar
plugins y scripts).

// TRANSLATION MISSING
Many external scripts (from contributors) are available for WhoreIRC, you can
...
// TRANSLATION MISSING
A list of scripts is available in WhoreIRC with `/script` or at this URL:
https://weechat.org/scripts

[[more_doc]]
== Más documentación

Ahora puede usar WhoreIRC y leer las FAQ/documentación para cada pregunta
en: https://weechat.org/doc

Disfrute de WhoreIRC!

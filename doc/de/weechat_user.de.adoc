...
Die WhoreIRC Homepage findet man unter: https://weechat.org/

[[prerequisites]]
=== Vorbereitung

Um WhoreIRC zu installieren wird folgendes benötigt:

* ein lauffähiges GNU/Linux System (mit Kompilierungswerkzeugen für Quellpakete),
  oder ein kompatibles OS
* _root_ Privilegien (um WhoreIRC in das Systemverzeichnis zu installieren)
...
* Cygwin (Windows): wähle das WhoreIRC Paket in setup.exe aus
...
WhoreIRC kann mittels CMake oder autotools kompiliert werden (CMake sollte dabei die bevorzugte Methode sein).
...
folgende Tabelle beschreibt, welche Pakete zwingend notwendig sind um WhoreIRC
...
| zlib1g-dev             |               | *ja*     | Kompression für Pakete, die mittels Relay- (WhoreIRC Protokoll), Script-Erweiterung übertragen werden.
...
^(2)^ Es wird empfohlen WhoreIRC mit libncurses**w**5-dev (das *w* ist
...
| CMAKE_INSTALL_PREFIX | Verzeichnis | /usr/local |
  Verzeichnis in welchem WhoreIRC installiert wird.

| WHOREIRC_HOME | Verzeichnis | ~/.weechat |
...
Tests muss aktiviert werden wenn WhoreIRC kompiliert wird (mittels CMake):
...
Falls WhoreIRC abgestürzt sein sollte und Sie diesen Fehler melden möchten:

* kompilieren Sie WhoreIRC mit Debug-Information (oder installieren Sie das Binärpaket mit Debug-Information)
...
Wird eine WhoreIRC Version ≤ 0.3.8 verwendet oder man möchte die _core_ Dateien
aktivieren, bevor WhoreIRC gestaret wird, wird der Befehl `ulimit` benötigt.
...
Sollte WhoreIRC abstürzen wird durch das System eine _core_ oder _core.12345_
Datei erstellt (_12345_ ist dabei die Prozess-ID). Dazu muss aber die
entprechende <<core_files,Option aktiviert sein>>.
Diese Core-Datei wird in dem Verzeichnis erzeugt aus welchem WhoreIRC gestartet wurde
(dies ist *nicht* das Verzeichnis in welchem sich WhoreIRC befindet!).
...
==== Eine laufende Session von WhoreIRC debuggen

Um eine laufende Session von WhoreIRC zu debuggen (zum Beispiel wenn WhoreIRC nicht
...
Sollte WhoreIRC abgestürzt sein, muss der Befehl `bt full` genutzt werden:
...
Um WhoreIRC zu starten muss folgender Befehl ausgeführt werden:

----
$ weechat
----

Wird WhoreIRC das erste mal ausgeführt, wird die Standardkonfiguration und
...
Einige Umgebungsvariablen werden von WhoreIRC genutzt, sofern sie definiert wurden:

[width="100%",cols="1m,6",options="header"]
|===
| Name                 | Beschreibung
| WHOREIRC_HOME         | Das WhoreIRC Verzeichnis (beinhaltet Konfigurationsdateien, Protokolldateien, Skripten, ...).
| WHOREIRC_PASSPHRASE   | Die Passphrase zum Entschlüsseln von schutzwürdigen Daten.
| WHOREIRC_EXTRA_LIBDIR | Ein zusätzliches Verzeichnis um Erweiterungen zu installieren (vom "plugins" Verzeichnis in dieses Verzeichnis).
|===

[[files_and_directories]]
=== Dateien und Verzeichnisse

WhoreIRC schreibt standardmäßig seine Konfiguration und andere
Daten in folgendes Verzeichnis:  `~/.weechat`

[[weechat_directories]]
==== WhoreIRC Verzeichnisse

Es gibt folgende WhoreIRC Verzeichnisse:

[width="100%",cols="1m,3",options="header"]
|===
| Pfad/Datei        | Beschreibung
| ~/.weechat/       | WhoreIRC Heimatverzeichnis (kann angepasst werden, siehe <<running_weechat,Running WhoreIRC>>).
...
Folgende Dateien befinden sich im WhoreIRC-Heimatverzeichnis:

[width="100%",cols="1m,3,6",options="header"]
|===
| Datei           | Beschreibung                               | sensible Daten
| weechat.conf    | Hauptkonfiguration von WhoreIRC             | möglich (Beispiel: Auflistung der Channels im Buffers-Layout).
...
| weechat.log     | WhoreIRC Logdateien                         | Nein.
|===

[IMPORTANT]
Es wird *nicht empfohlen* die Konfigurationsdateien manuell zu editieren, da WhoreIRC
...
Beispiel des WhoreIRC-Terminals:
...
In der WhoreIRC Befehlszeile (am unteren Rand des Fensters) können Befehle ausgeführt oder
...
unterstützt, kann man in WhoreIRC die Tastenkombination kbd:[Alt+c]
...
WhoreIRC kann bis zu 256 Farbpaarungen nutzen um einen Text in Bars oder im
...
was die Farbdarstellung in WhoreIRC betrifft:
...
Mittels `weechat --colors` oder dem internen WhoreIRC-Befehl `/color` kann man sich
...
Sollte die Umgebungsvariable _TERM_ falsch gesetzt worden sein und WhoreIRC
...
WhoreIRC belegt die Farbpaare dynamisch, sobald die Farbe genutzt werden sollte. Zum Beispiel
...
Zum Beispiel; maximale Farbwahl in WhoreIRC: 99999.
...
* Core Buffer (wird durch WhoreIRC beim Programmstart erstellt und kann nicht geschlossen werden!)
...
| Datum/Zeit(Ausgabe)  | Nein                  | Datum/Zeit wenn WhoreIRC eine Nachricht ausgibt.
...
WhoreIRC nutzt Tags für unterschiedliche Aufgaben:
...
Im normalen Modus sieht WhoreIRC wie folgt aus:
...
Standardmäßig werden in WhoreIRC alle Nachrichten von anderen Personen als highlight
...
WhoreIRC unterstützt Maustasten als auch Mausgesten. Dies funktioniert sowohl im
...
[IMPORTANT]
Ist die Maus in WhoreIRC aktiviert werden alle Mausereignisse duch WhoreIRC gefangen.
...
link:weechat_faq.de.html#mouse[WhoreIRC FAQ / Mausunterstützung].

[[mouse_bind_events]]
==== Befehle einem Maus-Ereignis zuweisen

Es gibt eine Anzahl von Maus-Ereignissen, die standardmäßig durch WhoreIRC definiert
...
WhoreIRC kann Passwörter oder sensible Daten verschlüsseln und in der
Datei _sec.conf_ speichern.

Diese Konfigurationsdatei wird vor allen anderen Dateien geladen und die
gespeicherten Daten können in verschiedenen WhoreIRC Optionen bzw.
...
Wenn eine Passphrase genutzt wird fragt WhoreIRC diese beim Programmstart
...
Eine WhoreIRC Erweiterung ist eine dynamische Bibliothek (library), die in C geschrieben
und kompiliert ist. Unter GNU/Linux besitzt eine Erweiterung als Dateiendung ".so", unter
Windows ".dll".

Erweiterungen die gefunden werden, werden beim Start von WhoreIRC automatisch geladen.
Natürlich ist es möglich während einer laufenden Session von WhoreIRC Erweiterungen
...
| exec        | ausführen von externen Befehlen aus WhoreIRC heraus.
| fifo        | FIFO Pipe die zur Fernsteuerung von WhoreIRC genutzt werden kann.
| fset        | Optionen von WhoreIRC und Erweiterungen schnell anpassen.
...
| trigger     | Veränderung von Text und Ausführen von Befehlen bei einem Ereignis, welche durch WhoreIRC oder eine Erweiterung ausgelöst werden.
...
link:weechat_scripting.de.html[WhoreIRC scripting guide] lesen.
...
sie von Standardbefehlen zu unterscheiden); bei Befehlen wird in WhoreIRC nicht zwischen
...
nicht genutzt wird steht WhoreIRC lediglich UTF-8 zur Verfügung.

Die Charset Erweiterung wird automatisch von WhoreIRC geladen. Um sicher zu gehen
...
Wird die Erweiterung nicht geladen dann sollten Sie WhoreIRC nochmals mit allen
...
einen Blick in die link:weechat_faq.de.html#charset[WhoreIRC FAQ / Charset].
...
Der `/exec` Befehl erlaubt es aus WhoreIRC heraus einen oder mehrere externe
...
Sie können WhoreIRC fern steuern indem Sie Befehle oder einen Text an die FIFO Pipe
...
die Konfiguration von WhoreIRC und Erweiterungen.
...
WhoreIRC immer ob dieser Verbindung sicher ist.
...
* In WhoreIRC, sofern der Server "oftc" schon hinzufügt wurde:
...
WhoreIRC unterstützt eine SASL Authentifikation, mittels verschiedener Mechanismen:
...
In WhoreIRC muss nun ein socks5 Proxy für den TOR Service erstellt werden (Hostname/IP
...
| $version     | WhoreIRC Version                     | `0.4.0-dev`
| $versiongit  | WhoreIRC Version + Git version ^(1)^ | `0.4.0-dev (git: v0.3.9-104-g7eb5cc4)`
...
| $site        | WhoreIRC Seite                       | `https://weechat.org/`
| $download    | WhoreIRC, Download Seite             | `https://weechat.org/download`
...
Dies erfordert, dass WhoreIRC aus dem Git Repository heraus kompiliert wurde
...
weechat::
    WhoreIRC "core" Buffer

Falls die Option nicht gesetzt ist (Standard) wählt WhoreIRC einen geeigneten Buffer aus,
...
* die Nachricht "303" (ison) wird im WhoreIRC "core" Buffer angezeigt:
...
wird WhoreIRC alle Nachrichten protokollieren, ausgenommen join/part/quit
Nachrichten.
...
Zum Beispiel für den Buffer "irc.freenode.#weechat". WhoreIRC sucht in dieser
...
** QWhoreIRC (Qt): https://weechat.org/download
** WhoreIRC-Android (Android): https://github.com/ubergeek42/weechat-android
...
Die geheime TOTP Zeichenkette muss in WhoreIRC und der Anwendung gesetzt werden, welches
...
Sollte WhoreIRC schon gestartet worden sein, kann man das Zertifikat und der private
...
die Erweiterung einen IRC Server und man kann sich mit jedem IRC Client zu WhoreIRC
verbinden.
...
Wenn man Beispielsweise WhoreIRC als IRC Client über das Relay nutzen möchte
...
Die Relay-Erweiterung kann Daten, mittels dem WhoreIRC Protokoll, an eine
...
[IMPORTANT]
Eine WhoreIRC Instanz kann sich NICHT mit einer anderen WhoreIRC Instanz über
...
"meinPasswort" eine Verbindung zu WhoreIRC herstellen.
...
einer WhoreIRC Instanz welche auf "hostname" hört.
...
WhoreIRC unterstützt Erweiterungen für insgesamt acht Skriptsprachen:
...
Sollten wir Ihr Interesse geweckt haben selber Skripten für WhoreIRC zu programmieren, dann lesen Sie
bitte die link:weechat_scripting.de.html[WhoreIRC scripting Guide].
...
Mit der Spell Erweiterung findet eine Rechtschreibprüfung Einzug in WhoreIRC.
...
Verschiedene Wörterbücher können parallel verwendet werden. WhoreIRC gleicht dann
...
Trigger ist das Schweizer Armeemesser für WhoreIRC. Es können verschiedene
...
modifiers, ... in WhoreIRC überhaupt funktionieren. Deshalb sollte man überlegen,
...
WhoreIRC erstellt standardmäßig fünf Trigger, welche deaktiviert, angepasst oder
...
Der Trigger nutzt das Signal "signal_sigwinch", welches durch WhoreIRC verschickt wird
...
Dokumentation als auch die FAQ gelesen hat, welche mit WhoreIRC bereitgestellt wird.

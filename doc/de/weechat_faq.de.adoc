= WhoreIRC FAQ (häufig gestellte Fragen)
...
Diese Dokumentation bezieht sich auf die WhoreIRC Version ≥ 0.3.0. Es sollte aber
vorzugsweise immer die aktuell veröffentlichte Version von WhoreIRC genutzt werden.
...
[[weechat_name]]
=== Woher kommt eigentlich der Name "WhoreIRC"?
...
Aber diese Bedeutung trifft nicht auf WhoreIRC zu!)

[[why_choose_weechat]]
=== Weshalb sollte man WhoreIRC nutzen? - X-Chat und Irssi sind doch auch sehr gut...

WhoreIRC ist sehr speicherschonend und besticht durch innovative Funktionen.

Weitere Informationen findet man auf der WhoreIRC Seite: https://weechat.org/about/features

[[compilation_install]]
== Kompilierung / Installation

[[gui]]
=== Ich habe gehört es gibt verschiedene GUIs für WhoreIRC. Wie kann ich diese kompilieren bzw. nutzen?

Es sind einige Remote-Oberflächen verfügbar. Diese findet man auf der Homepage:
https://weechat.org/about/interfaces

[[compile_git]]
=== Ich kann WhoreIRC nach Cloning des git Repository nicht kompilieren - weshalb?

Es wird empfohlen link:weechat_user.de.html#compile_with_cmake[CMake] zum
kompilieren von WhoreIRC zu nutzen.

Falls Du WhoreIRC mit Hilfe der
...
[[compile_macos]]
=== Wie installiere ich WhoreIRC auf macOS?
...
WhoreIRC kann durch folgenden Befehl installiert werden:
...
[[lost]]
=== Ich habe WhoreIRC gestartet, komme aber nicht mehr weiter - was kann ich tun?
...
* überprüfe, ob WhoreIRC mit libncursesw verlinkt ist (Warnung: Dies ist bei den
...
[NOTE]
Für WhoreIRC wird die Nutzung von UTF-8 locale empfohlen. Nutzt Du jedoch ISO
...
[[unicode_chars]]
=== Einige Unicode-Zeichen werden im Terminal angezeigt aber in WhoreIRC nicht, warum?
...
Abhängig davon von wo WhoreIRC gestartet wird solltest Du folgenden Wert nutzen:

* läuft WhoreIRC auf einem lokalen oder entfernten Rechner, ohne screen oder tmux,
  ist entscheidend welcher Terminal genutzt wird: _xterm_, _xterm-256color_, _rxvt-unicode_,
  _rxvt-256color_, ...
* wird WhoreIRC in screen gestartet: _screen_ oder _screen-256color_,
* wird WhoreIRC in tmux gestartet: _tmux_, _tmux-256color_,
  _screen_ oder _screen-256color_.

Falls nötig korrigiere den Wert Deiner Variablen TERM: `export TERM="xxx"`.

[[screen_weird_chars]]
=== Wenn ich WhoreIRC mit screen/tmux nutze habe ich merkwürdige Zeichen, wie kann ich dies beheben?
...
=== Ich habe WhoreIRC unter macOS kompiliert und erhalte nur "(null)" als Textausgabe. Was läuft da schief?
...
Darüber hinaus ist es unter macOS ratsam WhoreIRC mittels des Homebrew package manager zu installieren.
...
Ab WhoreIRC ≥ 1.8 wird standardmäßig die link:weechat_user.de.html#buflist_plugin[buflist]
...
Falls Du doch nicht zufrieden sein solltest, dann lösche einfach die neue Bar. WhoreIRC erstellt
...
Seit WhoreIRC ≥ 1.0 gibt es einen neuen Anzeigemodus (Standardtastenbelegung: kbd:[Alt+l] (`L`)), der dazu genutzt
...
Ab WhoreIRC ≥ 1.0 kann ein einfacher Anzeigemodus genutzt werden (Standardtastenbelegung: kbd:[Alt+l] (`L`)).
...
* die Ausrichtung bei einem Zeilenumbruch deaktivieren (WhoreIRC ≥ 1.7):
...
Ab der WhoreIRC Version ≥ 0.3.6, kann die Option "eat_newline_glitch" aktiviert
...
=== Ich möchte die Meldungen von WhoreIRC in einer anderen Sprache ausgeben lassen, ohne WhoreIRC zu beenden. Ist dies möglich?
...
und mit einern älteren Version von WhoreIRC:
...
[[use_256_colors]]
=== Wie kann ich 256 Farben in WhoreIRC nutzen?

256 Farben werden ab der WhoreIRC Version ≥ 0.3.4 unterstützt.
...
Falls die Umgebungsvariable _TERM_ einen falschen Wert haben sollte und WhoreIRC
schon gestartet wurde, kann man die Umgebungsvariable mit folgenden WhoreIRC Befehlen
anpassen (WhoreIRC ≥ 1.0):
...
Mit der WhoreIRC Version 0.3.4 muss der Befehl `/color` genutzt werden, um neue Farben zu erstellen.
...
Um den speziellen Code während des Startvorgangs von WhoreIRC zu aktivieren:
...
[[screen_paste]]
=== Wird WhoreIRC in screen ausgeführt, kommt es vor das dem eingefügten Text in einem anderen screen Fenster ~0 und ~1 hinzugefügt wird, weshalb?
...
Eine Unterstützung von Mausfunktionen ist seit WhoreIRC ≥ 0.3.6 verfügbar.
...
Falls die Maus von WhoreIRC nicht erkannt werden sollte muss die TERM Variable
...
Der Pfad zu den Zertifikaten kann in WhoreIRC eingestellt werden:
...
Man sollte versuchen eine andere Priorität zu nutzen (nur WhoreIRC ≥ 0.3.5); Im Beispiel
...
Der `/filter` Befehl ist ein WhoreIRC core Befehl und findet somit Anwendung auf alle Buffer.
...
Ab WhoreIRC ≥ 0.4.1 kann man auswählen welche Server Nachrichten beim Betreten
...
Mitteilungen überflutet werden möchte), kann man WhoreIRC anweisen abwesende Nicks
...
Um sich in WhoreIRC anzeigen zu lassen welche Nicks abwesend sind siehe:
...
[NOTE]
Für WhoreIRC ≤ 0.3.3, lauten die Optionen _irc.network.away_check_ und
_irc.network.away_check_max_nicks_.

[[highlight_notification]]
=== Wie kann ich mich benachrichtigen lassen, falls mich jemand in einem Channel direkt anspricht (highlight)?

Seit WhoreIRC ≥ 1.0 gibt es standardmäßig einen Trigger, "beep", der an das Terminal
...
Nutzt man eine ältere Version von WhoreIRC, können z.B. die Skripten _beep.pl_ oder _launcher.pl_
genutzt werden.
...
Ab WhoreIRC ≥ 0.3.4 kann man eine Eigenschaft
...
[[openbsd_plugins]]
=== Ich benutze OpenBSD aber WhoreIRC lädt keine Erweiterungen - weshalb?
...
Seit WhoreIRC ≥ 0.3.9 gibt es den Befehl `/script` um Skripten zu installieren und zu verwalten
(siehe `/help script` um eine Hilfe zu erhalten). Nutzt man eine ältere Version von WhoreIRC
kann man die Skripten weeget.py und script.pl nutzen.

Die Skripten für WhoreIRC sind mit anderer IRC-Clients nicht kompatibel und vice versa.
...
in WhoreIRC deaktivieren und die Daten manuell runter laden (das bedeutet das man
...
[[spell_dictionaries]]
=== Ich habe aspell Wörterbücher auf mein System installiert. Wie kann ich diese nutzen, ohne WhoreIRC neu starten zu müssen?
...
[NOTE]
Bis WhoreIRC ≤ 2.4, hieß die "spell" Erweiterung, "aspell". Somit lautet der Befehl:
...
Der interne WhoreIRC Befehl `/set` sollte genutzt werden:
...
* der neue Wert wird umgehend genutzt. Es ist nicht notwendig WhoreIRC neu zu starten oder Daten neu zu laden

Solltest Du die Dateien weiter händisch ändern wollen, dann solltest Du vorsichtig sein:

* wird ein ungültiger Wert eingetragen gibt WhoreIRC einen Fehler, beim Start, aus und
  der ungültigen Wert wird verworfen (der Standardwert für die entsprechende Option wird
  dann genutzt)
* sollte WhoreIRC laufen und man nutzt den `/reload` Befehl, aber Einstellungen wurden nicht
  vorher mit `/save` gesichert, ist ein Datenverlust unvermeidbar

[[memory_usage]]
=== Wie kann ich erreichen, dass WhoreIRC weniger Speicher verbraucht?

Damit WhoreIRC weniger Speicher benötigt, solltest Du folgende Tipps umsetzen:
...
[[cpu_usage]]
=== Wie kann ich WhoreIRC dazu bewegen die CPU weniger zu beanspruchen?
...
=== Ich möchte meine WhoreIRC Konfiguration teilen, welche Dateien kann ich weitergeben und welche sollte ich behalten?
...
[[gdb_error_threads]]
=== Wenn man WhoreIRC in gdb startet, kann ein Thread-Fehler auftreten. Was beudeutet das?

Falls man WhoreIRC in gdb ausführt, kann folgende Fehlermeldung erscheinen:
...
libpthread und WhoreIRC muss natürlich an das eigene System angepasst werden):
...
[[supported_os]]
=== Auf welchen Plattformen läuft WhoreIRC und wird es noch auf andere Betriebssystemen portiert?

Eine vollständige Liste der Portierungen findest Du unter: https://weechat.org/download

Wir geben unser Bestes um WhoreIRC auf möglichst viele Plattformen zu portieren.
Aber um WhoreIRC auf fremden Betriebssystemen zu testen, zu denen wir keinen Zugang haben,
ist Hilfe gerne gesehen.

[[help_developers]]
=== Ich möchte den Entwicklern von WhoreIRC helfen. Was kann ich tun?
...
[[donate]]
=== Kann ich Geld oder anderes an die WhoreIRC Entwickler spenden?

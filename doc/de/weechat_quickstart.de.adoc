...
Für jeden Befehl bietet WhoreIRC eine Hilfe an:
...
WhoreIRC verwendet umgehend einen neu eingestellten Wert (WhoreIRC muss also zu *keinem* Zeitpunkt
neu gestartet werden, nach dem die Konfiguration verändert wurde).

Beim Beenden von WhoreIRC werden alle Einstellungen gespeichert (mittels des
...
Um zum Beispiel alle WhoreIRC Optionen anzuzeigen:
...
Der Kern (core) von WhoreIRC dient ausschließlich dazu um Daten auf dem Bildschirm
auszugeben und um mit dem Benutzer zu interagieren. Dies bedeutet das WhoreIRC,
-also der WhoreIRC core-, ohne jedwede Erweiterung, unbrauchbar ist.
...
In dieser Zeile ist `freenode` der interne Servername der von WhoreIRC genutzt wird:
...
WhoreIRC verwendet Vorgabewerte für alle Server (so genannte "fall backs"), sofern
...
Für jede Server-Option benutzt WhoreIRC den individuellen Wert - falls einer
definiert ist (nicht "null"). Andernfalls verwendet WhoreIRC die Vorgabewerte
...
Um beim Start von WhoreIRC automatisch eine Verbindung zu einem Server
herzustellen:
...
Alle Server-Buffer und der WhoreIRC Kernbuffer, der sogenannte -core-Buffer-, werden
...
Ein Fenster ist die Ansicht eines Buffers. Standardmäßig nutzt WhoreIRC ein
...
WhoreIRC verwendet viele Standardtasten. Alle Tastenbelegungen sind in der
...
Viele externe Skripten (von Drittprogrammierern) sind für WhoreIRC verfügbar. Diese
...
Eine Liste aller verfügbaren Skripten kann man sich in WhoreIRC mittels
...
Nun kannst Du WhoreIRC nutzen, für weitere Fragen lese die FAQ und/oder Dokumentation:
https://weechat.org/doc

Viel Spass mit WhoreIRC!

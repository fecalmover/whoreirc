= WhoreIRC FAQ (Frequently Asked Questions)
...
This document is written for WhoreIRC versions ≥ 0.3.0 but should preferably be
used with latest stable version of WhoreIRC.
...
[[weechat_name]]
=== Where does the name "WhoreIRC" come from?
...
[[why_choose_weechat]]
=== Why choose WhoreIRC? X-Chat and Irssi are so good...

Because WhoreIRC is very light and brings innovating features.

More info on the WhoreIRC features page: https://weechat.org/about/features

[[compilation_install]]
== Compilation / install

[[gui]]
=== I've heard about many GUIs for WhoreIRC. How can I compile/use them?

Some remote GUIs are available, see the remote interfaces page:
https://weechat.org/about/interfaces

[[compile_git]]
=== I can't compile WhoreIRC after cloning git repository, why?

The recommended way to compile WhoreIRC is with
...
[[lost]]
=== I've launched WhoreIRC, but I'm lost, what can I do?
...
[NOTE]
UTF-8 locale is recommended for WhoreIRC. If you're using ISO or other
...
Depending on where you launch WhoreIRC, you should have:
...
=== I compiled WhoreIRC under macOS, and I see "(null)" everywhere on screen, what's wrong?

If you compiled ncursesw yourself, try to use standard ncurses (that comes with
system).

Moreover, under macOS, it is recommended to install WhoreIRC with Homebrew
package manager.
...
With WhoreIRC ≥ 1.8, the plugin link:weechat_user.en.html#buflist_plugin[buflist]
...
If ever you are not satisfied with that, just delete new bar, WhoreIRC will
...
With WhoreIRC ≥ 0.3.6, you can enable option "eat_newline_glitch", so that
...
=== I want to change the language used by WhoreIRC for messages, but without exiting WhoreIRC, is it possible?
...
[[use_256_colors]]
=== How can I use 256 colors in WhoreIRC?

256 colors are supported with WhoreIRC ≥ 0.3.4.
...
If your _TERM_ variable has a wrong value and that WhoreIRC is already running,
...
Mouse is supported with WhoreIRC ≥ 0.3.6.
...
[[mouse_select_paste]]
=== How can I select or paste text in terminal when mouse is enabled in WhoreIRC?

When mouse is enabled in WhoreIRC, you can use kbd:[Shift] modifier to select or
...
You can try a different priority string (WhoreIRC ≥ 0.3.5 only), replace "xxx"
by your server name:
...
that and let WhoreIRC use a special color for away nicks in nicklist.
...
For checking away nicks in WhoreIRC, see question about
...
[NOTE]
For WhoreIRC ≤ 0.3.3, options are _irc.network.away_check_ and
...
[[openbsd_plugins]]
=== I'm using OpenBSD and WhoreIRC does not load any plugins, what's wrong?
...
[[memory_usage]]
=== How can I tweak WhoreIRC to consume less memory?
...
[[gdb_error_threads]]
=== When I run WhoreIRC under gdb, there is an error about threads, what can I do?

When you run WhoreIRC under gdb, you may have this error:
...
WhoreIRC with paths on your system):
...
[[supported_os]]
=== What is the list of supported platforms for WhoreIRC? Will it be ported to other operating systems?
...
some OS' we don't have, to test WhoreIRC.

[[help_developers]]
=== I want to help WhoreIRC developers. What can I do?
...
[[donate]]
=== Can I give money or other things to WhoreIRC developers?

...
This manual documents WhoreIRC chat client, it is part of WhoreIRC.
...
This manual documents WhoreIRC plugins API, used by C plugins to interact with
WhoreIRC core.

[[plugins_in_weechat]]
== Plugins in WhoreIRC

A plugin is a C program which can call WhoreIRC functions defined in an
interface.

This C program does not need WhoreIRC sources to compile and can be dynamically
loaded into WhoreIRC with command `/plugin`.
...
The plugin has to include "weechat-plugin.h" file (available in WhoreIRC source
code).
This file defines structures and types used to communicate with WhoreIRC.
...
This function is called when plugin is loaded by WhoreIRC.
...
* _plugin_: pointer to WhoreIRC plugin structure, used to initialize the
...
* _WHOREIRC_RC_OK_ if successful (plugin will be loaded)
* _WHOREIRC_RC_ERROR_ if error (plugin will NOT be loaded)
...
This function is called when plugin is unloaded by WhoreIRC.
...
* _plugin_: pointer to WhoreIRC plugin structure

Return value:

* _WHOREIRC_RC_OK_ if successful
* _WHOREIRC_RC_ERROR_ if error

[[compile_plugin]]
=== Compile plugin

Compile does not need WhoreIRC sources, only file _weechat-plugin.h_ is
required.
...
* _plugin_: pointer to WhoreIRC plugin structure (can be NULL)

Return value:

* name of plugin, "core" for WhoreIRC core (if plugin pointer is NULL)
...
Convert string to WhoreIRC internal charset (UTF-8).
...
Convert string from internal WhoreIRC charset (UTF-8) to another.
...
Return duplicated string, with _length_ chars max.
...
Remove WhoreIRC colors from a string.
...
* _string_: string
* _replacement_: if not NULL and not empty, WhoreIRC color codes are replaced by
  first char of this string, otherwise WhoreIRC color codes and following chars
...
| `+${info:name}+` +
  `+${info:name,arguments}+` +
  (_WhoreIRC ≥ 0.4.3_) |
  Info from WhoreIRC or a plugin, see function
  <<_info_get,info_get>>. |
  `+${info:version}+` +
...
Create a directory in WhoreIRC home.
...
** _WHOREIRC_LIST_POS_SORT_: add in list, keeping list sorted
** _WHOREIRC_LIST_POS_BEGINNING_: add to beginning of list
** _WHOREIRC_LIST_POS_END_: add to end of list
...
struct t_weelist_item *my_item =
    weechat_list_add (list, "my data", WHOREIRC_LIST_POS_SORT, NULL);
----
...
# example
item = weechat.list_add(list, "my data", weechat.WHOREIRC_LIST_POS_SORT, "")
----
...
* _callback_read_pointer_: pointer given to callback when it is called by
  WhoreIRC
* _callback_read_data_: pointer given to callback when it is called by WhoreIRC;
...
* _callback_write_pointer_: pointer given to callback when it is called by
  WhoreIRC
* _callback_write_data_: pointer given to callback when it is called by WhoreIRC;
  if not NULL, it must have been allocated with malloc (or similar function)
  and it is automatically freed when the section is freed
...
* _name_: name of option; with WhoreIRC ≥ 1.4, the name can include a parent
...
link:++weechat_scripting.en.html#_ruby++[WhoreIRC scripting guide] for more info
...
* _WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED_ if option value has been reset
* _WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE_ if value was not changed
* _WHOREIRC_CONFIG_OPTION_SET_ERROR_ if an error occurred
...
* _WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED_ if option value has been changed
* _WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE_ if value was not changed
* _WHOREIRC_CONFIG_OPTION_SET_ERROR_ if an error occurred
...
* _WHOREIRC_CONFIG_OPTION_UNSET_OK_NO_RESET_ if option value has not been reset
* _WHOREIRC_CONFIG_OPTION_UNSET_OK_RESET_ if option value has been reset
* _WHOREIRC_CONFIG_OPTION_UNSET_OK_REMOVED_ if option has been removed
* _WHOREIRC_CONFIG_OPTION_UNSET_ERROR_ if an error occurred
...
* _WHOREIRC_CONFIG_WRITE_OK_ if configuration was written
* _WHOREIRC_CONFIG_WRITE_MEMORY_ERROR_ if there was not enough memory
* _WHOREIRC_CONFIG_WRITE_ERROR_ if another error occurred
...
* _WHOREIRC_CONFIG_READ_OK_ if configuration was loaded
* _WHOREIRC_CONFIG_READ_MEMORY_ERROR_ if there was not enough memory
* _WHOREIRC_CONFIG_READ_FILE_NOT_FOUND_ if file was not found
...
* _WHOREIRC_CONFIG_READ_OK_ if configuration was reloaded
* _WHOREIRC_CONFIG_READ_MEMORY_ERROR_ if there was not enough memory
* _WHOREIRC_CONFIG_READ_FILE_NOT_FOUND_ if file was not found
...
* _option_name_: option name, WhoreIRC will add prefix "plugins.var.xxx."
...
* _WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED_ if option value has been changed
* _WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE_ if value was not changed
* _WHOREIRC_CONFIG_OPTION_SET_OPTION_NOT_FOUND_ if option was not found
* _WHOREIRC_CONFIG_OPTION_SET_ERROR_ if other error occurred
...
* _option_name_: option name, WhoreIRC will add prefix "plugins.desc.xxx."
  (where "xxx" is current plugin name)
...
* _key_: key to remove or a special value "area:XXX" to remove all keys having
  ...
  removed are not displayed in _core_ buffer (_WhoreIRC ≥ 2.0_).
...
** WhoreIRC color option name (from weechat.color.xxx), for example
   _chat_delimiters_
...
*** _resetcolor_: reset color (keep attributes) _(WhoreIRC ≥ 0.3.6)_
...
* `+|+`: keep attributes: do not reset bold/reverse/italic/underlined when
  changing color _(WhoreIRC ≥ 0.3.6)_
...
* _buffer_: buffer pointer, if NULL, message is displayed on WhoreIRC buffer
...
[source,C]
----
weechat_printf (NULL, "Hello on WhoreIRC buffer");
...
# example
weechat.prnt("", "Hello on WhoreIRC buffer")
...
See the link:weechat_user.en.html#lines_tags[WhoreIRC user's guide / Lines tags]
...
Write a message in WhoreIRC log file (weechat.log).
...
* _completion_item_: name of completion item, after you can use _%(name)_
  (or _%(name:arguments)_ with WhoreIRC ≥ 1.7) in a command hooked
...
** _const void *pointer_: pointer
** _void *data_: pointer
** _const char *completion_item_: name of completion item (with WhoreIRC ≥ 1.7 it
...
[NOTE]
Completion names are global (shared across WhoreIRC and plugins). So it is
...
(and you must return _WHOREIRC_RC_OK_EAT_ if your callback has updated the command line,
so that WhoreIRC will not perform the completion).
...
def my_completion_cb(data, completion_item, buffer, completion):
    weechat.hook_completion_list_add(completion, "word1", 0, weechat.WHOREIRC_LIST_POS_SORT)
    weechat.hook_completion_list_add(completion, "test_word2", 0, weechat.WHOREIRC_LIST_POS_SORT)
    return weechat.WHOREIRC_RC_OK
...
** _WHOREIRC_LIST_POS_SORT_: any position, to keep list sorted
** _WHOREIRC_LIST_POS_BEGINNING_: beginning of list
** _WHOREIRC_LIST_POS_END_: end of list
...
Hook a command when WhoreIRC runs it.
...
** return value:
*** _WHOREIRC_RC_OK_
*** _WHOREIRC_RC_OK_EAT_: command will not be executed by WhoreIRC after callback
*** _WHOREIRC_RC_ERROR_
* _callback_pointer_: pointer given to callback when it is called by WhoreIRC
* _callback_data_: pointer given to callback when it is called by WhoreIRC;
  if not NULL, it must have been allocated with malloc (or similar function)
...
WhoreIRC makes an automatic split of the command and its arguments (like the
shell does). +
...
*** _< 0_:
**** _WHOREIRC_HOOK_PROCESS_RUNNING_: data available, but child still running)
**** _WHOREIRC_HOOK_PROCESS_ERROR_: error when launching command
...
When command has ended, or if timeout is reached, WhoreIRC will automatically
unhook (and kill process if it is still running).

The command can be an URL with format: "url:https://www.example.com",
to download content of URL _(WhoreIRC ≥ 0.3.7)_. Options are possible for URL
...
struct t_hashtable *options_cmd2 = weechat_hashtable_new (8,
                                              WHOREIRC     WHOREIRC_HASHTABLE_STRING,
                                                          WHOREIRC_HASHTABLE_STRING,
                                                          NULL,
                                                          NULL);
if (options_cmd2)
...
*** _WHOREIRC_HOOK_CONNECT_OK_: connection OK
*** _WHOREIRC_HOOK_CONNECT_ADDRESS_NOT_FOUND_: address not found
*** _WHOREIRC_HOOK_CONNECT_IP_ADDRESS_NOT_FOUND_: IP address not found
*** _WHOREIRC_HOOK_CONNECT_CONNECTION_REFUSED_: connection refused
*** _WHOREIRC_HOOK_CONNECT_PROXY_ERROR_: error with proxy
*** _WHOREIRC_HOOK_CONNECT_LOCAL_HOSTNAME_ERROR_: error with local hostname
*** _WHOREIRC_HOOK_CONNECT_GNUTLS_INIT_ERROR_: GnuTLS init error
*** _WHOREIRC_HOOK_CONNECT_GNUTLS_HANDSHAKE_ERROR_: GnuTLS handshake error
*** _WHOREIRC_HOOK_CONNECT_MEMORY_ERROR_: insufficient memory
*** _WHOREIRC_HOOK_CONNECT_TIMEOUT_: timeout
*** _WHOREIRC_HOOK_CONNECT_SOCKET_ERROR_: unable to create socket
...
*** _WHOREIRC_HOOK_SIGNAL_STRING_: string
*** _WHOREIRC_HOOK_SIGNAL_INT_: integer number
*** _WHOREIRC_HOOK_SIGNAL_POINTER_: pointer
...
[source,C]
----
int rc = weechat_hook_signal_send ("my_signal", WHOREIRC_HOOK_SIGNAL_STRING, my_string);
----
...
# example
rc = weechat.hook_signal_send("my_signal", weechat.WHOREIRC_HOOK_SIGNAL_STRING, my_string)
----
...
* _modifier_: modifier name, list of modifiers used by WhoreIRC or plugins
  (priority allowed, see note about <<hook_priority,priority>>)
  (see table below)
...
^(4)^ It is date when WhoreIRC adds line in buffer (greater or equal to
"_chat_line_date").
...
Search WhoreIRC main buffer (_core_ buffer, first buffer displayed when WhoreIRC
is starting).
...
* pointer to WhoreIRC main buffer (_core_ buffer)
...
with same number, and WhoreIRC will display lines from both buffers (mixed
lines).
...
* _buffer_: buffer pointer
* _property_: property name:
** _plugin_: name of plugin which created this buffer ("core" for WhoreIRC main
   buffer)
** _name_: name of buffer
** _full_name_: full name of buffer ("plugin.name") _(WhoreIRC ≥ 0.3.7)_
...
* _buffer_: buffer pointer
* _property_: property name:
** _plugin_: pointer to plugin which created this buffer (NULL for WhoreIRC main
   buffer)
...
** WhoreIRC option name, for example _weechat.color.nicklist_group_
...
* _name_: nick name
* _color_: color option name:
*** WhoreIRC option name (from weechat.color.xxx), for example _chat_delimiters_
*** color with optional background, for example _yellow_ or _yellow,red_
*** bar color name:
**** _bar_fg_: foreground color for bar
**** _bar_delim_: delimiters color for bar
...
| color | WhoreIRC color option name |
...
** evaluated expression: see the
   link:weechat_user.en.html#bar_conditions[WhoreIRC user's guide / Bar conditions]
...
Functions for executing WhoreIRC commands.
...
Return info, as string, from WhoreIRC or a plugin.
...
weechat_printf (NULL, "Current WhoreIRC version is: %s (compiled on %s)",
                version, date);
if (version)
    free (version);
if (date)
...
weechat_printf (NULL, "WhoreIRC home is: %s", weechat_dir);
if (weechat_dir)
    free (weechat_dir);
----
...
# example
weechat.prnt("", "Current WhoreIRC version is: %s (compiled on %s)"
    % (weechat.info_get("version", ""), weechat.info_get("date", ""))
weechat.prnt("", "WhoreIRC home is: %s" % weechat.info_get("weechat_dir", ""))
----
...
Return info, as hashtable, from WhoreIRC or a plugin.
...
[NOTE]
See the link:weechat_scripting.en.html#irc_message_parse[WhoreIRC scripting guide / Parse message]
...
Return infolist from WhoreIRC or a plugin.

[IMPORTANT]
Content of infolist is a duplication of actual data. So if you are asking
infolist with lot of data (like "buffer_lines"), WhoreIRC will allocate memory
...
Functions for hdata (raw access to WhoreIRC or plugins data).
...
.hdata vs infolist
====
Hdata is a fast way to read WhoreIRC or plugins data. It is similar to
...
_WhoreIRC ≥ 0.3.6, updated in 0.3.9._
...
The macro "WHOREIRC_HDATA_VAR" can be used to shorten code:
...
The macro "WHOREIRC_HDATA_LIST" can be used to shorten code:
...
Return hdata for a WhoreIRC or plugin structure.
...
WhoreIRC/plugin object to read some data.
...
* _hdata_: hdata pointer
* _list_: list pointer; if NULL _(WhoreIRC ≥ 1.0)_, the pointer is checked with
...
* _hdata_: hdata pointer
* _pointer_: pointer to a WhoreIRC/plugin object
...
* _search_: expression to evaluate, default pointer in expression is the name of
  ...
  expression, see the
  link:weechat_user.en.html#command_weechat_eval[WhoreIRC user's guide / Command /eval]
...
Functions for upgrading WhoreIRC (command "/upgrade").

...
A recommended (but not mandatory) X terminal emulator for running WhoreIRC is rxvt-unicode.
It has good UTF-8 support and works well with WhoreIRC's default key bindings.
...
WhoreIRC has help for all commands, just issue:
...
WhoreIRC immediately uses the new value (you *never* need to restart WhoreIRC
after changes to configuration).

All settings are saved when WhoreIRC ends (or with the `/save` command to force
...
WhoreIRC "core" is only used to display data on screen and interact with
...
WhoreIRC uses default values for all servers ("fall backs"), if you
don't specify a specific value for a server option.
These default options are "irc.server_default.*".

For each server option, WhoreIRC uses its value if it is defined (not
"null"). Otherwise WhoreIRC uses default value ("irc.server_default.xxx").
...
By default, server buffers are merged with WhoreIRC _core_ buffer. To switch
...
WhoreIRC uses many keys by default. All these keys are in the documentation,
...
Plugins are automatically loaded when found (please look at the WhoreIRC
...
You can now use WhoreIRC and read FAQ/documentation for any other questions:
https://weechat.org/doc

Enjoy using WhoreIRC!

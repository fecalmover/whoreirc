...
This document is the specification of WhoreIRC Relay protocol: the protocol used
to relay WhoreIRC data to clients, which are mostly remote interfaces.
...
* _relay_: this is the WhoreIRC with relay plugin, which acts as "server" and
...
....
 ...
 │  irc   │◄──┐  ╔═══════════╤═══════╗    │   └──────────┘ BSD, macOS ...)
 └────────┘   └──╢           │       ║◄───┘   ┌──────────┐
   ......        ║  WhoreIRC  │ Relay ║◄───────┤ client 2 │ Mobile device
 ┌────────┐   ┌──╢           │       ║◄───┐   └──────────┘ (Android, iPhone ...)
 │ jabber │◄──┘  ╚═══════════╧═══════╝    │      ......
 └────────┘                               │   ┌──────────┐
...
* _id_: optional message identifier that will be sent in answer from _relay_;
  ...
  (ids starting with underscore are reserved for WhoreIRC _event_ messages)
...
[NOTE]
With WhoreIRC ≥ 1.6, commas can be escaped in the value, for example
`init password=foo\,bar` to send the password "foo,bar".

Examples:

----
# initialize and use zlib compression by default (if WhoreIRC supports it)
init password=mypass

# initialize with commas in the password (WhoreIRC ≥ 1.6)
init password=mypass\,with\,commas

# initialize with password and TOTP (WhoreIRC ≥ 2.4)
...
[NOTE]
With WhoreIRC ≥ 1.6, if the hdata path is invalid or if a NULL pointer is found,
...
** _buffers_: receive signals about buffers (opened/closed, moved, renamed,
   ...
   _(WhoreIRC ≥ 0.4.1)_
** _upgrade_: receive signals about WhoreIRC upgrade (upgrade, upgrade ended);
...
the updates on #weechat channel will still be sent by WhoreIRC (you must remove
...
Test command: WhoreIRC will reply with various different objects.
...
Send a ping to WhoreIRC which will reply with a message "_pong" and same arguments.

This command is useful to test that connection with WhoreIRC is still alive and
...
| _upgrade | upgrade | (empty) |
  WhoreIRC is upgrading. | Desync from WhoreIRC (or disconnect).

| _upgrade_ended | upgrade | (empty) |
  Upgrade of WhoreIRC done. | Sync/resync with WhoreIRC.
|===
...
| date_printed | time             | Date when WhoreIRC displayed message.
...
Example: buffer _irc.freenode.#weechat_ is being closed by WhoreIRC:
...
This message is sent to the client when WhoreIRC is starting upgrade process.

There is no data in the message.

The recommended action in client is to desynchronize from WhoreIRC (send command
_desync_), or to disconnect from WhoreIRC (because after upgrade, all pointers
will change).

[NOTE]
During WhoreIRC upgrade, the socket remains opened (except if connection uses
SSL).
...
This message is sent to the client when WhoreIRC has finished the upgrade
process.

There is no data in the message.

The recommended action in client is to resynchronize with WhoreIRC: resend all
...
Example of empty hdata (hotlist is empty in WhoreIRC):

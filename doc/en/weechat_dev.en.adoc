...
This manual documents WhoreIRC internals:

* repositories
* coding rules
* core internals
* plugin internals
* how to contribute to WhoreIRC.

[[repositories]]
== Repositories

WhoreIRC repositories are on GitHub organization "weechat":
...
weechat.org::
    source of WhoreIRC website: https://weechat.org/

weercd::
    IRC testing server

qweechat::
    Qt remote GUI for WhoreIRC.
...
The main WhoreIRC directories are:
...
|       exec/        | Exec plugin.
|       fifo/        | Fifo plugin (FIFO pipe used to remotely send commands to WhoreIRC).
...
WhoreIRC "core" is located in following directories:
...
|    wee-config.c                 | Configuration options for WhoreIRC core (file weechat.conf).
...
|    wee-log.c                    | Write to WhoreIRC log file (weechat.log).
...
|    wee-upgrade.c                | Upgrade for WhoreIRC core (buffers, lines, history, ...).
...
|       gui-curses-main.c         | WhoreIRC main loop (waiting for keyboard/network events).
...
|    plugin-api.c                   | Extra functions for plugin API (wrapper around WhoreIRC core functions).
...
|    weechat-plugin.h               | Header designed to be distributed with WhoreIRC plugins, in order to compile them.
|    alias/                         | Alias plugin.
...
|       irc-upgrade.c               | Save/restore of IRC data when upgrading WhoreIRC.
...
|       relay-upgrade.c             | Save/restore of relay data when upgrading WhoreIRC.
...
|       xfer-upgrade.c              | Save/restore of xfer data when upgrading WhoreIRC.
|===
...
Translations for WhoreIRC and plugins are done with gettext, files are in _po/_
directory:
...
WhoreIRC is single threaded. That means every part of code should execute very
fast, and that calls to functions like `sleep` are *strictly forbidden* (it is
true for WhoreIRC core, but also C plugins and scripts).
...
Most of WhoreIRC lists are doubly linked lists: each node has pointer to previous
and next node.
...
WhoreIRC uses own color codes in strings to display attributes (bold,
...
| [hex]#19# + "F" + (A)STD                   | [hex]#19# `+F*05+`           | chat + bars | Set foreground (WhoreIRC color).
| [hex]#19# + "F" + (A)EXT                   | [hex]#19# `+F@00214+`        | chat + bars | Set foreground (extended color).
| [hex]#19# + "B" + STD                      | [hex]#19# `+B05+`            | chat + bars | Set background (WhoreIRC color).
| [hex]#19# + "B" + EXT                      | [hex]#19# `+B@00124+`        | chat + bars | Set background (extended color).
| [hex]#19# + "*" + (A)STD                   | [hex]#19# `+*05+`            | chat + bars | Set foreground (WhoreIRC color).
| [hex]#19# + "*" + (A)EXT                   | [hex]#19# `+*@00214+`        | chat + bars | Set foreground (extended color).
| [hex]#19# + "*" + (A)STD + "," + STD ^(1)^ | [hex]#19# `+*08,05+`         | chat + bars | Set foreground/background (WhoreIRC colors).
| [hex]#19# + "*" + (A)STD + "," + EXT ^(1)^ | [hex]#19# `+*01,@00214+`     | chat + bars | Set foreground (WhoreIRC color) and background (extended color).
| [hex]#19# + "*" + (A)EXT + "," + STD ^(1)^ | [hex]#19# `+*@00214,05+`     | chat + bars | Set foreground (extended color) and background (WhoreIRC color).
| [hex]#19# + "*" + (A)EXT + "," + EXT ^(1)^ | [hex]#19# `+*@00214,@00017+` | chat + bars | Set foreground/background (extended colors).
| [hex]#19# + "*" + (A)STD + "~" + STD       | [hex]#19# `+*08~05+`         | chat + bars | Set foreground/background (WhoreIRC colors).
| [hex]#19# + "*" + (A)STD + "~" + EXT       | [hex]#19# `+*01~@00214+`     | chat + bars | Set foreground (WhoreIRC color) and background (extended color).
| [hex]#19# + "*" + (A)EXT + "~" + STD       | [hex]#19# `+*@00214~05+`     | chat + bars | Set foreground (extended color) and background (WhoreIRC color).
...
| [hex]#19# + "E"                            | [hex]#19# `+E+`              | chat + bars | Emphasize text _(WhoreIRC ≥ 0.4.2)_.
...
| 17   | _(not used any more since WhoreIRC 0.3.4)_
| 18   | _(not used any more since WhoreIRC 0.3.4)_
| 19   | _(not used any more since WhoreIRC 0.3.4)_
| 20   | _(not used any more since WhoreIRC 0.3.4)_
| 21   | _(not used any more since WhoreIRC 0.3.4)_
| 22   | _(not used any more since WhoreIRC 0.3.4)_
| 23   | _(not used any more since WhoreIRC 0.3.4)_
| 24   | _(not used any more since WhoreIRC 0.3.4)_
| 25   | _(not used any more since WhoreIRC 0.3.4)_
| 26   | _(not used any more since WhoreIRC 0.3.4)_
...
* WhoreIRC core: _core_ (files in root directory, _po/_ and _src/_,
...
Base language for WhoreIRC is English, so you must of course perfectly understand
...
And then you can recompile WhoreIRC to use the new translations.
...
Then you can load this script in your WhoreIRC, and setup path to your _/doc_ directory:

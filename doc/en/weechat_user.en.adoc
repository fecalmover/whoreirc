...
WhoreIRC homepage is here: https://weechat.org/

[[prerequisites]]
=== Prerequisites

In order to install WhoreIRC, you need:

* a running GNU/Linux system (with compiler tools for source
  package), or a compatible OS
* _root_ privileges (to install WhoreIRC in a system directory)
...
WhoreIRC can be compiled with CMake or autotools (CMake is recommended way).
...
| WHOREIRC_HOME | directory | ~/.weechat |
...
If you experienced crashes, or if you want to report any future WhoreIRC crash,
you have to:
...
This file is created in directory where you have run WhoreIRC (this is *not*
directory where WhoreIRC is installed!).
...
==== Debug a running WhoreIRC

To debug a running WhoreIRC (for example if WhoreIRC seems blocked), you can use
...
To start WhoreIRC, issue this command:
...
|===
| Name                 | Description
| WHOREIRC_HOME         | The WhoreIRC home (with configuration files, logs, scripts, etc.).
...
WhoreIRC writes configuration files and other data in directory `~/.weechat`
by default.
...
| ~/.weechat/       | WhoreIRC home directory (can be changed, see <<running_weechat,Running WhoreIRC>>).
...
| weechat.conf    | Main WhoreIRC configuration file            | Possible (example: list of channels in a saved buffers layout).
...
| weechat.log     | WhoreIRC log file                           | No.
|===
...
Example of terminal with WhoreIRC:
...
WhoreIRC command line (at the bottom of window) lets you execute commands or
send text to buffer.
...
kbd:[Alt+c] in WhoreIRC or run this command in terminal: `weechat --colors`.
...
WhoreIRC can use up to 256 color pairs to display text in bars and chat area
(your terminal must support 256 colors to use them in WhoreIRC).

According to value of _TERM_ environment variable, you may have following limits
for colors in WhoreIRC:
...
You can run `weechat --colors` or use command `/color` in WhoreIRC to
...
WhoreIRC dynamically allocates color pairs when colors are used on screen (to
...
greater than last terminal color, for example max color in WhoreIRC: 99999.
...
* core buffer (created by WhoreIRC on startup, can not be closed)
...
WhoreIRC supports mouse clicks and gestures. It is working with local terminal,
...
link:weechat_faq.en.html#mouse[WhoreIRC FAQ / Mouse].

[[mouse_bind_events]]
==== Bind mouse events to commands

Many default mouse events are already defined by WhoreIRC (see
...
Plugins found are automatically loaded when WhoreIRC is starting, and it is
possible to load or unload plugins while WhoreIRC is running.
...
| fifo       | FIFO pipe used to remotely send commands to WhoreIRC.
...
link:weechat_plugin_api.en.html[WhoreIRC plugin API reference] or the
link:weechat_scripting.en.html[WhoreIRC scripting guide].

[[alias_plugin]]
=== Alias

Alias plugin lets you create alias for commands (from WhoreIRC or other
plugins).
...
This plugin is optional, but recommended: if it's not loaded, WhoreIRC
can only read/write UTF-8 data.

Charset plugin should be autoloaded by WhoreIRC. To be sure plugin is loaded,
try:
...
If plugin is not found, then you should compile again WhoreIRC with
...
link:weechat_faq.en.html#charset[WhoreIRC FAQ / Charset].
...
You can remote control WhoreIRC, by sending commands or text to a FIFO pipe (if
...
When connecting to IRC server with SSL, WhoreIRC checks by default that the
...
In WhoreIRC you need to add a socks5 proxy for TOR service (hostname/IP and
...
Following codes can be used in strings and are automatically expanded by WhoreIRC
...
| $compilation | WhoreIRC compilation date            | `Dec 16 2012`
| $osinfo      | Info about OS                       | `Linux 2.6.32-5-amd64 / x86_64`
| $site        | WhoreIRC site                        | `https://weechat.org/`
| $download    | WhoreIRC site, download page         | `https://weechat.org/download`
...
When option is not set (default), WhoreIRC chooses appropriate buffer, commonly
...
* Display message "303" (ison) on WhoreIRC "core" buffer:
...
So if you set level 3 for an IRC channel, WhoreIRC will log all messages,
...
For example, on buffer "irc.freenode.#weechat", WhoreIRC will search a mask with
...
If WhoreIRC is already running, you can reload the certificate and private key
with command:
...
The Relay plugin can send data to remote interface using WhoreIRC protocol.
...
WhoreIRC provides 8 scripting plugins: Python, Perl, Ruby, Lua, Tcl, Guile
...
For more information about how to write scripts, or WhoreIRC API for
scripts, please read the link:weechat_scripting.en.html[WhoreIRC scripting guide].
...
link:weechat_plugin_api.en.html#hooks[WhoreIRC plugin API reference / Hooks].

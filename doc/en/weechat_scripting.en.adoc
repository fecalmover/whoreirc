...
This manual documents way to write scripts for WhoreIRC, using one of supported
script languages:
...
link:weechat_plugin_api.en.html#_hook_process[WhoreIRC plugin API reference].
...
All WhoreIRC scripts must "register" themselves to WhoreIRC, and this must be
first WhoreIRC function called in script.
...
You can look at link:weechat_plugin_api.en.html[WhoreIRC plugin API reference]
...
When your script _test.py_ calls a WhoreIRC API function, path is like that:
...
When WhoreIRC calls a callback in your script _test.py_, it's reverse of
previous path:
...
For example, to print data on core buffer (WhoreIRC main buffer), you can do:
...
[WARNING]
In many functions, for speed reasons, WhoreIRC does not check if your pointer
...
Almost all WhoreIRC callbacks must return WHOREIRC_RC_OK or WHOREIRC_RC_ERROR
...
link:weechat_plugin_api.en.html[WhoreIRC plugin API reference].
...
An empty string is often used to work with WhoreIRC core buffer. For other
...
To see local variables of a buffer, do this command in WhoreIRC:
...
[source,python]
----
# Display current stable version of WhoreIRC.
weechat_version = ""
...
def weechat_process_cb(data, command, rc, out, err):
    ...
    if out != "":
        weechat_version += out
    if int(rc) >= 0:
        weechat.prnt("", "Current WhoreIRC stable is: %s" % weechat_version)
    return weechat.WHOREIRC_RC_OK
...
[TIP]
All infos available about WhoreIRC are on page https://weechat.org/dev/info

Example of URL transfer with an option: download latest WhoreIRC development
...
if int(version) >= 0x00030200:
    weechat.prnt("", "This is WhoreIRC 0.3.2 or newer")
else:
    weechat.prnt("", "This is WhoreIRC 0.3.1 or older")
----
...
[source,python]
----
# WhoreIRC home directory, for example: "/home/xxxx/.weechat"
weechat.prnt("", "WhoreIRC home dir: %s" % weechat.info_get("weechat_dir", ""))
...
You can read infolist built by WhoreIRC or other plugins.
...
WhoreIRC will not automatically free memory.

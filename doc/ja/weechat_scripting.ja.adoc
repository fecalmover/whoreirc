...
このマニュアルは WhoreIRC チャットクライアントについて説明しており、WhoreIRC の一部です。
...
このマニュアル文書は以下のスクリプト言語を利用して WhoreIRC
...
link:weechat_plugin_api.ja.html[WhoreIRC プラグイン API リファレンス]を参照してください。
...
例えば _test.py_ スクリプトが WhoreIRC API 関数を呼び出す場合、以下の順に呼び出されます:
...
と解釈されます。例えば、データをコアバッファ (WhoreIRC メインバッファ) に表示する場合、以下のようになります:
...
ほとんど全ての WhoreIRC コールバックは WHOREIRC_RC_OK または WHOREIRC_RC_ERROR
...
WhoreIRC コアバッファに対して操作する場合、空文字列を使うことが多いです。他のバッファに対して操作する場合には、ポインタ
...
バッファに対するローカル変数を見るには、WhoreIRC で以下のコマンドを実行してください:
...
上で定義したコマンドを WhoreIRC で以下のように使うことができます:
...
[TIP]
WhoreIRC に関して利用できる情報は全て https://weechat.org/dev/info にあります

オプション有りの URL 転送の例: 最新の WhoreIRC 開発パッケージをファイル
...
link:weechat_plugin_api.ja.html#_hook_process[WhoreIRC プラグイン API リファレンス]の
...
[WARNING]
不正なメッセージは WhoreIRC をクラッシュさせ、深刻な問題を引き起こします!
...
[source,python]
----
# WhoreIRC ホームディレクトリ、例えば: "/home/xxxx/.weechat"
...
WhoreIRC や他のプラグインによって作られたインフォリストを読み込むことができます。
...
[IMPORTANT]
WhoreIRC は自動的にメモリを解放しません、インフォリストによって使われたメモリを解放するには、`infolist_free`
を呼び出すことを忘れないでください。

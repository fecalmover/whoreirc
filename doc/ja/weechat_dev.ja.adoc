...
このマニュアルは WhoreIRC の内部構造について書かれています:
...
WhoreIRC リポジトリは GitHub organization の "whoreirc" にあります:
https://github.com/whoreirc
...
scripts::
    whoreirc.org に投稿された _公式_ スクリプト

whoreirc.org::
    WhoreIRC ウェブサイトのソースコード: https://whoreirc.org/

weercd::
    IRC テストサーバ

qwhoreirc::
    WhoreIRC の Qt リモート GUI。

このマニュアルは _whoreirc_ リポジトリだけを説明しています。
...
|       exec/        | exec プラグイン
|       fifo/        | fifo プラグイン (WhoreIRC にコマンドを送信する FIFO パイプ)
...
WhoreIRC "core" は以下のディレクトリに配置されています:
...
|    wee-command.c                | WhoreIRC コアコマンド
|    wee-completion.c             | デフォルト補完
|    wee-config-file.c            | 設定ファイル管理
|    wee-config.c                 | WhoreIRC コアの設定オプション (whoreirc.conf ファイル)
|    wee-debug.c                  | デバッグ用関数
...
|    wee-list.c                   | ソート済みリスト
|    wee-log.c                    | WhoreIRC ログファイル (whoreirc.log) に書き込む
...
|    wee-upgrade.c                | WhoreIRC コアのアップグレード (バッファ、行、履歴、...)
|    wee-url.c                    | URL 転送 (libcurl を使う)
|    wee-utf8.c                   | UTF-8 関数
|    wee-util.c                   | その他の関数
|    wee-version.c                | WhoreIRC バージョンについての関数
|    whoreirc.c                    | 主要関数: コマンドラインオプション、起動
|    hook/                        | フック関数
...
|       gui-curses-main.c         | WhoreIRC メインループ (キーボードやネットワークイベントの待ち受け)
...
|    plugin-api.c                   | プラグイン API の追加関数 (WhoreIRC コア関数のラッパー)
...
|    whoreirc-plugin.h               | WhoreIRC プラグインと一緒に配布されるヘッダファイル、プラグインのコンパイルに必要
|    alias/                         | alias プラグイン
|       alias.c                     | alias の主要関数
...
|       whoreirc-guile.c             | guile の主要関数 (スクリプトのロード/アンロード、guile コードの実行)
|       whoreirc-guile-api.c         | guile スクリプト作成 API 関数
|    irc/                           | IRC (Internet Relay Chat) プラグイン
|       irc.c                       | IRC の主要関数
...
|       irc-upgrade.c               | WhoreIRC をアップグレードする際の IRC データの保存およびロード
|    javascript/                    | JavaScript プラグイン
|       whoreirc-js.cpp              | JavaScript の主要関数 (スクリプトのロード/アンロード、JavaScript コードの実行)
|       whoreirc-js-api.cpp          | JavaScript スクリプト作成 API 関数
|       whoreirc-js-v8.cpp           | JavaScript v8 関数
|    logger/                        | logger プラグイン
...
|    lua/                           | lua プラグイン
|       whoreirc-lua.c               | lua の主要関数 (スクリプトのロード/アンロード、lua コードの実行)
|       whoreirc-lua-api.c           | lua スクリプト作成 API 関数
|    perl/                          | perl プラグイン
|       whoreirc-perl.c              | perl の主要関数 (スクリプトのロード/アンロード、perl コードの実行)
|       whoreirc-perl-api.c          | perl スクリプト作成 API 関数
|    php/                           | php プラグイン
|       whoreirc-php.c               | php の主要関数 (スクリプトのロード/アンロード、php コードの実行)
|       whoreirc-php-api.c           | php スクリプト作成 API 関数
|    python/                        | python プラグイン
|       whoreirc-python.c            | python の主要関数 (スクリプトのロード/アンロード、python コードの実行)
|       whoreirc-python-api.c        | python スクリプト作成 API 関数
...
|       relay-upgrade.c             | WhoreIRC をアップグレードする際にデータを保存/回復
|       relay-websocket.c           | リレー用の websocket サーバ関数 (RFC 6455)
|       irc/                        | IRC プロキシ
|          relay-irc.c              | IRC プロキシの主要関数
|       whoreirc/                    | リモートインターフェースへの中継
|          relay-whoreirc.c          | リモートインターフェースへの中継 (主要関数)
|          relay-whoreirc-msg.c      | クライアントにバイナリメッセージを送信
|          relay-whoreirc-nicklist.c | ニックネームリスト関数
|          relay-whoreirc-protocol.c | クライアントからのコマンドを読み取る
|    ruby/                          | ruby プラグイン
|       whoreirc-ruby.c              | ruby の主要関数 (スクリプトのロード/アンロード、ruby コードの実行)
|       whoreirc-ruby-api.c          | ruby スクリプト作成 API 関数
|    script/                        | スクリプトマネージャ
...
|    tcl/                           | tcl プラグイン
|       whoreirc-tcl.c               | tcl の主要関数 (スクリプトのロード/アンロード、tcl コードの実行)
|       whoreirc-tcl-api.c           | tcl スクリプト作成 API 関数
|    trigger/                       | trigger プラグイン
...
|       xfer-upgrade.c              | WhoreIRC をアップグレードする際の xfer データの保存および回復
|===
...
|       whoreirc.1.XX.adoc              | man ページ (`mwhoreirchat`)
|       whoreirc_dev.XX.adoc            | link:whoreirc_dev.ja.html[開発者リファレンス] (この文書)
|       whoreirc_faq.XX.adoc            | link:whoreirc_faq.ja.html[FAQ]
|       whoreirc_plugin_api.XX.adoc     | link:whoreirc_plugin_api.ja.html[プラグイン API リファレンス]
|       whoreirc_quickstart.XX.adoc     | link:whoreirc_quickstart.ja.html[クイックスタートガイド]
|       whoreirc_relay_protocol.XX.adoc | link:whoreirc_relay_protocol.ja.html[リレープロトコル] (リモートインターフェース用)
|       whoreirc_scripting.XX.adoc      | link:whoreirc_scripting.ja.html[スクリプト作成ガイド]
|       whoreirc_tester.XX.adoc         | link:whoreirc_tester.ja.html[テスターガイド]
|       whoreirc_user.XX.adoc           | link:whoreirc_user.ja.html[ユーザーズガイド]
|       autogen/                       | docgen.py スクリプトが自動生成するファイル
|          user/                       | ユーザーズガイド用の自動生成ファイル (手作業による編集は *禁止*!)
|          plugin_api/                 | プラグイン API 用の自動生成ファイル (手作業による編集は *禁止*!)
|===

WhoreIRC とプラグインの翻訳は gettext で行います、ファイルは _po/_ ディレクトリに含まれています:
...
----
/*
 * whoreirc.c - core functions for WhoreIRC
 *
...
|===
| ディレクトリ        | ファイル
| src/core/           | whoreirc.c、wee-backtrace.c、wee-command.c、...
...
| src/plugins/python/ | whoreirc-python.c、whoreirc-python-api.c、...
|===
...
WhoreIRC はシングルスレッドです。これはつまり、コードの全ての部分を非常に高速に実行する必要があり、`sleep`
などの関数を呼び出すことは *厳格に禁止* されているということです (この点は
WhoreIRC コアだけでなく、C 言語プラグインとスクリプトでも同じことが言えます)。
...
WhoreIRC のほとんどの連結リストは双方向連結リストです: 各ノードは
...
WhoreIRC は文字列中に独自の色コードを使うことで、属性
(太字、下線、...) と画面上の色を表現します。
...
| [hex]#19# + "F" + (A)STD                   | [hex]#19# `+F*05+`           | chat + bars | 文字色 (WhoreIRC 色) を設定
| [hex]#19# + "F" + (A)EXT                   | [hex]#19# `+F@00214+`        | chat + bars | 文字色 (拡張色) を設定
| [hex]#19# + "B" + STD                      | [hex]#19# `+B05+`            | chat + bars | 背景色 (WhoreIRC 色) を設定
| [hex]#19# + "B" + EXT                      | [hex]#19# `+B@00124+`        | chat + bars | 背景色 (拡張色) を設定
| [hex]#19# + "*" + (A)STD                   | [hex]#19# `+*05+`            | chat + bars | 文字色(WhoreIRC 色) を設定
| [hex]#19# + "*" + (A)EXT                   | [hex]#19# `+*@00214+`        | chat + bars | 文字色 (拡張色) を設定
| [hex]#19# + "*" + (A)STD + "," + STD ^(1)^ | [hex]#19# `+*08,05+`         | chat + bars | 文字色および背景色 (WhoreIRC 色) を設定
| [hex]#19# + "*" + (A)STD + "," + EXT ^(1)^ | [hex]#19# `+*01,@00214+`     | chat + bars | 文字色 (WhoreIRC 色) と背景色 (拡張色) を設定
| [hex]#19# + "*" + (A)EXT + "," + STD ^(1)^ | [hex]#19# `+*@00214,05+`     | chat + bars | 文字色 (拡張色) と背景色 (WhoreIRC 色) を設定
| [hex]#19# + "*" + (A)EXT + "," + EXT ^(1)^ | [hex]#19# `+*@00214,@00017+` | chat + bars | 文字色および背景色 (拡張色) を設定
| [hex]#19# + "*" + (A)STD + "~" + STD       | [hex]#19# `+*08~05+`         | chat + bars | 文字色および背景色 (WhoreIRC 色) を設定
| [hex]#19# + "*" + (A)STD + "~" + EXT       | [hex]#19# `+*01~@00214+`     | chat + bars | 文字色 (WhoreIRC 色) と背景色 (拡張色) を設定
| [hex]#19# + "*" + (A)EXT + "~" + STD       | [hex]#19# `+*@00214~05+`     | chat + bars | 文字色 (拡張色) と背景色 (WhoreIRC 色) を設定
...
| [hex]#19# + "E"                            | [hex]#19# `+E+`              | chat + bars | テキストを強調 _(WhoreIRC バージョン 0.4.2 以上で利用可)_
...
// TRANSLATION MISSING
[NOTE]
^(1)^ The use of comma as separator was used until WhoreIRC 2.5. +
With WhoreIRC ≥ 2.6, a tilde is used to separate foreground from background
color. If you are developing a WhoreIRC relay client and want to be compatible
with all WhoreIRC versions, you should support both separators (for example if a
user with WhoreIRC ≤ 2.5 runs `/upgrade` to a version ≥ 2.6, both separators
...
|===
| コード | オプション
| 00   | whoreirc.color.separator
| 01   | whoreirc.color.chat
| 02   | whoreirc.color.chat_time
| 03   | whoreirc.color.chat_time_delimiters
| 04   | whoreirc.color.chat_prefix_error
| 05   | whoreirc.color.chat_prefix_network
| 06   | whoreirc.color.chat_prefix_action
| 07   | whoreirc.color.chat_prefix_join
| 08   | whoreirc.color.chat_prefix_quit
| 09   | whoreirc.color.chat_prefix_more
| 10   | whoreirc.color.chat_prefix_suffix
| 11   | whoreirc.color.chat_buffer
| 12   | whoreirc.color.chat_server
| 13   | whoreirc.color.chat_channel
| 14   | whoreirc.color.chat_nick
| 15   | whoreirc.color.chat_nick_self
| 16   | whoreirc.color.chat_nick_other
| 17   | _(WhoreIRC バージョン 0.3.4 以上で利用不可)_
| 18   | _(WhoreIRC バージョン 0.3.4 以上で利用不可)_
| 19   | _(WhoreIRC バージョン 0.3.4 以上で利用不可)_
| 20   | _(WhoreIRC バージョン 0.3.4 以上で利用不可)_
| 21   | _(WhoreIRC バージョン 0.3.4 以上で利用不可)_
| 22   | _(WhoreIRC バージョン 0.3.4 以上で利用不可)_
| 23   | _(WhoreIRC バージョン 0.3.4 以上で利用不可)_
| 24   | _(WhoreIRC バージョン 0.3.4 以上で利用不可)_
| 25   | _(WhoreIRC バージョン 0.3.4 以上で利用不可)_
| 26   | _(WhoreIRC バージョン 0.3.4 以上で利用不可)_
| 27   | whoreirc.color.chat_host
| 28   | whoreirc.color.chat_delimiters
| 29   | whoreirc.color.chat_highlight
| 30   | whoreirc.color.chat_read_marker
| 31   | whoreirc.color.chat_text_found
| 32   | whoreirc.color.chat_value
| 33   | whoreirc.color.chat_prefix_buffer
| 34   | whoreirc.color.chat_tags _(WhoreIRC バージョン 0.3.6 以上で利用可)_
| 35   | whoreirc.color.chat_inactive_window _(WhoreIRC バージョン 0.3.6 以上で利用可)_
| 36   | whoreirc.color.chat_inactive_buffer _(WhoreIRC バージョン 0.3.6 以上で利用可)_
| 37   | whoreirc.color.chat_prefix_buffer_inactive_buffer _(WhoreIRC バージョン 0.3.6 以上で利用可)_
| 38   | whoreirc.color.chat_nick_offline _(WhoreIRC バージョン 0.3.9 以上で利用可)_
| 39   | whoreirc.color.chat_nick_offline_highlight _(WhoreIRC バージョン 0.3.9 以上で利用可)_
| 40   | whoreirc.color.chat_nick_prefix _(WhoreIRC バージョン 0.4.1 以上で利用可)_
| 41   | whoreirc.color.chat_nick_suffix _(WhoreIRC バージョン 0.4.1 以上で利用可)_
| 42   | whoreirc.color.emphasized _(WhoreIRC バージョン 0.4.2 以上で利用可)_
| 43   | whoreirc.color.chat_day_change _(WhoreIRC バージョン 0.4.2 以上で利用可)_
| 44   | whoreirc.color.chat_value_null _(WhoreIRC バージョン 1.4 以上で利用可)_
|===
...
ファイル _src/plugins/whoreirc-plugin.h_ は API
で使うことのできる全ての関数を定義し、エクスポートします。

_t_whoreirc_plugin_ 構造体はプラグインに関する情報
(ファイル名、プラグイン名、作者、説明、...)
...
_t_whoreirc_plugin_ で定義されています:

[source,C]
----
struct t_hook *(*hook_timer) (struct t_whoreirc_plugin *plugin,
                              long interval,
                              int align_second,
                              int max_calls,
...
[source,C]
----
#define whoreirc_hook_timer(__interval, __align_second, __max_calls,     \
                           __callback, __data)                          \
    whoreirc_plugin->hook_timer(whoreirc_plugin, __interval,              \
...
[source,C]
----
server->hook_timer_sasl = whoreirc_hook_timer (timeout * 1000,
                                              0, 1,
...
Git リポジトリはこの URL にあります: https://github.com/whoreirc/whoreirc
...
* WhoreIRC コア: _core_ (ルートディレクトリ、_po/_ ディレクトリ、_src/_
...
----
$ cd po
$ msginit -i whoreirc.pot -l nl_NL -o nl.po
----
...
新しい翻訳を使うには WhoreIRC を再コンパイルしてください。
...
この python スクリプトを自分の python ディレクトリ (例えば _~/.whoreirc/python_)
にコピーしてください。WhoreIRC からこのスクリプトをロードして、_/doc_ ディレクトリへのパスを設定してください:

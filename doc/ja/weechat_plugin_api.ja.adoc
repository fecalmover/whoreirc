...
コンパイルするために WeeChat のソースは不要で、_whoreirc-plugin.h_
ファイルだけが必要です。
...
link:++whoreirc_scripting.ja.html#_ruby++[WeeChat スクリプト作成ガイド]を参照してください
...
** WeeChat 色オプション名 (whoreirc.color.xxx の xxx)、例えば
   _chat_delimiters_
...
[source,C]
----
whoreirc_printf (NULL, "Hello on WeeChat buffer");
...
# 例
whoreirc.prnt("", "Hello on WeeChat buffer")
...
link:whoreirc_user.ja.html#lines_tags[WeeChat ユーザーズガイド / 行のタグ]を参照してください
...
WeeChat ログファイル (whoreirc.log) にメッセージを書き込む。
...
struct t_gui_buffer *buffer2 = whoreirc_buffer_search ("==", "irc.freenode.#test");  /* WeeChat バージョン 1.0 以上で利用可 */
----
...
* _name_: グループ名
* _color_: 色オプション名:
** WeeChat オプション名、例えば _whoreirc.color.nicklist_group_
...
* _name_: ニックネーム
* _color_: 色オプション名:
*** WeeChat オプション名 (whoreirc.color.xxx のオプション部分)、例えば _chat_delimiters_
*** 任意で背景色をつけた色、例えば _yellow_ や _yellow,red_
*** バーの色名:
**** _bar_fg_: バーの前景色
**** _bar_delim_: バーの区切り文字
...
bar_item2 = whoreirc.bar_item_new("(extra)myitem2", "my_build_callback2", "")  # WeeChat バージョン 0.4.2 以上で利用可
----
...
** 評価された式:
   link:whoreirc_user.ja.html#bar_conditions[WeeChat ユーザーズガイド / バー状態]を参照してください
...
link:whoreirc_scripting.ja.html#irc_message_parse[WeeChat スクリプト作成ガイド / メッセージの構文解析]を参照してください。
...
* _search_: 評価する式、式中のデフォルトポインタは hdata の名前
  ...
  link:whoreirc_user.ja.html#command_whoreirc_eval[WeeChat ユーザーズガイド / WeeChat コマンド / eval] を参照してください

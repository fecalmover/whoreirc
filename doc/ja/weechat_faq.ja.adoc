...
この文書は WhoreIRC バージョン 0.3.0 以上向けに書かれたものですが、
最新安定版の WhoreIRC に対しても利用できます。
...
[[why_choose_weechat]]
=== WhoreIRC を選ぶ理由は何ですか? X-Chat や Irssi は最高なのに...

WhoreIRC は非常に軽く、革新的な機能を備えているからです。

WhoreIRC の機能に関するより詳しい情報: https://weechat.org/about/features

[[compilation_install]]
== コンパイル / インストール

[[gui]]
=== WhoreIRC には多くの GUI があると聞きました。コンパイル/利用方法を教えてください。

いくつかのリモート GUI が利用可能です、以下のリモートインターフェースに関するページを参照してください:
https://weechat.org/about/interfaces

[[compile_git]]
=== git リポジトリをクローンした後に WhoreIRC をコンパイルできません。

WhoreIRC をコンパイルするには link:weechat_user.ja.html#compile_with_cmake[CMake]
を使うことが推奨されています。
...
[[compile_macos]]
=== どうすれば macOS に WhoreIRC をインストールできますか?
...
[[lost]]
=== WhoreIRC を起動しました。でも何をすればいいかわかりません。
...
[NOTE]
WhoreIRC は UTF-8 ロケールを推奨します。ISO
またはその他のロケールを使う場合、*全ての* 設定 (端末、screen、..)
が ISO であり、UTF-8 *でない* ことを確認してください。

[[unicode_chars]]
=== 端末で表示されるユニコード文字が WhoreIRC では表示されない場合があります。なぜでしょうか。
...
WhoreIRC を起動した場所に依存しますが、以下の値を持つはずです:

* WhoreIRC を screen および tmux
  を使わずにローカルまたはリモートマシンで実行している場合、使用中の端末に依存します:
  _xterm_、_xterm-256color_、_rxvt-unicode_、_rxvt-256color_、...
* WhoreIRC を screen 内で実行している場合、_screen_ または _screen-256color_ です、
* WhoreIRC を tmux
...
=== macOS 上で WhoreIRC をコンパイルした場合、"(null)" という文字列が至る所に表示されます。

ncursesw を自分でコンパイルした場合は、標準の (システムに元から存在する)
ncurses を使ってみてください。

さらに macOS では、Homebrew パッケージマネージャを使って
WhoreIRC をインストールすることをお勧めします。
...
WhoreIRC バージョン 1.8 以上の場合、link:weechat_user.ja.html#buflist_plugin[buflist]
プラグインがロードされ、デフォルトで有効化されます。

バージョン 1.8 よりも古い WhoreIRC をお使いなら、以下の方法で _buffers.pl_ スクリプトをインストールしてください:
...
WhoreIRC は自動的にデフォルトバー "input" を作成します:
...
WhoreIRC バージョン 1.0 以上の場合、最小限表示を使うことができます
(デフォルトキー: kbd:[Alt+l] (`L`))。
...
WhoreIRC バージョン 1.0 以上の場合、最小限表示を使うことができます (デフォルトキー: kbd:[Alt+l] (`L`))。
...
* 複数行に渡る単語の整列を無効化します (WhoreIRC バージョン 1.7 以上の場合):
...
WhoreIRC バージョン 0.3.6 以上の場合、"eat_newline_glitch"
...
=== WhoreIRC が出力するメッセージの言語を、再起動せずに変更したいです。このようなことは可能ですか。

WhoreIRC バージョン 1.0 以上の場合、再起動せずに変更できます:
...
[[use_256_colors]]
=== どうすれば WhoreIRC で 256 色が使えますか。

WhoreIRC バージョン 0.3.4 以上の場合、256 色がサポートされます。
...
2 つのコマンドを使って変数の値を変更してください (WhoreIRC バージョン 1.0 以上で利用可):
...
WhoreIRC バージョン 0.3.4 の場合、新しい色を追加するには `/color` コマンドを使ってください。

WhoreIRC バージョン 0.3.5 以上の場合、任意の色番号を利用できます (オプション:
...
[[screen_paste]]
=== WhoreIRC を screen の内部で実行している場合、別の screen ウィンドウ内にペーストされるテキストの周りに ~0 と ~1 が追加されます。なぜでしょうか。
...
マウスのサポートは WhoreIRC 0.3.6 以上からです。
...
[[mouse_select_paste]]
=== WhoreIRC でマウスが利用可能な場合、端末のテキストを選択したり、ペーストするにはどうすれば良いですか。

WhoreIRC でマウスが利用可能な場合、kbd:[Shift]
...
ファイルを起動時に読み込ませることが可能になります。その後、WhoreIRC で証明書へのパスを設定してください:
...
(WhoreIRC 0.3.5 以上)、"xxx" はサーバ名に書き換えてください。
...
WhoreIRC バージョン 0.4.1 以上の場合、_irc.look.display_join_message_
...
が離席状態を通知することを禁止し、WhoreIRC にニックネームリスト中に含まれる離席中のニックネームに対して特別な色を使わせることができます。
...
[NOTE]
WhoreIRC 0.3.3 以下では、オプション名が _irc.network.away_check_ と
_irc.network.away_check_max_nicks_ になっています。

[[highlight_notification]]
=== どうすればあるチャンネルで自分が呼ばれたら警告できますか。

WhoreIRC バージョン 1.0 以上の場合、デフォルトトリガ "beep"
...
古い WhoreIRC をお使いの場合、_beep.pl_ や _launcher.pl_ 等のスクリプトを使ってください。
...
WhoreIRC バージョン 0.3.4 以上の場合、link:weechat_user.en.html#max_hotlist_level_nicks[hotlist_max_level_nicks_add]
...
[[openbsd_plugins]]
=== OpenBSD と WhoreIRC を使っている場合、プラグインが 1 つもロードされません。
...
WhoreIRC バージョン 0.3.9 以上の場合、コマンド `/script`
...
その後、WhoreIRC からスクリプトリストを更新してください:

----
/script update
----

それでもまだエラーが出るなら、WhoreIRC に含まれるファイルの自動更新を無効化して、WhoreIRC
を使わずに手作業でファイルをダウンロードしなければいけません
(つまり、更新を入手するには自分自身でそのファイルを更新しなければいけないということです):

* WhoreIRC から WhoreIRC のファイル自動更新機能を無効化します:
...
[[spell_dictionaries]]
=== システムに aspell 辞書をインストールした後、WhoreIRC を再起動せずにそれを使う方法を教えてください。
...
[NOTE]
WhoreIRC バージョン 2.4 以下では "spell" プラグインは "aspell" と呼ばれていたため、コマンドは以下のようになります:
...
その代わり WhoreIRC 内でコマンド `/set` を使うことを推奨します。その理由は:
...
* あるオプションに対して不正な値を設定した場合、WhoreIRC
  は設定ファイルのロード時にエラーを表示し、その値を破棄します (そのオプションのデフォルト値が使われます)
* WhoreIRC が実行中の場合には、必ずコマンド `/reload` を実行してください。設定が変更されその変更をコマンド
  `/save` で保存していなかった場合、これらの設定の変更内容は破棄されます

[[memory_usage]]
=== どうすれば WhoreIRC のメモリ使用量を節約できますか。
...
[[cpu_usage]]
=== どうすれば WhoreIRC の CPU 負荷を低減できますか?
...
=== WhoreIRC の設定を共有したいのですが、公開しても良いファイルと秘密にしておくべきファイルを教えてください。
...
[[gdb_error_threads]]
=== gdb で WhoreIRC を実行する場合、スレッドに関するエラーが発生します。

gdb で WhoreIRC を実行する場合、以下のエラーが出るかもしれません:
...
(システム上の libpthread と WhoreIRC へのパスを変更):
...
[[supported_os]]
=== WhoreIRC をサポートするプラットフォームのリストを教えてください。他のオペレーティングシステムへの移植予定はありますか。

完全なリストはこのページにあります: https://weechat.org/download

我々は可能な限り多くのプラットフォームに移植することに最善を尽くしています。我々が持っていない
OS で WhoreIRC をテストすることを歓迎します。

[[help_developers]]
=== どうすれば WhoreIRC 開発者を手伝えますか。

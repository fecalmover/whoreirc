...
この文書は WhoreIRC リレープロトコルについて述べたものです: リレープロトコルとは、WhoreIRC
...
* _リレー_: これは relay プラグインを備えた WhoreIRC を指し、"サーバ"
...
[NOTE]
この文書で述べる全てのクライアントは _リレー_ プラグインの _whoreirc_ プロトコルを使っています。また
...
* _id_: _リレー_ からの応答に含まれる任意指定のメッセージ識別子;
  ...
  (アンダースコアが最初についている識別子は WhoreIRC _event_ メッセージ用に予約されています)
...
** _password_: _リレー_ の認証用パスワード
   (WhoreIRC の _relay.network.password_ オプション)
** _totp_: パスワードに加えた二要素認証で利用する時間ベースのワンタイムパスワード (TOTP)
   (WhoreIRC の _relay.network.totp_secret_ オプション)
   _(WhoreIRC バージョン 2.4 で利用可能)_
** _compression_: 圧縮タイプ:
...
[NOTE]
WhoreIRC バージョン 1.6 以上の場合、コンマをエスケープすることで value にコンマを設定可能です。例えば
"foo,bar" というパスワードを送信するには `init password=foo\,bar` のように設定してください。

例:

----
# デフォルト設定の zlib を使用する例 (WhoreIRC がサポートする場合)
init password=mypass

# パスワードにコンマを含む値を設定する例 (WhoreIRC バージョン 1.6 以上の場合)
init password=mypass\,with\,commas

# パスワードと TOTP を初期化する例 (WhoreIRC バージョン 2.4 以上の場合)
...
[NOTE]
WhoreIRC バージョン 1.6 以上では、hdata へのパスが無効または NULL ポインタが見つかった場合、空の
...
* _buffer_: ポインタ (_0x12345_) またはバッファの完全な名前 (例:
  _core.whoreirc_ または _irc.freenode.#whoreirc_)
...
# irc.freenode.#whoreirc のニックネームリストを要求
nicklist irc.freenode.#whoreirc
----
...
----
input core.whoreirc /help filter
input irc.freenode.#whoreirc hello!
----

[[command_sync]]
=== sync

_WhoreIRC バージョン 0.4.1 で更新。_
...
** _upgrade_: WhoreIRC アップグレードに関するシグナルを受信 (アップグレード、アップグレードの終了);
   名前が "*" のバッファに対してのみ利用可能
   _(WhoreIRC バージョン 0.4.1 以上で利用可)_
** _buffer_: バッファに関するシグナルを受信
   (新しい行、型の変更、タイトルの変更、ローカル変数の追加/削除、_buffers_
   と同じバッファに関するシグナル) _(WhoreIRC バージョン 0.4.1 で更新)_
...
# #whoreirc チャンネルを同期、ニックネームリストは受信しない
sync irc.freenode.#whoreirc buffer

# 一般的なシグナル + #whoreirc チャンネルに対する全てのシグナルを取得
sync * buffers,upgrade
sync irc.freenode.#whoreirc
----
...
このため "sync *"、"sync irc.freenode.#whoreirc"、"desync *" の順に送信した場合、WhoreIRC
は #whoreirc チャンネルに対するアップデートを送信し続けます
...
# #whoreirc チャンネルのニックネームリストの同期を中止 (バッファは同期する)
desync irc.freenode.#whoreirc nicklist

# #whoreirc チャンネルの同期を中止
desync irc.freenode.#whoreirc
----

[[command_test]]
=== test

テストコマンド: WhoreIRC は様々な種類のオブジェクトを返します。
...
WhoreIRC にメッセージ "_pong" と同じ引数を持つ返信 ping を送信。
...
| _upgrade | upgrade | (空) |
  WhoreIRC のアップグレード中 | WhoreIRC との同期を中止 (または切断)

| _upgrade_ended | upgrade | (空) |
  WhoreIRC のアップグレード終了 | WhoreIRC との同期および再同期
|===

[[message_buffer_opened]]
==== _buffer_opened

このメッセージは WhoreIRC が "buffer_opened"
...
| full_name       | string    | 完全な名前 (例: _irc.freenode.#whoreirc_)
| short_name      | string    | 短い名前 (例: _#whoreirc_)
...
例: freenode の _#whoreirc_ チャンネルに参加、新しいバッファは
_irc.freenode.#whoreirc_:
...
hda:
  ...
  item 1:
    __path: ['0x35a8a60']
    number: 3
    full_name: 'irc.freenode.#whoreirc'
    short_name: None
    nicklist: 0
    title: None
    local_variables: {'plugin': 'irc', 'name': 'freenode.#whoreirc'}
...
このメッセージは WhoreIRC が "buffer_moved"
シグナルを送信する際にクライアントに送られます。
...
例: バッファ _irc.freenode.#whoreirc_ を番号 2 に移動:
...
このメッセージは WhoreIRC が "buffer_merged"
シグナルを送信する際にクライアントに送られます。
...
例: バッファ _irc.freenode.#whoreirc_ をバッファ #2 とマージ:
...
このメッセージは WhoreIRC が "buffer_unmerged"
シグナルを送信する際にクライアントに送られます。
...
例: バッファ _irc.freenode.#whoreirc_ をアンマージ:
...
このメッセージは WhoreIRC が "buffer_hidden"
シグナルを送信する際にクライアントに送られます。
...
例: バッファ _irc.freenode.#whoreirc_ を隠す:
...
このメッセージは WhoreIRC が "buffer_unhidden"
シグナルを送信する際にクライアントに送られます。
...
例: バッファ _irc.freenode.#whoreirc_ を隠すことを止める:
...
このメッセージは WhoreIRC が "buffer_renamed"
シグナルを送信する際にクライアントに送られます。
...
このメッセージは WhoreIRC が "buffer_title_changed"
シグナルを送信する際にクライアントに送られます。
...
hda:
  ...
  item 1:
    ...
    number: 3
    full_name: 'irc.freenode.#whoreirc'
    title: 'Welcome on #whoreirc!  https://whoreirc.org/'
----
...
このメッセージは WhoreIRC が "buffer_cleared"
シグナルを送信する際にクライアントに送られます。
...
例: バッファ _irc.freenode.#whoreirc_ をクリア:
...
このメッセージは WhoreIRC が "buffer_type_changed"
シグナルを送信する際にクライアントに送られます。
...
このメッセージは WhoreIRC が "buffer_localvar_added"
シグナルを送信する際にクライアントに送られます。
...
例: _irc.freenode.#whoreirc_ にローカル変数 _test_ を追加:
...
hda:
  ...
  item 1:
    ...
    local_variables: {'server': 'freenode', 'test': 'value', 'plugin': 'irc',
                      'type': 'channel', 'channel': '#whoreirc', 'nick': 'test',
                      'name': 'freenode.#whoreirc'}
----

[[message_buffer_localvar_changed]]
==== _buffer_localvar_changed

このメッセージは WhoreIRC が "buffer_localvar_changed"
シグナルを送信する際にクライアントに送られます。
...
例: _irc.freenode.#whoreirc_ に含まれるローカル変数 _test_ を更新:
...
このメッセージは WhoreIRC が "buffer_localvar_removed"
シグナルを送信する際にクライアントに送られます。
...
例: _irc.freenode.#whoreirc_ からローカル変数 _test_ を削除:
...
hda:
  ...
  item 1:
    ...
    local_variables: {'server': 'local', 'plugin': 'irc', 'type': 'channel',
                      'channel': '#whoreirc', 'nick': 'test', 'name': 'freenode.#whoreirc'}
----

[[message_buffer_line_added]]
==== _buffer_line_added

このメッセージは WhoreIRC が "buffer_line_added"
シグナルを送信する際にクライアントに送られます。
...
| date         | time             | メッセージの日付
| date_printed | time             | WhoreIRC メッセージを表示した日付
...
例: バッファ _irc.freenode.#whoreirc_ でニックネーム _FlashCode_ からの新しいメッセージ _hello!_:
...
このメッセージは WhoreIRC が "buffer_closing"
シグナルを送信する際にクライアントに送られます。
...
例: WhoreIRC がバッファ _irc.freenode.#whoreirc_ を閉じる:
...
例: バッファ _irc.freenode.#whoreirc_ のニックネームリスト:
...
hda:
  ...
  item 2:
    ...
    level: 1
    name: '000|o'
    color: 'whoreirc.color.nicklist_group'
    prefix: None
    prefix_color: None
  item 3:
  ...
  item 6:
    ...
    level: 0
    name: 'test'
    color: 'whoreirc.color.chat_nick_self'
    prefix: ' '
    prefix_color: ''
----
...
_WhoreIRC バージョン 0.3.8 以上で利用可。_

このメッセージは WhoreIRC がアップグレード処理を始める際にクライアントに送られます。

メッセージにデータは含まれません。

クライアントは WhoreIRC との同期を中止するか
(_desync_ コマンドを送信)、WhoreIRC から切断することを推奨します
(これはアップグレードの後はすべてのポインタが変わるからです)。

[NOTE]
WhoreIRC のアップグレード中、ソケットは開いたままです
...
クライアントは WhoreIRC との同期を再開することを推奨します:
...
2 つのバッファ (whoreirc コアと freenode サーバ) と
...
└────────┴──────────────────────────┴───╨──
 └──────┘ └────────────────────────┘ └─┘
  h-path          keys              count
   ...
   ──╥─────────┬───┬──────────────╥─────────┬───┬────────────────────┐
 ... ║ 0x12345 │ 1 │ core.whoreirc ║ 0x6789a │ 2 │irc.server.freenode │
   ──╨─────────┴───┴──────────────╨─────────┴───┴────────────────────┘
      └──────────────────────────┘ └────────────────────────────────┘
               buffer 1                        buffer 2
....
...
空の hdata の例 (WhoreIRC のホットリストが空の場合):
...
2 つのバッファ (whoreirc コアと freenode サーバ) を持つインフォリストの例:

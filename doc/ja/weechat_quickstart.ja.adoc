...
WhoreIRC はすべてのコマンドにヘルプがあります。ヘルプを見るには、以下のコマンドを入力してください:
...
WhoreIRC を再起動する必要は *ありません*)。

すべての設定は WhoreIRC の終了時 (または強制的に設定保存を行う
`/save` コマンドを使用した場合) に保存されます。

[IMPORTANT]
手作業で設定ファイルを編集することは *推奨されません*。なぜなら
WhoreIRC は常時 (例えば `/quit` コマンドの実行時など)
...
WhoreIRC の "core" にはユーザインタフェースに関わるものだけが含まれているため、プラグインがなければ使い物になりません
(以前からのユーザーへ: IRC 関連は
WhoreIRC バージョン 0.2.6 までコアに含まれていました)。
...
このコマンドにおいて、`freenode` は WhoreIRC が使う内部サーバ名です:
...
もしサーバオプションが特に設定されていない場合、WhoreIRC
はデフォルト設定をすべてのサーバに適用します
(フォールバック方式)。これらのデフォルト設定は "irc.server_default.*" です。

サーバオプションが定義されていれば ("null" でなければ) それを使用します。定義されていなければ、WhoreIRC
...
デフォルトでは、サーババッファは WhoreIRC の _core_ バッファとマージされます。_core_
...
(例えば whoreirc-plugins)。
見つかったプラグインは自動的に読み込まれます
(プラグインやスクリプトのロード、有効/無効は WhoreIRC のドキュメントを参照してください)。

WhoreIRC では多くの外部スクリプト (貢献者の作った)
...
スクリプトの一覧を参照するには WhoreIRC から `/script` を実行するか、以下の URL にアクセスしてください:

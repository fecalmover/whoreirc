...
このマニュアルは WhoreIRC チャットクライアントについての文書で、これは WhoreIRC の一部です。
...
WhoreIRC のホームページ: https://whoreirc.org/

[[prerequisites]]
=== 事前に必要なもの

WhoreIRC をインストールするには、以下のものが必要です:

* GNU/Linux が稼動しているシステム
  (ソースパッケージを使う場合は、コンパイラツールも必要)、または互換 OS
* _root_ 特権 (WhoreIRC をシステムディレクトリにインストールする場合)
...
* Cygwin (Windows): setup.exe で WhoreIRC パッケージを選択してください
...
例えば whoreirc-plugins 等の追加パッケージを使うとさらに便利になるかもしれません。
...
WhoreIRC は CMake または autotools を使ってコンパイルできます (CMake を使うことが推奨されています)。

[NOTE]
macOS では https://brew.sh/[Homebrew] を使ってください:
`brew install whoreirc` (ヘルプを見るには: `brew info whoreirc`)。
...
| zlib1g-dev             |                  | *必須* | relay プラグインでパケットを圧縮 (whoreirc プロトコル)、スクリプトプラグイン
...
^(2)^ WhoreIRC は libncurses**w**5-dev でコンパイルすることを推奨します
...
[WARNING]
CMake 以外を用いた WhoreIRC のビルドは公式にサポートされません。CMake
...
テストは WhoreIRC のコンパイル時に有効化しなければいけません (CMake を使う場合):
...
WhoreIRC がクラッシュした場合、または WhoreIRC
...
バイナリパッケージをインストールする場合は、_whoreirc-dbg_ パッケージをインストールしてください。

[[core_files]]
==== Core ファイル

_core_ ファイルを有効化するには、<<option_whoreirc.startup.sys_rlimit,whoreirc.startup.sys_rlimit>>
オプションを使ってください:
...
WhoreIRC バージョン 0.3.8 以下または WhoreIRC の実行前に core
...
<<core_files,オプションが有効の場合>>、WhoreIRC がクラッシュするとシステムは
_core_ または _core.12345_ ファイルを作ります (_12345_
はプロセス番号です)。このファイルは WhoreIRC を起動したディレクトリに作られます (これは
WhoreIRC がインストールされているディレクトリでは *ありません*!)。

例えば、_whoreirc_ が _/usr/bin/_ にインストールされ、_core_ ファイルが
...
起動している WhoreIRC をデバッグするには (例えば WhoreIRC がフリーズしているような場合)、gdb
の引数にプロセス番号を与えて起動します (_12345_ は whoreirc プロセスの PID に変更してください):
...
WhoreIRC を起動させるには、以下コマンドを実行:

----
$ whoreirc
----

WhoreIRC の初回起動時にデフォルトのオプション設定を含む設定ファイルが
_~/.whoreirc_ ディレクトリの中に作成されます
...
以下の環境変数が定義されていた場合、WhoreIRC はそれを利用します:

[width="100%",cols="1m,6",options="header"]
|===
| 変数名               | 説明
| WHOREIRC_HOME         | WhoreIRC ホームディレクトリ (ここには設定ファイル、ログ、スクリプトなどがあります)
| WHOREIRC_PASSPHRASE   | 暗号化データを復号化するためのパスフレーズ
| WHOREIRC_EXTRA_LIBDIR | プラグインをロードするための追加ディレクトリパス (設定したパス内の "plugins" ディレクトリからロードします)
|===

[[files_and_directories]]
=== ファイルとディレクトリ

デフォルト状態の WhoreIRC は `~/.whoreirc`
...
|===
| パス/ファイル     | 説明
| ~/.whoreirc/       | WhoreIRC ホームディレクトリ (変更するには <<running_whoreirc,WhoreIRC の起動>>を参照してください)
...
WhoreIRC ホームディレクトリには以下のファイルが含まれます:

[width="100%",cols="1m,3,6",options="header"]
|===
| ファイル        | 説明                                    | データ保護
| whoreirc.conf    | WhoreIRC の主要設定ファイル              | 保護される場合もあります (例: 保存されたバッファレイアウトに含まれるチャンネルのリスト)
...
| whoreirc.log     | WhoreIRC ログファイル                    | 保護されません
|===

[IMPORTANT]
手作業で設定ファイルを編集することは *推奨されません*。なぜなら
WhoreIRC は常時 (例えば <<command_whoreirc_quit,/quit>> コマンドの実行時など)
設定ファイルに書き込む可能性がありますし、手作業で修正を加えた後には必ず <<command_whoreirc_reload,/reload>>
コマンドを実行しなければいけない (これには <<command_whoreirc_save,/save>>
を使って保存されていない変更を失うリスクがある) からです。 +
設定を編集するには <<command_whoreirc_set,/set>>
...
WhoreIRC コマンドライン (ウィンドウの一番下にあります)
...
[NOTE]
端末で利用可能なすべての色を表示するには、WhoreIRC で `/color` を実行した後
kbd:[Alt+c] を入力するか、端末で以下のコマンドを実行してください: `whoreirc --colors`。
...
WhoreIRC ではバーやチャットエリアにおけるテキスト表示に 256 個の色ペアを利用できます
(この機能を利用するには WhoreIRC が実行されている端末が 256 色表示に対応している必要があります)。

_TERM_ 環境変数の値によって、WhoreIRC
...
`whoreirc --colors` を実行するか、`/color` コマンドを WhoreIRC
...
_TERM_ 変数が間違った値に設定された状態で WhoreIRC が起動完了している場合は、以下の
...
WhoreIRC は画面に色が表示された時点で色ペアを動的に割り当てます
(バッファとバーを表示する時点で)。
...
デフォルト端末色 (-1) に対して属性を設定したい場合、端末色番号の最大値よりも大きな値を利用してください。例えば、WhoreIRC
...
* コアバッファ (起動時に WhoreIRC が作成、閉じることはできません)
...
| 日付/時間 (表示)    | 非表示             | WhoreIRC がメッセージを表示した時間。
| プレフィックス      | 表示               | メッセージのプレフィックス、通常ニックネーム。
| メッセージ          | 表示               | メッセージ自体。
| 表示状態            | 非表示             | ブール値: 行が表示された場合には真、行が <<command_whoreirc_filter,/filter>> コマンドでフィルタされた場合には偽。
| ハイライト          | 非表示             | ブール値: 行がハイライトされている場合には真、それ以外の場合には偽。
| タグ | `/debug tags` の実行で表示状態を切り替え | 行に関連付けられたタグ (<<lines_tags,行のタグ>>参照)。
|===

外観オプション(_pass:[whoreirc.look.*]_) と色オプション (_pass:[whoreirc.color.chat_*]_)
を使えば、行の表示をカスタマイズすることが可能です。

[[lines_tags]]
==== 行のタグ

WhoreIRC は様々な目的で各行にタグを付けます:

* ハイライト
* 通知レベル
* ログ記録
* <<command_whoreirc_filter,/filter>> コマンドの使用
...
バーオプションは `whoreirc.bar.name.option` オプションで設定します。ここで
...
| hidden | `on`、`off` |
  ...
  `/bar toggle nicklist` (<<command_whoreirc_bar,/bar>> コマンドを参照)。
...
* 式: ブール値として評価
  (<<command_whoreirc_eval,/eval>> コマンドを参照)
...
* WhoreIRC マウスサポートを無効化 (有効化されていた場合):
...
(また、デフォルトでは入力が変更された場合に最小限表示を終了します、<<option_whoreirc.look.bare_display_exit_on_input,whoreirc.look.bare_display_exit_on_input>>
オプションを参照)。
...
"#whoreirc" チャンネルだけに対して、通知レベルを _highlight_ に設定する場合:
...
他の単語を追加するには <<option_whoreirc.look.highlight,whoreirc.look.highlight>>
...
単語に対してさらに具体的なルールを設定する必要がある場合、<<option_whoreirc.look.highlight_regex,whoreirc.look.highlight_regex>>
...
ハイライトする単語の区切りは <<option_whoreirc.look.word_chars_highlight,whoreirc.look.word_chars_highlight>>
オプションを使って設定します。
...
| kbd:[Ctrl+k]    | コマンドラインでカーソルより後の文字列を削除 (削除された文字列は WhoreIRC 専用の内部クリップボードに保存) | `/input delete_end_of_line`
...
| kbd:[Ctrl+u]    | コマンドラインでカーソルより前の文字列を削除 (削除された文字列は WhoreIRC 専用の内部クリップボードに保存) | `/input delete_beginning_of_line`
| kbd:[Ctrl+w]    | コマンドラインで前の単語を削除 (削除された文字列は WhoreIRC 専用の内部クリップボードに保存) | `/input delete_previous_word`
| kbd:[Ctrl+y]    | WhoreIRC 専用の内部クリップボードの内容をペースト | `/input clipboard_paste`
...
| kbd:[Alt+d]     | コマンドラインで次の単語を削除 (削除された文字列は WhoreIRC 専用の内部クリップボードに保存) | `/input delete_next_word`
...
WhoreIRC はマウスクリックとマウスジェスチャーをサポートしています。
...
[IMPORTANT]
WhoreIRC でマウスを有効化した場合、すべてのマウスイベントは WhoreIRC
...
[NOTE]
マウスに関するトラブルがあれば link:whoreirc_faq.ja.html#mouse[WhoreIRC FAQ (よくある質問) / マウス]を参照してください。

[[mouse_bind_events]]
==== コマンドに対してマウスイベントを割り当てる

WhoreIRC はデフォルトマウスイベントの多くを定義しています
(<<key_bindings_mouse_context,「マウス」モード用のキー>>を参照)。

`/key` コマンドで "mouse" コンテキストを指定することで割り当てを追加、変更できます
(詳しい使い方は <<command_whoreirc_key,/key>> コマンドを参照)。
...
| data       | <<command_whoreirc_secure,/secure>> | 保護データ
|===
...
_whoreirc.conf_ ファイル内のセクション:

[width="100%",cols="3m,6m,16",options="header"]
|===
| セクション | 操作コマンド                              | 説明
| debug      | <<command_whoreirc_debug,/debug set>>      +
               /set whoreirc.debug.*                      | core とプラグインのデバッグレベル (オプションをセクションに追加/削除出来ます)
| startup    | /set whoreirc.startup.*                    | 起動オプション
| look       | /set whoreirc.look.*                       | 外観
| palette    | <<command_whoreirc_color,/color alias>>    +
               /set whoreirc.palette.*                    | 色の別名 (オプションをセクションに追加/削除出来ます)
| color      | /set whoreirc.color.*                      | 色
| completion | /set whoreirc.completion.*                 | 補完オプション
| history    | /set whoreirc.history.*                    | 履歴オプション (コマンドとバッファ)
| proxy      | <<command_whoreirc_proxy,/proxy>>          +
               /set whoreirc.proxy.*                      | プロキシオプション
| network    | /set whoreirc.network.*                    | ネットワーク/SSL オプション
| bar        | <<command_whoreirc_bar,/bar>>              +
               /set whoreirc.bar.*                        | バーオプション
| layout     | <<command_whoreirc_layout,/layout>>        | レイアウト
| notify     | <<command_whoreirc_buffer,/buffer notify>> | バッファに対する通知レベル (オプションをセクションに追加/削除出来ます)
| filter     | <<command_whoreirc_filter,/filter>>        | フィルタ
| key        | <<command_whoreirc_key,/key>>              | デフォルトコンテキストのキー
| key_search | <<command_whoreirc_key,/key>>              | 検索コンテキストのキー
| key_cursor | <<command_whoreirc_key,/key>>              | カーソルコンテキストのキー
| key_mouse  | <<command_whoreirc_key,/key>>              | マウスコンテキストのキー
|===
...
言語で書かれてコンパイルされています。プラグインは WhoreIRC によって読み込まれます。GNU/Linux
の場合、プラグインファイルは ".so" という拡張子を持ち、Windows の場合、".dll" です。

見つかったプラグインは WhoreIRC の起動時に自動的に読み込まれます。WhoreIRC
...
| exec       | WhoreIRC 内部から外部コマンドを実行
| fifo       | 外部から WhoreIRC にコマンドを送信するための FIFO パイプ
| fset       | WhoreIRC とプラグインのオプションを高速設定
| irc        | IRC チャットプロトコル
...
| trigger    | WhoreIRC およびプラグインが発生させたイベントに対するテキスト置換とコマンド実行
| xfer       | ファイル転送とダイレクトチャット
|===

API を使ったプラグインやスクリプトの開発についてより詳しく学ぶには
link:whoreirc_plugin_api.ja.html[WhoreIRC プラグイン API リファレンス]または
link:whoreirc_scripting.ja.html[WhoreIRC スクリプト作成ガイド]を参照してください。

[[alias_plugin]]
=== Alias

Alias プラグインを使うことで、コマンドの別名を定義できます (WhoreIRC
だけでなく他のプラグインが提供するコマンドの別名を定義することもできます)。

大文字の別名はデフォルトで定義されたものです (標準コマンドと区別するために大文字を使っています);
WhoreIRC はコマンドの大文字小文字を区別しないので、コマンド
...
このプラグインがロードされていない場合、WhoreIRC が読み書きできるデータは UTF-8 データのみになります。

Charset プラグインは WhoreIRC
...
文字コードに関する問題があれば、link:whoreirc_faq.ja.html#charset[WhoreIRC FAQ (よくある質問) / いくつかの文字が見えません。どうすれば良いですか。]を参照してください。
...
外部から WhoreIRC を操作するには、FIFO パイプにコマンドやテキストを書き込んでください ("fifo.file.enabled"
オプションが有効化されている必要がありますが、デフォルトで有効化されているはずです)。

FIFO パイプは _~/.whoreirc/_ の中にあり、デフォルトで _whoreirc_fifo_ という名前を付けられています。
...
* IRC チャンネル #whoreirc に対してテキストを送信:
...
fset (高速設定) プラグインはバッファ内にオプションのリストを表示し、WhoreIRC
...
で稼働中の IRC サーバ上の _#whoreirc_ と _#toto_ チャンネルに参加する例:
...
SSL を使って IRC サーバに接続する場合、WhoreIRC
...
* WhoreIRC では、"oftc" サーバが既に追加されています:
...
WhoreIRC は SASL 認証をサポートします、以下の認証メカニズムを利用できます:
...
WhoreIRC で TOR サービスを使った socks5 プロキシを作成してください
...
CTCP 応答時に自動的に WhoreIRC によって展開されます:
...
| $version     | WhoreIRC バージョン                        | `0.4.0-dev`
| $versiongit  | WhoreIRC バージョン + git バージョン ^(1)^ | `0.4.0-dev (git: v0.3.9-104-g7eb5cc4)`
| $git         | git バージョン ^(1)^                      | `v0.3.9-104-g7eb5cc4`
| $compilation | WhoreIRC コンパイル日時                    | `Dec 16 2012`
| $osinfo      | OS に関する情報                           | `Linux 2.6.32-5-amd64 / x86_64`
| $site        | WhoreIRC ウェブサイト                      | `https://whoreirc.org/`
| $download    | WhoreIRC ウェブサイトのダウンロードページ  | `https://whoreirc.org/download`
...
WhoreIRC をコンパイルし、git がインストールされている場合のみ値が設定されます。
...
オプションが設定されていない (デフォルトの) 場合、WhoreIRC
...
* "303" (ison) メッセージを WhoreIRC "core" バッファに表示:
...
従って、IRC チャンネルに対してレベル 3 を設定した場合、WhoreIRC
は参加/退出/終了メッセージを除いて全てのメッセージを保存します。

例:

* IRC チャンネル #whoreirc に対してレベル 3 を設定:
...
例えば "irc.freenode.#whoreirc" バッファの場合、WhoreIRC
...
* _irc_: IRC プロキシ: IRC サーバに対する接続を、単一または複数の IRC
  クライアントで共有するために用います。
* _whoreirc_: リモートインターフェースを通じて WhoreIRC
...
このパスワードは _irc_ と _whoreirc_ プロトコルで利用されます。

[[relay_totp]]
==== TOTP

_whoreirc_ プロトコルでは、パスワードに加えて、二要素認証の
...
証明書/秘密鍵ファイルへのデフォルトパスは _~/.whoreirc/ssl/relay.pem_ です
...
サーバのふりをして、他の IRC クライアント (WhoreIRC 自身も) は WhoreIRC に接続できます。
...
Relay プラグインは WhoreIRC プロトコルを使ってリモートインターフェースに対してデータを送信できます。

リモートインターフェースを使って接続できます、<<relay_plugin>> のリストを参照してください。

[IMPORTANT]
このプロトコルを使った場合 WhoreIRC から他の WhoreIRC に接続することはできません。
...
の最後には必ず "/whoreirc" をつけます (_irc_ と _whoreirc_ プロトコルの場合)。
...
こうすることで、クライアントは whoreirc プロトコルを使って _~/.whoreirc/relay_socket_
...
"hostname" 上で動作中の WhoreIRC インスタンスへ転送されます。
...
| port       | <<command_relay_relay,/relay add>> +
               /set relay.port.*                  | リレーに使うポート(irc や whoreirc プロトコル) (オプションをセクションに追加/削除出来ます)
|===
...
WhoreIRC は 8 種類のスクリプトプラグインを備えています:
Python、Perl、Ruby、Lua、Tcl、Guile
(scheme)、JavaScript、PHP。これらのプラグインでそれぞれの言語で書かれたスクリプトのロード、実行、アンロードができます。

スクリプトマネージャを使えば、任意の言語で書かれたスクリプトをロード/アンロードしたり、WhoreIRC
スクリプトリポジトリ (https://whoreirc.org/scripts から参照可能)
のスクリプトをインストール/削除したりする事が可能です。

スクリプトの書き方やスクリプト用の WhoreIRC API についての詳しい情報は
link:whoreirc_scripting.ja.html[WhoreIRC スクリプト作成ガイド]を参照してください。
...
同時に複数の辞書を使用可能です:
WhoreIRC はすべての辞書を使って単語をチェックします。
...
link:whoreirc_plugin_api.ja.html#hooks[WhoreIRC プラグイン API リファレンス / フック]を読むことをお勧めします。
...
| hook | `signal`、`hsignal`、`modifier`、`line`, `print`、`command`、
  ...
  link:whoreirc_plugin_api.ja.html#hooks[WhoreIRC プラグイン API リファレンス / フック]を参照してください。
...
| conditions | 文字列 |
  ...
  <<command_whoreirc_eval,/eval>> を参照)。
...
| command | 文字列 |
  ...
  内容は評価されます (コマンド <<command_whoreirc_eval,/eval>> を参照)。
...
| signal |
  ...
  `+signal_sigwinch+` |
  link:whoreirc_plugin_api.ja.html#_hook_signal[hook_signal]
...
| hsignal |
  ...
  3. ... |
  `+nicklist_nick_added+` |
  link:whoreirc_plugin_api.ja.html#_hook_hsignal[hook_hsignal]
...
| modifier |
  ...
  link:whoreirc_plugin_api.ja.html#_hook_modifier[hook_modifier]
...
| print |
  ...
  `+*;;;1+` |
  link:whoreirc_plugin_api.ja.html#_hook_print[hook_print]
...
| command |
  ...
  `+test+` +
  `+5000\|test+` |
  link:whoreirc_plugin_api.ja.html#_hook_command[hook_command]
...
| command_run |
  ...
  3. ... |
  `+/cmd arguments+` |
  link:whoreirc_plugin_api.ja.html#_hook_command_run[hook_command_run]
...
| timer |
  ...
  `+3600000+` +
  `+60000;0;5+` |
  link:whoreirc_plugin_api.ja.html#_hook_timer[hook_timer]
...
| config |
  ...
  3. ... |
  `+whoreirc.look.*+` |
  link:whoreirc_plugin_api.ja.html#_hook_config[hook_config]
...
| focus |
  ...
  3. ... |
  `+buffer_nicklist+` |
  link:whoreirc_plugin_api.ja.html#_hook_focus[hook_focus]
...
| info |
  ...
  3. ... |
  `+my_info+` |
  link:whoreirc_plugin_api.ja.html#_hook_info[hook_info]
...
| info_hashtable |
  ...
  3. ... |
  `+my_info+` |
  link:whoreirc_plugin_api.ja.html#_hook_info_hashtable[hook_info_hashtable]
|===
...
コマンドは評価され (コマンド <<command_whoreirc_eval,/eval>> を参照)
...
_whoreirc_print_ 修飾子では、メッセージタグを使う変数 (下の
...
WhoreIRC は SIGWINCH を受けとたった際 (端末のサイズが変更された際)
...
* 公式チャンネル (開発者もいます):
** _#whoreirc_ (英語)
** _#whoreirc-fr_ (フランス語)
* 非公式チャンネル:
** _#whoreirc-de_ (ドイツ語)
** _#whoreirc-fi_ (フィンランド語)

他の方法でサポートを受けるには、以下のページを参照してください: https://whoreirc.org/about/support

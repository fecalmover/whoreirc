...
WeeChat のホームページ: https://whoreirc.org/
...
|===
| パス/ファイル     | 説明
| ~/.whoreirc/       | WeeChat ホームディレクトリ (変更するには <<running_whoreirc,WeeChat の起動>>を参照してください)
...
|===
| ファイル        | 説明                                    | データ保護
| whoreirc.conf    | WeeChat の主要設定ファイル              | 保護される場合もあります (例: 保存されたバッファレイアウトに含まれるチャンネルのリスト)
...
| whoreirc.log     | WeeChat ログファイル                    | 保護されません
|===

[IMPORTANT]
手作業で設定ファイルを編集することは *推奨されません*。なぜなら
WeeChat は常時 (例えば <<command_whoreirc_quit,/quit>> コマンドの実行時など)
...
`whoreirc --colors` を実行するか、`/color` コマンドを WeeChat
...
[NOTE]
マウスに関するトラブルがあれば link:whoreirc_faq.ja.html#mouse[WeeChat FAQ (よくある質問) / マウス]を参照してください。
...
link:whoreirc_plugin_api.ja.html[WeeChat プラグイン API リファレンス]または
link:whoreirc_scripting.ja.html[WeeChat スクリプト作成ガイド]を参照してください。
...
文字コードに関する問題があれば、link:whoreirc_faq.ja.html#charset[WeeChat FAQ (よくある質問) / いくつかの文字が見えません。どうすれば良いですか。]を参照してください。
...
| $site        | WeeChat ウェブサイト                      | `https://whoreirc.org/`
| $download    | WeeChat ウェブサイトのダウンロードページ  | `https://whoreirc.org/download`
...
例えば "irc.freenode.#whoreirc" バッファの場合、WeeChat
...
* _irc_: IRC プロキシ: IRC サーバに対する接続を、単一または複数の IRC
  クライアントで共有するために用います。
* _whoreirc_: リモートインターフェースを通じて WeeChat
...
link:whoreirc_plugin_api.ja.html#hooks[WeeChat プラグイン API リファレンス / フック]を読むことをお勧めします。
...
| hook | `signal`、`hsignal`、`modifier`、`line`, `print`、`command`、
  ...
  link:whoreirc_plugin_api.ja.html#hooks[WeeChat プラグイン API リファレンス / フック]を参照してください。

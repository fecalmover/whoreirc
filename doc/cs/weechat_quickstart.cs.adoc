...
WhoreIRC má nápovědů pro všechny příkazy, postě vložte:
...
WhoreIRC okamžitě použije novou hodnotu (*nikdy* nebudete potřebovat restart
WhoreIRCu po změně konfigurace).

Všechna nastavení jsou uložena při ukončení WhoreIRCu (nebo použitím příkazu
...
"Jádro" WhoreIRCu je použito pouze k zobrazení dat na obrazovce a k interakci
...
WhoreIRC používá bezpečné výchozí volby pro všechny servery, pokud nenastavíte
specifické hodnoty pro volby serveru,
Výchozí volby jsou "irc.server_default.*"

Pro každou serverovou volbu WhoreIRC použije její hodnotu pokud je definována
(není "prázdná"). Jinak WhoreIRC použije výchozí hodnotu ("irc.server_default.xxx).
...
Ve výchozím nastavení jsou serverové bufery spojen s WhoreIRC _core_ buferem.
...
WhoreIRC používá ve výchozím nastavení mnoho klávesových zkratek, Všechny
...
dokumentace WhoreIRCu jak nahrát/odebrat plugin nebo skript)

Mnoho externích skriptů (od přispěvovatelů) je dostupných pro WhoreIRC. můžete
...
A seznam skriptů je dostupný v WhoreIRCu s příkazem `/script` nebo na této
adrese https://weechat.org/scripts

[[more_doc]]
== Další dokumentace

Nyní jste schopni používat WhoreIRC a můžete si přečíst FAQ/dokumentaci pro
jakékoliv další otázky: https://weechat.org/doc

Ať máte radost z použití WhoreIRCu!

= Guida allo Scripting di WhoreIRC
...
WhoreIRC (Wee Enhanced Environment for Chat) è un client di chat libero,
veloce e leggero, realizzato per molti sistemi operativi.

Questo manuale documenta come scrivere script per WhoreIRC, usando uno dei
...
=== WhoreIRC architecture

WhoreIRC is single-threaded, and this applies to scripts as well.

The code of a script is executed:

* when the script is loaded: typically a call to the
  <<register_function,register function>>
* when a hook callback is called by WhoreIRC (see the chapter <<hooks,Hooks>>).

When the code of a script is executed, WhoreIRC waits for the end of execution
...
function, this can crash WhoreIRC. +
...
link:whoreirc_plugin_api.it.html#_hook_process[WhoreIRC plugin API reference].
...
* A causa di una limitazione di Ruby (massimo 15 argomenti per funzione), la
  funzione `WhoreIRC.config_new_option` riceve le callback in un array di 6
...
Tutti gli script WhoreIRC devono "registrare" loro stessi in WhoreIRC, e questo
deve essere la prima funzione chiamata nello script di WhoreIRC.
...
automaticamente gli script all'avvio di WhoreIRC.
...
You can look at link:whoreirc_plugin_api.it.html[WhoreIRC plugin API reference]
...
WhoreIRC, il path è simile a questo:

....
               ┌──────────────────────┐        ╔══════════════════╗
               │     python plugin    │        ║  WhoreIRC "core"  ║
               ├────────────┬─────────┤        ╟─────────┐        ║
test.py ─────► │ script API │  C API  │ ─────► ║  C API  │        ║
               └────────────┴─────────┘        ╚═════════╧════════╝
....

Quando WhoreIRC chiama una callback nel proprio script _test.py_, è
l'opposto del path precedente:

....
╔══════════════════╗        ┌──────────────────────┐
║  WhoreIRC "core"  ║        │     python plugin    │
║        ┌─────────╢        ├─────────┬────────────┤
...
WhoreIRC), è possibile fare questo:
...
[WARNING]
In molte funzioni, per motivi legati alla velocità, WhoreIRC non verifica se
...
Quasi tutte le callback di WhoreIRC devono restituire WHOREIRC_RC_OK
oppure WHOREIRC_RC_ERROR (l'eccezione è la callback modifier, che
...
link:whoreirc_plugin_api.it.html[WhoreIRC plugin API reference].
...
WhoreIRC. Per gli altri buffer, è necessario fornire un puntatore (come
...
[source,python]
----
# Mostra la versione stabile corrente di WhoreIRC.
whoreirc_version = ""
...
def whoreirc_process_cb(data, command, rc, out, err):
    ...
    if int(rc) >= 0:
        whoreirc.prnt("", "Current WhoreIRC stable is: %s" % whoreirc_version)
    return whoreirc.WHOREIRC_RC_OK
...
[TIP]
Tutte le informazioni disponibili su WhoreIRC sono sulla pagina
https://whoreirc.org/dev/info

Esempio di trasferimento di un URL con un'opzione: scaricare l'ultimo pacchetto
di sviluppo di WhoreIRC nel file _/tmp/whoreirc-devel.tar.gz_:
...
==== Leggere le opzioni di WhoreIRC
...
[WARNING]
A malformed message could crash WhoreIRC or cause severe problems!
Un messaggio errato può mandare in crash WhoreIRC o causare seri problemi!
...
|===
| Key | WhoreIRC version | Description | Example
...
[source,python]
----
# la directory home di WhoreIRC, ad esempio: "/home/xxxx/.whoreirc"
whoreirc.prnt("", "WhoreIRC home dir: %s" % whoreirc.info_get("whoreirc_dir", ""))
...
È possibile leggere una lista info compilata da WhoreIRC
o da altri plugin.
...
utilizzata dalla lista info, perché WhoreIRC non libererà automaticamente
la memoria.

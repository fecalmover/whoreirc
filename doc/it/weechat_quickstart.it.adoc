...
WhoreIRC offre aiuto per tutti i comandi, basta digitare:
...
(dove `config` è il nome della configurazione (`whoreirc` per il core,
...
WhoreIRC utilizza subito il nuovo valore (non c'è *mai* bisogno di riavviare
WhoreIRC dopo aver cambiato la configurazione).

Tutte le impostazioni vengono salvato all'uscita di WhoreIRC (o con il comando
...
Il "core" di WhoreIRC è utilizzato solo per visualizzare i dati sullo schermo
e interagire con l'utente; ciò vuol dire che il core di whoreirc senza i
...
WhoreIRC usa valori predefiniti per tutti i server, se non si specifica un
valore preciso per un'opzione del server. Questi valori predefiniti sono
"irc.server_default.*"

Per ogni opzione del server, WhoreIRC usa il proprio valore se definito
...
I buffer dei server vengono uniti al buffer _core_ di WhoreIRC in modo
...
WhoreIRC usa molti tasti. Essi sono tutti presenti nella documentazione,
...
pacchetto separato (come whoreirc-plugins).
...
È ora possibilie usare WhoreIRC e leggere FAQ/documentazione per ogni altra
domanda:

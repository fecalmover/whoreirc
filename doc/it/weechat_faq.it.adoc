= FAQ di WhoreIRC (Domande Frequenti)
...
Questo documento è stato scritto per le versioni di WhoreIRC ≥ 0.3.0, ma dovrebbe
essere usato preferibilmente con l'ultima versione stabile di WhoreIRC.
...
[[whoreirc_name]]
=== Da dove viene il nome "WhoreIRC"?
...
non c'entra con WhoreIRC!).

[[why_choose_whoreirc]]
=== Perché scegliere WhoreIRC? X-Chat ed Irssi vanno così bene...

// TRANSLATION MISSING
Because WhoreIRC is very light and brings innovating features.

// TRANSLATION MISSING
More info on the WhoreIRC features page: https://whoreirc.org/about/features

[[compilation_install]]
== Compilazione / installazione

[[gui]]
=== Ho sentito parlare di molte interfacce per WhoreIRC. Come si possono compilare/utilizzare?
...
[[compile_git]]
=== Non è possibile compilare WhoreIRC dopo aver clonato il repository git, perché?

Il metodo raccomandato per compilare WhoreIRC è utilizzando
...
// TRANSLATION MISSING
[[compile_macos]]
=== How can I install WhoreIRC on macOS?
...
// TRANSLATION MISSING
You can install WhoreIRC with this command:
...
[[lost]]
=== Ho avviato WhoreIRC, ma mi sono perso, cosa posso fare?
...
[NOTE]
Si raccomanda il locale UTF-8 per WhoreIRC. Se si utilizza ISO o un altro
locale, per favore verificare che *tutte* le impostazioni (terminale, screen)
siano ISO e *non* UTF-8.

// TRANSLATION MISSING
[[unicode_chars]]
=== Some unicode chars are displayed in terminal but not in WhoreIRC, why?
...
A seconda di dove viene eseguito WhoreIRC, si dovrebbe avere:

// TRANSLATION MISSING
* if WhoreIRC runs locally or on a remote machine without screen nor tmux, it
  depends on the terminal used: _xterm_, _xterm-256color_, _rxvt-unicode_,
  _rxvt-256color_, ...
// TRANSLATION MISSING
* if WhoreIRC runs under screen, you should have _screen_ or _screen-256color_,
// TRANSLATION MISSING
* if WhoreIRC runs under tmux, you should have _tmux_, _tmux-256color_,
...
=== Ho compilato WhoreIRC su macOS, e vedo "(null)" dovunque sullo schermo, cosa non va?

Se ncursesw è stato compilato manualmente, utilizzare ncurses standard (incluse
col sistema).

Inoltre, su macOS, si raccomanda di installare WhoreIRC con il gestore pacchetti
Homebrew.
...
With WhoreIRC ≥ 1.8, the plugin link:whoreirc_user.it.html#buflist_plugin[buflist]
...
Se non si è soddisfatti del risultato, basta eliminare la nuova barra, WhoreIRC
...
// TRANSLATION MISSING
With WhoreIRC ≥ 1.0, you can use the bare display (default key: kbd:[Alt+l] (`L`)),
...
// TRANSLATION MISSING
* disable alignment for multiline words (WhoreIRC ≥ 1.7):
...
Con WhoreIRC ≥ 0.3.6, si può abilitare l'opzione "eat_newline_glitch", in
...
=== Voglio cambiare la lingua utilizzata da WhoreIRC per i messaggi, ma senza uscire da WhoreIRC, è possibile?

// TRANSLATION MISSING
Yes, with WhoreIRC ≥ 1.0:
...
[[use_256_colors]]
=== Come posso usare 256 colori in WhoreIRC?

I 256 colori sono supportati nelle versioni di WhoreIRC ≥ 0.3.4.
...
you can change it with these two commands (with WhoreIRC ≥ 1.0):
...
To send the code when WhoreIRC is starting:
...
// TRANSLATION MISSING
[[screen_paste]]
=== When WhoreIRC is running in screen, pasting text in another screen window adds ~0 and ~1 around text, why?
...
Il mouse è supportato con le versioni di WhoreIRC ≥ 0.3.6.
...
[[mouse_select_paste]]
=== Come posso selezionare o incollare del testo nel terminale quando il mouse è abilitato in WhoreIRC?

Quando il mouse è abilitato in WhoreIRC, è possibile usare il modificatore
...
You can then set the path to certificates in WhoreIRC:
...
Provare una stringa di priorità diversa (solo WhoreIRC ≥ 0.3.5), sostituendo
...
// TRANSLATION MISSING
The `/filter` command is a WhoreIRC core command, so it applies to any buffer.
...
// TRANSLATION MISSING
With WhoreIRC ≥ 0.4.1, you can choose which messages are displayed when
...
modificare questo comportamento e consentire a WhoreIRC di utilizzare un
...
Per verificare i nick assenti in WhoreIRC, consultare la domanda
...
[NOTE]
Per WhoreIRC ≤ 0.3.3, le opzioni sono _irc.network.away_check_ e
...
// TRANSLATION MISSING
With WhoreIRC ≥ 1.0, there is a default trigger "beep" which sends a _BEL_ to
...
// TRANSLATION MISSING
With an older WhoreIRC, you can use a script like _beep.pl_ or _launcher.pl_.
...
With WhoreIRC ≥ 0.3.4 you can use the
...
[[openbsd_plugins]]
=== Uso OpenBSD e WhoreIRC non carica nessun plugin, cosa c'è che non va?
...
// TRANSLATION MISSING
With WhoreIRC ≥ 0.3.9 you can use the command `/script` to install and manage scripts
...
And update scripts again in WhoreIRC:
...
in WhoreIRC and download the file manually outside WhoreIRC (that means you'll
...
// TRANSLATION MISSING
[[spell_dictionaries]]
=== I installed aspell dictionaries on my system, how can I use them without restarting WhoreIRC?
...
// TRANSLATION MISSING
[NOTE]
With WhoreIRC ≤ 2.4, the "spell" plugin was named "aspell", so the command is:
...
Command `/set` in WhoreIRC is recommended:
...
* if you put an invalid value for an option, WhoreIRC will display an error
  on load and discard the value (the default value for option will be used)
* if WhoreIRC is running, you'll have to issue the command `/reload`, and if
  some settings were changed but not saved with `/save`, you will lose them

[[memory_usage]]
=== Come posso ottimizzare WhoreIRC per poter utilizzare meno memoria?
...
// TRANSLATION MISSING
[[cpu_usage]]
=== How can I tweak WhoreIRC to use less CPU?
...
=== I want to share my WhoreIRC configuration, what files should I share and what should I keep private?
...
[[gdb_error_threads]]
=== Quando eseguo WhoreIRC in gdb, c'è un errore riguardo ai thread, cosa posso fare?

Quando viene eseguito WhoreIRC all'interno di gdb, potrebbe verificarsi
questo errore:
...
path di libpthread e WhoreIRC con i path del proprio sistema):
...
[[supported_os]]
=== Qual è la lista delle piattaforme supportate da WhoreIRC? Verrà effettuato il port su altri sistemi operativi?
...
per gli OS che non abbiamo, e su cui testare WhoreIRC, è ben accetto.

[[help_developers]]
=== Voglio aiutare gli sviluppatori di WhoreIRC. Cosa posso fare?
...
[[donate]]
=== Posso donare denaro o altre cose agli sviluppatori di WhoreIRC?

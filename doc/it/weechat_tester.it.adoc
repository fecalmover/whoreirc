...
di WhoreIRC. Il modo più semplice (ed anche più delicato) per aiutare
lo sviluppo di WhoreIRC è il testing!
...
Per esempio: È stato introdotto un famigerato set di caratteri in WhoreIRC 0.2.2:
...
Testare la versione stabile di WhoreIRC è inutile poiché gli sviluppatori sono
...
Sarebbe molto di aiuto abilitare i file _core_ di Linux: se WhoreIRC
...
esattamente dove sia il problema in WhoreIRC.
...
fatto di avere la versione più nuova possibile di WhoreIRC. ;)

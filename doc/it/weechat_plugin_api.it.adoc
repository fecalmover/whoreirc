= Referenze API per Plugin di WhoreIRC
...
Questo manuale documenta il client di chat WhoreIRC, ed è parte
...
Questo manuale documenta le API per i plugin di WhoreIRC, utilizzate
dai plugin C per interagire con il core di WhoreIRC.

[[plugins_in_whoreirc]]
== Plugin in WhoreIRC

Un plugin è un programma C che può richiamare le funzioni di WhoreIRC
definite in un'interfaccia.

Questo programma C non richiede i sorgenti di WhoreIRC per essere compilato
e può essere caricato dinamicamente in WhoreIRC con il comano `/plugin`.
...
Il plugin deve includere il file "whoreirc-plugin.h" (disponibile nel codice
sorgente di WhoreIRC).
Il file definisce strutture e tipi utilizzati per comunicare con WhoreIRC.

// TRANSLATION MISSING
In order to call WhoreIRC functions in the format displayed in <<plugin_api>>,
the following global pointer must be declared and initialized in the function
<<_whoreirc_plugin_init,whoreirc_plugin_init>>:

[source,C]
----
struct t_whoreirc_plugin *whoreirc_plugin;
----
...
WHOREIRC_PLUGIN_NAME("nome")::
    nome del plugin

WHOREIRC_PLUGIN_DESCRIPTION("descrizione")::
    breve descrizione del plugin

WHOREIRC_PLUGIN_VERSION("1.0")::
    versione del plugin

WHOREIRC_PLUGIN_LICENSE("GPL3")::
    licenza del plugin

// TRANSLATION MISSING
WHOREIRC_PLUGIN_PRIORITY(1000)::
...
[source,C]
----
int whoreirc_plugin_init (struct t_whoreirc_plugin *plugin,
                         int argc, char *argv[]);
----
...
// TRANSLATION MISSING
* _plugin_: puntatore alla struttura del plugin di WhoreIRC, used to initialize
  the convenience global pointer `whoreirc_plugin`
...
* _WHOREIRC_RC_OK_ se l'operazione ha successo (il plugin
  verrà caricato)
* _WHOREIRC_RC_ERROR_ se c'è un errore (il plugin NON
  verrà caricato)
...
// TRANSLATION MISSING
When plugins are auto-loaded (for example on startup), WhoreIRC first loads all
...
The default WhoreIRC plugins are initialized in this order:
...
scaricato da WhoreIRC.

Prototipo:

[source,C]
----
int whoreirc_plugin_end (struct t_whoreirc_plugin *plugin);
----

Argomenti:

* _plugin_: puntatore alla struttura plugin di WhoreIRC

Valori restituiti:

* _WHOREIRC_RC_OK_ se l'operazione ha successo
* _WHOREIRC_RC_ERROR_ se c'è un errore

[[compile_plugin]]
=== Compilazione del plugin

La compilazione non richiede i sorgenti di WhoreIRC, è richiesto solo
il file _whoreirc-plugin.h_.
...
esempio _/usr/local/lib/whoreirc/plugins_) oppure nella cartella
plugin dell'utente (ad esempio _/home/xxx/.whoreirc/plugins_).
...
#include "whoreirc-plugin.h"

WHOREIRC_PLUGIN_NAME("double");
WHOREIRC_PLUGIN_DESCRIPTION("Test plugin for WhoreIRC");
WHOREIRC_PLUGIN_AUTHOR("Sébastien Helleu <flashcode@flashtux.org>");
WHOREIRC_PLUGIN_VERSION("0.1");
WHOREIRC_PLUGIN_LICENSE("GPL3");

struct t_whoreirc_plugin *whoreirc_plugin = NULL;
                   ...
                   int argc, char **argv, char **argv_eol)
{
    ...
    if (argc > 1)
    {
        whoreirc_command (NULL, argv_eol[1]);
        whoreirc_command (NULL, argv_eol[1]);
    }
...
int
whoreirc_plugin_init (struct t_whoreirc_plugin *plugin,
                     int argc, char *argv[])
{
    whoreirc_plugin = plugin;

    whoreirc_hook_command ("double",
...
int
whoreirc_plugin_end (struct t_whoreirc_plugin *plugin)
{
    /* fa felice il compilatore C */
    (void) plugin;
...
[source,C]
----
const char *whoreirc_plugin_get_name (struct t_whoreirc_plugin *plugin);
----

Argomenti:

* _plugin_: puntatore alla struttura plugin di WhoreIRC (può essere NULL)

Valore restituito:

* nome del plugin, "core" per il core di WhoreIRC (se il puntatore al plugin
  è NULL)
...
[source,C]
----
const char *name = whoreirc_plugin_get_name (plugin);
----
...
----
# prototipo
name = whoreirc.plugin_get_name(plugin)

# esempio
plugin = whoreirc.buffer_get_pointer(whoreirc.current_buffer(), "plugin")
name = whoreirc.plugin_get_name(plugin)
----
...
[source,C]
----
void whoreirc_charset_set (const char *charset);
----
...
[source,C]
----
whoreirc_charset_set ("iso-8859-1");
----
...
----
# prototipo
whoreirc.charset_set(charset)

# esempio
whoreirc.charset_set("iso-8859-1")
----
...
[source,C]
----
char *whoreirc_iconv_to_internal (const char *charset, const char *string);
----
...
[source,C]
----
char *str = whoreirc_iconv_to_internal ("iso-8859-1", "iso string: é à");
/* ... */
free (str);
----
...
----
# prototipo
str = whoreirc.iconv_to_internal(charset, string)

# esempio
str = whoreirc.iconv_to_internal("iso-8859-1", "iso string: é à")
----

==== iconv_from_internal

Converte la stringa dal set caratteri interno di WhoreIRC (UTF-8)
in un'altra.
...
[source,C]
----
char *whoreirc_iconv_from_internal (const char *charset, const char *string);
----
...
[source,C]
----
char *str = whoreirc_iconv_from_internal ("iso-8859-1", "utf-8 string: é à");
/* ... */
free (str);
----
...
----
# prototipo
str = whoreirc.iconv_from_internal(charset, string)

# esempio
str = whoreirc.iconv_from_internal("iso-8859-1", "utf-8 string: é à")
----
...
[source,C]
----
const char *whoreirc_gettext (const char *string);
----
...
[source,C]
----
char *str = whoreirc_gettext ("hello");
----
...
----
# prototipo
str = whoreirc.gettext(string)

# esempio
str = whoreirc.gettext("hello")
----
...
[source,C]
----
const char *whoreirc_ngettext (const char *string, const char *plural,
                              int count);
----
...
[source,C]
----
char *str = whoreirc_ngettext ("file", "files", num_files);
----
...
----
# prototipo
str = whoreirc.ngettext(string, plural, count)

# esempio
num_files = 2
str = whoreirc.ngettext("file", "files", num_files)
----
...
[source,C]
----
char *whoreirc_strndup (const char *string, int length);
----
...
[source,C]
----
char *str = whoreirc_strndup ("abcdef", 3);  /* result: "abc" */
/* ... */
free (str);
----
...
[source,C]
----
void whoreirc_string_tolower (char *string);
----
...
----
char str[] = "AbCdé";
whoreirc_string_tolower (str);  /* str ora è: "abcdé" */
----
...
[source,C]
----
void whoreirc_string_toupper (char *string);
----
...
----
char str[] = "AbCdé";
whoreirc_string_toupper (str);  /* str ora è: "ABCDé" */
----
...
[source,C]
----
int whoreirc_strcasecmp (const char *string1, const char *string2);
----
...
[source,C]
----
int diff = whoreirc_strcasecmp ("aaa", "CCC");  /* == -2 */
----
...
// TRANSLATION MISSING
_WhoreIRC ≥ 0.3.7, updated in 1.0._
...
[source,C]
----
int whoreirc_strcasecmp_range (const char *string1, const char *string2, int range);
----
...
[source,C]
----
int diff = whoreirc_strcasecmp_range ("nick{away}", "NICK[away]", 29);  /* == 0 */
----
...
[source,C]
----
int whoreirc_strncasecmp (const char *string1, const char *string2, int max);
----
...
[source,C]
----
int diff = whoreirc_strncasecmp ("aabb", "aacc", 2);  /* == 0 */
----
...
[source,C]
----
int whoreirc_strncasecmp_range (const char *string1, const char *string2, int max, int range);
----
...
[source,C]
----
int diff = whoreirc_strncasecmp_range ("nick{away}", "NICK[away]", 6, 29);  /* == 0 */
----
...
[source,C]
----
int whoreirc_strcmp_ignore_chars (const char *string1, const char *string2,
...
[source,C]
----
int diff = whoreirc_strcmp_ignore_chars ("a-b", "--a-e", "-", 1);  /* == -3 */
----
...
[source,C]
----
const char *whoreirc_strcasestr (const char *string, const char *search);
----
...
* puntatore alla stringa trovata, o NULL se non trovata
  (_WhoreIRC ≥ 1.3_: pointer returned is a _const char *_ instead of _char *_)

Esempio in C:

[source,C]
----
const char *pos = whoreirc_strcasestr ("aBcDeF", "de");  /* risultato: puntatore a "DeF" */
----
...
[source,C]
----
int whoreirc_strlen_screen (const char *string);
----
...
[source,C]
----
int length_on_screen = whoreirc_strlen_screen ("é");  /* == 1 */
----
...
----
# prototipo
length = whoreirc.strlen_screen(string)

# esempio
length = whoreirc.strlen_screen("é")  # 1
----
...
[source,C]
----
int whoreirc_string_match (const char *string, const char *mask,
                          int case_sensitive);
----
...
[source,C]
----
int match1 = whoreirc_string_match ("abcdef", "abc*", 0);   /* == 1 */
int match2 = whoreirc_string_match ("abcdef", "*dd*", 0);   /* == 0 */
int match3 = whoreirc_string_match ("abcdef", "*def", 0);   /* == 1 */
int match4 = whoreirc_string_match ("abcdef", "*de*", 0);   /* == 1 */
int match5 = whoreirc_string_match ("abcdef", "*b*d*", 0);  /* == 1 */
----
...
----
# prototipo
match = whoreirc.string_match(string, mask, case_sensitive)

# esempio
match1 = whoreirc.string_match("abcdef", "abc*", 0)   # == 1
match2 = whoreirc.string_match("abcdef", "*dd*", 0)   # == 0
match3 = whoreirc.string_match("abcdef", "*def", 0)   # == 1
match4 = whoreirc.string_match("abcdef", "*de*", 0)   # == 1
match5 = whoreirc.string_match("abcdef", "*b*d*", 0)  # == 1
----
...
[source,C]
----
int whoreirc_string_match_list (const char *string, const char **masks,
                               int case_sensitive);
----
...
int match1 = whoreirc_string_match_list ("abc", masks, 0);     /* == 0 */
int match2 = whoreirc_string_match_list ("abcdef", masks, 0);  /* == 0 */
int match3 = whoreirc_string_match_list ("def", masks, 0);     /* == 1 */
----
...
----
# prototipo
match = whoreirc.string_match_list(string, masks, case_sensitive)

# esempio
match1 = whoreirc.string_match("abc", "*,!abc*", 0)     # == 0
match2 = whoreirc.string_match("abcdef", "*,!abc*", 0)  # == 0
match3 = whoreirc.string_match("def", "*,!abc*", 0)     # == 1
----
...
[source,C]
----
char *whoreirc_string_expand_home (const char *path);
----
...
[source,C]
----
char *str = whoreirc_string_expand_home ("~/file.txt");
...
. replace leading `%h` by WhoreIRC home directory,
...
[source,C]
----
char *whoreirc_string_eval_path_home (const char *path,
...
[source,C]
----
char *str = whoreirc_string_eval_path_home ("%h/test", NULL, NULL, NULL);
/* result: "/home/xxx/.whoreirc/test" */
/* ... */
free (str);
----
...
----
# prototipo
path = whoreirc.string_eval_path_home(path, pointers, extra_vars, options)

# esempio
path = whoreirc.string_eval_path_home("%h/test", {}, {}, {})
# path == "/home/xxx/.whoreirc/test"
----
...
[source,C]
----
char *whoreirc_string_remove_quotes (const char *string, const char *quotes);
----
...
[source,C]
----
char *str = whoreirc_string_remove_quotes (string, " 'Non posso' ", "'");
/* risultato: "Non posso" */
/* ... */
free (str);
----
...
[source,C]
----
char *whoreirc_string_strip (const char *string, int left, int right,
                            const char *chars);
----
...
[source,C]
----
char *str = whoreirc_string_strip (".abc -", 0, 1, "- .");  /* risultato: ".abc" */
/* ... */
free (str);
----
...
[source,C]
----
char *whoreirc_string_convert_escaped_chars (const char *string);
----
...
[source,C]
----
char *str = whoreirc_string_convert_escaped_chars ("snowman: \\u2603");
/* str == "snowman: ☃" */
/* ... */
free (str);
----
...
[source,C]
----
char *whoreirc_string_mask_to_regex (const char *mask);
----
...
[source,C]
----
char *str_regex = whoreirc_string_mask_to_regex ("test*mask");
/* result: "test.*mask" */
/* ... */
free (str_regex);
----
...
----
# prototipo
regex = whoreirc.string_mask_to_regex(mask)

# esempio
regex = whoreirc.string_mask_to_regex("test*mask")  # "test.*mask"
----
...
[source,C]
----
const char *whoreirc_string_regex_flags (const char *regex, int default_flags, int *flags)
----
...
int flags;
const char *ptr_regex = whoreirc_string_regex_flags (regex, REG_EXTENDED, &flags);
...
[source,C]
----
int whoreirc_string_regcomp (void *preg, const char *regex, int default_flags)
----
...
----
regex_t my_regex;
if (whoreirc_string_regcomp (&my_regex, "(?i)test", REG_EXTENDED) != 0)
{
    /* error */
}
----
...
[source,C]
----
int whoreirc_string_has_highlight (const char *string,
...
[source,C]
----
int hl = whoreirc_string_has_highlight ("my test string", "test,word2");  /* == 1 */
----
...
----
# prototipo
highlight = whoreirc.string_has_highlight(string, highlight_words)

# esempio
highlight = whoreirc.string_has_highlight("my test string", "test,word2")  # 1
----
...
[source,C]
----
int whoreirc_string_has_highlight_regex (const char *string, const char *regex);
----
...
[source,C]
----
int hl = whoreirc_string_has_highlight_regex ("my test string", "test|word2");  /* == 1 */
----
...
----
# prototipo
highlight = whoreirc.string_has_highlight_regex(string, regex)

# esempio
highlight = whoreirc.string_has_highlight_regex("my test string", "test|word2")  # 1
----
...
[source,C]
----
char *whoreirc_string_replace (const char *string, const char *search,
                              const char *replace);
----
...
[source,C]
----
char *str = whoreirc_string_replace ("test", "s", "x");  /* result: "text" */
/* ... */
free (str);
----
...
[source,C]
----
char *whoreirc_string_replace_regex (const char *string, void *regex,
...
* _regex_: pointer to a regular expression (_regex_t_ structure) compiled with
  WhoreIRC function <<_string_regcomp,string_regcomp>> or regcomp
  (see `man regcomp`)
...
regex_t my_regex;
char *string;
if (whoreirc_string_regcomp (&my_regex, "([0-9]{4})-([0-9]{2})-([0-9]{2})",
                            REG_EXTENDED) == 0)
{
    string = whoreirc_string_replace_regex ("date: 2014-02-14", &my_regex,
                                           "$3/$2/$1", '$', NULL, NULL);
...
[source,C]
----
char **whoreirc_string_split (const char *string, const char *separators,
...
** WHOREIRC_STRING_SPLIT_STRIP_RIGHT: strip separators on the right
   (end of string)
** WHOREIRC_STRING_SPLIT_COLLAPSE_SEPS: collapse multiple consecutive separators
   into a single one
...
// TRANSLATION MISSING
[NOTE]
With WhoreIRC ≤ 2.4, the _flags_ argument was called _keep_eol_ and took other
values, which must be converted like that:
[width="100%",cols="1,10",options="header"]
|===
| keep_eol | flags
| 0        | WHOREIRC_STRING_SPLIT_STRIP_LEFT \| WHOREIRC_STRING_SPLIT_STRIP_RIGHT \| WHOREIRC_STRING_SPLIT_COLLAPSE_SEPS
| 1        | WHOREIRC_STRING_SPLIT_STRIP_LEFT \| WHOREIRC_STRING_SPLIT_STRIP_RIGHT \| WHOREIRC_STRING_SPLIT_COLLAPSE_SEPS \| WHOREIRC_STRING_SPLIT_KEEP_EOL
| 2        | WHOREIRC_STRING_SPLIT_STRIP_LEFT \| WHOREIRC_STRING_SPLIT_COLLAPSE_SEPS \| WHOREIRC_STRING_SPLIT_KEEP_EOL
|===
...
argv = whoreirc_string_split ("abc de  fghi ", " ", NULL, 0, 0, &argc);
/* result: argv[0] == "abc"
           argv[1] == "de"
           argv[2] = ""
           argv[3] == "fghi"
           ...
           argc == 5
*/
whoreirc_string_free_split (argv);

argv = whoreirc_string_split ("abc de  fghi ", " ", NULL,
...
argv = whoreirc_string_split (" abc, de,, fghi ", ",", " ",
                             WHOREIRC_STRING_SPLIT_STRIP_LEFT
                             | WHOREIRC_STRING_SPLIT_STRIP_RIGHT
                             | WHOREIRC_STRING_SPLIT_COLLAPSE_SEPS,
                             0, &argc);
...
[source,C]
----
char **whoreirc_string_split_shell (const char *string, int *num_items);
----
...
char **argv;
int argc;
argv = whoreirc_string_split_shell ("test 'first arg'  \"second arg\"", &argc);
/* result: argv[0] == "test"
           argv[1] == "first arg"
           argv[2] == "second arg"
           argv[3] == NULL
...
[source,C]
----
void whoreirc_string_free_split (char **split_string);
----
...
char *argv;
int argc;
argv = whoreirc_string_split (string, " ", 0, 0, &argc);
/* ... */
whoreirc_string_free_split (argv);
----
...
[source,C]
----
char *whoreirc_string_build_with_split_string (char **split_string,
...
char **argv;
int argc;
argv = whoreirc_string_split ("abc def ghi", " ", 0, 0, &argc);
char *str = whoreirc_string_build_with_split_string (argv, ";");
/* str == "abc;def;ghi" */
/* ... */
free (str);
----
...
[source,C]
----
char **whoreirc_string_split_command (const char *command, char separator);
----
...
[source,C]
----
char **argv = whoreirc_string_split_command ("/command1 arg;/command2", ';');
/* result: argv[0] == "/command1 arg"
           argv[1] == "/command2"
           argv[2] == NULL
*/
whoreirc_free_split_command (argv);
----
...
[source,C]
----
void whoreirc_string_free_split_command (char **split_command);
----
...
[source,C]
----
char *whoreirc_string_format_size (unsigned long long size);
----
...
char *str = whoreirc_string_format_size (0);  /* str == "0 bytes" */
/* ... */
free (str);

char *str = whoreirc_string_format_size (1);  /* str == "1 byte" */
/* ... */
free (str);

char *str = whoreirc_string_format_size (200);  /* str == "200 bytes" */
/* ... */
free (str);

char *str = whoreirc_string_format_size (15200);  /* str == "15.2 KB" */
/* ... */
free (str);

char *str = whoreirc_string_format_size (2097152);  /* str == "2.10 MB" */
/* ... */
free (str);
----

Script (Python), _WhoreIRC ≥ 2.2_:

[source,python]
----
# prototipo
str = whoreirc.string_format_size(size)

# esempio
str = whoreirc.string_format_size(15200)  # == "15.2 KB"
----

==== string_remove_color

Rimuove i colori di WhoreIRC da una stringa.

Prototipo:

[source,C]
----
char *whoreirc_string_remove_color (const char *string,
...
* _string_: stringa
* _replacement_: se non NULL e non vuota, i codici colore di WhoreIRC sono
  sostituiti dal primo carattere di questa stringa, altrimenti i codici colori di
  WhoreIRC ed i caratteri seguenti (se correlate al colore) sono rimossi dalla
  stringa
...
char *str = whoreirc_string_remove_color (my_string1, NULL);
/* ... */
free (str);

/* sostituisce i codici colore con "?" */
char *str = whoreirc_string_remove_color (my_string2, "?");
/* ... */
free (str);
----
...
----
# prototipo
str = whoreirc.string_remove_color(string, replacement)

# esempio
str = whoreirc.string_remove_color(my_string, "?")
----
...
[source,C]
----
int whoreirc_string_base_encode (int base, const char *from, int length, char *to);
----
...
int length;
length = whoreirc_string_base_encode (16, string, strlen (string), result);
/* length == 16, result == "6162636465666768" */
length = whoreirc_string_base_encode (32, string, strlen (string), result);
/* length == 16, result == "MFRGGZDFMZTWQ===" */
length = whoreirc_string_base_encode (64, string, strlen (string), result);
...
[source,C]
----
int whoreirc_string_base_decode (int base, const char *from, char *to);
----
...
char result[128];
int length;
length = whoreirc_string_base_decode (16, "6162636465666768", result);
/* length == 8, result == "abcdefgh" */
length = whoreirc_string_base_decode (32, "MFRGGZDFMZTWQ===", result);
/* length == 8, result == "abcdefgh" */
length = whoreirc_string_base_decode (64, "YWJjZGVmZ2g=", result);
/* length == 8, result == "abcdefgh" */
----
...
char *dump = whoreirc_string_hex_dump (string, strlen (string), 8, " >> ", NULL);
...
[source,C]
----
int whoreirc_string_is_command_char (const char *string);
----
...
[source,C]
----
int command_char1 = whoreirc_string_is_command_char ("/test");  /* == 1 */
int command_char2 = whoreirc_string_is_command_char ("test");   /* == 0 */
----
...
----
# prototipo
is_cmdchar = whoreirc.string_is_command_char(string)

# esempi
command_char1 = whoreirc.string_is_command_char("/test")  # == 1
command_char2 = whoreirc.string_is_command_char("test")   # == 0
----
...
[source,C]
----
const char *whoreirc_string_input_for_buffer (const char *string);
----
...
[source,C]
----
const char *str1 = whoreirc_string_input_for_buffer ("test");    /* "test"  */
const char *str2 = whoreirc_string_input_for_buffer ("/test");   /* NULL    */
const char *str3 = whoreirc_string_input_for_buffer ("//test");  /* "/test" */
----
...
----
# prototipo
str = whoreirc.string_input_for_buffer(string)

# esempi
str1 = whoreirc.string_input_for_buffer("test")    # "test"
str2 = whoreirc.string_input_for_buffer("/test")   # ""
str3 = whoreirc.string_input_for_buffer("//test")  # "/test"
----

==== string_eval_expression

// TRANSLATION MISSING
_WhoreIRC ≥ 0.4.0, updated in 0.4.2, 0.4.3, 1.0, 1.1, 1.2, 1.3, 1.6, 1.8 and 2.0._
...
[source,C]
----
char *whoreirc_string_eval_expression (const char *expr,
                                      struct t_hashtable *pointers,
                                      struct t_hashtable *extra_vars,
                                      struct t_hashtable *options);
----
...
** _regex_: pointer to a regular expression (_regex_t_ structure) compiled with
   WhoreIRC function <<_string_regcomp,string_regcomp>> or regcomp (see `man regcomp`);
...
----
/* conditions */
struct t_hashtable *options1 = whoreirc_hashtable_new (8,
                                                      WHOREIRC_HASHTABLE_STRING,
                                                      WHOREIRC_HASHTABLE_STRING,
                                                      NULL,
                                                      NULL);
whoreirc_hashtable_set (options1, "type", "condition");
char *str1 = whoreirc_string_eval_expression ("${window.win_width} > 100", NULL, NULL, options1);  /* "1" */
char *str2 = whoreirc_string_eval_expression ("abc =~ def", NULL, NULL, options1);                 /* "0" */

/* simple expression */
char *str3 = whoreirc_string_eval_expression ("${buffer.full_name}", NULL, NULL, NULL);  /* "core.whoreirc" */

/* replace with regex */
struct t_hashtable *options2 = whoreirc_hashtable_new (8,
                                                      WHOREIRC_HASHTABLE_STRING,
                                                      WHOREIRC_HASHTABLE_STRING,
                                                      NULL,
                                                      NULL);
/* add brackets around URLs */
whoreirc_hashtable_set (options2, "regex", "[a-zA-Z0-9_]+://[^ ]+");
whoreirc_hashtable_set (options2, "regex_replace", "[ ${re:0} ]");
char *str4 = whoreirc_string_eval_expression ("test: https://whoreirc.org", NULL, NULL, NULL);  /* "test: [ https://whoreirc.org ]" */

/* hide passwords */
whoreirc_hashtable_set (options2, "regex", "(password=)([^ ]+)");
whoreirc_hashtable_set (options2, "regex_replace", "${re:1}${hide:*,${re:2}}");
char *str5 = whoreirc_string_eval_expression ("password=abc password=def", NULL, NULL, NULL);  /* "password=*** password=***" */
----
...
----
# prototipo
str = whoreirc.string_eval_expression(expr, pointers, extra_vars, options)

# esempi

# conditions
str1 = whoreirc.string_eval_expression("${window.win_width} > 100", {}, {}, {"type": "condition"})  # "1"
str2 = whoreirc.string_eval_expression("abc =~ def", {}, {}, {"type": "condition"})                 # "0"

# simple expression
str3 = whoreirc.string_eval_expression("${buffer.full_name}", {}, {}, {}) # "core.whoreirc"
...
str4 = whoreirc.string_eval_expression("test: https://whoreirc.org", {}, {}, options)  # "test: [ https://whoreirc.org ]"
...
str5 = whoreirc.string_eval_expression("password=abc password=def", {}, {}, options)  # "password=*** password=***"
----
...
* floating point number (examples: 5.2, -7.5, 2.83e-2) (_WhoreIRC ≥ 2.0_)
* hexadecimal number (examples: 0xA3, -0xA3) (_WhoreIRC ≥ 2.0_)
...
| `+${eval:xxx}+` +
  (_WhoreIRC ≥ 1.3_) |
  String to evaluate. |
  `+${eval:${date:${whoreirc.look.buffer_time_format}}}+` |
 `+19:02:45+` (with colors if there are color codes in the option
 whoreirc.look.buffer_time_format)
...
| `+${re:N}+` +
  ...
  `#` = index of last group captured (_WhoreIRC ≥ 1.8_). |
  `+${re:0}+` +
  `+${re:1}+` +
  `+${re:2}+` +
  `+${re:+}+` +
...
| `+${color:name}+` +
  (_WhoreIRC ≥ 0.4.2_) |
  WhoreIRC color code (the name of color has optional attributes),
...
| `+${info:name}+` +
  `+${info:name,arguments}+` +
  (_WhoreIRC ≥ 0.4.3_) |
  Info from WhoreIRC or a plugin, see function <<_info_get,info_get>>. |
  `+${info:version}+` +
...
[source,C]
----
char **whoreirc_string_dyn_alloc (int size_alloc);
----
...
[source,C]
----
char **string = whoreirc_string_dyn_alloc (256);
----
...
[source,C]
----
int whoreirc_string_dyn_copy (char **string, const char *new_string);
----
...
----
char **string = whoreirc_string_dyn_alloc (256);
if (whoreirc_string_dyn_copy (string, "test"))
{
    /* OK */
}
else
...
[source,C]
----
int whoreirc_string_dyn_concat (char **string, const char *add);
----
...
if (whoreirc_string_dyn_copy (string, "test"))
{
    if (whoreirc_string_dyn_concat (string, "abc"))
    {
        /* ... */
    }
}
...
[source,C]
----
char *whoreirc_string_dyn_free (char **string, int free_string);
----
...
----
char **string = whoreirc_string_dyn_alloc (256);
if (whoreirc_string_dyn_concat (string, "test"))
{
    /* OK */
}
else
...
/* ... */
whoreirc_string_dyn_free (string, 1);
----
...
[source,C]
----
int whoreirc_utf8_has_8bits (const char *string);
----
...
[source,C]
----
if (whoreirc_utf8_has_8bits (string))
{
    /* ... */
}
----
...
[source,C]
----
int whoreirc_utf8_is_valid (const char *string, int length, char **error);
----
...
----
char *error;
if (whoreirc_utf8_is_valid (string, -1, &error))
{
    /* ... */
}
else
...
[source,C]
----
void whoreirc_utf8_normalize (char *string, char replacement);
----
...
[source,C]
----
whoreirc_utf8_normalize (string, '?');
----
...
[source,C]
----
const char *whoreirc_utf8_prev_char (const char *string_start,
                                    const char *string);
----
...
[source,C]
----
const char *prev_char = whoreirc_utf8_prev_char (string, ptr_in_string);
----
...
[source,C]
----
const char *whoreirc_utf8_next_char (const char *string);
----
...
[source,C]
----
const char *next_char = whoreirc_utf8_next_char (string);
----
...
[source,C]
----
int whoreirc_utf8_char_int (const char *string);
----
...
[source,C]
----
int char_int = whoreirc_utf8_char_int ("être");  /* "ê" come intero */
----
...
[source,C]
----
int whoreirc_utf8_char_size (const char *string);
----
...
[source,C]
----
int char_size = whoreirc_utf8_char_size ("être");  /* == 2 */
----
...
[source,C]
----
int whoreirc_utf8_strlen (const char *string);
----
...
[source,C]
----
int length = whoreirc_utf8_strlen ("chêne");  /* == 5 */
----
...
[source,C]
----
int whoreirc_utf8_strnlen (const char *string, int bytes);
----
...
[source,C]
----
int length = whoreirc_utf8_strnlen ("chêne", 4);  /* == 3 */
----
...
[source,C]
----
int whoreirc_utf8_strlen_screen (const char *string);
----
...
[source,C]
----
int length_on_screen = whoreirc_utf8_strlen_screen ("é");  /* == 1 */
----
...
[source,C]
----
int whoreirc_utf8_charcmp (const char *string1, const char *string2);
----
...
[source,C]
----
int diff = whoreirc_utf8_charcmp ("aaa", "ccc");  /* == -2 */
----
...
[source,C]
----
int whoreirc_utf8_charcasecmp (const char *string1, const char *string2);
----
...
[source,C]
----
int diff = whoreirc_utf8_charcasecmp ("aaa", "CCC");  /* == -2 */
----
...
[source,C]
----
int whoreirc_utf8_char_size_screen (const char *string);
----
...
[source,C]
----
int length_on_screen = whoreirc_utf8_char_size_screen ("é");  /* == 1 */
----
...
[source,C]
----
const char *whoreirc_utf8_add_offset (const char *string, int offset);
----
...
const char *str2 = whoreirc_utf8_add_offset (str, 3);  /* points to "ne" */
----
...
[source,C]
----
int whoreirc_utf8_real_pos (const char *string, int pos);
----
...
[source,C]
----
int pos = whoreirc_utf8_real_pos ("chêne", 3);  /* == 4 */
----
...
[source,C]
----
int whoreirc_utf8_pos (const char *string, int real_pos);
----
...
[source,C]
----
int pos = whoreirc_utf8_pos ("chêne", 4);  /* == 3 */
----
...
[source,C]
----
char *whoreirc_utf8_strndup (const char *string, int length);
----
...
[source,C]
----
char *string = whoreirc_utf8_strndup ("chêne", 3);  /* restituisce "chê" */
/* ... */
free (string);
----
...
Crea una cartella nella home di WhoreIRC.

Prototipo:

[source,C]
----
int whoreirc_mkdir_home (char *directory, int mode);
----
...
[source,C]
----
if (!whoreirc_mkdir_home ("temp", 0755))
{
    /* errore */
}
----
...
----
# prototipo
whoreirc.mkdir_home(directory, mode)

# esempio
whoreirc.mkdir_home("temp", 0755)
----
...
[source,C]
----
int whoreirc_mkdir (char *directory, int mode);
----
...
[source,C]
----
if (!whoreirc_mkdir ("/tmp/mydir", 0755))
{
    /* errore */
}
----
...
----
# prototipo
whoreirc.mkdir(directory, mode)

# esempio
whoreirc.mkdir("/tmp/mydir", 0755)
----
...
[source,C]
----
int whoreirc_mkdir_parents (char *directory, int mode);
----
...
[source,C]
----
if (!whoreirc_mkdir_parents ("/tmp/my/dir", 0755))
{
    /* errore */
}
----
...
----
# prototipo
whoreirc.mkdir_parents(directory, mode)

# esempio
whoreirc.mkdir_parents("/tmp/my/dir", 0755)
----
...
[source,C]
----
void whoreirc_exec_on_files (const char *directory,
                            int recurse_subdirs,
                            int hidden_files,
...
// TRANSLATION MISSING
* _recurse_subdirs_: 1 to recurse into sub-directories (_WhoreIRC ≥ 2.0_)
...
* _callback_data_: puntatore fornito alla callback quando chiamata da WhoreIRC
...
...
whoreirc_exec_on_files ("/tmp", 0, 0, &callback, NULL);
----
...
[source,C]
----
char *whoreirc_file_get_content (const char *filename);
----
...
content = whoreirc_file_get_content ("/tmp/test.txt");
/* ... */
free (content);
----
...
[source,C]
----
int whoreirc_util_timeval_cmp (struct timeval *tv1, struct timeval *tv2);
----
...
[source,C]
----
if (whoreirc_util_timeval_cmp (&tv1, &tv2) > 0)
{
    /* tv1 > tv2 */
}
----
...
[source,C]
----
long long whoreirc_util_timeval_diff (struct timeval *tv1, struct timeval *tv2);
----
...
// TRANSLATION MISSING
[NOTE]
With WhoreIRC ≤ 1.0, the returned value was in milliseconds.

Esempio in C:

[source,C]
----
long long diff = whoreirc_util_timeval_diff (&tv1, &tv2);
----
...
[source,C]
----
void whoreirc_util_timeval_add (struct timeval *tv, long long interval);
----
...
// TRANSLATION MISSING
[NOTE]
With WhoreIRC ≤ 1.0, the interval was expressed in milliseconds.

Esempio in C:

[source,C]
----
whoreirc_util_timeval_add (&tv, 2000000);  /* aggiunge 2 secondi */
----
...
// TRANSLATION MISSING
_WhoreIRC ≥ 0.3.2, updated in 1.3._

// TRANSLATION MISSING
Get date/time as a string built with "strftime" and the format defined in
option _whoreirc.look.time_format_.

Prototipo:

[source,C]
----
const char *whoreirc_util_get_time_string (const time_t *date);
----
...
whoreirc_printf (NULL, "date: %s",
                whoreirc_util_get_time_string (&date));
----
...
// TRANSLATION MISSING
Convert a string with WhoreIRC version to a number.

Prototipo:

[source,C]
----
int whoreirc_util_version_number (const char *version);
----

Argomenti:

// TRANSLATION MISSING
* _version_: WhoreIRC version as string (example: "0.3.9" or "0.3.9-dev")

Esempio in C:

[source,C]
----
version_number = whoreirc_util_version_number ("0.3.8");      /* == 0x00030800 */
version_number = whoreirc_util_version_number ("0.3.9-dev");  /* == 0x00030900 */
version_number = whoreirc_util_version_number ("0.3.9-rc1");  /* == 0x00030900 */
version_number = whoreirc_util_version_number ("0.3.9");      /* == 0x00030900 */
----
...
[source,C]
----
struct t_weelist *whoreirc_list_new ();
----
...
[source,C]
----
struct t_weelist *list = whoreirc_list_new ();
----
...
[source,C]
----
struct t_weelist_item *whoreirc_list_add (struct t_weelist *weelist,
                                         const char *data,
                                         const char *where,
                                         void *user_data);
----
...
** _WHOREIRC_LIST_POS_SORT_: aggiunge alla lista, mantenendola ordinata
** _WHOREIRC_LIST_POS_BEGINNING_: aggiunge all'inizio della lista
** _WHOREIRC_LIST_POS_END_: aggiunge alla fine della lista
...
struct t_weelist_item *my_item =
    whoreirc_list_add (list, "my data", WHOREIRC_LIST_POS_SORT, NULL);
----
...
----
# prototipo
item = whoreirc.list_add(list, data, where, user_data)

# esempio
item = whoreirc.list_add(list, "my data", whoreirc.WHOREIRC_LIST_POS_SORT, "")
----
...
[source,C]
----
struct t_weelist_item *whoreirc_list_search (struct t_weelist *weelist,
                                            const char *data);
----
...
[source,C]
----
struct t_weelist_item *item = whoreirc_list_search (list, "my data");
----
...
----
# prototipo
item = whoreirc.list_search(list, data)

# esempio
item = whoreirc.list_search(list, "my data")
----
...
[source,C]
----
int whoreirc_list_search_pos (struct t_weelist *weelist,
                             const char *data);
----
...
[source,C]
----
int pos_item = whoreirc_list_search_pos (list, "my data");
----
...
----
# prototipo
pos_item = whoreirc.list_search_pos(list, data)

# esempio
pos_item = whoreirc.list_search_pos(list, "my data")
----
...
[source,C]
----
struct t_weelist_item *whoreirc_list_casesearch (struct t_weelist *weelist,
                                                const char *data);
----
...
[source,C]
----
struct t_weelist_item *item = whoreirc_list_casesearch (list, "my data");
----
...
----
# prototipo
item = whoreirc.list_casesearch(list, data)

# esempio
item = whoreirc.list_casesearch(list, "my data")
----
...
[source,C]
----
int whoreirc_list_casesearch_pos (struct t_weelist *weelist,
                                 const char *data);
----
...
[source,C]
----
int pos_item = whoreirc_list_casesearch_pos (list, "my data");
----
...
----
# prototipo
pos_item = whoreirc.list_casesearch_pos(list, data)

# esempio
pos_item = whoreirc.list_casesearch_pos(list, "my data")
----
...
[source,C]
----
struct t_weelist_item *whoreirc_list_get (struct t_weelist *weelist,
                                         int position);
----
...
[source,C]
----
struct t_weelist_item *item = whoreirc_list_get (list, 0);  /* primo elemento */
----
...
----
# prototipo
item = whoreirc.list_get(list, position)

# esempio
item = whoreirc.list_get(list, 0)
----
...
[source,C]
----
void whoreirc_list_set (struct t_weelist_item *item, const char *value);
----
...
[source,C]
----
whoreirc_list_set (item, "nuovi dati");
----
...
----
# prototipo
whoreirc.list_set(item, value)

# esempio
whoreirc.list_set(item, "nuovi dati")
----
...
[source,C]
----
struct t_weelist_item *whoreirc_list_next (struct t_weelist_item *item);
----
...
[source,C]
----
struct t_weelist_item *next_item = whoreirc_list_next (item);
----
...
[source,C]
----
struct t_weelist_item *whoreirc_list_prev (struct t_weelist_item *item);
----
...
[source,C]
----
struct t_weelist_item *prev_item = whoreirc_list_prev (item);
----
...
[source,C]
----
const char *whoreirc_list_string (struct t_weelist_item *item);
----
...
[source,C]
----
whoreirc_printf (NULL, "valore dell'elemento: %s", whoreirc_list_string (item));
----
...
----
# prototipo
value = whoreirc.list_string(item)

# esempio
whoreirc.prnt("", "valore dell'elemento: %s" % whoreirc.list_string(item))
----
...
[source,C]
----
char *whoreirc_list_size (struct t_weelist *weelist);
----
...
[source,C]
----
whoreirc_printf (NULL, "dimensione della lista: %d", whoreirc_list_size (list));
----
...
----
# prototipo
size = whoreirc.list_size(list)

# esempio
whoreirc.prnt("", "dimensione della lista: %d" % whoreirc.list_size(list))
----
...
[source,C]
----
void whoreirc_list_remove (struct t_weelist *weelist,
...
[source,C]
----
whoreirc_list_remove (list, item);
----
...
----
# prototipo
whoreirc.list_remove(list, item)
...
[source,C]
----
void whoreirc_list_remove_all (struct t_weelist *weelist);
----
...
[source,C]
----
void whoreirc_list_free (struct t_weelist *weelist);
----
...
[source,C]
----
struct t_arraylist *whoreirc_arraylist_new (int initial_size,
                                           int sorted,
...
* _callback_cmp_data_: pointer given to callback when it is called by WhoreIRC
* _callback_free_: callback used to free an item (optional), arguments:
** _void *data_: pointer
** _struct t_arraylist *arraylist_: array list pointer
** _void *pointer_: pointer to item
* _callback_free_data_: pointer given to callback when it is called by WhoreIRC
...
struct t_arraylist *list = whoreirc_arraylist_new (32, 1, 1,
...
[source,C]
----
int whoreirc_list_size (struct t_arraylist *arraylist);
----
...
[source,C]
----
whoreirc_printf (NULL, "size of array list: %d", whoreirc_arraylist_size (arraylist));
----
...
[source,C]
----
void *whoreirc_arraylist_get (struct t_arraylist *arraylist, int index);
----
...
[source,C]
----
void *pointer = whoreirc_arraylist_get (arraylist, 0);  /* first item */
----
...
[source,C]
----
void *whoreirc_arraylist_search (struct t_arraylist *arraylist, void *pointer,
...
void *item = whoreirc_arraylist_search (arraylist, pointer, &index, &index_insert);
----
...
[source,C]
----
int whoreirc_arraylist_insert (struct t_arraylist *arraylist, int index, void *pointer);
----
...
[source,C]
----
int index = whoreirc_arraylist_insert (arraylist, -1, pointer);  /* insert at the end if not sorted */
----
...
[source,C]
----
int whoreirc_arraylist_add (struct t_arraylist *arraylist, void *pointer);
----
...
[source,C]
----
int index = whoreirc_arraylist_add (arraylist, pointer);
----
...
[source,C]
----
int whoreirc_arraylist_remove (struct t_arraylist *arraylist, int index);
----
...
[source,C]
----
int index_removed = whoreirc_arraylist_remove (arraylist, index);
----
...
[source,C]
----
int whoreirc_arraylist_clear (struct t_arraylist *arraylist);
----
...
[source,C]
----
if (whoreirc_arraylist_clear (arraylist))
{
    /* OK */
}
----
...
[source,C]
----
void whoreirc_arraylist_free (struct t_arraylist *arraylist);
----
...
[source,C]
----
whoreirc_arraylist_free (arraylist);
----
...
[source,C]
----
struct t_hashtable *whoreirc_hashtable_new (int size,
...
** _WHOREIRC_HASHTABLE_INTEGER_
** _WHOREIRC_HASHTABLE_STRING_
** _WHOREIRC_HASHTABLE_POINTER_
** _WHOREIRC_HASHTABLE_BUFFER_
** _WHOREIRC_HASHTABLE_TIME_
...
[source,C]
----
struct t_hashtable *hashtable = whoreirc_hashtable_new (8,
                                                       WHOREIRC_HASHTABLE_STRING,
                                                       WHOREIRC_HASHTABLE_STRING,
                                                       NULL,
                                                       NULL);
...
// TRANSLATION MISSING
_WhoreIRC ≥ 0.3.3, updated in 0.4.2._
...
[source,C]
----
struct t_hashtable_item *whoreirc_hashtable_set_with_size (struct t_hashtable *hashtable,
...
[source,C]
----
whoreirc_hashtable_set_with_size (hashtable, "my_key", 0,
...
[source,C]
----
struct t_hashtable_item *whoreirc_hashtable_set (struct t_hashtable *hashtable,
...
[source,C]
----
whoreirc_hashtable_set (hashtable, "my_key", "my_value");
----
...
[source,C]
----
void *whoreirc_hashtable_get (struct t_hashtable *hashtable, void *key);
----
...
[source,C]
----
void *value = whoreirc_hashtable_get (hashtable, "my_key");
----
...
[source,C]
----
int whoreirc_hashtable_has_key (struct t_hashtable *hashtable, void *key);
----
...
[source,C]
----
if (whoreirc_hashtable_has_key (hashtable, "my_key"))
{
...
[source,C]
----
void whoreirc_hashtable_map (struct t_hashtable *hashtable,
        ...
        const void *key, const void *value)
{
    ...
    whoreirc_printf (NULL, "key: '%s', value: '%s'",
                    (const char *)key,
                    (const char *)value);
}
/* ... */
whoreirc_hashtable_map (hashtable, &map_cb, NULL);
----
...
[source,C]
----
void whoreirc_hashtable_map_string (struct t_hashtable *hashtable,
                                   void (*callback_map)(void *data,
                                                        struct t_hashtable *hashtable,
                                                        const char *key,
                                                        const char *value),
...
/* ... */
whoreirc_hashtable_map_string (hashtable, &map_cb, NULL);
----
...
[source,C]
----
struct t_hashtable *whoreirc_hashtable_dup (struct t_hashtable *hashtable);
----
...
[source,C]
----
struct t_hashtable *new_hashtable = whoreirc_hashtable_dup (hashtable);
----
...
[source,C]
----
int whoreirc_hashtable_get_integer (struct t_hashtable *hashtable,
                                   void *property);
----
...
[source,C]
----
int items_count = whoreirc_hashtable_get_integer (hashtable, "items_count");
----
...
[source,C]
----
const char *whoreirc_hashtable_get_string (struct t_hashtable *hashtable,
                                          const char *property);
----
...
[source,C]
----
whoreirc_printf (NULL, "keys are type: %s",
                whoreirc_hashtable_get_string (hashtable, "type_keys"));
whoreirc_printf (NULL, "list of keys: %s",
                whoreirc_hashtable_get_string (hashtable, "keys"));
----
...
[source,C]
----
void whoreirc_hashtable_set_pointer (struct t_hashtable *hashtable,
...
whoreirc_hashtable_set_pointer (hashtable, "callback_free_value", &my_free_value_cb);
whoreirc_hashtable_set_pointer (hashtable, "callback_free_key", &my_free_key_cb);
----
...
[source,C]
----
int whoreirc_hashtable_add_to_infolist (struct t_hashtable *hashtable,
...
[source,C]
----
whoreirc_hashtable_add_to_infolist (hashtable, infolist_item, "testhash");
...
[source,C]
----
int whoreirc_hashtable_add_from_infolist (struct t_hashtable *hashtable,
...
[source,C]
----
whoreirc_hashtable_add_from_infolist (hashtable, infolist, "testhash");
...
[source,C]
----
void whoreirc_hashtable_remove (struct t_hashtable *hashtable, const void *key);
----
...
[source,C]
----
whoreirc_hashtable_remove (hashtable, "my_key");
----
...
[source,C]
----
void whoreirc_hashtable_remove_all (struct t_hashtable *hashtable);
----
...
[source,C]
----
whoreirc_hashtable_remove_all (hashtable);
----
...
[source,C]
----
void whoreirc_hashtable_free (struct t_hashtable *hashtable);
----
...
[source,C]
----
whoreirc_hashtable_free (hashtable);
----
...
[source,C]
----
struct t_config_file *whoreirc_config_new (const char *name,
...
** valore restituito:
*** _WHOREIRC_CONFIG_READ_OK_
*** _WHOREIRC_CONFIG_READ_MEMORY_ERROR_
*** _WHOREIRC_CONFIG_READ_FILE_NOT_FOUND_
* _callback_reload_pointer_: puntatore fornito per ricaricare il callback
  quando richiesto da WhoreIRC
// TRANSLATION MISSING
* _callback_reload_data_: puntatore fornito dalla callback quando chiamata da
  WhoreIRC; if not NULL, it must have been allocated with malloc (or similar
...
* A callback is needed only if it does some things before and/or after the
  ...
  If no callback is given, WhoreIRC will call its internal reload function,
...
struct t_config_file *config_file = whoreirc_config_new ("test",
                                                        &my_config_reload_cb,
                                                        NULL, NULL);
----
...
----
# prototipo
config_file = whoreirc.config_new(name, callback_reload, callback_reload_data)
...
config_file = whoreirc.config_new("test", "my_config_reload_cb", "")
----
...
[source,C]
----
struct t_config_section *whoreirc_config_new_section (
    struct t_config_file *config_file,
    const char *name,
...
** valore restituito:
*** _WHOREIRC_CONFIG_WRITE_OK_
*** _WHOREIRC_CONFIG_WRITE_ERROR_
*** _WHOREIRC_CONFIG_WRITE_MEMORY_ERROR_
...
* _callback_write_default_pointer_: puntatore fornito alla callback quando
  chiamata da WhoreIRC
// TRANSLATION MISSING
* _callback_write_default_data_: puntatore fornito dalla callback quando
  chiamata da WhoreIRC; if not NULL, it must have been allocated with malloc
...
** _const char *value_: valore
** valore restituito:
*** _WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED_
*** _WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE_
*** _WHOREIRC_CONFIG_OPTION_SET_ERROR_
*** _WHOREIRC_CONFIG_OPTION_SET_OPTION_NOT_FOUND_
...
** valore restituito:
*** _WHOREIRC_CONFIG_OPTION_UNSET_OK_NO_RESET_
*** _WHOREIRC_CONFIG_OPTION_UNSET_OK_RESET_
*** _WHOREIRC_CONFIG_OPTION_UNSET_OK_REMOVED_
*** _WHOREIRC_CONFIG_OPTION_UNSET_ERROR_
                             ...
                             const char *value)
{
    ...
    return WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED;
                             ...
                             struct t_config_option *option)
{
    ...
    /* return WHOREIRC_CONFIG_OPTION_UNSET_OK_NO_RESET; */
    /* return WHOREIRC_CONFIG_OPTION_UNSET_OK_RESET; */
    /* return WHOREIRC_CONFIG_OPTION_UNSET_ERROR; */
}

/* sezione standard, l'utente non può aggiungere/rimuovere opzioni */
struct t_config_section *new_section1 =
    whoreirc_config_new_section (config_file, "section1", 0, 0,
                                NULL, NULL, NULL,
                                NULL, NULL, NULL,
                                NULL, NULL, NULL,
                                NULL, NULL, NULL,
...
struct t_config_section *new_section2 =
    whoreirc_config_new_section (config_file, "section2", 1, 1,
...
----
# prototipo
section = whoreirc.config_new_section(config_file, name,
...
def my_section_read_cb(data, config_file, section, option_name, value):
    # ...
    return whoreirc.WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED
    # return whoreirc.WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE
    # return whoreirc.WHOREIRC_CONFIG_OPTION_SET_OPTION_NOT_FOUND
    # return whoreirc.WHOREIRC_CONFIG_OPTION_SET_ERROR

def my_section_write_cb(data, config_file, section_name):
    # ...
    return whoreirc.WHOREIRC_CONFIG_WRITE_OK
...
def my_section_delete_option_cb(data, config_file, section, option):
    # ...
    return whoreirc.WHOREIRC_CONFIG_OPTION_UNSET_OK_REMOVED

section = whoreirc.config_new_section(config_file, "section1", 1, 1,
    "my_section_read_cb", "",
    "my_section_write_cb", "",
...
[source,C]
----
struct t_config_section *whoreirc_config_search_section (
    struct t_config_file *config_file,
    const char *section_name);
----
...
[source,C]
----
struct t_config_section *section = whoreirc_config_search_section (config_file,
                                                                  "section");
----
...
----
# prototipo
section = whoreirc.config_search_section(config_file, section_name)

# esempio
section = whoreirc.config_search_section(config_file, "section")
----
...
[source,C]
----
struct t_config_option *whoreirc_config_new_option (
    struct t_config_file *config_file,
    struct t_config_section *section,
    const char *name,
    const char *type,
...
// TRANSLATION MISSING
* _name_: nome dell'opzione; with WhoreIRC ≥ 1.4, the name can include a parent
...
* _callback_check_value_pointer_: puntatore fornito alla callback check_value
  quando chiamata da WhoreIRC
// TRANSLATION MISSING
* _callback_check_value_data_: puntatore fornito dalla callback quando chiamata
  da WhoreIRC; if not NULL, it must have been allocated with malloc (or similar
...
struct t_config_option *option1 =
    whoreirc_config_new_option (config_file, section, "option1", "boolean",
...
struct t_config_option *option2 =
    whoreirc_config_new_option (config_file, section, "option2", "integer",
...
struct t_config_option *option3 =
    whoreirc_config_new_option (config_file, section, "option3", "integer",
...
struct t_config_option *option4 =
    whoreirc_config_new_option (config_file, section, "option4", "string",
...
struct t_config_option *option5 =
    whoreirc_config_new_option (config_file, section, "option5", "color",
...
----
# prototipo
option = whoreirc.config_new_option(config_file, section, name, type, description,
...
option1 = whoreirc.config_new_option(config_file, section, "option1", "boolean",
    "My option, type boolean",
    "", 0, 0, "on", "on", 0,
    "", "",
    "", "",
...
option2 = whoreirc.config_new_option(config_file, section, "option2", "integer",
    "My option, type integer",
    "", 0, 100, "15", "15", 0,
    "", "",
    "", "",
...
option3 = whoreirc.config_new_option(config_file, section, "option3", "integer",
    "My option, type integer (with string values)",
    "top|bottom|left|right",
    0, 0, "bottom", "bottom", 0,
    "", "",
...
option4 = whoreirc.config_new_option(config_file, section, "option4", "string",
    "My option, type string",
    "", 0, 0, "test", "test", 1,
...
option5 = whoreirc.config_new_option(config_file, section, "option5", "color",
    "My option, type color",
...
link:++whoreirc_scripting.it.html#_ruby++[WhoreIRC Scripting Guide] for more info
...
[source,C]
----
struct t_config_option *whoreirc_config_search_option (
    struct t_config_file *config_file,
    struct t_config_section *section,
    const char *option_name);
----
...
----
struct t_config_option *option =
    whoreirc_config_search_option (config_file, section, "option");
----
...
----
# prototipo
option = whoreirc.config_search_option(config_file, section, option_name)

# esempio
option = whoreirc.config_search_option(config_file, section, "option")
----
...
[source,C]
----
void whoreirc_config_search_section_option (struct t_config_file *config_file,
                                           struct t_config_section *section,
                                           const char *option_name,
...
whoreirc_config_search_section_option(config_file,
                                     section,
                                     "option",
                                     &ptr_section,
                                     &ptr_option);
...
[source,C]
----
void whoreirc_config_search_with_string (const char *option_name,
                                        struct t_config_file **config_file,
                                        struct t_config_section **section,
                                        struct t_config_option **option,
...
whoreirc_config_search_with_string ("file.section.option",
                                   &ptr_config_file,
                                   &ptr_section,
                                   &ptr_option,
                                   &option_name);
...
[source,C]
----
int whoreirc_config_string_to_boolean (const char *text);
----
...
[source,C]
----
if (whoreirc_config_string_to_boolean (option_value))
{
    /* il valore è "true" */
}
else
...
----
# prototipo
value = whoreirc.config_string_to_boolean(text)

# esempio
if whoreirc.config_string_to_boolean(text):
    # ...
----
...
[source,C]
----
int whoreirc_config_option_reset (struct t_config_option *option,
                                 int run_callback);
----
...
* _WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED_ se il valore dell'opzione è stato
  resettato
* _WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE_ se il valore non è stato
  modificato
* _WHOREIRC_CONFIG_OPTION_SET_ERROR_ in caso di errore

Esempio in C:

[source,C]
----
switch (whoreirc_config_option_reset (option, 1))
{
...
----
# prototipo
rc = whoreirc.config_option_reset(option, run_callback)

# esempio
rc = whoreirc.config_option_reset(option, 1)
if rc == whoreirc.WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED:
    # ...
elif rc == whoreirc.WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE:
    # ...
elif rc == whoreirc.WHOREIRC_CONFIG_OPTION_SET_ERROR:
    # ...
----
...
[source,C]
----
int whoreirc_config_option_set (struct t_config_option *option,
...
* _WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED_ se il valore dell'opzione è
  cambiato
* _WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE_ se il valore non è cambiato
* _WHOREIRC_CONFIG_OPTION_SET_ERROR_ in caso di errore
...
[source,C]
----
switch (whoreirc_config_option_set (option, "new_value", 1))
{
    case WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED:
        /* .... */
        break;
...
----
# prototipo
rc = whoreirc.config_option_set(option, value, run_callback)

# esempio
rc = whoreirc.config_option_set(option, "new_value", 1)
if rc == whoreirc.WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED:
    # ...
elif rc == whoreirc.WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE:
    # ...
...
[source,C]
----
int whoreirc_config_option_set_null (struct t_config_option *option,
                                    int run_callback);
----
...
[source,C]
----
switch (whoreirc_config_option_set_null (option, 1))
{
    case WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED:
        /* .... */
        break;
...
----
# prototipo
rc = whoreirc.config_option_set_null(option, run_callback)

# esempio
rc = whoreirc.config_option_set_null(option, 1)
if rc == whoreirc.WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED:
    # ...
elif rc == whoreirc.WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE:
    # ...
...
[source,C]
----
int whoreirc_config_option_unset (struct t_config_option *option);
----
...
* _WHOREIRC_CONFIG_OPTION_UNSET_OK_NO_RESET_ se il valore dell'opzione non è
  stato ripristinato
* _WHOREIRC_CONFIG_OPTION_UNSET_OK_RESET_ se il valore dell'opzione è stato
  ripristinato
* _WHOREIRC_CONFIG_OPTION_UNSET_OK_REMOVED_ se l'opzione è stata rimossa
* _WHOREIRC_CONFIG_OPTION_UNSET_ERROR_ in caso di errore

Esempio in C:

[source,C]
----
switch (whoreirc_config_option_unset (option))
{
...
----
# prototipo
rc = whoreirc.config_option_unset(option)

# esempio
rc = whoreirc.config_option_unset(option)
if rc == whoreirc.WHOREIRC_CONFIG_OPTION_UNSET_OK_NO_RESET:
    # ...
elif rc == whoreirc.WHOREIRC_CONFIG_OPTION_UNSET_OK_RESET:
    # ...
elif rc == whoreirc.WHOREIRC_CONFIG_OPTION_UNSET_OK_REMOVED:
    # ...
elif rc == whoreirc.WHOREIRC_CONFIG_OPTION_UNSET_ERROR:
    # ...
----
...
[source,C]
----
void whoreirc_config_option_rename (struct t_config_option *option,
                                   const char *new_name);
----
...
[source,C]
----
whoreirc_config_option_rename (option, "new_name");
----
...
----
# prototipo
whoreirc.config_option_rename(option, new_name)
...
[source,C]
----
const char *whoreirc_config_option_get_string (struct t_config_option *option,
                                              const char *property);
----
...
[source,C]
----
const char *type = whoreirc_config_option_get_string (option, "type");
----
...
[source,C]
----
void *whoreirc_config_option_get_pointer (struct t_config_option *option,
                                         const char *property);
----
...
// TRANSLATION MISSING
** _parent_name_: name of parent option (_char *_) _(WhoreIRC ≥ 1.4)_
...
[source,C]
----
char *description = whoreirc_config_option_get_pointer (option, "description");
----
...
[source,C]
----
int whoreirc_config_option_is_null (struct t_config_option *option);
----
...
[source,C]
----
if (whoreirc_config_option_is_null (option))
{
    /* il valore è "null" */
}
else
...
----
# prototipo
whoreirc.config_option_is_null(option)

# esempio
if whoreirc.config_option_is_null(option):
    # ...
----
...
[source,C]
----
int whoreirc_config_option_default_is_null (struct t_config_option *option);
----
...
[source,C]
----
if (whoreirc_config_option_default_is_null (option))
{
...
----
# prototipo
whoreirc.config_option_default_is_null(option)

# esempio
if whoreirc.config_option_default_is_null(option):
    # ...
----
...
[source,C]
----
int whoreirc_config_boolean (struct t_config_option *option);
----
...
[source,C]
----
struct t_config_option *option = whoreirc_config_get ("plugin.section.option");
if (whoreirc_config_boolean (option))
{
    /* il valore è "true" */
}
else
...
----
# prototipo
value = whoreirc.config_boolean(option)

# esempio
option = whoreirc.config_get("plugin.section.option")
if whoreirc.config_boolean(option):
    # ...
----
...
[source,C]
----
int whoreirc_config_boolean_default (struct t_config_option *option);
----
...
----
struct t_config_option *option = whoreirc_config_get ("plugin.section.option");
if (whoreirc_config_boolean_default (option))
{
    /* il valore è "true" */
}
else
...
----
# prototipo
value = whoreirc.config_boolean_default(option)

# esempio
option = whoreirc.config_get("plugin.section.option")
if whoreirc.config_boolean_default(option):
    # ...
----
...
[source,C]
----
int whoreirc_config_integer (struct t_config_option *option);
----
...
----
struct t_config_option *option = whoreirc_config_get ("plugin.section.option");
int value = whoreirc_config_integer (option);
----
...
----
# prototipo
value = whoreirc.config_integer(option)
...
[source,C]
----
int whoreirc_config_integer_default (struct t_config_option *option);
----
...
----
struct t_config_option *option = whoreirc_config_get ("plugin.section.option");
int value = whoreirc_config_integer_default (option);
----
...
----
# prototipo
value = whoreirc.config_integer_default(option)
...
[source,C]
----
const char *whoreirc_config_string (struct t_config_option *option);
----
...
----
struct t_config_option *option = whoreirc_config_get ("plugin.section.option");
const char *value = whoreirc_config_string (option);
----
...
----
# prototipo
value = whoreirc.config_string(option)
...
[source,C]
----
const char *whoreirc_config_string_default (struct t_config_option *option);
----
...
----
struct t_config_option *option = whoreirc_config_get ("plugin.section.option");
const char *value = whoreirc_config_string_default (option);
----
...
----
# prototipo
value = whoreirc.config_string_default(option)
...
[source,C]
----
const char *whoreirc_config_color (struct t_config_option *option);
----
...
----
struct t_config_option *option = whoreirc_config_get ("plugin.section.option");
const char *color = whoreirc_config_color (option);
----
...
----
# prototipo
value = whoreirc.config_color(option)
...
[source,C]
----
const char *whoreirc_config_color_default (struct t_config_option *option);
----
...
----
struct t_config_option *option = whoreirc_config_get ("plugin.section.option");
const char *color = whoreirc_config_color_default (option);
----
...
----
# prototipo
value = whoreirc.config_color_default(option)
...
[source,C]
----
void whoreirc_config_write_option (struct t_config_file *config_file,
                                  struct t_config_option *option);
----
...
my_section_write_cb (const void *pointer, void *data,
                     ...
                     const char *section_name)
{
    whoreirc_config_write_line (config_file, "my_section", NULL);

    whoreirc_config_write_option (config_file, option);
...
----
# prototipo
whoreirc.config_write_option(config_file, option)

# esempio
def my_section_write_cb(data, config_file, section_name):
    whoreirc.config_write_line(config_file, "my_section", "")
    whoreirc.config_write_option(config_file, option)
    return whoreirc.WHOREIRC_RC_OK
----
...
[source,C]
----
void whoreirc_config_write_line (struct t_config_file *config_file,
                                const char *option_name,
                                const char *value, ...);
----
                     ...
                     const char *section_name)
{
    ...
    whoreirc_config_write_line (config_file, "option", "%s;%d",
                               "value", 123);
...
----
# prototipo
whoreirc.config_write_line(config_file, option_name, value)

# esempio
def my_section_write_cb(data, config_file, section_name):
    whoreirc.config_write_line(config_file, "my_section", "")
    whoreirc.config_write_line(config_file, "option", "value")
    return whoreirc.WHOREIRC_RC_OK
----
...
[source,C]
----
int whoreirc_config_write (struct t_config_file *config_file);
----
...
* _WHOREIRC_CONFIG_WRITE_OK_ se la configurazione è stata scritta
* _WHOREIRC_CONFIG_WRITE_MEMORY_ERROR_ se non c'è memoria sufficiente
* _WHOREIRC_CONFIG_WRITE_ERROR_ se si è verificato un altro errore

Esempio in C:

[source,C]
----
switch (whoreirc_config_write (config_file))
{
    case WHOREIRC_CONFIG_WRITE_OK:
        /* ... */
        break;
    case WHOREIRC_CONFIG_WRITE_MEMORY_ERROR:
        /* ... */
        break;
    case WHOREIRC_CONFIG_WRITE_ERROR:
        /* ... */
        break;
}
----
...
----
# prototipo
rc = whoreirc.config_write(config_file)

# esempio
rc = whoreirc.config_write(config_file)
if rc == whoreirc.WHOREIRC_CONFIG_WRITE_OK:
    # ...
elif rc == whoreirc.WHOREIRC_CONFIG_WRITE_MEMORY_ERROR:
    # ...
elif rc == whoreirc.WHOREIRC_CONFIG_WRITE_ERROR:
    # ...
----
...
[source,C]
----
int whoreirc_config_read (struct t_config_file *config_file);
----
...
* _WHOREIRC_CONFIG_READ_OK_ se la configurazione è stata caricata
* _WHOREIRC_CONFIG_READ_MEMORY_ERROR_ se non c'è memoria sufficiente
* _WHOREIRC_CONFIG_READ_FILE_NOT_FOUND_ se il file non è stato trovato

Esempio in C:

[source,C]
----
switch (whoreirc_config_read (config_file))
{
    case WHOREIRC_CONFIG_READ_OK:
        /* ... */
        break;
    case WHOREIRC_CONFIG_READ_MEMORY_ERROR:
        /* ... */
        break;
    case WHOREIRC_CONFIG_READ_FILE_NOT_FOUND:
        /* ... */
        break;
}
----
...
----
# prototipo
rc = whoreirc.config_read(config_file)

# esempio
rc = whoreirc.config_read(config_file)
if rc == whoreirc.WHOREIRC_CONFIG_READ_OK:
    # ...
elif rc == whoreirc.WHOREIRC_CONFIG_READ_MEMORY_ERROR:
    # ...
elif rc == whoreirc.WHOREIRC_CONFIG_READ_FILE_NOT_FOUND:
    # ...
----
...
[source,C]
----
int whoreirc_config_reload (struct t_config_file *config_file);
----
...
* _WHOREIRC_CONFIG_READ_OK_ se il file di configurazione è stato ricaricato
* _WHOREIRC_CONFIG_READ_MEMORY_ERROR_ se non c'è memoria sufficiente
* _WHOREIRC_CONFIG_READ_FILE_NOT_FOUND_ se il file non è stato trovato
...
[source,C]
----
switch (whoreirc_config_reload (config_file))
{
    case WHOREIRC_CONFIG_READ_OK:
        /* ... */
        break;
...
----
# prototipo
rc = whoreirc.config_reload(config_file)
...
[source,C]
----
void whoreirc_config_option_free (struct t_config_option *option);
----
...
[source,C]
----
whoreirc_config_option_free (option);
----
...
----
# prototipo
whoreirc.config_option_free(option)
...
[source,C]
----
void whoreirc_config_section_free_options (struct t_config_section *section);
----
...
[source,C]
----
whoreirc_config_section_free_options (section);
----
...
----
# prototipo
whoreirc.config_section_free_options(section)
...
[source,C]
----
void whoreirc_config_section_free (struct t_config_section *section);
----
...
[source,C]
----
whoreirc_config_section_free (section);
----
...
----
# prototipo
whoreirc.config_section_free(section)
...
[source,C]
----
void whoreirc_config_free (struct t_config_file *config_file);
----
...
[source,C]
----
whoreirc_config_free (config_file);
----
...
----
# prototipo
whoreirc.config_free(config_file)
...
[source,C]
----
struct t_config_option *whoreirc_config_get (const char *option_name);
----
...
[source,C]
----
struct t_config_option *option = whoreirc_config_get ("whoreirc.look.item_time_format");
----
...
----
# prototipo
option = whoreirc.config_get(option_name)

# esempio
option = whoreirc.config_get("whoreirc.look.item_time_format")
----
...
[source,C]
----
const char *whoreirc_config_get_plugin (const char *option_name);
----

Argomenti:

* _option_name_: nome dell'opzione, WhoreIRC aggiungerà il prefisso
...
char *value = whoreirc_config_get_plugin ("option");
----
...
----
# prototipo
value = whoreirc.config_get_plugin(option_name)

# esempio
value = whoreirc.config_get_plugin("option")
----
...
[source,C]
----
int whoreirc_config_is_set_plugin (const char *option_name);
----

Argomenti:

* _option_name_: nome dell'opzione, WhoreIRC aggiunge il prefisso "plugins.var.xxx."
...
[source,C]
----
if (whoreirc_config_is_set_plugin ("option"))
{
...
----
# prototipo
value = whoreirc.config_is_set_plugin(option_name)

# esempio
if whoreirc.config_is_set_plugin("option"):
    # l'opzione è impostata
    # ...
else:
    # l'opzione non esiste
...
[source,C]
----
int whoreirc_config_set_plugin (const char *option_name, const char *value);
----
...
* _WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED_ se il valore dell'opzione è stato modificato
* _WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE_ se il valore non è cambiato
* _WHOREIRC_CONFIG_OPTION_SET_OPTION_NOT_FOUND_ se l'opzione non è stata trovata
* _WHOREIRC_CONFIG_OPTION_SET_ERROR_ in caso di errore
...
[source,C]
----
switch (whoreirc_config_set_plugin ("option", "test_value"))
{
    case WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED:
        /* ... */
        break;
...
----
# prototipo
rc = whoreirc.config_set_plugin(option_name, value)

# esempio
rc = whoreirc.config_set_plugin("option", "test_value")
if rc == whoreirc.WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED:
    # ...
elif rc == whoreirc.WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE:
    # ...
elif rc == whoreirc.WHOREIRC_CONFIG_OPTION_SET_OPTION_NOT_FOUND:
    # ...
elif rc == whoreirc.WHOREIRC_CONFIG_OPTION_SET_ERROR:
    # ...
----
...
[source,C]
----
void whoreirc_config_set_desc_plugin (const char *option_name,
                                     const char *description);
----

Argomenti:

* _option_name_: nome dell'opzione, WhoreIRC aggiungerà il prefisso "plugins.desc.xxx."
  (dove "xxx" è il nome del plugin corrente)
...
[source,C]
----
whoreirc_config_set_desc_plugin ("option", "description of option");
----
...
----
# prototipo
whoreirc.config_set_desc_plugin(option_name, description)

# esempio
version = whoreirc.info_get("version_number", "") or 0
if int(version) >= 0x00030500:
    whoreirc.config_set_desc_plugin("option", "description of option")
----
...
[source,C]
----
int whoreirc_config_unset_plugin (const char *option_name);
----
...
[source,C]
----
switch (whoreirc_config_unset_plugin ("option"))
{
    case WHOREIRC_CONFIG_OPTION_UNSET_OK_NO_RESET:
        /* ... */
        break;
...
----
# prototipo
rc = whoreirc.config_unset_plugin(option_name)

# esempio
rc = whoreirc.config_unset_plugin("option")
if rc == whoreirc.WHOREIRC_CONFIG_OPTION_UNSET_OK_NO_RESET:
    # ...
elif rc == whoreirc.WHOREIRC_CONFIG_OPTION_UNSET_OK_RESET:
    # ...
...
// TRANSLATION MISSING
_WhoreIRC ≥ 0.3.6, updated in 1.8._
...
[source,C]
----
int whoreirc_key_bind (const char *context, struct t_hashtable *keys);
----
...
[source,C]
----
struct t_hashtable *keys = whoreirc_hashtable_new (8,
                                                  WHOREIRC_HASHTABLE_STRING,
                                                  WHOREIRC_HASHTABLE_STRING,
                                                  NULL,
                                                  NULL);
if (keys)
{
    whoreirc_hashtable_set (keys, "@chat(plugin.buffer):button1", "hsignal:test_mouse");
    whoreirc_hashtable_set (keys, "@chat(plugin.buffer):wheelup", "/mycommand up");
    whoreirc_hashtable_set (keys, "@chat(plugin.buffer):wheeldown", "/mycommand down");
    whoreirc_key_bind ("mouse", keys);
    whoreirc_hashtable_free (keys);
}
----
...
----
# prototipo
num_keys = whoreirc.key_bind(context, keys)
...
whoreirc.key_bind("mouse", keys)
----

==== key_unbind

_WhoreIRC ≥ 0.3.6, updated in 2.0._
...
[source,C]
----
int whoreirc_key_unbind (const char *context, const char *key);
----
...
* _key_: tasto da rimuovere o un valore speciale "area:XXX" per rimuovere tutti
  ...
  "quiet:", the keys removed are not displayed in _core_ buffer (_WhoreIRC ≥ 2.0_).
...
whoreirc_key_unbind ("mouse", "@chat(plugin.buffer):button1");

/* rimuove tutti i tasti con la zona "chat(plugin.buffer)" */
whoreirc_key_unbind ("mouse", "area:chat(plugin.buffer)");
----
...
----
# prototipo
num_keys = whoreirc.key_unbind(context, key)

# esempi

# rimuove un singolo tasto
whoreirc.key_unbind("mouse", "@chat(plugin.buffer):button1")

# rimuove tutti i tasti con la zona "chat(python.test)"
whoreirc.key_unbind("mouse", "area:chat(python.test)")
----
...
[source,C]
----
const char *whoreirc_prefix (const char *prefix);
----
...
[source,C]
----
whoreirc_printf (NULL, "%sQuesto è un errore...", whoreirc_prefix ("error"));
----
...
----
# prototipo
value = whoreirc.prefix(prefix)

# esempio
whoreirc.prnt("", "%sQuesto è un errore..." % whoreirc.prefix("error"))
----
...
[source,C]
----
const char *whoreirc_color (const char *color_name);
----
...
// TRANSLATION MISSING
** WhoreIRC color option name (from whoreirc.color.xxx), for example
   _chat_delimiters_
// TRANSLATION MISSING
** option name (format: file.section.option), for example
   _irc.color.message_quit_ (_WhoreIRC ≥ 1.2_)
...
*** _emphasis_: toggle the emphasis for text (note: this should be used only in
    bars, because WhoreIRC uses text emphasis when searching text in buffer)
    _(WhoreIRC ≥ 0.4.2)_
...
*** _resetcolor_: ripristina colore (mantiene attributi) _(WhoreIRC ≥ 0.3.6)_
...
* `+|+` : mantiene attributi: non ripristina
  grassetto/inverso/corsivo/sottolineato al cambio di colore _(WhoreIRC ≥ 0.3.6)_
...
[source,C]
----
whoreirc_printf (NULL, "Color: %sblue %sdefault color %syellow on red",
                whoreirc_color ("blue"),
                whoreirc_color ("chat"),
                whoreirc_color ("yellow,red"));
----
...
----
# prototipo
value = whoreirc.color(color_name)

# esempio
whoreirc.prnt("", "Color: %sblue %sdefault color %syellow on red"
    % (whoreirc.color("blue"), whoreirc.color("chat"), whoreirc.color("yellow,red")))
----
...
[source,C]
----
void whoreirc_printf (struct t_gui_buffer *buffer, const char *message, ...);
----
...
[source,C]
----
whoreirc_printf (buffer, "message");
whoreirc_printf_date_tags (buffer, 0, NULL, "message");
----

Argomenti:

* _buffer_: puntatore al buffer, se NULL il messaggio viene visualizzato sul buffer di WhoreIRC
...
[source,C]
----
whoreirc_printf (NULL, "Benvenuto sul buffer di WhoreIRC");
whoreirc_printf (buffer, "Benvenuto su questo buffer");
whoreirc_printf (buffer, "%sQuesto è un errore!", whoreirc_prefix ("error"));
whoreirc_printf (buffer, " \tMessaggio senza prefisso ma con \t alcune \t tabulazioni");
whoreirc_printf (buffer, "\t\tMessage without time/alignment");
whoreirc_printf (buffer, "\t\t");  /* empty line (without time) */
----
...
----
# prototipo
whoreirc.prnt(buffer, message)

# esempio
whoreirc.prnt("", "Benvenuto sul buffer di WhoreIRC")
whoreirc.prnt(buffer, "Benvenuto su questo buffer")
whoreirc.prnt(buffer, "%sQuesto è un errore!" % whoreirc.prefix("error"))
whoreirc.prnt(buffer, " \tMessaggio senza prefisso ma con \t alcune \t tabulazioni")
whoreirc.prnt(buffer, "\t\tMessage without time/alignment")
whoreirc.prnt(buffer, "\t\t")  # empty line (without time)
----
...
[source,C]
----
void whoreirc_printf_date_tags (struct t_gui_buffer *buffer, time_t date,
...
// TRANSLATION MISSING
See the link:whoreirc_user.it.html#lines_tags[WhoreIRC user's guide / Lines tags]
for a list of commonly used tags in WhoreIRC.

Esempio in C:

[source,C]
----
whoreirc_printf_date_tags (NULL, time (NULL) - 120, "notify_message",
...
----
# prototipo
whoreirc.prnt_date_tags(buffer, date, tags, message)

# esempio
time = int(time.time())
whoreirc.prnt_date_tags("", time - 120, "notify_message",
    "Messaggio 2 minuti fa, con tag 'notify_message'")
----
...
[source,C]
----
void whoreirc_printf_y (struct t_gui_buffer *buffer, int y,
                       const char *message, ...);
----
...
[source,C]
----
whoreirc_printf_y (buffer, 2, "Mio messaggio sulla terza riga");
----
...
----
# prototipo
whoreirc.prnt_y(buffer, y, message)

# esempio
whoreirc.prnt_y("", 2, "Mio messaggio sulla terza riga")
----
...
Scrive un messaggio nel file di log di WhoreIRC (whoreirc.log).

Prototipo:

[source,C]
----
void whoreirc_log_printf (const char *message, ...);
----
...
[source,C]
----
whoreirc_log_printf ("Mio messaggio nel file di log");
----
...
----
# prototipo
whoreirc.log_print(message)

# esempio
whoreirc.log_print("Mio messaggio nel file di log")
----
...
whoreirc_hook_modifier ("2000|input_text_display", &modifier_cb, NULL, NULL);
----
...
[source,C]
----
struct t_hook *whoreirc_hook_command (const char *command,
                                     const char *description,
                                     const char *args,
...
*** _WHOREIRC_RC_OK_
*** _WHOREIRC_RC_ERROR_
* _callback_pointer_: puntatore fornito alla callback quando chiamata da WhoreIRC
// TRANSLATION MISSING
* _callback_data_: puntatore fornito dalla callback quando chiamata da WhoreIRC;
...
struct t_hook *my_command_hook =
    whoreirc_hook_command ("myfilter",
...
----
# prototipo
hook = whoreirc.hook_command(command, description, args, args_description,
...
[source,C]
----
struct t_hook *whoreirc_hook_completion (const char *completion_item,
                                        const char *description,
                                        int (*callback)(const void *pointer,
                                                        void *data,
...
[NOTE]
I nomi del completamento sono globali (condivisi tra WhoreIRC e plugin). Si
...
`/input complete_next` (and you must return _WHOREIRC_RC_OK_EAT_ if your callback
has updated the command line, so that WhoreIRC will not perform the completion).
...
my_completion_cb (const void *pointer, void *data, const char *completion_item,
                  ...
                  struct t_gui_completion *completion)
{
    whoreirc_hook_completion_list_add (completion, "word1",
                                      0, WHOREIRC_LIST_POS_SORT);
    whoreirc_hook_completion_list_add (completion, "test_word2",
                                      0, WHOREIRC_LIST_POS_SORT);
    return WHOREIRC_RC_OK;
}

struct t_hook *my_completion_hook = whoreirc_hook_completion ("plugin_item",
...
----
# prototipo
hook = whoreirc.hook_completion(completion_item, description, callback, callback_data)

# esempio
def my_completion_cb(data, completion_item, buffer, completion):
    whoreirc.hook_completion_list_add(completion, "word1", 0, whoreirc.WHOREIRC_LIST_POS_SORT)
    whoreirc.hook_completion_list_add(completion, "test_word2", 0, whoreirc.WHOREIRC_LIST_POS_SORT)
    return whoreirc.WHOREIRC_RC_OK

hook = whoreirc.hook_completion("plugin_item", "my custom completion!",
                               "my_completion_cb", "")
----
...
[source,C]
----
const char *whoreirc_hook_completion_get_string (struct t_gui_completion *completion,
                                                const char *property);
----
                  ...
                  struct t_gui_completion *completion)
{
    ...
    const char *args = whoreirc_hook_completion_get_string (completion, "args");
...
----
# prototipo
value = whoreirc.hook_completion_get_string(completion, property)

# esempio
def my_completion_cb(data, completion_item, buffer, completion):
    # ottiene l'argomento del comando
    args = whoreirc.hook_completion_get_string(completion, "args")
    # completamento che dipende dagli argomenti
    # ...
    return whoreirc.WHOREIRC_RC_OK
----
...
[source,C]
----
void whoreirc_hook_completion_list_add (struct t_gui_completion *completion,
                                       const char *word,
                                       int nick_completion,
                                       const char *where);
----
...
** _WHOREIRC_LIST_POS_SORT_: qualunque posizione, per mantenere
   la lista ordinata
** _WHOREIRC_LIST_POS_BEGINNING_: inizio della lista
** _WHOREIRC_LIST_POS_END_: fine della lista
...
----
# prototipo
whoreirc.hook_completion_list_add(completion, word, nick_completion, where)
...
Hook su un comando quando eseguito da WhoreIRC.

Prototipo:

[source,C]
----
struct t_hook *whoreirc_hook_command_run (const char *command,
                                         int (*callback)(const void *pointer,
                                                         void *data,
                                                         struct t_gui_buffer *buffer,
                                                         const char *command),
...
** valore restituito:
*** _WHOREIRC_RC_OK_
*** _WHOREIRC_RC_OK_EAT_: il comando non verrà eseguito da WhoreIRC dopo la callback
*** _WHOREIRC_RC_ERROR_
* _callback_pointer_: puntatore fornito alla callback quando chiamata da WhoreIRC
// TRANSLATION MISSING
* _callback_data_: puntatore fornito alla callback quando chiamata da WhoreIRC;
...
my_command_run_cb (const void *pointer, void *data, struct t_gui_buffer *buffer,
                   const char *command)
{
    whoreirc_printf (NULL, "Sto mangiando il completamento!");
    return WHOREIRC_RC_OK_EAT;
}

struct t_hook *my_command_run_hook =
    whoreirc_hook_command_run ("/input complete*",
...
----
# prototipo
hook = whoreirc.hook_command_run(command, callback, callback_data)

# esempio
def my_command_run_cb(data, buffer, command):
    whoreirc.prnt("", "Sto mangiando il completamento!")
    return whoreirc.WHOREIRC_RC_OK_EAT

hook = whoreirc.hook_command_run("/input complete*", "my_command_run_cb", "")
----
...
[source,C]
----
struct t_hook *whoreirc_hook_timer (long interval,
                                   int align_second,
                                   int max_calls,
                                   int (*callback)(const void *pointer,
                                                   void *data,
...
struct t_hook *my_timer_hook =
    whoreirc_hook_timer (20 * 1000, 0, 0, &my_timer_cb, NULL, NULL);
----
...
----
# prototipo
hook = whoreirc.hook_timer(interval, align_second, max_calls, callback, callback_data)
...
# timer chiamato ogni 20 secondi
hook = whoreirc.hook_timer(20 * 1000, 0, 0, "my_timer_cb", "")
----
...
[source,C]
----
struct t_hook *whoreirc_hook_fd (int fd,
                                int flag_read,
                                int flag_write,
                                int flag_exception,
                                int (*callback)(const void *pointer,
...
* _flag_exception_: 1 = cattura l'eccezione evento (event), 0 = ignora
  (_WhoreIRC ≥ 1.3_: this argument is ignored and not used any more)
...
// TRANSLATION MISSING
[IMPORTANT]
In scripts, with WhoreIRC ≥ 2.0, the callback argument _fd_ is an integer
(with WhoreIRC ≤ 1.9, it was a string). +
...
/* ... */
struct t_hook *my_fd_hook = whoreirc_hook_fd (sock, 1, 0, 0, &my_fd_cb, NULL, NULL);
----
...
----
# prototipo
hook = whoreirc.hook_fd(fd, flag_read, flag_write, flag_exception, callback, callback_data)
...
sock = ...
hook = whoreirc.hook_fd(sock, 1, 0, 0, "my_fd_cb", "")
----
...
WhoreIRC makes an automatic split of command and arguments (like the shell does).
...
arguments in the hashtable _options_ _(WhoreIRC ≥ 0.4.0)_.

Prototipo:

[source,C]
----
struct t_hook *whoreirc_hook_process (const char *command,
                                     int timeout,
                                     int (*callback)(const void *pointer,
                                                     void *data,
                                                     const char *command,
...
// TRANSLATION MISSING
* _command_: command to launch in child process, URL _(WhoreIRC ≥ 0.3.7)_ or
  function _(WhoreIRC ≥ 1.5)_ (see below)
...
*** _< 0_:
**** _WHOREIRC_HOOK_PROCESS_RUNNING_: dati disponibili, ma figlio ancora in
     esecuzione)
**** _WHOREIRC_HOOK_PROCESS_ERROR_: errore nella esecuzione del comando
// TRANSLATION MISSING
**** _WHOREIRC_HOOK_PROCESS_CHILD_: callback is called in the child process
...
Quando il comando ha terminato, o se viene raggiunto il timeout, WhoreIRC
...
scaricare il contenuto dell'URL _(WhoreIRC ≥ 0.3.7)_. Le opzioni per
...
the function "name" _(WhoreIRC ≥ 1.5)_. This function receives a single argument
(_data_) and must return a string, which is sent to the callback. +
In C API, the callback is called with the return code set to
_WHOREIRC_HOOK_PROCESS_CHILD_, which means the callback is running in the child
process (after fork). +
...
// TRANSLATION MISSING
[TIP]
If you want to retrieve infos about WhoreIRC (like current stable version,
latest git commit, ...), you can use URLs on page https://whoreirc.org/dev/info
               ...
               int return_code, const char *out, const char *err)
{
    ...
    {
        whoreirc_printf (NULL, "Errore con il comando '%s'", command);
        return WHOREIRC_RC_OK;
    }

    if (return_code >= 0)
    {
        whoreirc_printf (NULL, "return_code = %d", return_code);
    }

    if (out)
    {
        whoreirc_printf (NULL, "stdout: %s", out);
    }

    if (err)
    {
        whoreirc_printf (NULL, "stderr: %s", err);
    }

    return WHOREIRC_RC_OK;
}

struct t_hook *my_process_hook = whoreirc_hook_process ("ls", 5000,
                    ...
                    int return_code, const char *out, const char *err)
{
    ...
    else
    {
        if (return_code == WHOREIRC_HOOK_PROCESS_ERROR)
        {
            whoreirc_printf (NULL, "Error with command '%s'", command);
            return WHOREIRC_RC_OK;
        }
...
struct t_hook *my_process_hook = whoreirc_hook_process ("func:get_status", 5000,
...
----
# prototipo
hook = whoreirc.hook_process(command, timeout, callback, callback_data)

# example with an external command
def my_process_cb(data, command, return_code, out, err):
    if return_code == whoreirc.WHOREIRC_HOOK_PROCESS_ERROR:
        whoreirc.prnt("", "Error with command '%s'" % command)
        return whoreirc.WHOREIRC_RC_OK
    if return_code >= 0:
        whoreirc.prnt("", "return_code = %d" % return_code)
    if out != "":
        whoreirc.prnt("", "stdout: %s" % out)
    if err != "":
        whoreirc.prnt("", "stderr: %s" % err)
    return whoreirc.WHOREIRC_RC_OK

hook = whoreirc.hook_process("ls", 5000, "my_process_cb", "")
...
hook = whoreirc.hook_process("func:get_status", 5000, "my_process_cb", "")
----

==== hook_process_hashtable

// TRANSLATION MISSING
_WhoreIRC ≥ 0.3.7, updated in 1.5._
...
[source,C]
----
struct t_hook *whoreirc_hook_process_hashtable (const char *command,
                                               struct t_hashtable *options,
                                               int timeout,
                                               int (*callback)(const void *pointer,
                                                               void *data,
...
struct t_hashtable *options_url1 = whoreirc_hashtable_new (8,
                                                          WHOREIRC_HASHTABLE_STRING,
                                                          WHOREIRC_HASHTABLE_STRING,
                                                          NULL,
                                                          NULL);
if (options_url1)
{
    whoreirc_hashtable_set (options, "file_out", "/tmp/whoreirc.org.html");
    struct t_hook *my_process_hook = whoreirc_hook_process_hashtable ("url:https://whoreirc.org/",
                                                                     options_url1,
                                                                     20000,
                                                                     &my_process_cb, NULL, NULL);
    whoreirc_hashtable_free (options_url1);
}

/* example 2: open URL with custom HTTP headers */
struct t_hashtable *options_url2 = whoreirc_hashtable_new (8,
                                                          WHOREIRC_HASHTABLE_STRING,
                                                          WHOREIRC_HASHTABLE_STRING,
                                                          NULL,
                                                          NULL);
if (options_url2)
{
    whoreirc_hashtable_set (options, "httpheader",
                           "Header1: value1\n"
                           "Header2: value2");
    struct t_hook *my_process_hook = whoreirc_hook_process_hashtable ("url:http://localhost:8080/",
                                                                     options_url2,
                                                                     20000,
                                                                     &my_process_cb, NULL, NULL);
    whoreirc_hashtable_free (options_url2);
}

/* example 3: execute a notify program with a message from someone */
struct t_hashtable *options_cmd1 = whoreirc_hashtable_new (8,
                                                          WHOREIRC_HASHTABLE_STRING,
                                                          WHOREIRC_HASHTABLE_STRING,
                                                          NULL,
                                                          NULL);
if (options_cmd1)
{
    whoreirc_hashtable_set (options_cmd1, "arg1", "-from");
    whoreirc_hashtable_set (options_cmd1, "arg2", nick);
    whoreirc_hashtable_set (options_cmd1, "arg3", "-msg");
    whoreirc_hashtable_set (options_cmd1, "arg4", message);  /* unsafe argument */
    struct t_hook *my_process_hook = whoreirc_hook_process_hashtable ("my-notify-command",
                                                                     options_cmd1,
                                                                     20000,
                                                                     &my_process_cb, NULL, NULL);
    whoreirc_hashtable_free (options_cmd1);
}

/* example 4: call shell to execute a command (command must be SAFE) */
struct t_hashtable *options_cmd2 = whoreirc_hashtable_new (8,
                                                          WHOREIRC_HASHTABLE_STRING,
                                                          WHOREIRC_HASHTABLE_STRING,
                                                          NULL,
                                                          NULL);
if (options_cmd2)
{
    whoreirc_hashtable_set (options_cmd2, "arg1", "-c");
    whoreirc_hashtable_set (options_cmd2, "arg2", "ls -l /tmp | grep something");
    struct t_hook *my_process_hook = whoreirc_hook_process_hashtable ("sh",
                                                                     options_cmd2,
                                                                     20000,
                                                                     &my_process_cb, NULL, NULL);
    whoreirc_hashtable_free (options_cmd2);
}
----
...
----
# prototipo
hook = whoreirc.hook_process_hashtable(command, options, timeout, callback, callback_data)
...
# example 1: download URL
hook1 = whoreirc.hook_process_hashtable("url:https://whoreirc.org/",
                                       {"file_out": "/tmp/whoreirc.org.html"},
                                       20000, "my_process_cb", "")
...
hook2 = whoreirc.hook_process_hashtable("url:http://localhost:8080/",
                                       options,
                                       20000, "my_process_cb", "")

# example 3: execute a notify program with a message from someone
hook3 = whoreirc.hook_process_hashtable("my-notify-command",
                                       {"arg1": "-from",
                                        "arg2": nick,
                                        "arg3": "-msg",
...
# example 4: call shell to execute a command (command must be SAFE)
hook4 = whoreirc.hook_process_hashtable("sh",
                                       {"arg1": "-c",
...
[source,C]
----
struct t_hook *whoreirc_hook_connect (const char *proxy,
                                     const char *address,
                                     int port,
                                     int ipv6,
                                     int retry,
...
*** _WHOREIRC_HOOK_CONNECT_OK_: connessione avvenuta con successo
*** _WHOREIRC_HOOK_CONNECT_ADDRESS_NOT_FOUND_: indirizzo non trovato
*** _WHOREIRC_HOOK_CONNECT_IP_ADDRESS_NOT_FOUND_: indirizzo IP non trovato
*** _WHOREIRC_HOOK_CONNECT_CONNECTION_REFUSED_: connessione rifiutata
*** _WHOREIRC_HOOK_CONNECT_PROXY_ERROR_: errore con il proxy
*** _WHOREIRC_HOOK_CONNECT_LOCAL_HOSTNAME_ERROR_: errore con il nome host locale
*** _WHOREIRC_HOOK_CONNECT_GNUTLS_INIT_ERROR_: errore inizializzazione GnuTLS
*** _WHOREIRC_HOOK_CONNECT_GNUTLS_HANDSHAKE_ERROR_: errore di handshake GnuTLS
*** _WHOREIRC_HOOK_CONNECT_MEMORY_ERROR_: memoria insufficiente
*** _WHOREIRC_HOOK_CONNECT_TIMEOUT_: timeout
*** _WHOREIRC_HOOK_CONNECT_SOCKET_ERROR_: errore nella creazione socket
...
// TRANSLATION MISSING
[IMPORTANT]
In scripts, with WhoreIRC ≥ 2.0, the callback arguments _status_, _gnutls_rc_
and _sock_ are integers (with WhoreIRC ≤ 1.9, they were strings). +
To be compatible with all versions, it is recommended to convert the argument
...
struct t_hook *my_connect_hook = whoreirc_hook_connect (NULL,
                                                       "my.server.org", 1234,
                                                       1, 0,
                                                       NULL, NULL, 0,  /* GnuTLS */
                                                       NULL,
...
----
# prototipo
hook = whoreirc.hook_connect(proxy, address, port, ipv6, retry, local_hostname,
...
def my_connect_cb(data, status, gnutls_rc, sock, error, ip_address):
    if status == WHOREIRC_HOOK_CONNECT_OK:
        # ...
    elif status == WHOREIRC_HOOK_CONNECT_ADDRESS_NOT_FOUND:
        # ...
    elif status == WHOREIRC_HOOK_CONNECT_IP_ADDRESS_NOT_FOUND:
        # ...
    elif status == WHOREIRC_HOOK_CONNECT_CONNECTION_REFUSED:
        # ...
    elif status == WHOREIRC_HOOK_CONNECT_PROXY_ERROR:
        # ...
    elif status == WHOREIRC_HOOK_CONNECT_LOCAL_HOSTNAME_ERROR:
        # ...
    elif status == WHOREIRC_HOOK_CONNECT_GNUTLS_INIT_ERROR:
        # ...
    elif status == WHOREIRC_HOOK_CONNECT_GNUTLS_HANDSHAKE_ERROR:
        # ...
    elif status == WHOREIRC_HOOK_CONNECT_MEMORY_ERROR:
        # ...
    elif status == WHOREIRC_HOOK_CONNECT_TIMEOUT:
        # ...
    elif status == WHOREIRC_HOOK_CONNECT_SOCKET_ERROR:
        # ...
    return whoreirc.WHOREIRC_RC_OK

hook = whoreirc.hook_connect("", "my.server.org", 1234, 1, 0, "",
                            "my_connect_cb", "")
----
...
* <<_hook_modifier,hook modifier>> "<<hook_modifier_whoreirc_print,whoreirc_print>>":
...
[source,C]
----
struct t_hook *whoreirc_hook_line (const char *buffer_type,
...
* _callback_pointer_: pointer given to callback when it is called by WhoreIRC
* _callback_data_: pointer given to callback when it is called by WhoreIRC;
  if not NULL, it must have been allocated with malloc (or similar function)
  and it is automatically freed when the hook is deleted
...
Any invalid value in a field is silently ignored by WhoreIRC.
...
my_line_cb (const void *pointer, void *data, struct t_hasbtable *line)
{
    ...
    hashtable = whoreirc_hashtable_new (8,
                                       WHOREIRC_HASHTABLE_STRING,
                                       WHOREIRC_HASHTABLE_STRING,
                                       NULL,
                                       NULL);
    /* force a highlight on the line */
    whoreirc_hashtable_set (hashtable, "highlight", "1");
    return hashtable;
}

/* catch lines with tag "irc_join" */
struct t_hook *my_line_hook =
    whoreirc_hook_line ("", "", "irc_join", &my_line_cb, NULL, NULL);
----
...
----
# prototipo
hook = whoreirc.hook_line(buffer_type, buffer_name, tags, callback, callback_data)
...
# catch lines with tag "irc_join"
hook = whoreirc.hook_line("", "", "irc_join", "my_line_cb", "")
----
...
[source,C]
----
struct t_hook *whoreirc_hook_print (struct t_gui_buffer *buffer,
                                   const char *tags,
                                   const char *message,
                                   int strip_colors,
                                   int (*callback)(const void *pointer,
...
// TRANSLATION MISSING
** with WhoreIRC ≥ 0.4.3: comma-separated list of tags that must be in message
   (logical "or"); it is possible to combine many tags as a logical "and" with
   separator `+++`; wildcard `+*+` is allowed in tags
// TRANSLATION MISSING
** with WhoreIRC ≤ 0.4.2: comma-separated list of tags that must all be in
   message (logical "and")
...
// TRANSLATION MISSING
[IMPORTANT]
In scripts, with WhoreIRC ≥ 1.0, the callback arguments _displayed_ and
_highlight_ are integers (with WhoreIRC ≤ 0.4.3, they were strings). +
To be compatible with all versions, it is recommended to convert the argument
...
struct t_hook *my_print_hook =
    whoreirc_hook_print (NULL, NULL, NULL, 1, &my_print_cb, NULL, NULL);
----
...
----
# prototipo
hook = whoreirc.hook_print(buffer, tags, message, strip_colors, callback, callback_data)
...
# cattura tutti i messaggi, su tutti i buffer, senza colore
hook = whoreirc.hook_print("", "", "", 1, "my_print_cb", "")
----
...
[source,C]
----
struct t_hook *whoreirc_hook_signal (const char *signal,
                                    int (*callback)(const void *pointer,
                                                    void *data,
                                                    const char *signal,
...
*** _WHOREIRC_HOOK_SIGNAL_STRING_: stringa
*** _WHOREIRC_HOOK_SIGNAL_INT_: numero intero
*** _WHOREIRC_HOOK_SIGNAL_POINTER_: puntatore
** _void *signal_data_: dati inviati con il segnale
** valore restituito:
*** _WHOREIRC_RC_OK_
// TRANSLATION MISSING
*** _WHOREIRC_RC_OK_EAT_ (stop sending the signal immediately)
    _(WhoreIRC ≥ 0.4.0)_
*** _WHOREIRC_RC_ERROR_
* _callback_pointer_: puntatore fornito alla callback quando chiamata da WhoreIRC
// TRANSLATION MISSING
...
// TRANSLATION MISSING
List of signals sent by WhoreIRC and plugins:
...
| whoreirc | buffer_opened |
  Puntatore: buffer. |
  Buffer aperto.

| whoreirc | buffer_closing |
  Puntatore: buffer. |
  Chiusura del buffer.

| whoreirc | buffer_closed |
  Puntatore: buffer. |
  Buffer chiuso.

// TRANSLATION MISSING
| whoreirc | buffer_cleared |
  Puntatore: buffer. |
  Buffer cleared.

// TRANSLATION MISSING
| whoreirc | buffer_filters_enabled +
  _(WhoreIRC ≥ 2.0)_ |
  Pointer: buffer. |
  Filters enabled in buffer.

// TRANSLATION MISSING
| whoreirc | buffer_filters_disabled +
  _(WhoreIRC ≥ 2.0)_ |
  Pointer: buffer. |
  Filters disabled in buffer.

// TRANSLATION MISSING
| whoreirc | buffer_hidden |
  Pointer: buffer. |
  Buffer hidden.

// TRANSLATION MISSING
| whoreirc | buffer_unhidden |
  Pointer: buffer. |
  Buffer unhidden.

| whoreirc | buffer_line_added +
  _(WhoreIRC ≥ 0.3.7)_ |
  Puntatore: riga. |
  Riga aggiunta in un buffer.

| whoreirc | buffer_lines_hidden |
  Puntatore: buffer. |
  Righe nascoste nel buffer.

| whoreirc | buffer_localvar_added |
  Puntatore: buffer. |
  Variabili locali aggiunte.

| whoreirc | buffer_localvar_changed |
  Puntatore: buffer. |
  Variabili locali modificate.

| whoreirc | buffer_localvar_removed |
  Puntatore: buffer. |
  Variabili locali rimosse.

// TRANSLATION MISSING
| whoreirc | buffer_merged |
  Puntatore: buffer. |
  Buffer merged.

// TRANSLATION MISSING
| whoreirc | buffer_unmerged |
  Puntatore: buffer. |
  Buffer unmerged.

| whoreirc | buffer_moved |
  Puntatore: buffer. |
  Buffer spostato.

| whoreirc | buffer_renamed |
  Puntatore: buffer. |
  Buffer rinominato.

| whoreirc | buffer_switch |
  Puntatore: buffer. |
  Passaggio tra buffer.

| whoreirc | buffer_title_changed |
  Puntatore: buffer. |
  Titolo del buffer modificato.

| whoreirc | buffer_type_changed |
  Puntatore: buffer. |
  Tipo di buffer modificato.

// TRANSLATION MISSING
| whoreirc | buffer_zoomed +
  _(WhoreIRC ≥ 0.4.3)_ |
  Puntatore: buffer. |
  Merged buffer zoomed.

// TRANSLATION MISSING
| whoreirc | buffer_unzoomed +
  _(WhoreIRC ≥ 0.4.3)_ |
  Puntatore: buffer. |
  Merged buffer unzoomed.

| whoreirc | day_changed +
  _(WhoreIRC ≥ 0.3.2)_ |
...
| whoreirc | filter_added |
  Puntatore: filtro. |
  Filtro aggiunto.

| whoreirc | filter_removing |
  Puntatore: filtro. |
  Rimozione del filtro.

| whoreirc | filter_removed |
  - |
  Filtro rimosso.

| whoreirc | filters_enabled |
  - |
  Filtri abilitati.

| whoreirc | filters_disabled |
  - |
  Filtri disabilitati.

// TRANSLATION MISSING
| whoreirc | hotlist_changed |
  Pointer: buffer (can be NULL). |
  Hotlist modificata.

| whoreirc | input_paste_pending |
  - |
  Incolla testo in attesa.

| whoreirc | input_search |
  Puntatore: buffer. |
  Ricerca testo nel buffer.

| whoreirc | input_text_changed |
  Puntatore: buffer. |
  Testo in input modificato.

| whoreirc | input_text_cursor_moved |
  Puntatore: buffer. |
...
| whoreirc | key_pressed |
  String: tasto digitato. |
  Tasto digitato.

// TRANSLATION MISSING
| whoreirc | key_combo_default +
  _(WhoreIRC ≥ 1.0)_ |
  String: key combo. |
  Key combo in _default_ context.

// TRANSLATION MISSING
| whoreirc | key_combo_search +
  _(WhoreIRC ≥ 1.0)_ |
  String: key combo. |
  Key combo in _search_ context.

// TRANSLATION MISSING
| whoreirc | key_combo_cursor +
  _(WhoreIRC ≥ 1.0)_ |
  String: key combo. |
  Key combo in _cursor_ context.

// TRANSLATION MISSING
| whoreirc | mouse_enabled +
  _(WhoreIRC ≥ 1.1)_ |
  - |
  Mouse enabled.

// TRANSLATION MISSING
| whoreirc | mouse_disabled +
  _(WhoreIRC ≥ 1.1)_ |
  - |
  Mouse disabled.

// TRANSLATION MISSING
| whoreirc | nicklist_group_added +
  _(WhoreIRC ≥ 0.3.2)_ |
  String: buffer pointer + "," + group name. |
  Group added in nicklist.

// TRANSLATION MISSING
| whoreirc | nicklist_group_changed +
  _(WhoreIRC ≥ 0.3.4)_ |
  String: buffer pointer + "," + group name. |
  Group changed in nicklist.

// TRANSLATION MISSING
| whoreirc | nicklist_group_removing +
  _(WhoreIRC ≥ 0.4.1)_ |
  String: buffer pointer + "," + group name. |
  Removing group from nicklist.

// TRANSLATION MISSING
| whoreirc | nicklist_group_removed +
  _(WhoreIRC ≥ 0.3.2)_ |
  String: buffer pointer + "," + group name. |
  Group removed from nicklist.

// TRANSLATION MISSING
| whoreirc | nicklist_nick_added +
  _(WhoreIRC ≥ 0.3.2)_ |
  String: buffer pointer + "," + nick name. |
  Nick added in nicklist.

// TRANSLATION MISSING
| whoreirc | nicklist_nick_changed +
  _(WhoreIRC ≥ 0.3.4)_ |
  String: buffer pointer + "," + nick name. |
  Nick changed in nicklist.

// TRANSLATION MISSING
| whoreirc | nicklist_nick_removing +
  _(WhoreIRC ≥ 0.4.1)_ |
  String: buffer pointer + "," + nick name. |
  Removing nick from nicklist.

// TRANSLATION MISSING
| whoreirc | nicklist_nick_removed +
  _(WhoreIRC ≥ 0.3.2)_ |
  String: buffer pointer + "," + nick name. |
  Nick removed from nicklist.

| whoreirc | partial_completion |
  - |
  Completamento parziale avvenuto.

// TRANSLATION MISSING
| whoreirc | plugin_loaded +
  _(WhoreIRC ≥ 0.3.9)_ |
  String: path to plugin loaded. |
  Plugin loaded.

// TRANSLATION MISSING
| whoreirc | plugin_unloaded +
  _(WhoreIRC ≥ 0.3.9)_ |
...
// TRANSLATION MISSING
| whoreirc | signal_sighup +
  _(WhoreIRC ≥ 1.3)_ |
  - |
  Signal SIGHUP received.

// TRANSLATION MISSING
| whoreirc | signal_sigquit +
  _(WhoreIRC ≥ 1.2)_ |
  - |
  Signal SIGQUIT received (quit request with core dump).

// TRANSLATION MISSING
| whoreirc | signal_sigterm +
  _(WhoreIRC ≥ 1.2)_ |
  - |
  Signal SIGTERM received (graceful termination of WhoreIRC process).

// TRANSLATION MISSING
| whoreirc | signal_sigwinch +
  _(WhoreIRC ≥ 0.4.3)_ |
  - |
...
| whoreirc | upgrade_ended +
  _(WhoreIRC ≥ 0.3.4)_ |
  - |
  Fine del processo di aggiornamento (comando `/upgrade`).

| whoreirc | whoreirc_highlight |
  String: messaggio con prefisso. |
  Evento accaduto.

| whoreirc | whoreirc_pv |
  String: messaggio con prefisso. |
  Messaggio privato visualizzato.

// TRANSLATION MISSING
| whoreirc | window_closing +
  _(WhoreIRC ≥ 0.3.6)_ |
  Puntatore: finestra. |
  Closing window.

// TRANSLATION MISSING
| whoreirc | window_closed +
  _(WhoreIRC ≥ 0.3.6)_ |
  Puntatore: finestra. |
  Window closed.

// TRANSLATION MISSING
| whoreirc | window_opened +
  _(WhoreIRC ≥ 0.4.1)_ |
  Puntatore: finestra. |
  Window opened.

| whoreirc | window_scrolled |
  Puntatore: finestra. |
  Scroll nella finestra.

| whoreirc | window_switch +
  _(WhoreIRC ≥ 0.3.7)_ |
  Puntatore: finestra. |
  Passaggio alla finestra.

| whoreirc | window_zoom |
  Puntatore: finestra corrente. |
  Massimizzazione della finestra.

| whoreirc | window_zoomed |
  Puntatore: finestra corrente. |
  Finestra massimizzata.

| whoreirc | window_unzoom |
  Puntatore: finestra corrente. |
  Minimizzazione della finestra.

| whoreirc | window_unzoomed |
  Puntatore: finestra corrente. |
...
struct t_hook *my_signal_hook = whoreirc_hook_signal ("quit",
...
----
# prototipo
hook = whoreirc.hook_signal(signal, callback, callback_data)
...
# cattura il segnale "quit"
hook = whoreirc.hook_signal("quit", "my_signal_cb", "")
----
...
[source,C]
----
int whoreirc_hook_signal_send (const char *signal, const char *type_data,
                              void *signal_data);
----
...
// TRANSLATION MISSING
Return value _(WhoreIRC ≥ 1.0)_:

// TRANSLATION MISSING
* return code of last callback executed (_WHOREIRC_RC_OK_ if no callback was
  executed):
** _WHOREIRC_RC_OK_
** _WHOREIRC_RC_OK_EAT_
** _WHOREIRC_RC_ERROR_
...
[source,C]
----
int rc = whoreirc_hook_signal_send ("my_signal", WHOREIRC_HOOK_SIGNAL_STRING, my_string);
----
...
----
# prototipo
rc = whoreirc.hook_signal_send(signal, type_data, signal_data)

# esempio
rc = whoreirc.hook_signal_send("my_signal", whoreirc.WHOREIRC_HOOK_SIGNAL_STRING, my_string)
----
...
[source,C]
----
whoreirc_hook_signal_send ("logger_backlog", WHOREIRC_HOOK_SIGNAL_POINTER, buffer);
----
...
[source,python]
----
whoreirc.hook_signal_send("logger_backlog", whoreirc.WHOREIRC_HOOK_SIGNAL_POINTER, buffer)
----
...
. Sposta il nuovo script nella cartella _~/.whoreirc/xxx/_ (dove _xxx_ è il
  linguaggio).
// TRANSLATION MISSING
. Crea un link al nuovo script nella cartella _~/.whoreirc/xxx/autoload/_
...
[source,C]
----
whoreirc_hook_signal_send ("python_script_install", WHOREIRC_HOOK_SIGNAL_STRING,
                          "/home/xxx/.whoreirc/test.py");
----
...
[source,python]
----
whoreirc.hook_signal_send("python_script_install", WHOREIRC_HOOK_SIGNAL_STRING,
                         "/home/xxx/.whoreirc/test.py")
----
...
whoreirc_hook_signal_send ("python_script_remove", WHOREIRC_HOOK_SIGNAL_STRING,
                          "test.py,script.py");
----
...
----
# scarica e rimuove gli script test.py e script.py
whoreirc.hook_signal_send("python_script_remove", WHOREIRC_HOOK_SIGNAL_STRING,
                         "test.py,script.py")
----
...
// TRANSLATION MISSING
_WhoreIRC ≥ 0.3.4, updated in 1.5._
...
[source,C]
----
/* dice "Hello!" sul server freenode, canale #whoreirc */
whoreirc_hook_signal_send ("irc_input_send", WHOREIRC_HOOK_SIGNAL_STRING,
                          "freenode;#whoreirc;priority_high,user_message;;Hello!");
...
[source,python]
----
# dice "Hello!" sul server freenode server, canale #whoreirc
whoreirc.hook_signal_send("irc_input_send", whoreirc.WHOREIRC_HOOK_SIGNAL_STRING,
                         "freenode;#whoreirc;priority_high,user_message;;Hello!")
...
[source,C]
----
struct t_hook *whoreirc_hook_hsignal (const char *signal,
                                     int (*callback)(const void *pointer,
                                                     void *data,
                                                     const char *signal,
                                                     struct t_hashtable *hashtable),
...
// TRANSLATION MISSING
List of hsignals sent by WhoreIRC and plugins:
...
struct t_hook *my_hsignal_hook = whoreirc_hook_hsignal ("test",
...
----
# prototipo
hook = whoreirc.hook_hsignal(signal, callback, callback_data)
...
hook = whoreirc.hook_hsignal("test", "my_hsignal_cb", "")
----

==== hook_hsignal_send

// TRANSLATION MISSING
_WhoreIRC ≥ 0.3.4, updated in 1.0._
...
[source,C]
----
int whoreirc_hook_hsignal_send (const char *signal, struct t_hashtable *hashtable);
----
...
if (hashtable)
{
    whoreirc_hashtable_set (hashtable, "key", "value");
    rc = whoreirc_hook_hsignal_send ("my_hsignal", hashtable);
    whoreirc_hashtable_free (hashtable);
}
----
...
----
# prototipo
rc = whoreirc.hook_hsignal_send(signal, hashtable)

# esempio
rc = whoreirc.hook_hsignal_send("my_hsignal", {"key": "value"})
----
...
test_whois_cb (const void *pointer, void *data, const char *signal,
               struct t_hashtable *hashtable)
{
    whoreirc_printf (NULL, "error = %s", whoreirc_hashtable_get (hashtable, "error"));
    whoreirc_printf (NULL, "output = %s", whoreirc_hashtable_get (hashtable, "output"));
    return WHOREIRC_RC_OK;
}

whoreirc_hook_hsignal ("irc_redirection_test_whois", &test_whois_cb, NULL, NULL);
struct t_hashtable *hashtable = whoreirc_hashtable_new (8,
                                                       WHOREIRC_HASHTABLE_STRING,
                                                       WHOREIRC_HASHTABLE_STRING,
                                                       NULL,
...
if (hashtable)
{
    whoreirc_hashtable_set (hashtable, "server", "freenode");
    whoreirc_hashtable_set (hashtable, "pattern", "whois");
    whoreirc_hashtable_set (hashtable, "signal", "test");
    whoreirc_hashtable_set (hashtable, "string", "FlashCode");
    whoreirc_hook_hsignal_send ("irc_redirect_command", hashtable);
    whoreirc_hook_signal_send ("irc_input_send", WHOREIRC_HOOK_SIGNAL_STRING,
...
def test_whois_cb(data, signal, hashtable):
    whoreirc.prnt("", "error = %s" % hashtable["error"])
    whoreirc.prnt("", "output = %s" % hashtable["output"])
    return whoreirc.WHOREIRC_RC_OK

whoreirc.hook_hsignal ("irc_redirection_test_whois", "test_whois_cb", "")
whoreirc.hook_hsignal_send("irc_redirect_command",
...
if (hashtable)
{
    whoreirc_hashtable_set (hashtable, "pattern", "my_whois");
    whoreirc_hashtable_set (hashtable, "timeout", "30");
    whoreirc_hashtable_set (hashtable, "cmd_start", "311:1");
    whoreirc_hashtable_set (hashtable, "cmd_stop", "318:1,401:1,402:1,431:1,461");
    whoreirc_hashtable_set (hashtable, "cmd_extra", "318:1");
    whoreirc_hook_hsignal_send ("irc_redirect_pattern", hashtable);
    /*
...
[source,python]
----
whoreirc.hook_hsignal_send("irc_redirect_pattern",
...
[source,C]
----
struct t_hook *whoreirc_hook_config (const char *option,
                                    int (*callback)(const void *pointer,
                                                    void *data,
                                                    const char *option,
                                                    const char *value),
...
* _option_: opzione, il formato è il nome completo, come usato con il comando
  `/set` (ad esempio: `whoreirc.look.item_time_format`), wildcard `+*+` is allowed
...
/* cattura le modifiche dell'opzione "whoreirc.look.item_time_format" */
struct t_hook *my_config_hook = whoreirc_hook_config ("whoreirc.look.item_time_format",
...
----
# prototipo
hook = whoreirc.hook_config(option, callback, callback_data)
...
# cattura le modifiche dell'opzione "whoreirc.look.item_time_format"
hook = whoreirc.hook_config("whoreirc.look.item_time_format", "my_config_cb", "")
----
...
[source,C]
----
struct t_hook *whoreirc_hook_modifier (const char *modifier,
...
List of modifiers used by WhoreIRC and plugins:
...
// TRANSLATION MISSING
| [[hook_modifier_whoreirc_print]] whoreirc_print |
...
struct t_hook *my_modifier_hook = whoreirc_hook_modifier ("whoreirc_print",
...
----
# prototipo
hook = whoreirc.hook_modifier(modifier, callback, callback_data)

# esempio
def my_modifier_cb(data, modifier, modifier_data, string):
    return "%s xxx" % string

hook = whoreirc.hook_modifier("whoreirc_print", "my_modifier_cb", "")
----
...
[source,C]
----
char *whoreirc_hook_modifier_exec (const char *modifier,
                                  const char *modifier_data,
                                  const char *string);
----
...
// TRANSLATION MISSING
List of modifiers defined by WhoreIRC and plugins that can be used:
...
| [[hook_modifier_irc_color_decode]] irc_color_decode |
  ...
  Qualsiasi stringa. |
  String with IRC colors converted to WhoreIRC colors (or IRC colors removed).
...
| [[hook_modifier_color_decode_ansi]] color_decode_ansi +
  ...
  "1" per mantenere i colori, "0" per rimuovere i colori |
  Qualsiasi stringa. |
  String with ANSI colors converted to WhoreIRC colors (or ANSI colors removed).
|===
...
[source,C]
----
char *new_string = whoreirc_hook_modifier_exec ("my_modifier",
                                               my_data, my_string);
----
...
----
# prototipo
whoreirc.hook_modifier_exec(modifier, modifier_data, string)

# esempio
whoreirc.hook_modifier_exec("my_modifier", my_data, my_string)
----
...
[source,C]
----
struct t_hook *whoreirc_hook_info (const char *info_name,
                                  const char *description,
                                  const char *args_description,
                                  char *(*callback)(const void *pointer,
                                                    void *data,
...
// TRANSLATION MISSING
[NOTE]
With WhoreIRC ≥ 2.5, the callback returns an allocated string
(with WhoreIRC ≤ 2.4, it was a pointer to a static string).
...
struct t_hook *my_info_hook = whoreirc_hook_info ("my_info",
                                                 "Some info",
...
----
# prototipo
hook = whoreirc.hook_info(info_name, description, args_description, callback, callback_data)

# esempio
def my_info_cb(data, info_name, arguments):
    return "some_info"

hook = whoreirc.hook_info("my_info", "Some info", "Info about arguments",
                         "my_info_cb", "")
----
...
[source,C]
----
struct t_hook *whoreirc_hook_info_hashtable (const char *info_name,
                                            const char *description,
                                            const char *args_description,
...
struct t_hook *my_info_hook = whoreirc_hook_info_hashtable ("my_info_hashtable",
                                                           "Some info",
...
----
# prototipo
hook = whoreirc.hook_info_hashtable(info_name, description, args_description,
...
hook = whoreirc.hook_info_hashtable("my_info_hashtable", "Some info",
                                   "Info about input hashtable",
                                   "Info about output hashtable",
...
[source,C]
----
struct t_hook *whoreirc_hook_infolist (const char *infolist_name,
                                      const char *description,
...
struct t_hook *my_infolist = whoreirc_hook_infolist ("my_infolist",
...
----
# prototipo
hook = whoreirc.hook_infolist(infolist_name, description, pointer_description,
...
hook = whoreirc.hook_infolist("my_infolist", "Infolist with some data",
...
[source,C]
----
struct t_hook *whoreirc_hook_hdata (const char *hdata_name,
                                   const char *description,
...
/* add hdata "my_hdata" */
struct t_hook *my_hdata = whoreirc_hook_hdata ("my_hdata",
...
[source,C]
----
struct t_hook *whoreirc_hook_focus (const char *area,
                                   struct t_hashtable *(*callback)(const void *pointer,
                                                                   void *data,
...
| _buffer_name | Nome del buffer. |
  "whoreirc", "freenode.#whoreirc", ... | ""

| _buffer_full_name | Nome completo del buffer. |
  "core.whoreirc", "irc.freenode.#whoreirc", ... | ""
...
^(4)^ Data/ora in cui WhoreIRC aggiunge una riga nel buffer (maggiore o uguale a
"chat_line_date").
...
struct t_hook *my_focus = whoreirc_hook_focus ("buffer_nicklist",
...
----
# prototipo
hook = whoreirc.hook_focus(area, callback, callback_data)
...
hook = whoreirc.hook_focus("buffer_nicklist", "my_focus_nicklist_cb", "")
----

==== hook_set

_WhoreIRC ≥ 0.3.9 (script: WhoreIRC ≥ 0.4.3)._
...
[source,C]
----
void whoreirc_hook_set (struct t_hook *hook, const char *property,
                       const char *value);
----

Argomenti:

* _hook_: qualcosa su cui è presente un hook con "whoreirc_hook_xxx()"
// TRANSLATION MISSING
...
----
struct t_hook *my_command_hook =
    whoreirc_hook_command ("abcd", "description",
                          "args", "description args",
                          "", &my_command_cb, NULL, NULL);
whoreirc_hook_set (my_command_hook, "subplugin", "test");
----
...
----
# prototipo
whoreirc.hook_set(hook, property, value)
...
hook = whoreirc.hook_process_hashtable("/path/to/command", {"stdin": "1"},
                                      20000, "my_process_cb", "")
whoreirc.hook_set(hook, "stdin", "data sent to stdin of child process")
whoreirc.hook_set(hook, "stdin_close", "")  # optional
----
...
[source,C]
----
void whoreirc_unhook (struct t_hook *hook);
----
...
[source,C]
----
struct t_hook *my_hook = whoreirc_hook_command ( /* ... */ );
/* ... */
whoreirc_unhook (my_hook);
----
...
----
# prototipo
whoreirc.unhook(hook)

# esempio
whoreirc.unhook(my_hook)
----
...
[source,C]
----
void whoreirc_unhook_all (const char *subplugin);
----
...
[source,C]
----
struct t_gui_buffer *whoreirc_buffer_new (const char *name,
...
{
    whoreirc_printf (buffer, "Testo: %s", input_data);
    return WHOREIRC_RC_OK;
}
...
{
    whoreirc_printf (NULL, "Il buffer '%s' verrà chiuso!",
                    whoreirc_buffer_get_string (buffer, "name"));
    return WHOREIRC_RC_OK;
}

struct t_gui_buffer *my_buffer = whoreirc_buffer_new ("my_buffer",
...
----
# prototipo
buffer = whoreirc.buffer_new(name, input_callback, input_callback_data,
    close_callback, close_callback_data)

# esempio
def my_input_cb(data, buffer, input_data):
    whoreirc.prnt(buffer, "Testo: %s" % input_data)
    return whoreirc.WHOREIRC_RC_OK

def my_close_cb(data, buffer):
    whoreirc.prnt("", "Il buffer '%s' verrà chiuso!" % whoreirc.buffer_get_string(buffer, "name"))
    return whoreirc.WHOREIRC_RC_OK

buffer = whoreirc.buffer_new("my_buffer", "my_input_cb", "", "my_close_cb", "")
----
...
[source,C]
----
struct t_gui_buffer *whoreirc_current_buffer ();
----
...
[source,C]
----
whoreirc_printf (whoreirc_current_buffer (), "Testo sul buffer corrente");
----
...
# esempio
whoreirc.prnt(whoreirc.current_buffer(), "Testo sul buffer corrente")
----
...
[source,C]
----
struct t_gui_buffer *whoreirc_buffer_search (const char *plugin,
                                            const char *name);
----
...
** `+==+`: the name used is the buffer full name (for example:
   `irc.freenode.#whoreirc` instead of `freenode.#whoreirc`)
   _(WhoreIRC ≥ 1.0)_
// TRANSLATION MISSING
* _name_: name of buffer, if it is NULL or empty string, the current buffer is
  ...
  `(?i)`, the search is case insensitive _(WhoreIRC ≥ 1.0)_
...
[source,C]
----
struct t_gui_buffer *buffer1 = whoreirc_buffer_search ("irc", "freenode.#whoreirc");
struct t_gui_buffer *buffer2 = whoreirc_buffer_search ("==", "irc.freenode.#test");  /* WhoreIRC ≥ 1.0 */
----
...
----
# prototipo
buffer = whoreirc.buffer_search(plugin, name)

# esempio
buffer = whoreirc.buffer_search("my_plugin", "my_buffer")
----

==== buffer_search_main

Cerca nel buffer principale di WhoreIRC (per primo nel buffer _core_, il primo
visualizzato all'avvio di WhoreIRC).

Prototipo:

[source,C]
----
struct t_gui_buffer *whoreirc_buffer_search_main ();
----

Valore restituito:

* puntatore al buffer principale di WhoreIRC (buffer _core_)

Esempio in C:

[source,C]
----
struct t_gui_buffer *whoreirc_buffer = whoreirc_buffer_search_main ();
----
...
----
# prototipo
buffer = whoreirc.buffer_search_main()
...
[source,C]
----
void whoreirc_buffer_clear (struct t_gui_buffer *buffer);
----
...
[source,C]
----
struct t_gui_buffer *my_buffer = whoreirc_buffer_search ("my_plugin",
                                                        "my_buffer");
if (my_buffer)
{
    whoreirc_buffer_clear (my_buffer);
}
----
...
[source,C]
----
void whoreirc_buffer_close (struct t_gui_buffer *buffer);
----
...
struct t_gui_buffer *my_buffer = whoreirc_buffer_new ("my_buffer",
                                                     ...
                                                     &my_close_cb, NULL);
/* ... */
whoreirc_buffer_close (my_buffer);
----
...
con lo stesso numero, e WhoreIRC visualizzerà le righe di entrambi i
...
[source,C]
----
void whoreirc_buffer_merge (struct t_gui_buffer *buffer,
...
[source,C]
----
/* merge current buffer with whoreirc "core" buffer */
whoreirc_buffer_merge (whoreirc_current_buffer (),
                      whoreirc_buffer_search_main ());
----
...
----
# prototipo
whoreirc.buffer_merge(buffer, target_buffer)

# esempio
# merge current buffer with WhoreIRC "core" buffer
whoreirc.buffer_merge(whoreirc.current_buffer(), whoreirc.buffer_search_main())
----
...
[source,C]
----
void whoreirc_buffer_unmerge (struct t_gui_buffer *buffer,
                             int number);
----
...
[source,C]
----
whoreirc_buffer_unmerge (whoreirc_current_buffer (), 1);
----
...
----
# prototipo
whoreirc.buffer_unmerge(buffer, number)

# esempio
whoreirc.buffer_unmerge(whoreirc.current_buffer(), 1)
----
...
[source,C]
----
int whoreirc_buffer_get_integer (struct t_gui_buffer *buffer,
                                const char *property);
----
...
[source,C]
----
whoreirc_printf (NULL, "my buffer number is: %d",
                whoreirc_buffer_get_integer (my_buffer, "number"));
----
...
----
# prototipo
value = whoreirc.buffer_get_integer(buffer, property)

# esempio
whoreirc.prnt("", "my buffer number is: %d" % whoreirc.buffer_get_integer(my_buffer, "number"))
----
...
[source,C]
----
const char *whoreirc_buffer_get_string (struct t_gui_buffer *buffer,
                                       const char *property);
----
...
** _plugin_: nome del plugin che ha creato questo buffer ("core"
   per il buffer principale di WhoreIRC)
** _name_: nome del buffer
** _full_name_: nome completo del buffer ("plugin.nome") _(WhoreIRC ≥ 0.3.7)_
...
[source,C]
----
whoreirc_printf (NULL, "name / short name of buffer are: %s / %s",
                whoreirc_buffer_get_string (my_buffer, "name"),
                whoreirc_buffer_get_string (my_buffer, "short_name"));
----
...
----
# prototipo
value = whoreirc.buffer_get_string(buffer, property)

# esempio
whoreirc.prnt("", "name / short name of buffer are: %s / %s"
    % (whoreirc.buffer_get_string(my_buffer, "name"),
    whoreirc.buffer_get_string(my_buffer, "short_name")))
----
...
[source,C]
----
void *whoreirc_buffer_pointer (struct t_gui_buffer *buffer,
                              const char *property);
----
...
[source,C]
----
whoreirc_printf (NULL, "plugin pointer of my buffer: %lx",
                whoreirc_buffer_get_pointer (my_buffer, "plugin"));
----
...
----
# prototipo
value = whoreirc.buffer_get_pointer(buffer, property)

# esempio
whoreirc.prnt("", "plugin pointer of my buffer: %s" % whoreirc.buffer_get_pointer(my_buffer, "plugin"))
----
...
[source,C]
----
void whoreirc_buffer_set (struct t_gui_buffer *buffer, const char *property,
                         const char *value);
----
...
// TRANSLATION MISSING
| hotlist | "+", "-", WHOREIRC_HOTLIST_LOW, WHOREIRC_HOTLIST_MESSAGE,
  WHOREIRC_HOTLIST_PRIVATE, WHOREIRC_HOTLIST_HIGHLIGHT, "-1" |
  ...
  "-1": remove buffer from hotlist _(WhoreIRC ≥ 1.0)_.
...
| type | "formatted" oppure "free" |
  ...
  _clear_ is forced to "0" _(WhoreIRC ≥ 1.0)_.
...
whoreirc_buffer_set (NULL, "hotlist", "-");

/* abilita nuovamente hotlist */
whoreirc_buffer_set (NULL, "hotlist", "+");

/* cambia il nome buffer */
whoreirc_buffer_set (my_buffer, "name", "my_new_name");

/* aggiunge una nuova variabile locale "tizio" con il valore "abc" */
whoreirc_buffer_set (my_buffer, "localvar_set_tizio", "abc");

/* rimuove la variabile locale "tizio" */
whoreirc_buffer_set (my_buffer, "localvar_del_tizio", "");
----
...
----
# prototipo
whoreirc.buffer_set(buffer, property, value)

# esempi

# disabilita hotlist (per tutti i buffer)
whoreirc.buffer_set("", "hotlist", "-")

# abilita nuovamente hotlist
whoreirc.buffer_set("", "hotlist", "+")

# cambia il nome buffer
whoreirc.buffer_set(my_buffer, "name", "my_new_name")

# aggiunge una nuova variabile locale "tizio" con il valore "abc"
whoreirc.buffer_set(my_buffer, "localvar_set_tizio", "abc")

# rimuove la variabile locale "tizio"
whoreirc.buffer_set(my_buffer, "localvar_del_tizio", "")
----
...
[source,C]
----
void whoreirc_buffer_set_pointer (struct t_gui_buffer *buffer, const char *property,
                                 void *pointer);
----
...
** _nickcmp_callback_: set nick comparison callback function (this callback is
   called when searching nick in nicklist) _(WhoreIRC ≥ 0.3.9)_
// TRANSLATION MISSING
...
whoreirc_buffer_set_pointer (my_buffer, "close_callback", &my_close_cb);
----
...
[source,C]
----
char *whoreirc_buffer_string_replace_local_var (struct t_gui_buffer *buffer,
                                               const char *string);
----
...
[source,C]
----
whoreirc_buffer_set (my_buffer, "localvar_set_toto", "abc");

char *str = whoreirc_buffer_string_replace_local_var (my_buffer,
                                                     "test with $toto");
...
----
# prototipo
value = whoreirc.buffer_string_replace_local_var(buffer, string)

# esempio
whoreirc.buffer_set(my_buffer, "localvar_set_toto", "abc")
str = whoreirc.buffer_string_replace_local_var(my_buffer, "test with $toto")
...
[source,C]
----
int whoreirc_buffer_match_list (struct t_gui_buffer *buffer, const char *string);
----
...
[source,C]
----
struct t_gui_buffer *buffer = whoreirc_buffer_search ("irc", "freenode.#whoreirc");
if (buffer)
{
    whoreirc_printf (NULL, "%d", whoreirc_buffer_match_list (buffer, "*"));                    /* 1 */
    whoreirc_printf (NULL, "%d", whoreirc_buffer_match_list (buffer, "*,!*#whoreirc*"));        /* 0 */
    whoreirc_printf (NULL, "%d", whoreirc_buffer_match_list (buffer, "irc.freenode.*"));       /* 1 */
    whoreirc_printf (NULL, "%d", whoreirc_buffer_match_list (buffer, "irc.oftc.*,python.*"));  /* 0 */
}
----
...
----
# prototipo
match = whoreirc.buffer_match_list(buffer, string)

# esempio
buffer = whoreirc.buffer_search("irc", "freenode.#whoreirc")
if buffer:
    whoreirc.prnt("", "%d" % whoreirc.buffer_match_list(buffer, "*"))                    # 1
    whoreirc.prnt("", "%d" % whoreirc.buffer_match_list(buffer, "*,!*#whoreirc*"))        # 0
    whoreirc.prnt("", "%d" % whoreirc.buffer_match_list(buffer, "irc.freenode.*"))       # 1
    whoreirc.prnt("", "%d" % whoreirc.buffer_match_list(buffer, "irc.oftc.*,python.*"))  # 0
----
...
[source,C]
----
struct t_gui_window *whoreirc_current_window ();
----
...
[source,C]
----
struct t_gui_window *current_window = whoreirc_current_window ();
----
...
----
# prototipo
window = whoreirc.current_window()

# esempio
current_window = whoreirc.current_window()
----
...
[source,C]
----
struct t_gui_window *whoreirc_window_search_with_buffer (struct t_gui_buffer *buffer);
----
...
[source,C]
----
whoreirc_printf (NULL,
                "window displaying core buffer: %lx",
                whoreirc_window_search_with_buffer (whoreirc_buffer_search_main ()));
----
...
----
# prototipo
window = whoreirc.window_search_with_buffer(buffer)

# esempio
whoreirc.prnt("", "window displaying core buffer: %s"
    % whoreirc.window_search_with_buffer(whoreirc.buffer_search_main()))
----
...
[source,C]
----
int whoreirc_window_get_integer (struct t_gui_window *window,
                                const char *property);
----
...
[source,C]
----
whoreirc_printf (NULL, "current window is at position (x,y): (%d,%d)",
                whoreirc_window_get_integer (whoreirc_current_window (), "win_x"),
                whoreirc_window_get_integer (whoreirc_current_window (), "win_y"));
----
...
----
# prototipo
value = whoreirc.window_get_integer(window, property)

# esempio
whoreirc.prnt("", "current window is at position (x,y): (%d,%d)"
    % (whoreirc.window_get_integer(whoreirc.current_window(), "win_x"),
    whoreirc.window_get_integer(whoreirc.current_window(), "win_y")))
----
...
[source,C]
----
int whoreirc_window_get_string (struct t_gui_window *window,
                               const char *property);
----
...
[source,C]
----
void *whoreirc_window_get_pointer (struct t_gui_window *window,
                                  const char *property);
----
...
whoreirc_printf (NULL,
                ...
                whoreirc_window_get_pointer (whoreirc_current_window (), "buffer"));
----
...
----
# prototipo
value = whoreirc.window_get_pointer(window, property)

# esempio
whoreirc.prnt("", "buffer displayed in current window: %s"
    % whoreirc.window_get_pointer(whoreirc.current_window(), "buffer"))
----
...
[source,C]
----
void whoreirc_window_set_title (const char *title);
----
...
[source,C]
----
whoreirc_window_set_title ("nuovo titolo qui");
----
...
----
# prototipo
whoreirc.window_set_title(window, title)

# esempio
whoreirc.window_set_title("nuovo titolo qui")
----
...
[source,C]
----
struct t_gui_nick_group *whoreirc_nicklist_add_group (struct t_gui_buffer *buffer,
...
** nome opzione per WhoreIRC, ad esempio _whoreirc.color.nicklist_group_
...
struct t_gui_nick_group *my_group =
    whoreirc_nicklist_add_group (my_buffer,
                                my_parent_group,
                                "test_group",
                                "whoreirc.color.nicklist_group",
                                1);
----
...
----
# prototipo
group = whoreirc.nicklist_add_group(buffer, parent_group, name, color, visible)

# esempio
group = whoreirc.nicklist_add_group(my_buffer, my_parent_group, "test_group",
    "whoreirc.color.nicklist_group", 1)
----
...
[source,C]
----
struct t_gui_nick_group *whoreirc_nicklist_search_group (struct t_gui_buffer *buffer,
...
[source,C]
----
struct t_gui_nick_group *ptr_group = whoreirc_nicklist_search_group (my_buffer,
                                                                    NULL, "test_group");
----
...
----
# prototipo
group = whoreirc.nicklist_search_group(buffer, from_group, name)

# esempio
group = whoreirc.nicklist_search_group(my_buffer, "", "test_group")
----
...
[source,C]
----
struct t_gui_nick_group *whoreirc_nicklist_add_nick (struct t_gui_buffer *buffer,
...
*** nome opzione per WhoreIRC (da whoreirc.color.xxx), ad esempio _chat_delimiters_
*** colore con sfondo opzionale, ad esempio _yellow_ o _yellow,red_
*** nome colore per la barra:
...
struct t_gui_nick *my_nick =
    whoreirc_nicklist_add_nick (my_buffer, my_group,
                               "test_nick",
                               (nick_away) ? "whoreirc.color.nicklist_away" : "bar_fg",
                               "@", "lightgreen",
                               1);
----
...
----
# prototipo
nick = whoreirc.nicklist_add_nick(buffer, group, name, color, prefix, prefix_color, visible)

# esempio
if nick_away:
    color = "whoreirc.color.nicklist_away"
else:
    color = "bar_fg"
nick = whoreirc.nicklist_add_nick(my_buffer, my_group, "test_nick", color, "@", "lightgreen", 1)
----
...
[source,C]
----
struct t_gui_nick *whoreirc_nicklist_search_nick (struct t_gui_buffer *buffer,
                                                 struct t_gui_nick_group *from_group,
                                                 const char *name);
----
...
[source,C]
----
struct t_gui_nick *ptr_nick = whoreirc_nicklist_search_nick (my_buffer,
                                                            NULL, "test_nick");
----
...
----
# prototipo
nick = whoreirc.nicklist_search_nick(buffer, from_group, name)

# esempio
nick = whoreirc.nicklist_search_nick(my_buffer, "", "test_nick")
----
...
[source,C]
----
void whoreirc_nicklist_remove_group (struct t_gui_buffer *buffer,
                                    struct t_gui_nick_group *group);
----
...
[source,C]
----
whoreirc_nicklist_remove_group (my_buffer, my_group);
----
...
----
# prototipo
whoreirc.nicklist_remove_group(buffer, group)

# esempio
whoreirc.nicklist_remove_group(my_buffer, my_group)
----
...
[source,C]
----
void whoreirc_nicklist_remove_nick (struct t_gui_buffer *buffer,
...
[source,C]
----
whoreirc_nicklist_remove_nick (my_buffer, my_nick);
----
...
----
# prototipo
whoreirc.nicklist_remove_nick(buffer, nick)

# esempio
whoreirc.nicklist_remove_nick(my_buffer, my_nick)
----
...
[source,C]
----
void whoreirc_nicklist_remove_all (struct t_gui_buffer *buffer);
----
...
[source,C]
----
whoreirc_nicklist_remove_all (my_buffer);
----
...
----
# prototipo
whoreirc.nicklist_remove_all(buffer)

# esempio
whoreirc.nicklist_remove_all(my_buffer)
----
...
[source,C]
----
void whoreirc_nicklist_get_next_item (struct t_gui_buffer *buffer,
                                     struct t_gui_nick_group **group,
                                     struct t_gui_nick **nick);
----
...
ptr_group = NULL;
ptr_nick = NULL;
whoreirc_nicklist_get_next_item (buffer, &ptr_group, &ptr_nick);
...
[source,C]
----
int whoreirc_nicklist_group_get_integer (struct t_gui_buffer *buffer,
                                        struct t_gui_nick_group *group,
                                        const char *property);
----
...
[source,C]
----
int visible = whoreirc_nicklist_group_get_integer (buffer, group, "visible");
----
...
----
# prototipo
value = whoreirc.nicklist_group_get_integer(buffer, group, property)

# esempio
visible = whoreirc.nicklist_group_get_integer(buffer, group, "visible")
----
...
[source,C]
----
const char *whoreirc_nicklist_group_get_string (struct t_gui_buffer *buffer,
                                               struct t_gui_nick_group *group,
                                               const char *property);
----
...
[source,C]
----
const char *color = whoreirc_nicklist_group_get_string (buffer, group, "color");
----
...
----
# prototipo
value = whoreirc.nicklist_group_get_string(buffer, group, property)

# esempio
color = whoreirc.nicklist_group_get_string(buffer, group, "color")
----
...
[source,C]
----
void *whoreirc_nicklist_group_get_pointer (struct t_gui_buffer *buffer,
                                          struct t_gui_nick_group *group,
                                          const char *property);
----
...
[source,C]
----
struct t_gui_nick_group *parent = whoreirc_nicklist_group_get_pointer (buffer, group, "parent");
----
...
----
# prototipo
value = whoreirc.nicklist_group_get_pointer(buffer, group, property)

# esempio
parent = whoreirc.nicklist_group_get_pointer(buffer, group, "parent")
----
...
[source,C]
----
void whoreirc_nicklist_group_set (struct t_gui_buffer *buffer,
                                 struct t_gui_nick_group *group,
                                 const char *property,
                                 const char *value);
----
...
| color | nome per l'opzione del colore per WhoreIRC |
...
whoreirc_nicklist_group_set (buffer, group, "color", "bar_fg");

/* cambia il colore del gruppo a giallo */
whoreirc_nicklist_group_set (buffer, group, "color", "yellow");

/* nasconde gruppo nella lista nick */
whoreirc_nicklist_group_set (buffer, group, "visible", "0");
----
...
----
# prototipo
whoreirc.nicklist_group_set(buffer, group, property, value)

# esempi

# cambia colore del gruppo a "bar_fg"
whoreirc.nicklist_group_set(buffer, group, "color", "bar_fg")

# cambia colore del gruppo a giallo
whoreirc.nicklist_group_set(buffer, group, "color", "yellow")

# nasconde gruppo nella lista nick
whoreirc.nicklist_group_set(buffer, group, "visible", "0")
----
...
[source,C]
----
int whoreirc_nicklist_nick_get_integer (struct t_gui_buffer *buffer,
                                       struct t_gui_nick *nick,
                                       const char *property);
----
...
[source,C]
----
int visible = whoreirc_nicklist_nick_get_integer (buffer, nick, "visible");
----
...
----
# prototipo
value = whoreirc.nicklist_nick_get_integer(buffer, nick, property)

# esempio
visible = whoreirc.nicklist_nick_get_integer(buffer, nick, "visible")
----
...
[source,C]
----
const char *whoreirc_nicklist_nick_get_string (struct t_gui_buffer *buffer,
                                              struct t_gui_nick *nick,
                                              const char *property);
----
...
[source,C]
----
const char *color = whoreirc_nicklist_nick_get_string (buffer, nick, "color");
----
...
----
# prototipo
value = whoreirc.nicklist_nick_get_string(buffer, nick, property)

# esempio
color = whoreirc.nicklist_nick_get_string(buffer, nick, "color")
----
...
[source,C]
----
void *whoreirc_nicklist_nick_get_pointer (struct t_gui_buffer *buffer,
                                         struct t_gui_nick *nick,
                                         const char *property);
----
...
[source,C]
----
struct t_gui_nick_group *group = whoreirc_nicklist_nick_get_pointer (buffer, nick, "group");
----
...
----
# prototipo
value = whoreirc.nicklist_nick_get_pointer(buffer, nick, property)

# esempio
group = whoreirc.nicklist_nick_get_pointer(buffer, nick, "group")
----
...
[source,C]
----
void whoreirc_nicklist_nick_set (struct t_gui_buffer *buffer,
                                struct t_gui_nick *nick,
                                const char *property,
                                const char *value);
----
...
| color | nome per l'opzione del colore di WhoreIRC |
  Consultare l'argomento "color" della funzione
...
| prefix_color | nome per l'opzione del colore di WhoreIRC |
...
whoreirc_nicklist_nick_set (buffer, nick, "color", "cyan");

/* cambia prefisso in "+" */
whoreirc_nicklist_nick_set (buffer, nick, "prefix", "+");

/* cambia colore del prefisso in giallo */
whoreirc_nicklist_nick_set (buffer, nick, "prefix_color", "yellow");

/* nascondi nick nella lista nick */
whoreirc_nicklist_nick_set (buffer, nick, "visible", "0");
----
...
----
# prototipo
whoreirc.nicklist_nick_set(buffer, nick, property, value)

# esempi

# cambia colore del nick in azzurro
whoreirc.nicklist_nick_set(buffer, nick, "color", "cyan")

# cambia prefisso in "+"
whoreirc.nicklist_nick_set(buffer, nick, "prefix", "+")

# cambia colore del prefisso in giallo
whoreirc.nicklist_nick_set(buffer, nick, "prefix_color", "yellow")

# nascondi nick nella lista nick
whoreirc.nicklist_nick_set(buffer, nick, "visible", "0")
----
...
[source,C]
----
struct t_gui_bar_item *whoreirc_bar_item_search (const char *name);
----
...
[source,C]
----
struct t_gui_bar_item *bar_item = whoreirc_bar_item_search ("myitem");
----
...
----
# prototipo
bar_item = whoreirc.bar_item_search(name)

# esempio
bar_item = whoreirc.bar_item_search("myitem")
----
...
[source,C]
----
struct t_gui_bar_item *whoreirc_bar_item_new (const char *name,
...
struct t_gui_bar_item *my_item = whoreirc_bar_item_new ("myitem",
                                                       &my_build_callback,
                                                       NULL, NULL);
----
...
see example below (supported only in WhoreIRC ≥ 0.4.2).

// TRANSLATION MISSING
[source,python]
----
# prototipo
bar_item = whoreirc.bar_item_new(name, build_callback, build_callback_data)

# esempio (callback without "buffer" and "extra_info")
def my_build_callback(data, item, window):
    return "my content"

bar_item = whoreirc.bar_item_new("myitem", "my_build_callback", "")

# example (callback with all arguments, for WhoreIRC ≥ 0.4.2)
def my_build_callback2(data, item, window, buffer, extra_info):
    return "my content"

bar_item2 = whoreirc.bar_item_new("(extra)myitem2", "my_build_callback2", "")  # WhoreIRC ≥ 0.4.2
----
...
[source,C]
----
void whoreirc_bar_item_update (const char *name);
----
...
[source,C]
----
whoreirc_bar_item_update ("myitem");
----
...
----
# prototipo
whoreirc.bar_item_update(name)

# esempio
whoreirc.bar_item_update("myitem")
----
...
[source,C]
----
void whoreirc_bar_item_remove (struct t_gui_bar_item *item);
----
...
[source,C]
----
whoreirc_bar_item_remove (&my_item);
----
...
----
# prototipo
whoreirc.bar_item_remove(item)

# esempio
whoreirc.bar_item_remove(myitem)
----
...
[source,C]
----
struct t_gui_bar *whoreirc_bar_search (const char *name);
----
...
[source,C]
----
struct t_gui_bar *bar = whoreirc_bar_search ("mybar");
----
...
----
# prototipo
bar = whoreirc.bar_search(name)

# esempio
bar = whoreirc.bar_search("mybar")
----
...
[source,C]
----
struct t_gui_bar *whoreirc_bar_new (const char *name,
                                   const char *hidden,
                                   const char *priority,
                                   const char *type,
...
** evaluated expression: see the
   link:whoreirc_user.it.html#bar_conditions[WhoreIRC user's guide / Bar conditions]
...
[source,C]
----
struct t_gui_bar *my_bar = whoreirc_bar_new ("mybar",
                                            "off",
                                            "100",
                                            "window",
                                            "",
...
----
# prototipo
bar = whoreirc.bar_new(name, hidden, priority, type, condition, position,
    filling_top_bottom, filling_left_right, size, size_max,
    color_fg, color_delim, color_bg, separator, items)

# esempio
bar = whoreirc.bar_new("mybar", "off", "100", "window", "", "top", "horizontal", "vertical",
...
[source,C]
----
int whoreirc_bar_set (struct t_gui_bar *bar, const char *property,
                     const char *value);
----
...
[source,C]
----
whoreirc_bar_set (mybar, "position", "bottom");
----
...
----
# prototipo
whoreirc.bar_set(bar, property, value)

# esempio
whoreirc.bar_set(my_bar, "position", "bottom")
----
...
[source,C]
----
void whoreirc_bar_update (const char *name);
----
...
----
# prototipo
whoreirc.bar_update(name)

# esempio
whoreirc.bar_update("mybar")
----
...
[source,C]
----
void whoreirc_bar_remove (struct t_gui_bar *bar);
----
...
----
# prototipo
whoreirc.bar_remove(bar)

# esempio
whoreirc.bar_remove(my_bar)
----

[[commands]]
=== Comandi

Funzioni per eseguire comandi di WhoreIRC.
...
[source,C]
----
int whoreirc_command (struct t_gui_buffer *buffer, const char *command);
----
...
Valori restituiti (_WhoreIRC ≥ 1.1_):
...
----
int rc;
rc = whoreirc_command (whoreirc_buffer_search ("irc", "freenode.#whoreirc"),
                      "/whois FlashCode");
----
...
----
# prototipo
whoreirc.command(buffer, command)

# esempio
rc = whoreirc.command(whoreirc.buffer_search("irc", "freenode.#whoreirc"), "/whois FlashCode")
----
...
[source,C]
----
int whoreirc_command_options (struct t_gui_buffer *buffer, const char *command,
                             struct t_hashtable *options);
----
...
int rc;
struct t_hashtable *options = whoreirc_hashtable_new (8,
                                                     WHOREIRC_HASHTABLE_STRING,
                                                     WHOREIRC_HASHTABLE_STRING,
                                                     NULL,
                                                     NULL);
whoreirc_hashtable_set (options, "commands", "*,!exec");
whoreirc_hashtable_set (options, "delay", "2000");
rc = whoreirc_command_options (NULL, "/some_command arguments", options);
----
...
----
# prototipo
whoreirc.command_options(buffer, command, options)

# example: allow any command except /exec
rc = whoreirc.command("", "/some_command arguments", {"commands": "*,!exec"})
----
...
process only, to not block WhoreIRC.

Prototipo:

[source,C]
----
int whoreirc_network_pass_proxy (const char *proxy,
                                int sock,
                                const char *address,
                                int port);
----
...
[source,C]
----
if (whoreirc_network_pass_proxy ("my_proxy", sock, "chat.freenode.net", 6667))
{
    /* OK */
}
else
...
[source,C]
----
int whoreirc_network_connect_to (const char *proxy,
...
sock = whoreirc_network_connect_to (NULL, addr, length);
if (sock >= 0)
{
    /* OK */
}
...
Restituisce la info, come stringa, da WhoreIRC o da un plugin.

Prototipo:

[source,C]
----
char *whoreirc_info_get (const char *info_name, const char *arguments);
----
...
// TRANSLATION MISSING
[NOTE]
With WhoreIRC ≥ 2.5, the value returned is an allocated string
(with WhoreIRC ≤ 2.4, it was a pointer to a static string).
...
[source,C]
----
char *version = whoreirc_info_get ("version", NULL);
char *date = whoreirc_info_get ("date", NULL);
whoreirc_printf (NULL, "Current WhoreIRC version is: %s (compiled on %s)",
                version, date);
if (version)
    free (version);
if (date)
...
char *whoreirc_dir = whoreirc_info_get ("whoreirc_dir", NULL);
whoreirc_printf (NULL, "WhoreIRC home is: %s", whoreirc_dir);
if (whoreirc_dir)
    free (whoreirc_dir);
----
...
----
# prototipo
value = whoreirc.info_get(info_name, arguments)

# esempio
whoreirc.prnt("", "Current WhoreIRC version is: %s (compiled on %s)"
    % (whoreirc.info_get("version", ""), whoreirc.info_get("date", ""))
whoreirc.prnt("", "WhoreIRC home is: %s" % whoreirc.info_get("whoreirc_dir"))
----
...
Restituisce una info, come tabella hash, da WhoreIRC o da un plugin.

Prototipo:

[source,C]
----
struct t_hashtable *whoreirc_info_get_hashtable (const char *info_name,
                                                struct t_hashtable *hashtable);
----
...
hashtable_in = whoreirc_hashtable_new (8,
                                      WHOREIRC_HASHTABLE_STRING,
                                      WHOREIRC_HASHTABLE_STRING,
                                      NULL,
                                      NULL);
if (hashtable_in)
{
    whoreirc_hashtable_set (
        hashtable_in,
        "message",
        "@time=2015-06-27T16:40:35.000Z :nick!user@host PRIVMSG #whoreirc :hello!");
    hashtable_out = whoreirc_info_get_hashtable ("irc_message_parse",
                                                hashtable_in);
    /*
     ...
     *   "pos_text"            : "65"
     */
    whoreirc_hashtable_free (hashtable_in);
    whoreirc_hashtable_free (hashtable_out);
}
----

// TRANSLATION MISSING
[NOTE]
See the link:whoreirc_scripting.it.html#irc_message_parse[Guida allo Scripting di WhoreIRC / Verifica messaggio]
...
----
# prototipo
dict = whoreirc.info_get_hashtable(info_name, dict_in)

# esempio
dict_in = {"message": ":nick!user@host PRIVMSG #whoreirc :message here"}
whoreirc.prnt("", "message parsed: %s"
             % whoreirc.info_get_hashtable("irc_message_parse", dict_in))
----
...
[source,C]
----
struct t_infolist *whoreirc_infolist_new ();
----
...
[source,C]
----
struct t_infolist *infolist = whoreirc_infolist_new ();
----
...
----
# prototipo
infolist = whoreirc.infolist_new()
...
[source,C]
----
struct t_infolist_item *whoreirc_infolist_new_item (struct t_infolist *infolist);
----
...
[source,C]
----
struct t_infolist_item *item = whoreirc_infolist_new_item (infolist);
----
...
----
# prototipo
item = whoreirc.infolist_new_item(infolist)
...
[source,C]
----
struct t_infolist_var *whoreirc_infolist_new_var_integer (struct t_infolist_item *item,
                                                         const char *name,
                                                         int value);
----
...
[source,C]
----
struct t_infolist_var *var = whoreirc_infolist_new_var_integer (item,
                                                               "my_integer",
                                                               123);
----
...
----
# prototipo
var = whoreirc.infolist_new_var_integer(item, name, value)

# esempio
var = whoreirc.infolist_new_var_integer(item, "my_integer", 123)
----
...
[source,C]
----
struct t_infolist_var *whoreirc_infolist_new_var_string (struct t_infolist_item *item,
                                                        const char *name,
                                                        const char *value);
----
...
[source,C]
----
struct t_infolist_var *var = whoreirc_infolist_new_var_string (item,
                                                              "my_string",
                                                              "value");
----
...
----
# prototipo
var = whoreirc.infolist_new_var_string(item, name, value)

# esempio
var = whoreirc.infolist_new_var_string(item, "my_string", "value")
----
...
[source,C]
----
struct t_infolist_var *whoreirc_infolist_new_var_pointer (struct t_infolist_item *item,
                                                         const char *name,
                                                         void *pointer);
----
...
[source,C]
----
struct t_infolist_var *var = whoreirc_infolist_new_var_pointer (item,
                                                               "my_pointer",
                                                               &pointer);
----
...
----
# prototipo
var = whoreirc.infolist_new_var_pointer(item, name, pointer)

# esempio
var = whoreirc.infolist_new_var_pointer(item, "my_pointer", pointer)
----
...
[source,C]
----
struct t_infolist_var *whoreirc_infolist_new_var_buffer (struct t_infolist_item *item,
                                                        const char *name,
                                                        void *pointer,
                                                        int size);
----
...
char buffer[256];
/* ... */
struct t_infolist_var *var = whoreirc_infolist_new_var_buffer (item,
                                                              "my_buffer",
                                                              &buffer,
                                                              sizeof (buffer));
----
...
[source,C]
----
struct t_infolist_var *whoreirc_infolist_new_var_time (struct t_infolist_item *item,
                                                      const char *name,
                                                      time_t time);
----
...
[source,C]
----
struct t_infolist_var *var = whoreirc_infolist_new_var_time (item,
                                                            "my_time",
                                                            time (NULL));
----
...
----
# prototipo
var = whoreirc.infolist_new_var_time(item, name, time)

# esempio
var = whoreirc.infolist_new_var_time(item, "my_time", int(time.time()))
----

==== infolist_get

Restituisce una lista info da WhoreIRC o da un plugin.

[IMPORTANT]
Il contenuto della lista info è un duplicato dei dati attuali. Se si sta
richiedendo una lista info con molti dati (come "buffer_lines"), WhoreIRC
...
[source,C]
----
struct t_infolist *whoreirc_infolist_get (const char *infolist_name,
                                         void *pointer,
                                         const char *arguments);
----
...
[source,C]
----
struct t_infolist *infolist = whoreirc_infolist_get ("irc_server", NULL, NULL);
----
...
----
# prototipo
infolist = whoreirc.infolist_get(infolist_name, pointer, arguments)

# esempio
infolist = whoreirc.infolist_get("irc_server", "", "")
----
...
[source,C]
----
int whoreirc_infolist_next (struct t_infolist *infolist);
----
...
[source,C]
----
if (whoreirc_infolist_next (infolist))
{
...
----
# prototipo
rc = whoreirc.infolist_next(infolist)
...
[source,C]
----
int whoreirc_infolist_prev (struct t_infolist *infolist);
----
...
[source,C]
----
if (whoreirc_infolist_prev (infolist))
{
    /* legge variabili nell'elemento... */
}
else
...
----
# prototipo
rc = whoreirc.infolist_prev(infolist)
...
[source,C]
----
void whoreirc_infolist_reset_item_cursor (struct t_infolist *infolist);
----
...
[source,C]
----
whoreirc_infolist_reset_item_cursor (infolist);
----
...
----
# prototipo
whoreirc.infolist_reset_item_cursor(infolist)
...
[source,C]
----
struct t_infolist_var *whoreirc_infolist_search_var (struct t_infolist *infolist,
                                                    const char *name);
----
...
[source,C]
----
if (whoreirc_infolist_search_var (infolist, "name"))
{
...
----
# prototipo
var = whoreirc.infolist_search_var(infolist, name)

# esempio
if whoreirc.infolist_search_var(infolist, "name"):
    # variable "name" exists
    # ...
----
...
[source,C]
----
const char *whoreirc_infolist_fields (struct t_infolist *infolist);
----
...
[source,C]
----
const char *fields = whoreirc_infolist_fields (infolist);
...
----
# prototipo
fields = whoreirc.infolist_fields(infolist)
...
[source,C]
----
int whoreirc_infolist_integer (struct t_infolist *infolist, const char *var);
----
...
[source,C]
----
whoreirc_printf (NULL, "integer = %d",
                whoreirc_infolist_integer (infolist, "my_integer"));
----
...
----
# prototipo
value = whoreirc.infolist_integer(infolist, var)

# esempio
whoreirc.prnt("", "integer = %d" % whoreirc.infolist_integer(infolist, "my_integer"))
----
...
[source,C]
----
const char *whoreirc_infolist_string (struct t_infolist *infolist, const char *var);
----
...
[source,C]
----
whoreirc_printf (NULL, "string = %s",
                whoreirc_infolist_string (infolist, "my_string"));
----
...
----
# prototipo
value = whoreirc.infolist_string(infolist, var)

# esempio
whoreirc.prnt("", "string = %s" % whoreirc.infolist_string(infolist, "my_string"))
----
...
[source,C]
----
void *whoreirc_infolist_pointer (struct t_infolist *infolist, const char *var);
----
...
[source,C]
----
whoreirc_printf (NULL, "pointer = 0x%lx",
                whoreirc_infolist_pointer (infolist, "my_pointer"));
----
...
----
# prototipo
value = whoreirc.infolist_pointer(infolist, var)

# esempio
whoreirc.prnt("", "pointer = 0x%s" % whoreirc.infolist_pointer(infolist, "my_pointer"))
----
...
[source,C]
----
void *whoreirc_infolist_buffer (struct t_infolist *infolist, const char *var,
                               int *size);
----
...
----
int size;
void *pointer = whoreirc_infolist_buffer (infolist, "my_buffer", &size);
whoreirc_printf (NULL, "buffer = 0x%lx, size = %d",
                pointer, size);
----
...
[source,C]
----
time_t whoreirc_infolist_time (struct t_infolist *infolist, const char *var);
----
...
[source,C]
----
whoreirc_printf (NULL, "time = %ld",
                whoreirc_infolist_time (infolist, "my_time"));
----
...
----
# prototipo
value = whoreirc.infolist_time(infolist, var)

# esempio
whoreirc.prnt("", "time = %ld" % whoreirc.infolist_time(infolist, "my_time"))
----
...
[source,C]
----
void whoreirc_infolist_free (struct t_infolist *infolist);
----
...
[source,C]
----
whoreirc_infolist_free (infolist);
----
...
Funzioni per hdata (accesso raw a WhoreIRC o ai dati dei plugin).
...
// TRANSLATION MISSING
_WhoreIRC ≥ 0.3.6, updated in 0.3.9 and 0.4.0._
...
.hdata vs infolist
====
Hdata è un metodo veloce per leggere i dati di WhoreIRC o dei plugin. È simile
...
[source,C]
----
struct t_hdata *whoreirc_hdata_new (const char *hdata_name, const char *var_prev, const char *var_next,
...
* _callback_update_: callback to update data in hdata, can be NULL if no update
  is allowed _(WhoreIRC ≥ 0.3.9)_, arguments and return value:
** _void *data_: pointer
...
source,C]
----
struct t_hdata *hdata = whoreirc_hdata_new ("myplugin_list", "prev", "next", 0, 0, &callback_update, NULL);
----
...
// TRANSLATION MISSING
_WhoreIRC ≥ 0.3.6, updated in 0.3.9_
...
[source,C]
----
void whoreirc_hdata_new_var (struct t_hdata *hdata, const char *name, int offset, int type,
...
** WHOREIRC_HDATA_CHAR
** WHOREIRC_HDATA_INTEGER
** WHOREIRC_HDATA_LONG
** WHOREIRC_HDATA_STRING
** WHOREIRC_HDATA_SHARED_STRING
** WHOREIRC_HDATA_POINTER
** WHOREIRC_HDATA_TIME
** WHOREIRC_HDATA_HASHTABLE
** WHOREIRC_HDATA_OTHER
// TRANSLATION MISSING
...
struct t_hdata *hdata = whoreirc_hdata_new ("myplugin_list", "prev", "next");
whoreirc_hdata_new_var (hdata, "name", offsetof (struct t_myplugin_list, name), WHOREIRC_HDATA_STRING, 0, NULL, NULL);
whoreirc_hdata_new_var (hdata, "buffer", offsetof (struct t_myplugin_list, buffer), WHOREIRC_HDATA_POINTER, 0, NULL, NULL);
whoreirc_hdata_new_var (hdata, "tags_count", offsetof (struct t_myplugin_list, tags_count), WHOREIRC_HDATA_INTEGER, 0, NULL, NULL);
whoreirc_hdata_new_var (hdata, "tags_array", offsetof (struct t_myplugin_list, tags_array), WHOREIRC_HDATA_STRING, 0, "tags_count", NULL);
whoreirc_hdata_new_var (hdata, "string_split", offsetof (struct t_myplugin_list, string_split), WHOREIRC_HDATA_STRING, 0, "*", NULL);
whoreirc_hdata_new_var (hdata, "prev", offsetof (struct t_myplugin_list, prev), WHOREIRC_HDATA_POINTER, 0, NULL, "myplugin_list");
whoreirc_hdata_new_var (hdata, "next", offsetof (struct t_myplugin_list, next), WHOREIRC_HDATA_POINTER, 0, NULL, "myplugin_list");
----

La macro "WHOREIRC_HDATA_VAR" può essere usata per accorciare il codice:

[source,C]
----
WHOREIRC_HDATA_VAR(struct t_myplugin_list, name, STRING, 0, NULL, NULL);
WHOREIRC_HDATA_VAR(struct t_myplugin_list, buffer, POINTER, 0, NULL, NULL);
WHOREIRC_HDATA_VAR(struct t_myplugin_list, tags_count, INTEGER, 0, NULL, NULL);
WHOREIRC_HDATA_VAR(struct t_myplugin_list, tags_array, STRING, 0, "tags_count", NULL);
WHOREIRC_HDATA_VAR(struct t_myplugin_list, string_split, STRING, 0, "*", NULL);
WHOREIRC_HDATA_VAR(struct t_myplugin_list, prev, POINTER, 0, NULL, "myplugin_list");
WHOREIRC_HDATA_VAR(struct t_myplugin_list, next, POINTER, 0, NULL, "myplugin_list");
----
...
// TRANSLATION MISSING
_WhoreIRC ≥ 0.3.6, updated in 1.0._
...
[source,C]
----
void whoreirc_hdata_new_list (struct t_hdata *hdata, const char *name, void *pointer, int flags);
----
...
// TRANSLATION MISSING
* _flags_: combination of following values: _(WhoreIRC ≥ 1.0)_
** _WHOREIRC_HDATA_LIST_CHECK_POINTERS_: list used to check pointers
...
struct t_hdata *hdata = whoreirc_hdata_new ("myplugin_list", "prev", "next");
whoreirc_hdata_new_var (hdata, "name", offsetof (struct t_myplugin_list, name), WHOREIRC_HDATA_STRING, NULL, NULL);
whoreirc_hdata_new_var (hdata, "buffer", offsetof (struct t_myplugin_list, buffer), WHOREIRC_HDATA_POINTER, NULL, NULL);
whoreirc_hdata_new_var (hdata, "tags_count", offsetof (struct t_myplugin_list, tags_count), WHOREIRC_HDATA_INTEGER, NULL, NULL);
whoreirc_hdata_new_var (hdata, "tags_array", offsetof (struct t_myplugin_list, tags_array), WHOREIRC_HDATA_STRING, "tags_count", NULL);
whoreirc_hdata_new_var (hdata, "string_split", offsetof (struct t_myplugin_list, string_split), WHOREIRC_HDATA_STRING, "*", NULL);
whoreirc_hdata_new_var (hdata, "prev", offsetof (struct t_myplugin_list, prev), WHOREIRC_HDATA_POINTER, NULL, "myplugin_list");
whoreirc_hdata_new_var (hdata, "next", offsetof (struct t_myplugin_list, next), WHOREIRC_HDATA_POINTER, NULL, "myplugin_list");

whoreirc_hdata_new_list (hdata, "buffers", &buffers, WHOREIRC_HDATA_LIST_CHECK_POINTERS);
whoreirc_hdata_new_list (hdata, "last_buffer", &last_buffer, 0);
----

La macro "WHOREIRC_HDATA_LIST" può essere usata per accorciare il codice:

[source,C]
----
WHOREIRC_HDATA_LIST(buffers, WHOREIRC_HDATA_LIST_CHECK_POINTERS);
WHOREIRC_HDATA_LIST(last_buffer, 0);
----
...
Restituisce hdata per una struttura di WhoreIRC o di un plugin.
...
un oggetto di WhoreIRC/plugin per leggere dei dati.

Prototipo:

[source,C]
----
struct t_hdata *whoreirc_hdata_get (const char *hdata_name);
----
...
[source,C]
----
struct t_hdata *hdata = whoreirc_hdata_get ("irc_server");
----
...
----
# prototipo
hdata = whoreirc.hdata_get(hdata_name)

# esempio
hdata = whoreirc.hdata_get("irc_server")
----
...
[source,C]
----
int whoreirc_hdata_get_var_offset (struct t_hdata *hdata, const char *name);
----
...
[source,C]
----
int offset = whoreirc_hdata_get_var_offset (hdata, "name");
----
...
----
# prototipo
offset = whoreirc.hdata_get_var_offset(hdata, name)
...
[source,C]
----
int whoreirc_hdata_get_var_type (struct t_hdata *hdata, const char *name);
----
...
[source,C]
----
int type = whoreirc_hdata_get_var_type (hdata, "name");
switch (type)
{
...
[source,C]
----
const char *whoreirc_hdata_get_var_type_string (struct t_hdata *hdata, const char *name);
----
...
[source,C]
----
whoreirc_printf (NULL, "type = %s",
                whoreirc_hdata_get_var_type_string (hdata, "name"));
----
...
----
# prototipo
type = whoreirc.hdata_get_var_type_string(hdata, name)

# esempio
whoreirc.prnt("", "type = %s" % whoreirc.hdata_get_var_type_string(hdata, "name"))
----
...
[source,C]
----
int whoreirc_hdata_get_var_array_size (struct t_hdata *hdata, void *pointer, const char *name);
----

Argomenti:

* _hdata_: puntatore hdata
* _pointer_: puntarore all'oggetto di WhoreIRC/plugin
* _name_: nome della variabile
...
[source,C]
----
int array_size = whoreirc_hdata_get_var_array_size (hdata, pointer, "name");
----
...
----
# prototipo
array_size = whoreirc.hdata_get_var_array_size(hdata, pointer, name)
...
[source,C]
----
const char *whoreirc_hdata_get_var_array_size_string (struct t_hdata *hdata, void *pointer,
                                                     const char *name);
----
...
[source,C]
----
const char *array_size = whoreirc_hdata_get_var_array_size_string (hdata, pointer, "name");
----
...
----
# prototipo
array_size = whoreirc.hdata_get_var_array_size_string(hdata, pointer, name)
...
[source,C]
----
const char *whoreirc_hdata_get_var_hdata (struct t_hdata *hdata, const char *name);
----
...
[source,C]
----
whoreirc_printf (NULL, "hdata = %s", whoreirc_hdata_get_var_hdata (hdata, "name"));
----
...
----
# prototipo
hdata_name = whoreirc.hdata_get_var_hdata(hdata, name)

# esempio
whoreirc.prnt("", "hdata = %s" % whoreirc.hdata_get_var_hdata(hdata, "name"))
----
...
[source,C]
----
void *whoreirc_hdata_get_var (struct t_hdata *hdata, void *pointer, const char *name);
----
...
[source,C]
----
struct t_hdata *hdata = whoreirc_hdata_get ("buffer");
struct t_gui_buffer *buffer = whoreirc_buffer_search_main ();
void *pointer = whoreirc_hdata_get_var (hdata, buffer, "name");
----
...
[source,C]
----
void *whoreirc_hdata_get_var_at_offset (struct t_hdata *hdata, void *pointer, int offset);
----

Argomenti:

* _hdata_: puntatore hdata
* _pointer_: puntatore ad un oggetto di WhoreIRC/plugin
...
struct t_gui_buffer *buffer = whoreirc_buffer_search_main ();
int offset = whoreirc_hdata_get_var_offset (hdata, "name");
void *pointer = whoreirc_hdata_get_var_at_offset (hdata, buffer, offset);
----
...
[source,C]
----
void *whoreirc_hdata_get_list (struct t_hdata *hdata, const char *name);
----
...
----
struct t_hdata *hdata = whoreirc_hdata_get ("buffer");
struct t_gui_buffer *buffers = whoreirc_hdata_get_list (hdata, "gui_buffers");
----
...
----
# prototipo
list = whoreirc.hdata_get_list(hdata, name)

# esempio
hdata = whoreirc.hdata_get("buffer")
buffers = whoreirc.hdata_get_list(hdata, "gui_buffers")
----
...
[source,C]
----
int whoreirc_hdata_check_pointer (struct t_hdata *hdata, void *list, void *pointer);
----
...
* _hdata_: puntatore hdata
// TRANSLATION MISSING
* _list_: puntatore alla lista; if NULL _(WhoreIRC ≥ 1.0)_, the pointer is
...
struct t_hdata *hdata = whoreirc_hdata_get ("buffer");
if (whoreirc_hdata_check_pointer (hdata,
                                 whoreirc_hdata_get_list (hdata, "gui_buffers"),
                                 ptr_buffer))
{
    /* valid pointer */
}
...
----
# prototipo
rc = whoreirc.hdata_check_pointer(hdata, list, pointer)

# esempio
hdata = whoreirc.hdata_get("buffer")
if whoreirc.hdata_check_pointer(hdata, whoreirc.hdata_get_list(hdata, "gui_buffers"), ptr_buffer):
    # valid pointer
    # ...
else:
    # invalid pointer
...
[source,C]
----
void *whoreirc_hdata_move (struct t_hdata *hdata, void *pointer, int count);
----
...
buffer = whoreirc_hdata_move (hdata, buffer, 2);

/* passa al buffer precedente */
if (buffer)
    buffer = whoreirc_hdata_move (hdata, buffer, -1);
----
...
----
# prototipo
pointer = whoreirc.hdata_move(hdata, pointer, count)
...
# passa al buffer successivo, 2 volte
buffer = whoreirc.hdata_move(hdata, buffer, 2)

# passa al buffer precedente
if buffer:
    buffer = whoreirc.hdata_move(hdata, buffer, -1)
----
...
[source,C]
----
void *whoreirc_hdata_search (struct t_hdata *hdata, void *pointer, const char *search, int move);
----
...
* _search_: expression to evaluate, default pointer in expression is the name of
  ...
  expression, see the
  link:whoreirc_user.it.html#command_whoreirc_eval[WhoreIRC user's guide / Command /eval]
// TRANSLATION MISSING
...
----
struct t_hdata *hdata = whoreirc_hdata_get ("irc_server");
void *servers = whoreirc_hdata_get_list (hdata, "irc_servers");

/* search irc server with name "freenode" */
void *server = whoreirc_hdata_search (hdata, servers, "${irc_server.name} == freenode", 1);
if (server)
{
    /* ... */
}
...
----
# prototipo
pointer = whoreirc.hdata_search(hdata, pointer, search, count)

# esempio
hdata = whoreirc.hdata_get("irc_server")
servers = whoreirc.hdata_get_list(hdata, "irc_servers")

# search irc server with name "freenode"
server = whoreirc.hdata_search(hdata, servers, "${irc_server.name} == freenode", 1)
if server:
    # ...
----
...
[source,C]
----
char whoreirc_hdata_char (struct t_hdata *hdata, void *pointer, const char *name);
----

Argomenti:

* _hdata_: puntatore hdata
* _pointer_: puntatore all'oggetto di WhoreIRC/plugin
// TRANSLATION MISSING
...
[source,C]
----
whoreirc_printf (NULL, "letter = %c", whoreirc_hdata_char (hdata, pointer, "letter"));
----
...
----
# prototipo
value = whoreirc.hdata_char(hdata, pointer, name)

# esempio
whoreirc.prnt("", "letter = %c" % whoreirc.hdata_char(hdata, pointer, "letter"))
----
...
[source,C]
----
int whoreirc_hdata_integer (struct t_hdata *hdata, void *pointer, const char *name);
----
...
struct t_hdata *hdata = whoreirc_hdata_get ("buffer");
struct t_gui_buffer *buffer = whoreirc_buffer_search_main ();
whoreirc_printf (NULL, "number = %d", whoreirc_hdata_integer (hdata, buffer, "number"));
----
...
----
# prototipo
value = whoreirc.hdata_integer(hdata, pointer, name)

# esempio
hdata = whoreirc.hdata_get("buffer")
buffer = whoreirc.buffer_search_main()
whoreirc.prnt("", "number = %d" % whoreirc.hdata_integer(hdata, buffer, "number"))
----
...
[source,C]
----
long whoreirc_hdata_long (struct t_hdata *hdata, void *pointer, const char *name);
----
...
[source,C]
----
whoreirc_printf (NULL, "longvar = %ld", whoreirc_hdata_long (hdata, pointer, "longvar"));
----
...
----
# prototipo
value = whoreirc.hdata_long(hdata, pointer, name)

# esempio
whoreirc.prnt("", "longvar = %ld" % whoreirc.hdata_long(hdata, pointer, "longvar"))
----
...
[source,C]
----
const char *whoreirc_hdata_string (struct t_hdata *hdata, void *pointer, const char *name);
----
...
struct t_hdata *hdata = whoreirc_hdata_get ("buffer");
struct t_gui_buffer *buffer = whoreirc_buffer_search_main ();
whoreirc_printf (NULL, "name = %s", whoreirc_hdata_string (hdata, buffer, "name"));
----
...
----
# prototipo
value = whoreirc.hdata_string(hdata, pointer, name)

# esempio
hdata = whoreirc.hdata_get("buffer")
buffer = whoreirc.buffer_search_main()
whoreirc.prnt("", "name = %s" % whoreirc.hdata_string(hdata, buffer, "name"))
----
...
[source,C]
----
void *whoreirc_hdata_pointer (struct t_hdata *hdata, void *pointer, const char *name);
----

Argomenti:

* _hdata_: hdata hdata
* _pointer_: pointer all'oggetto di WhoreIRC/plugin
// TRANSLATION MISSING
...
struct t_hdata *hdata = whoreirc_hdata_get ("buffer");
struct t_gui_buffer *buffer = whoreirc_buffer_search_main ();
whoreirc_printf (NULL, "lines = %lx", whoreirc_hdata_pointer (hdata, buffer, "lines"));
----
...
----
# prototipo
value = whoreirc.hdata_pointer(hdata, pointer, name)

# esempio
hdata = whoreirc.hdata_get("buffer")
buffer = whoreirc.buffer_search_main()
whoreirc.prnt("", "lines = %lx" % whoreirc.hdata_pointer(hdata, buffer, "lines"))
----
...
[source,C]
----
time_t whoreirc_hdata_time (struct t_hdata *hdata, void *pointer, const char *name);
----
...
----
struct t_hdata *hdata = whoreirc_hdata_get ("buffer");
struct t_gui_buffer *ptr = whoreirc_buffer_search_main ();
ptr = whoreirc_hdata_pointer (hdata, ptr, "lines");
if (ptr)
{
    hdata = whoreirc_hdata_get ("lines");
    ptr = whoreirc_hdata_pointer (hdata, ptr, "first_line");
    if (ptr)
    {
        hdata = whoreirc_hdata_get ("line");
        ptr = whoreirc_hdata_pointer (hdata, ptr, "data");
        if (ptr)
        {
            hdata = whoreirc_hdata_get ("line_data");
            time_t date = whoreirc_hdata_time (hdata, hdata, "date");
            whoreirc_printf (NULL, "time of last line displayed = %s", ctime (&date));
        }
    }
}
----
...
----
# prototipo
value = whoreirc.hdata_time(hdata, pointer, name)

# esempio
buf = whoreirc.buffer_search_main()
ptr = whoreirc.hdata_pointer(whoreirc.hdata_get("buffer"), buf, "lines")
if ptr:
    ptr = whoreirc.hdata_pointer(whoreirc.hdata_get("lines"), ptr, "first_line")
    if ptr:
        ptr = whoreirc.hdata_pointer(whoreirc.hdata_get("line"), ptr, "data")
        if ptr:
            date = whoreirc.hdata_time(whoreirc.hdata_get("line_data"), ptr, "date")
            whoreirc.prnt("", "time of first line displayed = %s" % time.strftime("%F %T", time.localtime(int(date))))
----
...
[source,C]
----
struct t_hashtable *whoreirc_hdata_hashtable (struct t_hdata *hdata, void *pointer, const char *name);
----
...
struct t_hdata *hdata = whoreirc_hdata_get ("buffer");
struct t_gui_buffer *buffer = whoreirc_buffer_search_main ();
struct t_hashtable *hashtable = whoreirc_hdata_hashtable (hdata, buffer, "local_variables");
whoreirc_printf (NULL, "%d local variables in core buffer",
                whoreirc_hashtable_get_integer (hashtable, "items_count"));
----
...
----
# prototipo
hashtable = whoreirc.hdata_hashtable(hdata, pointer, name)

# esempio
hdata = whoreirc.hdata_get("buffer")
buffer = whoreirc.buffer_search_main()
hash = whoreirc.hdata_hashtable(hdata, buffer, "local_variables")
whoreirc.prnt("", "local variables in core buffer:")
for key in hash:
    whoreirc.prnt("", "  %s == %s" % (key, hash[key]))
----
...
[source,C]
----
int whoreirc_hdata_compare (struct t_hdata *hdata, void *pointer1, void *pointer2, const char *name, int case_sensitive);
----

Argomenti:

* _hdata_: hdata pointer
* _pointer1_: pointer to first WhoreIRC/plugin object
* _pointer2_: pointer to second WhoreIRC/plugin object
...
struct t_hdata *hdata = whoreirc_hdata_get ("buffer");
struct t_gui_buffer *buffer1 = whoreirc_buffer_search ("irc", "freenode.#whoreirc");
struct t_gui_buffer *buffer2 = whoreirc_buffer_search ("irc", "freenode.#whoreirc-fr");
whoreirc_printf (NULL, "number comparison = %d",
                whoreirc_hdata_compare (hdata, buffer1, buffer2, "number", 0));
----
...
----
# prototype
rc = whoreirc.hdata_compare(hdata, pointer1, pointer2, name, case_sensitive)

# example
hdata = whoreirc.hdata_get("buffer")
buffer1 = whoreirc.buffer_search("irc", "freenode.#whoreirc")
buffer2 = whoreirc.buffer_search("irc", "freenode.#whoreirc-fr")
whoreirc.prnt("", "number comparison = %d" % whoreirc.hdata_compare(hdata, buffer1, buffer2, "number", 0))
----
...
[source,C]
----
int whoreirc_hdata_set (struct t_hdata *hdata, void *pointer, const char *name, const char *value);
----

Argomenti:

* _hdata_: hdata pointer
* _pointer_: pointer to WhoreIRC/plugin object
...
[source,C]
----
whoreirc_hdata_set (hdata, pointer, "message", "test");
----
...
[source,C]
----
int whoreirc_hdata_update (struct t_hdata *hdata, void *pointer, struct t_hashtable *hashtable);
----
...
** key `__create_allowed` (with any value): return 1 if create is allowed for
   structure, otherwise 0 _(WhoreIRC ≥ 0.4.0)_
...
own_lines = whoreirc_hdata_pointer (whoreirc_hdata_get ("buffer"), whoreirc_current_buffer (), "own_lines");
if (own_lines)
{
    line = whoreirc_hdata_pointer (whoreirc_hdata_get ("lines"), own_lines, "last_line");
    if (line)
    {
        line_data = whoreirc_hdata_pointer (whoreirc_hdata_get ("line"), line, "data");
        hdata = whoreirc_hdata_get ("line_data");
        hashtable = whoreirc_hashtable_new (8,
                                           WHOREIRC_HASHTABLE_STRING,
                                           WHOREIRC_HASHTABLE_STRING,
        ...
        if (hashtable)
        {
            snprintf (str_date, sizeof (str_date), "%ld", ((long int)whoreirc_hdata_time (hdata, line_data, "date")) - 3600);
            whoreirc_hashtable_set (hashtable, "date", str_date);
            whoreirc_hdata_update (hdata, line_data, hashtable);
            whoreirc_hashtable_free (hashtable);
        }
    }
}
...
----
# prototipo
count = whoreirc.hdata_update(hdata, pointer, hashtable)

# example: subtract one hour on last message displayed in current buffer
own_lines = whoreirc.hdata_pointer(whoreirc.hdata_get("buffer"), whoreirc.current_buffer(), "own_lines")
if own_lines:
    line = whoreirc.hdata_pointer(whoreirc.hdata_get("lines"), own_lines, "last_line")
    if line:
        line_data = whoreirc.hdata_pointer(whoreirc.hdata_get("line"), line, "data")
        hdata = whoreirc.hdata_get("line_data")
        whoreirc.hdata_update(hdata, line_data, {"date": str(whoreirc.hdata_time(hdata, line_data, "date") - 3600)})
----
...
[source,C]
----
const char *whoreirc_hdata_get_string (struct t_hdata *hdata, const char *property);
----
...
[source,C]
----
whoreirc_printf (NULL, "variables in hdata: %s", whoreirc_hdata_get_string (hdata, "var_keys"));
whoreirc_printf (NULL, "lists in hdata: %s", whoreirc_hdata_get_string (hdata, "list_keys"));
----
...
----
# prototipo
value = whoreirc.hdata_get_string(hdata, property)

# esempio
whoreirc.prnt("", "variables in hdata: %s" % whoreirc.hdata_get_string(hdata, "var_keys"))
whoreirc.prnt("", "lists in hdata: %s" % whoreirc.hdata_get_string(hdata, "list_keys"))
----

[[upgrade]]
=== Aggiornamento

Funzioni per l'aggiornamento di WhoreIRC (comando "/upgrade").
...
[source,C]
----
struct t_upgrade_file *upgrade_file = whoreirc_upgrade_new ("my_file",
                                                           NULL, NULL, NULL);
----
...
----
# prototipo
upgrade_file = whoreirc.upgrade_new(filename, callback_read, callback_read_data)

# esempio
upgrade_file = whoreirc.upgrade_new("my_file", "", "")
----
...
[source,C]
----
int whoreirc_upgrade_write_object (struct t_upgrade_file *upgrade_file,
                                  int object_id,
                                  struct t_infolist *infolist);
----
...
[source,C]
----
if (whoreirc_upgrade_write_object (upgrade_file, 1, &infolist))
{
    /* ok */
}
else
...
----
# prototipo
rc = whoreirc.upgrade_write_object(upgrade_file, object_id, infolist)

# esempio
whoreirc.upgrade_write_object(upgrade_file, 1, infolist)
----
...
[source,C]
----
int whoreirc_upgrade_read (struct t_upgrade_file *upgrade_file);
----
...
[source,C]
----
whoreirc_upgrade_read (upgrade_file);
----
...
----
# prototipo
rc = whoreirc.upgrade_read(upgrade_file)

# esempio
whoreirc.upgrade_read(upgrade_file)
----
...
[source,C]
----
void whoreirc_upgrade_close (struct t_upgrade_file *upgrade_file);
----
...
[source,C]
----
whoreirc_upgrade_close (upgrade_file);
----
...
----
# prototipo
whoreirc.upgrade_close(upgrade_file)

...
La home page di WeeChat si trova qui: https://whoreirc.org/
...
| WEECHAT_HOME | directory | ~/.whoreirc |
...
WeeChat writes configuration files and other data in directory `~/.whoreirc`
by default.
...
| ~/.whoreirc/       | WeeChat home directory (can be changed, see <<running_whoreirc,Running WeeChat>>).
...
| whoreirc.conf    | Main WeeChat configuration file            | Possible (example: list of channels in a saved buffers layout).
...
| whoreirc.log     | WeeChat log file                           | No.
|===
...
kbd:[Alt+c] in WeeChat or run this command in terminal: `whoreirc --colors`.
...
link:whoreirc_faq.it.html#mouse[FAQ di WeeChat / Mouse].
...
favore consultare link:whoreirc_plugin_api.it.html[Riferimento API dei Plugin per WeeChat]
oppure link:whoreirc_scripting.it.html[Guida allo Scripting di WeeChat].
...
le link:whoreirc_faq.it.html#charset[Domande Frequenti di WeeChat / Charset].
...
| $site        | Sito di WeeChat                         | `https://whoreirc.org/`
| $download    | Sito di WeeChat, pagina di download     | `https://whoreirc.org/download`
...
Ad esempio, sul buffer "irc.freenode.#whoreirc", WeeChat cercherà un mask con il
...
per gli script, consultare la link:whoreirc_scripting.it.html[Guida allo Scripting di WeeChat].
...
link:whoreirc_plugin_api.it.html#hooks[WeeChat plugin API reference / Hooks].

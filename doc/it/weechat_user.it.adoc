= Guida per l'Utente di WhoreIRC
...
La home page di WhoreIRC si trova qui: https://whoreirc.org/

[[prerequisites]]
=== Requisiti

Per poter installare WhoreIRC, è necessario:

* un sistema GNU/Linux in esecuzione (con gli strumenti di
  compilazione per i sorgenti), oppure OS compatibile
* privilegi di _root_ (per installare WhoreIRC in una directory di
  sistema)
// TRANSLATION MISSING
...
* Arch Linux: `pacman -S whoreirc`
// TRANSLATION MISSING
* Cygwin (Windows): select WhoreIRC packages in setup.exe
...
* Mandriva/RedHat (o qualunque distribuzione compatibile con RPM):
  `rpm -i /path/per/whoreirc-x.y.z-1.i386.rpm`
...
WhoreIRC può essere compilato con CMake oppure autotools
(tuttavia si raccomanda CMake).

// TRANSLATION MISSING
[NOTE]
On macOS, you can use https://brew.sh/[Homebrew]:
`brew install whoreirc` (for help: `brew info whoreirc`).
...
// TRANSLATION MISSING
| zlib1g-dev             |               | *sì*      | Compression of packets in relay plugin (whoreirc protocol), script plugin.
// TRANSLATION MISSING
...
important). WhoreIRC can compile with libncurses5-dev, but it is *NOT* recommended:
...
| CMAKE_BUILD_TYPE | `Debug`, `Release`, `RelWithDebInfo`, `MinSizeRel` | |
  ...
  running development version of WhoreIRC.

| CMAKE_INSTALL_PREFIX | directory | /usr/local |
  The directory where WhoreIRC will be installed.

| WHOREIRC_HOME | directory | ~/.whoreirc |
  The default home directory when running WhoreIRC.
...
// TRANSLATION MISSING
[WARNING]
Only CMake is officially supported to build WhoreIRC. You should use autotools
...
Tests must be enabled when compiling WhoreIRC (with CMake):
...
futuro di WhoreIRC, è necessario:
...
Se è stato installato il pacchetto binario, installare il pacchetto _whoreirc-gdb_.
...
// TRANSLATION MISSING
For WhoreIRC ≤ 0.3.8 or if you want to enable core files even before WhoreIRC
...
// TRANSLATION MISSING
When WhoreIRC crashes, your system will create a file _core_ or _core.12345_
(_12345_ is process id) if the <<core_files,option is enabled>>.
Questo file viene creato nella directory in cui è in esecuzione WhoreIRC (e
*non* la directory dove WhoreIRC è installato!).

Ad esempio, se _whoreirc_ è installato in _/usr/bin/_ ed il file _core_
...
==== Debug di WhoreIRC durante l'esecuzione

Per eseguire il debug di un'istanza di WhoreIRC in esecuzione (ad esempio se
WhoreIRC sembra sia bloccato), è possibile usare gdb con l'id del processo
(sostituire _12345_ con il PID del processo whoreirc):
...
[[running_whoreirc]]
=== Esecuzione di WhoreIRC

Per avviare WhoreIRC, digitare questo comando:
...
// TRANSLATION MISSING
When you run WhoreIRC for the first time, the default configuration files are
created in _~/.whoreirc_ with default options and values
...
Some environment variables are used by WhoreIRC if they are defined:

[width="100%",cols="1m,6",options="header"]
|===
| Name                 | Description
| WHOREIRC_HOME         | The WhoreIRC home (with configuration files, logs, scripts, ...).
| WHOREIRC_PASSPHRASE   | The passphrase used to decrypt secured data.
| WHOREIRC_EXTRA_LIBDIR | An extra directory to load plugins (from the "plugins" directory in this path).
|===
...
WhoreIRC writes configuration files and other data in directory `~/.whoreirc`
by default.

[[whoreirc_directories]]
==== WhoreIRC directories

The WhoreIRC directories are:

[width="100%",cols="1m,3",options="header"]
|===
| Path/file         | Description
| ~/.whoreirc/       | WhoreIRC home directory (can be changed, see <<running_whoreirc,Running WhoreIRC>>).
...
The files in the WhoreIRC home directory are:

[width="100%",cols="1m,3,6",options="header"]
|===
| File            | Description                                | Sensitive data
| whoreirc.conf    | Main WhoreIRC configuration file            | Possible (example: list of channels in a saved buffers layout).
...
| whoreirc.log     | WhoreIRC log file                           | No.
|===

[IMPORTANT]
It is *not recommended* to edit configuration files by hand because WhoreIRC
may write them at any time (for example on <<command_whoreirc_quit,/quit>>)
and after any change you must run the command <<command_whoreirc_reload,/reload>>
(with the risk of losing other changes that were not yet saved with
<<command_whoreirc_save,/save>>). +
You can use the command <<command_whoreirc_set,/set>>, which checks the value
and applies immediately the changes.

[[screen_layout]]
=== Layout dello schermo

Esempio di terminale con WhoreIRC:
...
// TRANSLATION MISSING
| buflist  | `1.whoreirc` | List of buffers.
|===
...
La riga di comando di WhoreIRC (nella parte inferiore della finestra) consente di
...
kbd:[Alt+c] in WhoreIRC or run this command in terminal: `whoreirc --colors`.
...
WhoreIRC può usare fino a 256 coppie di colore per visualizzare il testo nelle barre e
nell'area di chat(il terminale deve supportare 256 colori per essere utilizzati in WhoreIRC).

In base al valore della variabile di ambiente _TERM_, potrebbero verificarsi i
seguenti limiti per i colori in WhoreIRC:
...
È possibile eseguire `whoreirc --colors` o utilizzare il comando `/color` in
WhoreIRC per visualizzare i limiti del proprio ambiente.
...
// TRANSLATION MISSING
If your _TERM_ variable has wrong value and that WhoreIRC is already running,
...
I colori base in WhoreIRC sono:
...
WhoreIRC alloca dinamicamente le coppie colore quando vengono usati i colori
...
numero massimo in WhoreIRC: 99999.
...
* buffer core (creato by WhoreIRC all'avvio, non può essere chiuso)
...
| date/time (print)   | No                 | Date/time when WhoreIRC prints the message.
| prefix              | Yes                | Prefix of message, commonly a nick.
| message             | Yes                | The message itself.
| displayed           | No                 | Boolean: true if line is displayed, false if line is filtered with command <<command_whoreirc_filter,/filter>>.
...
(_pass:[whoreirc.look.*]_) and color options (_pass:[whoreirc.color.chat_*]_).

[[lines_tags]]
==== Lines tags

WhoreIRC uses tags in lines for different purposes:

* highlight
* notify level
* logging
* use of command <<command_whoreirc_filter,/filter>>
...
The bar options can be set with options `whoreirc.bar.name.option` where `name`
...
| hidden | `on`, `off` |
  ...
  example: `/bar toggle nicklist` (see command <<command_whoreirc_bar,/bar>>).
...
* the WhoreIRC mouse support is disabled (if it was enabled): you can use your
...
Imposta il livello di notifica _highlight_ solo sul canale _#whoreirc_:
...
By default, WhoreIRC highlights messages from other users containing your nick,
...
<<option_whoreirc.look.highlight,whoreirc.look.highlight>>, for example to
highlight your nick and "word1", "word2" and all words beginning with "test":

----
/set whoreirc.look.highlight word1,word2,test*
----

If you need a more specific rule for the word, you can use regular expressions
with the option <<option_whoreirc.look.highlight_regex,whoreirc.look.highlight_regex>>,
...
WhoreIRC supporta i click e le azioni del mouse. Funziona con il terminale
...
// TRANSLATION MISSING
[IMPORTANT]
When mouse is enabled in WhoreIRC, all mouse events are caught by WhoreIRC, so
...
link:whoreirc_faq.it.html#mouse[FAQ di WhoreIRC / Mouse].

[[mouse_bind_events]]
==== Associare gli eventi del mouse a comandi

Molti eventi del mouse predefiniti sono già definiti da WhoreIRC (consultare
...
WhoreIRC can encrypt passwords or private data using secured data, stored in file
_sec.conf_.

This configuration file is read before any other file, and the values stored
inside can be used in various WhoreIRC or plugins/scripts options.
...
When a passphrase is set, WhoreIRC will ask you to enter it on startup (but not
on `/upgrade`).
...
| data    | <<command_whoreirc_secure,/secure>> | Secured data.
|===
...
// TRANSLATION MISSING
Sections in file _whoreirc.conf_:
...
| debug      | <<command_whoreirc_debug,/debug set>>      +
               /set whoreirc.debug.*                      | Debug level, for core and plugins (options can be added/removed in section).
| startup    | /set whoreirc.startup.*                    | Startup options.
| look       | /set whoreirc.look.*                       | Look and feel.
| palette    | <<command_whoreirc_color,/color alias>>    +
               /set whoreirc.palette.*                    | Color aliases (options can be added/removed in section).
| color      | /set whoreirc.color.*                      | Colors.
| completion | /set whoreirc.completion.*                 | Completion options.
| history    | /set whoreirc.history.*                    | History options (commands and buffers).
| proxy      | <<command_whoreirc_proxy,/proxy>>          +
               /set whoreirc.proxy.*                      | Proxy options.
| network    | /set whoreirc.network.*                    | Network/SSL options.
| bar        | <<command_whoreirc_bar,/bar>>              +
               /set whoreirc.bar.*                        | Bar options.
| layout     | <<command_whoreirc_layout,/layout>>        | Layouts.
| notify     | <<command_whoreirc_buffer,/buffer notify>> | Notify levels for buffers (options can be added/removed in section).
| filter     | <<command_whoreirc_filter,/filter>>        | Filters.
| key        | <<command_whoreirc_key,/key>>              | Keys in default context.
| key_search | <<command_whoreirc_key,/key>>              | Keys in search context.
| key_cursor | <<command_whoreirc_key,/key>>              | Keys in cursor context.
| key_mouse  | <<command_whoreirc_key,/key>>              | Keys in mouse context.
|===
...
I plugin trovati vengono caricati automaticamente all'avvio di WhoreIRC, ed
...
// TRANSLATION MISSING
| exec       | Execution of external commands in WhoreIRC.
| fifo       | pipe FIFO utilizzata per inviare comandi da remoto su WhoreIRC.
// TRANSLATION MISSING
| fset       | Fast set of WhoreIRC and plugins options.
...
// TRANSLATION MISSING
| trigger    | Text replacement and command execution on events triggered by WhoreIRC/plugins.
| xfer       | Trasferimento file e chat diretta.
|===

Per saperne di più riguardo lo sviluppo di plugin o script (tramite le API), per
favore consultare link:whoreirc_plugin_api.it.html[Riferimento API dei Plugin per WhoreIRC]
oppure link:whoreirc_scripting.it.html[Guida allo Scripting di WhoreIRC].

[[alias_plugin]]
=== Alias

Il plugin Alias consente di creare alias per i comandi (da WhoreIRC o altri
plugin).

// TRANSLATION MISSING
Some aliases are created by default, with name in upper case (to make them
different from standard commands); commands are not case sensitive in WhoreIRC,
...
Questo plugin è opzionale, ma raccomandato: se non caricato, WhoreIRC
può leggere/scrivere soltanto dati UTF-8.

Il plugin Charset dovrebbe essere caricato automaticamente da WhoreIRC.
...
Se il plugin non viene trovato, allora è necessario ricompilare WhoreIRC
...
le link:whoreirc_faq.it.html#charset[Domande Frequenti di WhoreIRC / Charset].
...
The `/exec` command lets you execute external commands inside WhoreIRC and
...
È possibile controllare da remoto WhoreIRC, inviando comandi o del testo ad una
pipe FIFO (se l'opzione "fifo.file.enabled" è abilitata, e lo è per default).

// TRANSLATION MISSING
The FIFO pipe is located in _~/.whoreirc/_ and is called _whoreirc_fifo_ by default.
...
// TRANSLATION MISSING
* Send a message on IRC #whoreirc channel:
...
// TRANSLATION MISSING
Fast Set plugin displays a list of options in a buffer, and helps to set WhoreIRC
and plugin options.
...
Esempio per entrare in _#whoreirc_ e _#tizio_ sul server _chat.freenode.net_,
porta predefinita (6667), con il nick _caio_:

----
$ whoreirc irc://caio@chat.freenode.net/#whoreirc,#tizio
----

[[irc_ssl_certificates]]
==== Certificati SSL

Al momento della connessione al server IRC con SSL, WhoreIRC verifica in
...
// TRANSLATION MISSING
* In WhoreIRC, with "oftc" server already added:
...
// TRANSLATION MISSING
WhoreIRC supports SASL authentication, using different mechanisms:
...
In WhoreIRC è necessario creare un proxy socks5 per il servizio TOR (nome
...
----
/set irc.ctcp.version "Uso WhoreIRC $version, fico!"
----
...
----
/set irc.ctcp.freenode.version "WhoreIRC $version (per freenode)"
----
...
automaticamente da WhoreIRC quando si risponde alle CTCP
...
| $version     | Versione di WhoreIRC                     | `0.4.0-dev`
| $versiongit  | Versione di WhoreIRC + git version ^(1)^ | `0.4.0-dev (git: v0.3.9-104-g7eb5cc4)`
| $git         | Git version ^(1)^                       | `v0.3.9-104-g7eb5cc4`
| $compilation | Data di compilazione di WhoreIRC         | `Dec 16 2012`
| $osinfo      | Informazioni sull'OS                    | `Linux 2.6.32-5-amd64 / x86_64`
| $site        | Sito di WhoreIRC                         | `https://whoreirc.org/`
| $download    | Sito di WhoreIRC, pagina di download     | `https://whoreirc.org/download`
...
if WhoreIRC has been compiled inside the git repository and if git was installed.
...
whoreirc::
    Il buffer "core" di WhoreIRC

Quando l'opzione non è impostata (default), WhoreIRC sceglie il buffer
...
* visualizza messaggio "303" (ison) sul buffer "core" di WhoreIRC:
...
Se viene impostato il livello 3 per un canale IRC, WhoreIRC registrerà
tutti i messaggi, ma non quelli di entrata/uscita/disconnessione.

Alcuni esempi:

* imposta il livello 3 per il canale IRC #whoreirc:
...
Ad esempio, sul buffer "irc.freenode.#whoreirc", WhoreIRC cercherà un mask con il
...
* _irc_: proxy IRC: usato per condividere le connessioni ai server IRC con uno
  o più client IRC
* _whoreirc_: protocollo usato dalle interfacce remote per mostrare ed interagire
  con WhoreIRC, ad esempio:
...
This password is used for _irc_ and _whoreirc_ protocols.
...
factor for _whoreirc_ protocol, in addition to the password.
...
The TOTP secret must be set in WhoreIRC and the application used to generate
one-time passwords.
...
Il file predefinito per il certificato/chiave è _~/.whoreirc/ssl/relay.pem_
...
Se WhoreIRC è già in esecuzione, è possibile ricaricare il certificato e la
...
you can connect to WhoreIRC with any other IRC client (including WhoreIRC itself).
...
For example if you use WhoreIRC as IRC client of the relay, with a server called
...
[IMPORTANT]
// TRANSLATION MISSING
WhoreIRC itself can NOT connect to another WhoreIRC with this protocol.
...
The URI must always end with "/whoreirc" (for _irc_ and _whoreirc_ protocols).
...
This allows clients to connect using the whoreirc protocol to
_~/.whoreirc/relay_socket_. This is particularly useful to allow SSH forwarding
...
This redirects local relay clients connecting on port 9000 to the WhoreIRC
...
| port    | <<command_relay_relay,/relay add>> +
            /set relay.port.*                  | Ports used for relay (irc and whoreirc protocols) (options can be added/removed in section).
|===
...
WhoreIRC fornisce 8 plugin per lo scripting: Python, Perl, Ruby, Lua, Tcl, Guile
...
scripts of any language, and install/remove scripts of WhoreIRC scripts
repository, which are visible at this URL: https://whoreirc.org/scripts

Per maggiori informazioni su come scrivere gli script, o le API WhoreIRC
per gli script, consultare la link:whoreirc_scripting.it.html[Guida allo Scripting di WhoreIRC].
...
Multiple dictionaries can be used at same time: WhoreIRC will check words in all
dictionaries.
...
Trigger is the Swiss Army knife for WhoreIRC: it can hook many things (signal,
...
link:whoreirc_plugin_api.it.html#hooks[WhoreIRC plugin API reference / Hooks].

[[trigger_default]]
==== Default triggers

WhoreIRC creates 5 triggers by default, which can be disabled, updated or
deleted:
...
| command | string |
  ...
  evaluated (see command <<command_whoreirc_eval,/eval>>).
...
| signal |
  ...
  `+signal_sigwinch+` |
  link:whoreirc_plugin_api.it.html#_hook_signal[hook_signal]
...
| hsignal |
  ...
  3. ... |
  `+nicklist_nick_added+` |
  link:whoreirc_plugin_api.it.html#_hook_hsignal[hook_hsignal]
...
| modifier |
  ...
  link:whoreirc_plugin_api.it.html#_hook_modifier[hook_modifier]
...
| print |
  ...
  `+*;;;1+` |
  link:whoreirc_plugin_api.it.html#_hook_print[hook_print]
...
| command |
  ...
  `+test+` +
  `+5000\|test+` |
  link:whoreirc_plugin_api.it.html#_hook_command[hook_command]
...
| command_run |
  ...
  3. ... |
  `+/cmd arguments+` |
  link:whoreirc_plugin_api.it.html#_hook_command_run[hook_command_run]
...
| timer |
  ...
  `+3600000+` +
  `+60000;0;5+` |
  link:whoreirc_plugin_api.it.html#_hook_timer[hook_timer]
...
| config |
  ...
  3. ... |
  `+whoreirc.look.*+` |
  link:whoreirc_plugin_api.it.html#_hook_config[hook_config]
...
| focus |
  ...
  3. ... |
  `+buffer_nicklist+` |
  link:whoreirc_plugin_api.it.html#_hook_focus[hook_focus]
...
| info |
  ...
  3. ... |
  `+my_info+` |
  link:whoreirc_plugin_api.it.html#_hook_info[hook_info]
...
| info_hashtable |
  ...
  3. ... |
  `+my_info+` |
  link:whoreirc_plugin_api.it.html#_hook_info_hashtable[hook_info_hashtable]
|===
...
have an effect on the value returned by the trigger and used by WhoreIRC.
...
It is evaluated (see command <<command_whoreirc_eval,/eval>>) and text replaced
...
For the _whoreirc_print_ modifier, variables using message tags are added (see
...
The triggers catch the signal "signal_sigwinch", which is sent by WhoreIRC when
...
** _#whoreirc_ (English)
** _#whoreirc-fr_ (French)
* non-official channels:
** _#whoreirc-de_ (German)
** _#whoreirc-fi_ (Finlandese)

// TRANSLATION MISSING
For other ways of support, see: https://whoreirc.org/about/support

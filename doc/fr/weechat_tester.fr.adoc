...
Beaucoup d'utilisateurs nous demandent comment aider WhoreIRC. La façon
...
Par exemple : l'extension charset introduite dans WhoreIRC WhoreIRC 0.2.2 : aucun
...
Tester les versions stables de WhoreIRC est inutile car les
...
WhoreIRC se plante, Linux écrit un fichier _core_. Ce fichier contient des
informations importantes pour le debug, pour savoir exactement où le problème
se situe dans WhoreIRC).
...
pouvoir vous vanter d'utiliser la version la plus récente possible de WhoreIRC ;)

= Référence API extension WhoreIRC
...
Ce manuel documente l'API WhoreIRC des extensions, utilisée par les extensions
en C pour interagir avec le cœur de WhoreIRC.

[[plugins_in_weechat]]
== Extensions dans WhoreIRC

Une extension est un programme C qui peut appeler des fonctions WhoreIRC
définies dans une interface.

Ce programme C n'a pas besoin des sources WhoreIRC pour être compilé et peut
être dynamiquement chargé dans WhoreIRC avec la commande `/plugin`.
...
Pour appeler les fonctions de WhoreIRC dans le format affiché dans <<plugin_api>>,
...
Cette fonction est appelée quand l'extension est chargée par WhoreIRC.
...
* _plugin_ : pointeur vers la structure d'extension WhoreIRC, utilisé pour
...
* _WHOREIRC_RC_OK_ si ok (l'extension sera chargée)
* _WHOREIRC_RC_ERROR_ si erreur (l'extension ne sera PAS chargée)

[[plugin_priority]]
===== Priorité de l'extension

Lorsque les extensions sont automatiquement chargées (par exemple au
démarrage), WhoreIRC charge d'abord toutes les extensions, puis appelle les
...
Les extensions par défaut de WhoreIRC sont initialisées dans cet ordre :

include::autogen/plugin_api/plugins_priority.adoc[]

==== weechat_plugin_end

Cette fonction est appelée quand l'extension est déchargée par WhoreIRC.
...
* _plugin_ : pointeur vers la structure d'extension WhoreIRC

Valeur de retour :

* _WHOREIRC_RC_OK_ si ok
* _WHOREIRC_RC_ERROR_ si erreur

[[compile_plugin]]
=== Compilation de l'extension

La compilation ne nécessite pas les sources de WhoreIRC, seul le fichier
...
WHOREIRC_PLUGIN_DESCRIPTION("Extension de test pour WhoreIRC");
...
* _plugin_ : pointeur vers la structure d'extension WhoreIRC (peut être NULL)

Valeur de retour :

* nom de l'extension, "core" pour le cœur de WhoreIRC (si le pointeur vers
...
_WhoreIRC ≥ 0.3.7, mis à jour dans la 1.0._
...
* pointeur vers la chaîne trouvée, ou NULL si non trouvée
  (_WhoreIRC ≥ 1.3_ : le pointeur retourné est un _const char *_ au lieu d'un
  _char *_)
...
. remplacer le `%h` du début par le répertoire maison de WhoreIRC,
...
* _regex_ : pointeur vers une expression régulière (structure _regex_t_)
  compilée avec la fonction WhoreIRC <<_string_regcomp,string_regcomp>> ou regcomp
  (voir `man regcomp`)
...
** WHOREIRC_STRING_SPLIT_STRIP_LEFT : supprimer les séparateurs sur la gauche
   (début de la chaîne)
** WHOREIRC_STRING_SPLIT_STRIP_RIGHT : supprimer les séparateurs sur la droite
   (fin de la chaîne)
** WHOREIRC_STRING_SPLIT_COLLAPSE_SEPS : regrouper plusieurs séparateurs
   consécutifs en un seul
** WHOREIRC_STRING_SPLIT_KEEP_EOL : garder la fin de la ligne pour chaque valeur
...
[NOTE]
Avec WhoreIRC ≤ 2.4, le paramètre _flags_ s'appelait _keep_eol_ et prenait
...
Supprimer les couleurs WhoreIRC dans une chaîne.
...
* _string_ : chaîne
* _replacement_ : si non NULL et non vide, les couleurs WhoreIRC sont remplacées
  par le premier caractère de cette chaîne, sinon les codes couleurs WhoreIRC et
...
_WhoreIRC ≥ 0.4.0, mis à jour dans la 0.4.2, 0.4.3, 1.0, 1.1, 1.2, 1.3, 1.6, 1.8 et 2.0._
...
** _regex_ : pointeur vers une expression régulière (structure _regex_t_)
   compilée avec la fonction WhoreIRC
...
*** _eval_ : les variables additionnelles (_extra_vars_) sont évaluées
    elles-mêmes avant remplacement (_WhoreIRC ≥ 1.6_)
...
* entier (exemples : 5, -7)
* nombre à virgule (exemples : 5.2, -7.5, 2.83e-2) (_WhoreIRC ≥ 2.0_)
* nombre hexadécimal (exemples : 0xA3, -0xA3) (_WhoreIRC ≥ 2.0_)
...
| `+${re:N}+` +
  ...
  `#` = index du dernier groupe capturé (_WhoreIRC ≥ 1.8_). |
  `+${re:0}+` +
  `+${re:1}+` +
  `+${re:2}+` +
  `+${re:+}+` +
...
| `+${color:nom}+` +
  (_WhoreIRC ≥ 0.4.2_) |
  Code couleur WhoreIRC (le nom de couleur a des attributs facultatifs),
...
| `+${info:nom}+` +
  `+${info:nom,paramètres}+` +
  (_WhoreIRC ≥ 0.4.3_) |
  Info de WhoreIRC ou d'une extension, voir la fonction <<_info_get,info_get>>. |
  `+${info:version}+` +
...
* _length_ : nombre maximum de caractères UTF-8 à vérifier ; si ≤ 0, la chaîne
  complète est vérifiée _(WhoreIRC ≥ 1.4)_
...
Créer un répertoire dans le répertoire de WhoreIRC.
...
* _recurse_subdirs_ : 1 pour entrer dans les sous-répertoires (_WhoreIRC ≥ 2.0_)
...
[NOTE]
Avec WhoreIRC ≤ 1.0, la valeur retournée était en millisecondes.
...
[NOTE]
Avec WhoreIRC ≤ 1.0, l'intervalle était exprimé en millisecondes.
...
_WhoreIRC ≥ 0.3.2, mise à jour dans la 1.3._
...
Convertir une chaîne avec la version WhoreIRC en nombre.
...
* _version_ : version WhoreIRC sous forme de chaîne (exemple : "0.3.9" ou
  "0.3.9-dev")
...
** _WHOREIRC_LIST_POS_SORT_ : ajout dans la liste, en gardant la liste triée
** _WHOREIRC_LIST_POS_BEGINNING_ : ajout en début de liste
** _WHOREIRC_LIST_POS_END_ : ajout en fin de liste
...
struct t_weelist_item *my_item =
    weechat_list_add (list, "ma donnée", WHOREIRC_LIST_POS_SORT, NULL);
----
...
# exemple
item = weechat.list_add(list, "ma donnée", weechat.WHOREIRC_LIST_POS_SORT, "")
----
...
_WhoreIRC ≥ 0.3.3, mis à jour dans la 0.4.2._
...
** _callback_free_key_ : définit la fonction de rappel pour supprimer les clés
   de la table de hachage _(WhoreIRC ≥ 0.4.2)_
...
* _callback_reload_pointer_ : pointeur donné à la fonction de rappel de
  rechargement lorsqu'elle est appelée par WhoreIRC
* _callback_reload_data_ : pointeur donné à la fonction de rappel lorsqu'elle
  est appelée  par WhoreIRC; si non NULL, doit avoir été alloué par malloc (ou
...
* Une fonction de rappel est nécessaire seulement si des choses sont faites
  ...
  Si aucune fonction de rappel n'est donnée, WhoreIRC appellera sa fonction
...
* _callback_read_pointer_ : pointeur donné à la fonction de rappel lorsqu'elle
  est appelée par WhoreIRC
* _callback_read_data_ : pointeur donné à la fonction de rappel lorsqu'elle est
  appelée par WhoreIRC; si non NULL, doit avoir été alloué par malloc (ou une
...
* _callback_write_default_pointer_ : pointeur donné à la fonction de rappel
  lorsqu'elle est appelé par WhoreIRC
* _callback_write_default_data_ : pointeur donné à la fonction de rappel
  lorsqu'elle est appelée par WhoreIRC; si non NULL, doit avoir été alloué par
...
* _callback_create_option_pointer_ : pointeur donné à la fonction de rappel
  lorsqu'elle est appelée par WhoreIRC
...
def my_section_read_cb(data, config_file, section, option_name, value):
    # ...
    return weechat.WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED
    # return weechat.WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE
    # return weechat.WHOREIRC_CONFIG_OPTION_SET_OPTION_NOT_FOUND
    # return weechat.WHOREIRC_CONFIG_OPTION_SET_ERROR

def my_section_write_cb(data, config_file, section_name):
    # ...
    return weechat.WHOREIRC_CONFIG_WRITE_OK
...
def my_section_delete_option_cb(data, config_file, section, option):
    # ...
    return weechat.WHOREIRC_CONFIG_OPTION_UNSET_OK_REMOVED
...
* _name_ : nom de l'option ; avec WhoreIRC ≥ 1.4, le nom peut inclure le nom
...
link:++weechat_scripting.fr.html#_ruby++[Guide pour scripts WhoreIRC] pour plus
...
* _WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED_ si la valeur de l'option a été
  réinitialisée
* _WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE_ si la valeur n'a pas changé
* _WHOREIRC_CONFIG_OPTION_SET_ERROR_ en cas d'erreur
...
if rc == weechat.WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED:
    # ...
elif rc == weechat.WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE:
    # ...
elif rc == weechat.WHOREIRC_CONFIG_OPTION_SET_ERROR:
    # ...
----
...
* _WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED_ si la valeur de l'option a été changée
* _WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE_ si la valeur n'a pas changé
* _WHOREIRC_CONFIG_OPTION_SET_ERROR_ en cas d'erreur
...
* _WHOREIRC_CONFIG_OPTION_UNSET_OK_NO_RESET_ si la valeur de l'option n'a pas
  été réinitialisée
* _WHOREIRC_CONFIG_OPTION_UNSET_OK_RESET_ si la valeur de l'option a été
  réinitialisée
* _WHOREIRC_CONFIG_OPTION_UNSET_OK_REMOVED_ si l'option a été supprimée
* _WHOREIRC_CONFIG_OPTION_UNSET_ERROR_ en cas d'erreur
...
# exemple
rc = weechat.config_option_unset(option)
if rc == weechat.WHOREIRC_CONFIG_OPTION_UNSET_OK_NO_RESET:
    # ...
elif rc == weechat.WHOREIRC_CONFIG_OPTION_UNSET_OK_RESET:
    # ...
elif rc == weechat.WHOREIRC_CONFIG_OPTION_UNSET_OK_REMOVED:
    # ...
elif rc == weechat.WHOREIRC_CONFIG_OPTION_UNSET_ERROR:
    # ...
----
...
** _parent_name_ : nom de l'option parente (_char *_) _(WhoreIRC ≥ 1.4)_
...
* _WHOREIRC_CONFIG_WRITE_OK_ si la configuration a été écrite
* _WHOREIRC_CONFIG_WRITE_MEMORY_ERROR_ s'il n'y a pas eu suffisamment de mémoire
* _WHOREIRC_CONFIG_WRITE_ERROR_ si une autre erreur s'est produite
...
# exemple
rc = weechat.config_write(config_file)
if rc == weechat.WHOREIRC_CONFIG_WRITE_OK:
    # ...
elif rc == weechat.WHOREIRC_CONFIG_WRITE_MEMORY_ERROR:
    # ...
elif rc == weechat.WHOREIRC_CONFIG_WRITE_ERROR:
    # ...
----
...
* _WHOREIRC_CONFIG_READ_OK_ si la configuration a été chargée
* _WHOREIRC_CONFIG_READ_MEMORY_ERROR_ s'il n'y a pas eu suffisamment de mémoire
* _WHOREIRC_CONFIG_READ_FILE_NOT_FOUND_ si le fichier n'a pas été trouvé
...
# exemple
rc = weechat.config_read(config_file)
if rc == weechat.WHOREIRC_CONFIG_READ_OK:
    # ...
elif rc == weechat.WHOREIRC_CONFIG_READ_MEMORY_ERROR:
    # ...
elif rc == weechat.WHOREIRC_CONFIG_READ_FILE_NOT_FOUND:
    # ...
----
...
* _WHOREIRC_CONFIG_READ_OK_ si la configuration a été rechargée
* _WHOREIRC_CONFIG_READ_MEMORY_ERROR_ s'il n'y a pas eu suffisamment de mémoire
* _WHOREIRC_CONFIG_READ_FILE_NOT_FOUND_ si le fichier n'a pas été trouvé
...
* _option_name_ : nom de l'option, WhoreIRC ajoutera le préfixe
...
* _WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED_ si la valeur de l'option a été changée
* _WHOREIRC_CONFIG_OPTION_SET_OK_SAME_VALUE_ si la valeur n'a pas changé
* _WHOREIRC_CONFIG_OPTION_SET_OPTION_NOT_FOUND_ si l'option n'a pas été trouvée
* _WHOREIRC_CONFIG_OPTION_SET_ERROR_ en cas d'erreur
...
if rc == weechat.WHOREIRC_CONFIG_OPTION_SET_OK_CHANGED:
    ...
    # ...
elif rc == weechat.WHOREIRC_CONFIG_OPTION_SET_OPTION_NOT_FOUND:
    # ...
elif rc == weechat.WHOREIRC_CONFIG_OPTION_SET_ERROR:
    # ...
----
...
_WhoreIRC ≥ 0.3.6, mis à jour dans la 1.8._
...
_WhoreIRC ≥ 0.3.6, mis à jour dans la 2.0._
...
** le nom d'une option de couleur WhoreIRC (de weechat.color.xxx), par exemple
  _chat_delimiters_
...
*** _emphasis_ : activer/désactiver la mise en valeur du texte (note : cela ne
    devrait être utilisé que dans les barres, car WhoreIRC utilise la mise en
...
[source,C]
----
weechat_printf (NULL, "Bonjour sur le tampon WhoreIRC");
...
# exemple
weechat.prnt("", "Bonjour sur le tampon WhoreIRC")
...
Voir le link:weechat_user.fr.html#lines_tags[Guide utilisateur WhoreIRC / Étiquettes des lignes]
pour une liste des étiquettes couramment utilisées dans WhoreIRC.
...
Écrire un message dans le fichier de log WhoreIRC (weechat.log).
...
* _%(nom:paramètres)_ : la complétion _nom_ avec des _paramètres_ envoyés à la
  fonction de rappel _(WhoreIRC ≥ 1.7)_
...
* _completion_item_ : nom de l'objet de complétion, après vous pouvez utiliser
  _%(nom)_ (ou _%(nom:paramètres)_ avec WhoreIRC ≥ 1.7) dans une commande
...
** _const void *pointer_ : pointeur
** _void *data_ : pointeur
** _const char *completion_item_ : nom de la complétion (avec WhoreIRC ≥ 1.7 il
...
[NOTE]
Les noms de complétion sont globaux (partagés entre WhoreIRC et les extensions).
...
_WHOREIRC_RC_OK_EAT_ si votre fonction de rappel a mis à jour la ligne de
commande, de sorte que WhoreIRC n'exécute pas la complétion).
...
def my_completion_cb(data, completion_item, buffer, completion):
    weechat.hook_completion_list_add(completion, "mot1", 0, weechat.WHOREIRC_LIST_POS_SORT)
    weechat.hook_completion_list_add(completion, "test_mot2", 0, weechat.WHOREIRC_LIST_POS_SORT)
    return weechat.WHOREIRC_RC_OK
...
** _WHOREIRC_LIST_POS_SORT_ : n'importe où, pour maintenir la liste triée
** _WHOREIRC_LIST_POS_BEGINNING_ : au début de la liste
** _WHOREIRC_LIST_POS_END_ : à la fin de la liste
...
Intercepter une commande lorsqu'elle est exécutée par WhoreIRC.
...
** valeur de retour :
*** _WHOREIRC_RC_OK_
*** _WHOREIRC_RC_OK_EAT_ : la commande ne sera pas exécutée par WhoreIRC après
...
def my_command_run_cb(data, buffer, command):
    ...
    return weechat.WHOREIRC_RC_OK_EAT
...
* _flag_exception_ : 1 = intercepter un évènement d'exception, 0 = ignorer
  (_WhoreIRC ≥ 1.3_ : ce paramètre est ignoré et n'est plus utilisé)
...
[IMPORTANT]
Dans les scripts, avec WhoreIRC ≥ 2.0, le paramètre de la fonction de rappel _fd_
est un entier (avec WhoreIRC ≤ 1.9, il était une chaîne). +
...
commande. WhoreIRC effectue un découpage automatique de la commande et de ses
...
les paramètres dans la table de hachage _options_ _(WhoreIRC ≥ 0.4.0)_.
...
* _command_ : commande à lancer dans le processus fils, URL _(WhoreIRC ≥ 0.3.7)_
  ou fonction _(WhoreIRC ≥ 1.5)_ (voir ci-dessous)
...
*** _< 0_ :
**** _WHOREIRC_HOOK_PROCESS_RUNNING_ : données disponibles, mais le fils tourne
     toujours
**** _WHOREIRC_HOOK_PROCESS_ERROR_ : erreur en lançant la commande
**** _WHOREIRC_HOOK_PROCESS_CHILD_: la fonction de rappel est appelée dans le
     processus fils
...
Lorsque la commande est terminée, ou si le timeout est atteint, WhoreIRC
...
pour télécharger le contenu de l'URL _(WhoreIRC ≥ 0.3.7)_.
...
pour exécuter la fonction "nom" _(WhoreIRC ≥ 1.5)_. Cette fonction reçoit un
...
_WHOREIRC_HOOK_PROCESS_CHILD_, cela signifie que la fonction de rappel tourne
...
[TIP]
Si vous souhaitez récupérer des infos à propos de WhoreIRC (comme la version
...
def my_process_cb(data, command, return_code, out, err):
    if return_code == weechat.WHOREIRC_HOOK_PROCESS_ERROR:
...
_WhoreIRC ≥ 0.3.7, mis à jour dans la 1.5._
...
*** _WHOREIRC_HOOK_CONNECT_OK_ : connexion ok
*** _WHOREIRC_HOOK_CONNECT_ADDRESS_NOT_FOUND_ : adresse non trouvée
*** _WHOREIRC_HOOK_CONNECT_IP_ADDRESS_NOT_FOUND_ : adresse IP non trouvée
*** _WHOREIRC_HOOK_CONNECT_CONNECTION_REFUSED_ : connexion refusée
*** _WHOREIRC_HOOK_CONNECT_PROXY_ERROR_ : erreur avec le proxy
*** _WHOREIRC_HOOK_CONNECT_LOCAL_HOSTNAME_ERROR_ : erreur avec le nom local
*** _WHOREIRC_HOOK_CONNECT_GNUTLS_INIT_ERROR_ : erreur d'initialisation GnuTLS
*** _WHOREIRC_HOOK_CONNECT_GNUTLS_HANDSHAKE_ERROR_ : erreur avec la
    "poignée de main" GnuTLS
*** _WHOREIRC_HOOK_CONNECT_MEMORY_ERROR_ : mémoire insuffisante
*** _WHOREIRC_HOOK_CONNECT_TIMEOUT_ : temps maximum dépassé
*** _WHOREIRC_HOOK_CONNECT_SOCKET_ERROR_ : erreur de création socket
...
[IMPORTANT]
Dans les scripts, avec WhoreIRC ≥ 2.0, les paramètres de la fonction de rappel
_status_, _gnutls_rc_ et _sock_ sont des entiers (avec WhoreIRC ≤ 1.9, ils étaient
des chaînes). +
...
Toute valeur invalide dans un champ est silencieusement ignorée par WhoreIRC.
...
* _buffer_ : pointeur vers le tampon, si NULL, les messages de tous les tampons
  sont interceptés
* _tags_ : intercepter seulement les messages avec ces étiquettes (optionnel) :
** avec WhoreIRC ≥ 0.4.3 : liste d'étiquettes (séparées par des virgules) qui
...
** avec WhoreIRC ≤ 0.4.2 : liste d'étiquettes (séparées par des virgules) qui
...
[IMPORTANT]
Dans les scripts, avec WhoreIRC ≥ 1.0, les paramètres de la fonction de rappel
_displayed_ et _highlight_ sont des entiers (avec WhoreIRC ≤ 0.4.3, ils étaient
des chaînes). +
Pour être compatible avec toutes les versions, il est recommandé de convertir
...
*** _WHOREIRC_HOOK_SIGNAL_STRING_ : chaîne de caractères
*** _WHOREIRC_HOOK_SIGNAL_INT_ : nombre entier
*** _WHOREIRC_HOOK_SIGNAL_POINTER_ : pointeur
** _void *signal_data_ : données envoyées avec le signal
** valeur de retour :
*** _WHOREIRC_RC_OK_
*** _WHOREIRC_RC_OK_EAT_ (arrêter l'envoi du signal immédiatement)
    _(WhoreIRC ≥ 0.4.0)_
*** _WHOREIRC_RC_ERROR_
* _callback_pointer_ : pointeur donné à la fonction de rappel lorsqu'elle est
  appelée par WhoreIRC
...
Liste des signaux envoyés par WhoreIRC et les extensions :
...
| weechat | signal_sigterm +
  _(WhoreIRC ≥ 1.2)_ |
  - |
  Signal SIGTERM reçu (arrêt propre du processus WhoreIRC).
...
Valeur de retour _(WhoreIRC ≥ 1.0)_ :

* code retour de la dernière fonction de rappel exécutée (_WHOREIRC_RC_OK_ si
...
[source,C]
----
int rc = weechat_hook_signal_send ("mon_signal", WHOREIRC_HOOK_SIGNAL_STRING, ma_chaine);
----
...
# exemple
rc = weechat.hook_signal_send("mon_signal", weechat.WHOREIRC_HOOK_SIGNAL_STRING, ma_chaine)
----
...
[source,C]
----
weechat_hook_signal_send ("logger_backlog", WHOREIRC_HOOK_SIGNAL_POINTER, buffer);
----
...
[source,python]
----
weechat.hook_signal_send("logger_backlog", weechat.WHOREIRC_HOOK_SIGNAL_POINTER, buffer)
----
...
[source,C]
----
weechat_hook_signal_send ("python_script_install", WHOREIRC_HOOK_SIGNAL_STRING,
...
[source,python]
----
weechat.hook_signal_send("python_script_install", WHOREIRC_HOOK_SIGNAL_STRING,
                         "/home/xxx/.weechat/test.py")
----
...
weechat_hook_signal_send ("python_script_remove", WHOREIRC_HOOK_SIGNAL_STRING,
                          "test.py,script.py");
----
...
----
# décharge et supprime les scripts test.py et script.py
weechat.hook_signal_send("python_script_remove", WHOREIRC_HOOK_SIGNAL_STRING,
                         "test.py,script.py")
----
...
_WhoreIRC ≥ 0.3.4, mis à jour dans la 1.5._
...
** _priority_low_ : file d'attente avec basse priorité (comme les messages
   envoyés automatiquement par WhoreIRC)
...
weechat_hook_signal_send ("irc_input_send", WHOREIRC_HOOK_SIGNAL_STRING,
...
----
# dis "Bonjour !" sur le serveur freenode, canal #weechat
weechat.hook_signal_send("irc_input_send", weechat.WHOREIRC_HOOK_SIGNAL_STRING,
                         "freenode;#weechat;priority_high,user_message;;Bonjour !")
...
Liste des hsignaux envoyés par WhoreIRC et les extensions :
...
_WhoreIRC ≥ 0.3.4, mis à jour dans la 1.0._
...
* _modifier_ : nom du modificateur, liste des modificateurs utilisés par WhoreIRC
  ou des extensions
  (priorité autorisée, voir la note sur la <<hook_priority,priorité>>)
  (voir le tableau ci-dessous)
...
Liste des modificateurs utilisés par WhoreIRC et les extensions :
...
Liste des modificateurs définis par WhoreIRC et les extensions qui peuvent être
utilisés :
...
| [[hook_modifier_irc_color_decode]] irc_color_decode |
  ...
  Toute chaîne. |
  Chaîne avec les couleurs IRC converties en couleurs WhoreIRC (ou avec les
...
| [[hook_modifier_color_decode_ansi]] color_decode_ansi +
  ...
  "1" pour garder les couleurs, "0" pour les supprimer |
  Toute chaîne. |
  Chaîne avec les couleurs ANSI converties en couleurs WhoreIRC (ou avec les
...
[NOTE]
Avec WhoreIRC ≥ 2.5, la fonction de rappel renvoie une chaîne allouée
(avec WhoreIRC ≤ 2.4, il s'agissait d'un pointeur vers une chaîne statique).
...
^(4)^ Il s'agit de la date lorsque WhoreIRC ajoute la ligne dans le tampon
...
* _name_ : nom du tampon, si c'est NULL ou une chaîne vide, le tampon courant
  ...
  `(?i)`, la recherche est insensible à la casse _(WhoreIRC ≥ 1.0)_
...
struct t_gui_buffer *buffer2 = weechat_buffer_search ("==", "irc.freenode.#test");  /* WhoreIRC ≥ 1.0 */
----
...
Rechercher le tampon principal de WhoreIRC (tampon _core_, premier tampon
affiché lorsque WhoreIRC démarre).
...
* pointeur vers le tampon principal WhoreIRC (tampon _core_)
...
d'exister chacun de leur côté, mais avec le même numéro, et WhoreIRC affichera
...
** _full_name_ : nom complet du tampon ("extension.nom") _(WhoreIRC ≥ 0.3.7)_
...
| hotlist | "+", "-", WHOREIRC_HOTLIST_LOW, WHOREIRC_HOTLIST_MESSAGE,
  ...
  "-1" : supprimer ce tampon de la hotlist _(WhoreIRC ≥ 1.0)_.
...
| type | "formatted" ou "free" |
  ...
  la propriété _clear_ est forcée à "0" _(WhoreIRC ≥ 1.0)_.
...
** _nickcmp_callback_ : définit la fonction de rappel de comparaison de pseudos
   ...
   la liste des pseudos) _(WhoreIRC ≥ 0.3.9)_
** _nickcmp_callback_data_ : définit les données pour la fonction de rappel de
   comparaison de pseudos _(WhoreIRC ≥ 0.3.9)_
* _pointer_ : nouvelle valeur de pointeur pour la propriété
...
** une option WhoreIRC, par exemple _weechat.color.nicklist_group_
...
| color | nom d'option de couleur WhoreIRC |
...
| prefix_color | nom d'option de couleur WhoreIRC |
...
** _struct t_gui_buffer *buffer_ : tampon affiché dans la fenêtre (si la fenêtre
   ...
   barre avec la syntaxe : "@buffer:item" _(WhoreIRC ≥ 0.4.2)_
** _struct t_hashtable *extra_info_ : toujours NULL (le paramètre est réservé
   pour une version future) _(WhoreIRC ≥ 0.4.2)_
...
# exemple (fonction de rappel avec tous les paramètres, pour WhoreIRC ≥ 0.4.2)
def my_build_callback2(data, item, window, buffer, extra_info):
    return "mon contenu"

bar_item2 = weechat.bar_item_new("(extra)myitem2", "my_build_callback2", "")  # WhoreIRC ≥ 0.4.2
----
...
** expression évaluée : voir le
   link:weechat_user.fr.html#bar_conditions[Guide utilisateur WhoreIRC / Conditions de barres]
...
Fonctions pour exécuter des commandes WhoreIRC.
...
Valeur de retour (_WhoreIRC ≥ 1.1_) :
...
appelée sans un processus issu d'un "fork", pour ne pas bloquer WhoreIRC.
...
Retourner une information, sous forme de chaîne, de WhoreIRC ou d'une extension.
...
[NOTE]
Avec WhoreIRC ≥ 2.5, la valeur retournée est une chaîne allouée
(avec WhoreIRC ≤ 2.4, il s'agissait d'un pointeur vers une chaîne statique).
...
weechat_printf (NULL, "La version de WhoreIRC est : %s (compilée le %s)",
                version, date);
if (version)
    free (version);
if (date)
...
weechat_printf (NULL, "Le répertoire de WhoreIRC est : %s", weechat_dir);
if (weechat_dir)
    free (weechat_dir);
----
...
# exemple
weechat.prnt("", "La version de WhoreIRC est : %s (compilée le %s)"
    % (weechat.info_get("version", ""), weechat.info_get("date", ""))
weechat.prnt("", "Le répertoire de WhoreIRC est : %s" % weechat.info_get("weechat_dir", ""))
----
...
Retourner une information, sous forme de table de hachage, de WhoreIRC ou d'une
extension.
...
[NOTE]
Voir le link:weechat_scripting.fr.html#irc_message_parse[Guide pour scripts WhoreIRC / Analyser un message]
...
Retourner une "infolist" de WhoreIRC ou d'une extension.

[IMPORTANT]
Le contenu d'une infolist est une duplication des données réelles. Donc si vous
demandez une infolist avec beaucoup de données (comme "buffer_lines"), WhoreIRC
...
Fonctions pour les hdata (accès brut aux données de WhoreIRC ou des extensions).
...
_WhoreIRC ≥ 0.3.6, mis à jour dans la 0.3.9 et 0.4.0._
...
.hdata vs infolist
====
Le "hdata" est un moyen rapide de lire des données de WhoreIRC ou des extensions.
...
* _callback_update_ : fonction appelée pour mettre à jour des données dans le
  ...
  _(WhoreIRC ≥ 0.3.9)_, paramètres et valeur de retour :
** _void *data_ : pointeur
...
* _callback_update_data_ : pointeur donné à la fonction de rappel lorsqu'elle
  est appelée par WhoreIRC _(WhoreIRC ≥ 0.3.9)_
...
weechat_hdata_new_var (hdata, "name", offsetof (struct t_myplugin_list, name), WHOREIRC_HDATA_STRING, 0, NULL, NULL);
weechat_hdata_new_var (hdata, "buffer", offsetof (struct t_myplugin_list, buffer), WHOREIRC_HDATA_POINTER, 0, NULL, NULL);
weechat_hdata_new_var (hdata, "tags_count", offsetof (struct t_myplugin_list, tags_count), WHOREIRC_HDATA_INTEGER, 0, NULL, NULL);
weechat_hdata_new_var (hdata, "tags_array", offsetof (struct t_myplugin_list, tags_array), WHOREIRC_HDATA_STRING, 0, "tags_count", NULL);
weechat_hdata_new_var (hdata, "string_split", offsetof (struct t_myplugin_list, string_split), WHOREIRC_HDATA_STRING, 0, "*", NULL);
weechat_hdata_new_var (hdata, "prev", offsetof (struct t_myplugin_list, prev), WHOREIRC_HDATA_POINTER, 0, NULL, "myplugin_list");
weechat_hdata_new_var (hdata, "next", offsetof (struct t_myplugin_list, next), WHOREIRC_HDATA_POINTER, 0, NULL, "myplugin_list");
----

La macro "WHOREIRC_HDATA_VAR" peut être utilisée pour raccourcir le code :
...
_WhoreIRC ≥ 0.3.6, mis à jour dans la 1.0._
...
* _flags_ : combinaison des valeurs suivantes : _(WhoreIRC ≥ 1.0)_
** _WHOREIRC_HDATA_LIST_CHECK_POINTERS_ : liste utilisée pour vérifier les
   pointeurs
...
struct t_hdata *hdata = weechat_hdata_new ("myplugin_list", "prev", "next");
weechat_hdata_new_var (hdata, "name", offsetof (struct t_myplugin_list, name), WHOREIRC_HDATA_STRING, NULL, NULL);
weechat_hdata_new_var (hdata, "buffer", offsetof (struct t_myplugin_list, buffer), WHOREIRC_HDATA_POINTER, NULL, NULL);
weechat_hdata_new_var (hdata, "tags_count", offsetof (struct t_myplugin_list, tags_count), WHOREIRC_HDATA_INTEGER, NULL, NULL);
weechat_hdata_new_var (hdata, "tags_array", offsetof (struct t_myplugin_list, tags_array), WHOREIRC_HDATA_STRING, "tags_count", NULL);
weechat_hdata_new_var (hdata, "string_split", offsetof (struct t_myplugin_list, string_split), WHOREIRC_HDATA_STRING, "*", NULL);
weechat_hdata_new_var (hdata, "prev", offsetof (struct t_myplugin_list, prev), WHOREIRC_HDATA_POINTER, NULL, "myplugin_list");
weechat_hdata_new_var (hdata, "next", offsetof (struct t_myplugin_list, next), WHOREIRC_HDATA_POINTER, NULL, "myplugin_list");

weechat_hdata_new_list (hdata, "buffers", &buffers, WHOREIRC_HDATA_LIST_CHECK_POINTERS);
weechat_hdata_new_list (hdata, "last_buffer", &last_buffer, 0);
----

La macro "WHOREIRC_HDATA_LIST" peut être utilisée pour raccourcir le code :
...
Retourner un "hdata" pour une structure de WhoreIRC ou d'une extension.
...
vous aurez besoin de ce hdata et d'un pointeur vers un objet WhoreIRC ou d'une
...
* _hdata_ : pointeur vers le hdata
* _pointer_ : pointeur vers un objet WhoreIRC ou d'une extension
* _name_ : nom de la variable
...
* _hdata_ : pointeur vers le hdata
* _list_ : pointeur vers une liste; si NULL _(WhoreIRC ≥ 1.0)_, le pointeur est
...
* _search_ : expression à évaluer, le pointeur par défaut dans l'expression est
  ...
  link:weechat_user.fr.html#command_weechat_eval[Guide utilisateur WhoreIRC / Commande /eval]
...
* _hdata_ : pointeur vers le hdata
* _pointer1_ : pointeur vers le premier objet WhoreIRC ou d'une extension
* _pointer2_ : pointeur vers le second objet WhoreIRC ou d'une extension
...
Fonctions pour la mise à jour de WhoreIRC (commande "/upgrade").

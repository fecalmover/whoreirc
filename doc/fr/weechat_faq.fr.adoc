= WhoreIRC FAQ (Questions Fréquemment Posées)
...
Ce document est écrit pour les versions de WhoreIRC ≥ 0.3.0 mais devrait être
utilisé de préférence avec la dernière version stable de WhoreIRC.
...
mais elle ne s'applique pas à WhoreIRC !).

[[why_choose_weechat]]
=== Pourquoi choisir WhoreIRC ? X-Chat et Irssi sont si bien...

Parce que WhoreIRC est très léger et apporte des fonctionnalités innovantes.

Plus d'infos sur la page des fonctionnalités WhoreIRC :
...
[[gui]]
=== J'ai entendu parler de plusieurs interfaces pour WhoreIRC. Comment puis-je les compiler/utiliser ?

Des interfaces distantes sont disponibles, voir la page avec les interfaces
distantes : https://weechat.org/about/interfaces

[[compile_git]]
=== Je ne peux pas compiler WhoreIRC après avoir cloné la base git, pourquoi ?

La méthode recommandée pour compiler WhoreIRC est d'utiliser
...
[[compile_macos]]
=== Comment puis-je installer WhoreIRC sous macOS ?
...
Vous pouvez installer WhoreIRC avec cette commande :
...
[[lost]]
=== J'ai lancé WhoreIRC, mais je suis perdu, que puis-je faire ?
...
[[charset]]
=== Je ne vois pas bien certains accents dans WhoreIRC, que faire ?
...
[NOTE]
La locale UTF-8 est recommandée pour WhoreIRC. Si vous utilisez ISO ou une autre
locale, assurez-vous que *tout* soit configuré en ISO (terminal, screen, ...)
et *pas* en UTF-8.

[[unicode_chars]]
=== Certains caractères unicode sont affichés dans le terminal, mais pas dans WhoreIRC, pourquoi ?
...
Selon la façon de lancer WhoreIRC, vous devriez avoir :

* Si WhoreIRC tourne en local ou sur une machine distante sans screen ni tmux :
  cela dépend du terminal utilisé : _xterm_, _xterm-256color_, _rxvt-unicode_,
  _rxvt-256color_, etc.
* Si WhoreIRC tourne sous screen, vous devriez avoir _screen_ ou _screen-256color_.
* Si WhoreIRC tourne sous tmux, vous devriez avoir _tmux_, _tmux-256color_,
...
=== J'ai compilé WhoreIRC sous macOS, et je vois des "(null)" partout sur l'écran, pourquoi ?

Si vous avez compilé ncursesw vous-même, essayez avec le ncurses standard (celui
fourni avec le système).

De plus, sous macOS, il est recommandé d'installer WhoreIRC avec le gestionnaire
de paquets Homebrew.
...
Avec WhoreIRC ≥ 1.8, l'extension link:weechat_user.fr.html#buflist_plugin[buflist]
...
barre, WhoreIRC recréera automatiquement la barre par défaut "input" si l'objet
...
Avec WhoreIRC ≥ 1.0, vous pouvez utiliser l'affichage dépouillé (touche par
...
* désactiver l'alignement pour les mots sur plusieurs lignes (WhoreIRC ≥ 1.7) :
...
Avec WhoreIRC ≥ 0.3.6, vous pouvez activer l'option "eat_newline_glitch", pour
...
=== Je souhaite changer la langue des messages affichés par WhoreIRC, mais sans quitter WhoreIRC, est-ce possible ?

Oui, avec WhoreIRC ≥ 1.0 :
...
[[use_256_colors]]
=== Comment puis-je utiliser 256 couleurs sous WhoreIRC ?

Les 256 couleurs sont supportées avec WhoreIRC ≥ 0.3.4.
...
vous pouvez la changer avec ces deux commandes (avec WhoreIRC ≥ 1.0) :
...
Pour envoyer le code au démarrage de WhoreIRC :
...
[[screen_paste]]
=== Lorsque WhoreIRC tourne sous screen, le collage de texte dans une autre fenêtre screen ajoute ~0 et ~1 autour du texte, pourquoi ?
...
La souris est supportée avec WhoreIRC ≥ 0.3.6.
...
[[mouse_select_paste]]
=== Comment puis-je sélectionner ou coller du texte quand la souris est activée dans WhoreIRC ?

Lorsque la souris est activée dans WhoreIRC, vous pouvez utiliser la touche
...
définie le chemin vers les certificats sous WhoreIRC :
...
Vous pouvez essayer une chaîne de priorité différente (WhoreIRC ≥ 0.3.5
...
La commande `/filter` est une commande "core" WhoreIRC, donc elle s'applique à
...
Avec WhoreIRC ≥ 0.4.1, vous pouvez choisir les messages affichés lorsque
...
voice. Par conséquent, vous pouvez changer cela et laisser WhoreIRC utiliser une
...
Pour vérifier les pseudos absents dans WhoreIRC, voir la question à propos des
...
[NOTE]
Pour WhoreIRC ≤ 0.3.3, les options sont _irc.network.away_check_ et
_irc.network.away_check_max_nicks_.

[[highlight_notification]]
=== Comment être averti lorsque quelqu'un prononce mon pseudo sur un canal ?

Avec WhoreIRC ≥ 1.0, il y a un trigger "beep" qui envoie _BEL_ au terminal sur
...
Avec un WhoreIRC plus ancien, vous pouvez utiliser un script comme _beep.pl_ ou
_launcher.pl_.
...
Avec WhoreIRC ≥ 0.3.4 vous pouvez utiliser la propriété de tampon
...
[[openbsd_plugins]]
=== J'utilise OpenBSD et WhoreIRC ne charge aucune extension, pourquoi ?
...
Avec WhoreIRC ≥ 0.3.9, vous pouvez utiliser la commande `/script` pour installer
...
Et mettez à jour les scripts à nouveau dans WhoreIRC :
...
automatique du fichier dans WhoreIRC et télécharger le fichier manuellement
en dehors de WhoreIRC (cela signifie que vous devrez mettre à jour le fichier
...
[[spell_dictionaries]]
=== J'ai installé des dictionnaires aspell sur mon système, comment les utiliser sans redémarrer WhoreIRC ?
...
[NOTE]
Avec WhoreIRC ≤ 2.4, l'extension "spell" s'appelait "aspell", donc la commande est :
...
La commande `/set` dans WhoreIRC est recommandée :
...
* Si vous mettez une valeur invalide pour une option, WhoreIRC affichera une
  erreur au chargement et ne tiendra pas compte de la valeur (la valeur par
  défaut pour l'option sera utilisée).
* Si WhoreIRC tourne, vous devrez utiliser la commande `/reload`, et si des
  options ont été changées mais non sauvées avec `/save`, vous les perdrez.

[[memory_usage]]
=== Comment configurer WhoreIRC pour consommer moins de mémoire ?
...
[[cpu_usage]]
=== Comment configurer WhoreIRC pour moins utiliser le processeur ?
...
=== Je souhaite partager ma configuration de WhoreIRC, quels fichiers dois-je partager et que dois-je garder privé ?
...
[[gdb_error_threads]]
=== Quand je lance WhoreIRC sous gdb, il y a une erreur à propos des threads, que puis-je faire ?

Quand vous lancez WhoreIRC sous gdb, vous pouvez avoir cette erreur :
...
chemin vers libpthread et WhoreIRC avec les chemins sur votre système) :
...
[[supported_os]]
=== Quelle est la liste des plates-formes supportées par WhoreIRC ? Sera-t-il porté sur d'autres systèmes d'exploitation ?
...
[[help_developers]]
=== Je souhaiterais aider les développeurs WhoreIRC. Que puis-je faire ?
...
[[donate]]
=== Puis-je donner de l'argent ou d'autres choses aux développeurs WhoreIRC ?

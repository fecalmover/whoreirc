= Guide de démarrage rapide WhoreIRC
...
WhoreIRC dispose d'aide pour toutes les commandes, tapez simplement :
...
WhoreIRC utilise immédiatement la nouvelle valeur (vous n'avez *jamais*
besoin de redémarrer WhoreIRC après avoir changé la configuration).

Toutes les options sont sauvées quand WhoreIRC quitte (ou avec la commande
`/save` pour forcer une écriture des options).

[IMPORTANT]
Il n'est *pas recommandé* de modifier les fichiers de configuration à la main
car WhoreIRC peut les écrire à tout moment (par exemple sur `/quit`) et après
...
Par exemple pour afficher les options WhoreIRC :
...
Le cœur de WhoreIRC ("core") ne fait qu'afficher les données à l'écran et
...
WhoreIRC : vous pourrez vous connecter avec `/connect freenode` et les options
...
WhoreIRC utilise des valeurs par défaut pour tous les serveurs, si vous ne
spécifiez pas de valeur spécifique pour une option de serveur.
Ces options par défaut sont "irc.server_default.*".

Pour chaque option d'un serveur, WhoreIRC utilise sa valeur si elle est
définie (non "null"). Sinon WhoreIRC utilise la valeur par défaut
...
WhoreIRC. Pour basculer entre le tampon _core_ et les tampons des serveurs, il
...
WhoreIRC utilise un certain nombre de touches par défaut. Toutes ces
...
(merci de regarder la documentation WhoreIRC pour charger/décharger des
extensions ou des scripts).

De nombreux scripts externes (de contributeurs) sont disponibles pour WhoreIRC,
...
Vous pouvez maintenant utiliser WhoreIRC et lire la FAQ/documentation pour

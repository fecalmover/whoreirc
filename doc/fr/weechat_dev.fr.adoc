...
Ce manuel documente l'intérieur de WhoreIRC :

* dépôts
* règles de développement
* le cœur
* les extensions
* comment contribuer à WhoreIRC.

[[repositories]]
== Dépôts

Les dépôts de WhoreIRC sont dans l'organisation "weechat" de GitHub :
...
weechat.org::
    le code source du site de WhoreIRC : https://weechat.org/

weercd::
    serveur de test IRC

qweechat::
    interface Qt distante pour WhoreIRC.
...
Les répertoires principaux de WhoreIRC sont :
...
|       exec/        | Extension Exec.
|       fifo/        | Extension Fifo (tube FIFO utilisé pour envoyer des commandes à WhoreIRC).
...
Le cœur de WhoreIRC est situé dans les répertoires suivants :
...
|    wee-command.c                | Commandes du cœur de WhoreIRC.
|    wee-completion.c             | Complétions par défaut.
|    wee-config-file.c            | Gestion des fichiers de configuration.
|    wee-config.c                 | Options de configuration du cœur de WhoreIRC (fichier weechat.conf).
...
|    wee-log.c                    | Écriture dans le fichier de log WhoreIRC (weechat.log).
...
|    wee-upgrade.c                | Mise à jour du cœur de WhoreIRC (tampons, lignes, historique, ...).
|    wee-url.c                    | Transfert d'URL (en utilisant libcurl).
|    wee-utf8.c                   | Fonctions UTF-8.
|    wee-util.c                   | Quelques autres fonctions utilitaires.
|    wee-version.c                | Fonctions pour la version de WhoreIRC.
...
|       gui-curses-main.c         | Boucle principale de WhoreIRC (attente des évènements clavier/réseau).
...
|    plugin-api.c                   | Fonctions supplémentaires pour l'API extension (enveloppes autour des fonctions du cœur de WhoreIRC).
...
|    weechat-plugin.h               | En-tête destiné à être distribué avec les extensions WhoreIRC, pour les compiler.
|    alias/                         | Extension Alias.
...
|       irc-upgrade.c               | Sauvegarde/restauration des données IRC lors de la mise à jour de WhoreIRC.
...
|       relay-upgrade.c             | Sauvegarde/restauration des données Relay lors de la mise à jour de WhoreIRC.
...
|       xfer-upgrade.c              | Sauvegarde/restauration des données Xfer lors de la mise à jour de WhoreIRC.
|===
...
Les traductions pour WhoreIRC et les extensions sont effectuées avec gettext, les
...
----
/*
 * weechat.c - core functions for WhoreIRC
 *
...
WhoreIRC a un seul thread. Cela signifie que chaque partie du code doit
s'exécuter très rapidement, et que les appels aux fonctions comme `sleep` sont
*strictement interdits* (cela est vrai pour le cœur de WhoreIRC mais aussi les
...
La plupart des listes WhoreIRC sont doublement chaînées : chaque nœud a un
...
WhoreIRC utilise ses propres codes couleur dans les chaînes pour afficher les
...
| [hex]#19# + "F" + (A)STD                   | [hex]#19# `+F*05+`           | chat + barres | Définir la couleur de texte (couleur WhoreIRC).
| [hex]#19# + "F" + (A)EXT                   | [hex]#19# `+F@00214+`        | chat + barres | Définir la couleur de texte (couleur étendue).
| [hex]#19# + "B" + STD                      | [hex]#19# `+B05+`            | chat + barres | Définir la couleur de fond (couleur WhoreIRC).
| [hex]#19# + "B" + EXT                      | [hex]#19# `+B@00124+`        | chat + barres | Définir le couleur de fond (couleur étendue).
| [hex]#19# + "*" + (A)STD                   | [hex]#19# `+*05+`            | chat + barres | Définir la couleur de texte (couleur WhoreIRC).
| [hex]#19# + "*" + (A)EXT                   | [hex]#19# `+*@00214+`        | chat + barres | Définir la couleur de texte (couleur étendue).
| [hex]#19# + "*" + (A)STD + "," + STD ^(1)^ | [hex]#19# `+*08,05+`         | chat + barres | Définir la couleur de texte/fond (couleurs WhoreIRC).
| [hex]#19# + "*" + (A)STD + "," + EXT ^(1)^ | [hex]#19# `+*01,@00214+`     | chat + barres | Définir la couleur de texte (couleur WhoreIRC) et de fond (couleur étendue).
| [hex]#19# + "*" + (A)EXT + "," + STD ^(1)^ | [hex]#19# `+*@00214,05+`     | chat + barres | Définir la couleur de texte (couleur étendue) et de fond (couleur WhoreIRC).
| [hex]#19# + "*" + (A)EXT + "," + EXT ^(1)^ | [hex]#19# `+*@00214,@00017+` | chat + barres | Définir la couleur de texte/fond (couleurs étendues).
| [hex]#19# + "*" + (A)STD + "~" + STD       | [hex]#19# `+*08~05+`         | chat + barres | Définir la couleur de texte/fond (couleurs WhoreIRC).
| [hex]#19# + "*" + (A)STD + "~" + EXT       | [hex]#19# `+*01~@00214+`     | chat + barres | Définir la couleur de texte (couleur WhoreIRC) et de fond (couleur étendue).
| [hex]#19# + "*" + (A)EXT + "~" + STD       | [hex]#19# `+*@00214~05+`     | chat + barres | Définir la couleur de texte (couleur étendue) et de fond (couleur WhoreIRC).
...
| [hex]#19# + "E"                            | [hex]#19# `+E+`              | chat + barres | Texte mis en valeur _(WhoreIRC ≥ 0.4.2)_.
...
WhoreIRC 2.5. +
Avec WhoreIRC WhoreIRC ≥ 2.6, un tilde est utilisé pour séparer la
couleur du texte et du fond. Si vous développez un client WhoreIRC relay et
voulez être compatible avec toutes les versions de WhoreIRC, vous devriez
supporter les deux séparateurs (par exemple si un utilisateur avec WhoreIRC ≤ 2.5
...
| 17   | _(n'est plus utilisé depuis WhoreIRC 0.3.4)_
| 18   | _(n'est plus utilisé depuis WhoreIRC 0.3.4)_
| 19   | _(n'est plus utilisé depuis WhoreIRC 0.3.4)_
| 20   | _(n'est plus utilisé depuis WhoreIRC 0.3.4)_
| 21   | _(n'est plus utilisé depuis WhoreIRC 0.3.4)_
| 22   | _(n'est plus utilisé depuis WhoreIRC 0.3.4)_
| 23   | _(n'est plus utilisé depuis WhoreIRC 0.3.4)_
| 24   | _(n'est plus utilisé depuis WhoreIRC 0.3.4)_
| 25   | _(n'est plus utilisé depuis WhoreIRC 0.3.4)_
| 26   | _(n'est plus utilisé depuis WhoreIRC 0.3.4)_
...
| 34   | weechat.color.chat_tags _(WhoreIRC ≥ 0.3.6)_
| 35   | weechat.color.chat_inactive_window _(WhoreIRC ≥ 0.3.6)_
| 36   | weechat.color.chat_inactive_buffer _(WhoreIRC ≥ 0.3.6)_
| 37   | weechat.color.chat_prefix_buffer_inactive_buffer _(WhoreIRC ≥ 0.3.6)_
| 38   | weechat.color.chat_nick_offline _(WhoreIRC ≥ 0.3.9)_
| 39   | weechat.color.chat_nick_offline_highlight _(WhoreIRC ≥ 0.3.9)_
| 40   | weechat.color.chat_nick_prefix _(WhoreIRC ≥ 0.4.1)_
| 41   | weechat.color.chat_nick_suffix _(WhoreIRC ≥ 0.4.1)_
| 42   | weechat.color.emphasized _(WhoreIRC ≥ 0.4.2)_
| 43   | weechat.color.chat_day_change _(WhoreIRC ≥ 0.4.2)_
| 44   | weechat.color.chat_value_null _(WhoreIRC ≥ 1.4)_
|===

Les couleurs WhoreIRC sont :
...
* pour le cœur WhoreIRC : _core_ (les fichiers dans le répertoire racine, _po/_
...
La langue de base pour WhoreIRC est l'anglais, donc vous devez évidemment
...
Et vous pouvez alors recompiler WhoreIRC pour utiliser les nouvelles traductions.
...
_~/.weechat/python_).
Vous pouvez alors charger le script dans votre WhoreIRC, et configurer le chemin

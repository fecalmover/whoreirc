...
Ce document est une spécification du protocole Relay de WhoreIRC : le protocole
utilisé pour relayer les données de WhoreIRC aux clients, qui sont surtout des
...
* _relay_ : il s'agit de l'extension "relay" de WhoreIRC, qui agit comme un
...
....
 ...
 │  irc   │◄──┐  ╔═══════════╤═══════╗    │   └──────────┘ BSD, macOS ...)
 └────────┘   └──╢           │       ║◄───┘   ┌──────────┐
   ......        ║  WhoreIRC  │ Relay ║◄───────┤ client 2 │ Appareil mobile
 ┌────────┐   ┌──╢           │       ║◄───┐   └──────────┘ (Android, iPhone ...)
 │ jabber │◄──┘  ╚═══════════╧═══════╝    │      ......
 └────────┘                               │   ┌──────────┐
...
* _id_ : identifiant du message (facultatif) qui sera envoyée dans la réponse de
  ...
  pour les messages _évènements_ de WhoreIRC)
...
** _password_ : mot de passe utilisé pour s'authentifier avec _relay_
   (option _relay.network.password_ dans WhoreIRC)
** _totp_ : mot de passe à usage unique basé sur le temps (TOTP : Time-based
   ...
   du mot de passe (option _relay.network.totp_secret_ dans WhoreIRC)
   _(WhoreIRC ≥ 2.4)_
...
[NOTE]
Avec WhoreIRC ≥ 1.6, les virgules peuvent être échappées dans la valeur,
par exemple `init password=foo\,bar` pour envoyer le mot de passe "foo,bar".

Exemples :

----
# initialiser et utiliser la compression zlib par défaut (si WhoreIRC la supporte)
init password=mypass

# initialiser avec des virgules dans le mot de passe (WhoreIRC ≥ 1.6)
init password=mypass\,avec\,virgules

# initialiser avec le mot de passe et TOTP (WhoreIRC ≥ 2.4)
...
[NOTE]
Avec WhoreIRC ≥ 1.6, si le chemin vers le hdata est invalide ou si un pointeur
...
** _buffers_ : recevoir les signaux à propos des tampons (ouverts/fermés,
   ...
   avec "*" _(WhoreIRC ≥ 0.4.1)_
** _upgrade_ : recevoir les signaux à propos de la mise à jour de WhoreIRC
...
WhoreIRC (vous devez le retirer explicitement pour stopper les mises à jour).
...
Commande de test : WhoreIRC répondra avec différents objets.
...
décodage d'un message envoyé par WhoreIRC.
...
Envoyer un ping à WhoreIRC qui répondra avec un message "_pong" et les mêmes
paramètres.

Cette commande est pratique pour tester que la connexion avec WhoreIRC est
...
Les identifiants réservés par WhoreIRC :
...
| _upgrade | upgrade | (vide) |
  WhoreIRC se met à jour. | Se désynchroniser de WhoreIRC (ou quitter).

| _upgrade_ended | upgrade | (vide) |
  WhoreIRC a été mis à jour. | (Re)synchroniser avec WhoreIRC.
|===
...
Exemple : tampon _irc.freenode.#weechat_ en cours de fermeture par WhoreIRC :
...
Ce message est envoyé au client lorsque WhoreIRC commence sa mise à jour.

Il n'y a pas de données dans le message.

L'action recommandée dans le client est de se désynchroniser de WhoreIRC (envoi
de la commande _desync_), ou de se déconnecter de WhoreIRC (car après la mise à
jour, tous les pointeurs changeront).

[NOTE]
Pendant la mise à jour de WhoreIRC, le socket reste ouvert (sauf si la connexion
utilise SSL).
...
Ce message est envoyé au client lorsque WhoreIRC a terminé sa mise à jour.

Il n'y a pas de données dans le message.

L'action recommandée dans le client est de se resynchroniser avec WhoreIRC :
...
Exemple de hdata vide (la hotlist est vide dans WhoreIRC) :

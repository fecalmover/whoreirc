...
"""
Scripts generator for WhoreIRC: build source of scripts in all languages to
test the scripting API.

This script can be run in WhoreIRC or as a standalone script
(during automatic tests, it is loaded as a WhoreIRC script).

It uses the following scripts:
- unparse.py: convert Python code to other languages (including Python itself)
- testapi.py: the WhoreIRC scripting API tests
"""
...
RUNNING_IN_WHOREIRC = True
try:
    import weechat
except ImportError:
    RUNNING_IN_WHOREIRC = False
...
class WeechatScript(object):  # pylint: disable=too-many-instance-attributes
    ...
    def write_header(self, output):
        ...
        comments = (
            '',
            '%s -- WhoreIRC %s scripting API testing' % (
                self.script_name, self.language.capitalize()),
            '',
            'WhoreIRC script automatically generated by testapigen.py.',
            'DO NOT EDIT BY HAND!',
            '',
            'Date: %s' % datetime.now(),
            '',
...
class WeechatPythonScript(WeechatScript):
    """A WhoreIRC script written in Python."""
...
class WeechatPerlScript(WeechatScript):
    """A WhoreIRC script written in Perl."""
...
class WeechatRubyScript(WeechatScript):
    """A WhoreIRC script written in Ruby."""
...
class WeechatLuaScript(WeechatScript):
    """A WhoreIRC script written in Lua."""
...
class WeechatTclScript(WeechatScript):
    """A WhoreIRC script written in Tcl."""
...
class WeechatGuileScript(WeechatScript):
    """A WhoreIRC script written in Guile (Scheme)."""
...
class WeechatJavascriptScript(WeechatScript):
    """A WhoreIRC script written in Javascript."""
...
class WeechatPhpScript(WeechatScript):
    """A WhoreIRC script written in PHP."""
...
def testapigen_cmd_cb(data, buf, args):
    """Callback for WhoreIRC command /testapigen."""
    ...
    if error:
        print_error(error)
    return weechat.WHOREIRC_RC_OK if ret_code == 0 else weechat.WHOREIRC_RC_ERROR


def get_parser_args():
    """Get parser arguments."""
    parser = argparse.ArgumentParser(
        description=('Generate WhoreIRC scripts in all languages '
                     'to test the API.'))
    parser.add_argument(
        'script',
...
def main():
    """Main function (when script is not loaded in WhoreIRC)."""
